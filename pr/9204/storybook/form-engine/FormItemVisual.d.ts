...
export interface BAIFormItemVisualProps {
    ...
    fieldId?: string;
    /**
     * The JSON-encoded name path, published as `data-bai-form-item-name` so
     * scroll-to-error can map a failing field onto its item without depending
     * on the control's `id` (a child may bring its own).
     */
    itemName?: string;
    htmlFor?: string;
    className?: string;
