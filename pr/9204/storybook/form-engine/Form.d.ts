...
export interface FormProps<Values = any> extends Omit<React.FormHTMLAttributes<HTMLFormElement>, 'onSubmit' | 'children' | 'onChange' | 'onReset'> {
    ...
    /**
     * Scrolls the first invalid field into view and focuses it when a
     * whole-form `validateFields()` rejects. Off unless set here or on
     * `<FormConfigProvider>`; a single call can override either way.
     */
