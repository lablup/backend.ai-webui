...
- `@smoke` carries a role suffix for the post-install smoke CLI
  (`packages/backend.ai-webui-smoke-cli`): a test with **only** `@smoke` must perform
  ...
  the runner injects one role's credentials only. A describe that mixes both helpers
  cannot carry a role tag; tag the individual tests instead. `--grep @smoke --list`
  shows the union of both roles; the runner does the partition. Full rules:
