...
- **React Build**: Vite 6 with `@vitejs/plugin-react`
...
4. **Do NOT review generated or vendored files.** Skip the contents of:
   - `**/__generated__/**` — Relay compiler output (deterministically regenerated by `pnpm relay`; the real source is the `graphql` tags in `.tsx`/`.ts` files plus `data/schema.graphql`)
   - `pnpm-lock.yaml`, `**/*.lock`, `**/*.lock.yml`
   - `*.d.ts` declaration files and other build artifacts under `dist/` or `build/`

   These files cannot be meaningfully changed by the author and any comment on them is noise. The only thing worth checking is whether the **set** of generated files in the diff is consistent with the schema/query changes (missing or stray artifacts) — not their line-by-line content.
...
- Development uses `pnpm run dev` (TypeScript watch + Relay watch + React dev server under Portless)
- Build process: multi-stage with resource copying and React build (Vite with vite-plugin-pwa service worker generation)
...
- Follow the GitHub Stacked PRs strategy (`gh stack`) for complex features
