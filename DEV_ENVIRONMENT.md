...
Pins the tab has made are kept as a set, listed in the overlay's dock: the dock's rows reorder them, remove them, and edit a pin's note (✏️). Editing gives the pin a **new id** — the id is a fingerprint of the anchor, note included, and the `review-pins` CLI verifies it against the block's marker — so a comment already pasted keeps the old pin: press "Copy all" and paste it again.
