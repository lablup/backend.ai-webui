...
Pins the tab has made are kept as a set, listed in the overlay's dock: the dock's rows reorder them, remove them, and edit a pin's note (✏️). Editing changes the pin's **id** whenever it changes the note the anchor carries — the id is a fingerprint of the anchor, and the `review-pins` CLI verifies it against the block's marker. `review-pins parse` leaves walkthrough stops out unless `--include-stops` is given, so a walkthrough comment never reads as findings. Only the first `NOTE_MAX` characters ride in the anchor, so an edit past that cap keeps the id while the block still changes. Either way a comment already pasted keeps the old text: press "Copy all" and paste it again.

**Guided mode (a walkthrough).** A `#bai=v3` link whose parts are all **stops** — pins the implementing session authored, carrying what changed (`ch`) and what to check (`ck`) — is not a reviewer's set and does not merge into the draft: it opens as a read-only **walkthrough** in its own `sessionStorage` key, so the reviewer's own pins and the dock's "Copy all" are untouched. Each located stop's element gets a docs-style **mark** (tint by `added`/`modified`, dashed orange outline, ordinal badge, `✓ viewed` / `✎ comment`), a **navigator** pill sits bottom-right (`N pages · M changes │ i / M · k viewed │ ✎ Copy N comments │ ‹ › ☰`), and a popover shows the current stop with its `old → new` line, its GitHub code links and a comment box. `‹ ›` step in set order and cross pages through the app's own router (the host publishes `navigate` on `window.__BAI_REVIEW__`; a document without one falls back to a full reload), and a stop behind a modal stays "waiting" with its `via` sentence until the modal opens. A part the decoder cannot read — what a chat client's truncation leaves behind — does not veto the mode: the link opens on the stops that survived, and the banner and the `☰` panel say how many were lost. Keys are read by `e.code` so an IME cannot eat them: `n`/`]`, `p`/`[`, `v` viewed, `m` comment, `c` ref, `Esc`. Viewed ticks and comments live in `localStorage` keyed by the head the stops were minted for; `✎ Copy N comments` writes one ordinary `bai-review` block per comment — the stop's anchor with every stop field (`ck`, `ch`, `code`, …) stripped and the comment as its note, so `pnpm run review-pins parse` counts them among the findings it answers instead of skipping them as stops — each carrying a `re: stop k · <id>` line that names the stop it answers. `c` (`Copy ref`) writes the same stripped ordinary pin for the stop under the cursor.

`/__review/state` (GET only, no parameters) answers `{pr, repo, branch, source, root, head}`. `pr` / `repo` / `branch` / `source` are the write side's, so a copied block can carry the PR number. `root` is `git rev-parse --show-toplevel` on the box, and it is what the client strips off react-grab's source paths: a workspace package lives above the Vite root and is served through `/@fs/<absolute path>`, so without it a stack frame would carry the driver's home directory into a public PR comment. When the endpoint does not answer, the client drops the source location instead of printing it. `head` is `git rev-parse HEAD` on the box — the boot record carries no sha — and it is what the walkthrough banner compares against the head its stops were minted for.
