...
- [ ] `NotificationState`에 `origin`과 `category` 필드가 추가되어야 한다
...
- [ ] 토스트가 최대 동시 표시 개수(N개, 열린 질문 #1 참고) 이상 표시된 상태에서 "모두 닫기" 버튼이 나타난다
...
6. R4 다중 액션 버튼 도입 시, 기존 `NotificationState`의 `onCancel`/`onRetry` 필드를 새 액션 배열로 통합할 것인가, 아니면 병행 유지할 것인가?
7. origin=`import` 세션 완료 시 마운트할 vfolder를 어떻게 식별할 것인가? (세션 생성 시점의 mount 목록에서 추출 vs 별도 전달)
