...
─────────────────────────────────────────────────────────
  ↑ Select 오른쪽에 ⓘ 배치. 선택 전 비활성, 선택 후 클릭 → Modal
...
- [ ] 새로고침 버튼
- [ ] "Create Preset" 버튼 → 생성 페이지(`/admin-serving/deployment-presets/new`)로 이동
- [ ] 테이블 행 Actions → Edit 버튼 클릭 시 수정 페이지(`/admin-serving/deployment-presets/:presetId/edit`)로 이동

**생성 / 수정 페이지** — 별도 페이지, `SessionLauncherPage`와 동일한 step 구조:

라우트:
- 생성: `/admin-serving/deployment-presets/new`
- 수정: `/admin-serving/deployment-presets/:presetId/edit`

Step 구성 (수평 `Steps` 컴포넌트 + Previous/Next 네비게이션):

| Step | 필드 |
|---|---|
| 1. Basic Info | `name`(필수), `description`, `runtimeVariantId`(필수, 수정 시 read-only 표시), `imageId`(필수) |
...
- 완료 또는 Cancel 시 목록 탭(`/admin-serving?tab=deployment-presets`)으로 복귀

**삭제**
- [ ] 목록에서 Delete 아이콘 클릭 → `BAIDeleteConfirmModal` (이름 표시 + 단순 확인)
...
- [ ] 생성/수정 페이지도 superadmin만 접근 가능 (비-superadmin 접근 시 목록으로 리디렉트)
- [ ] 일반 사용자는 selector 상세 보기만 접근 가능
