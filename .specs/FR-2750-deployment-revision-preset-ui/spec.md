...
**생성 / 수정 폼** — 섹션 구조:
- 기본 정보: `name`(필수), `description`, `runtimeVariantId`(필수), `imageId`(필수)
- 클러스터: `clusterMode`, `clusterSize`
- 리소스: `resourceSlots`, `resourceOpts`
- 실행 환경: `startupCommand`, `bootstrapScript`, `environ`, `presetValues`
- 배포 기본값: `openToPublic`, `replicaCount`, `revisionHistoryLimit`, `deploymentStrategy`
- 고급: `modelDefinition`
- 수정 폼에만: `rank`

**삭제**
- [ ] preset 이름을 직접 입력해야 삭제 확정 (typed confirmation)
...
1. **Runtime variant 목록 쿼리**: 어느 GQL 쿼리로 가져올지
2. **Image selector**: 기존 컴포넌트 재사용 가능 여부 (`imageId: UUID!` 매핑)
3. **삭제 시 참조 처리**: 사용 중인 deployment revision이 있을 때 백엔드 동작 (cascade vs 거절)
