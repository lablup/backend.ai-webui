...
- [ ] `adminPrometheusQueryPresets` 쿼리를 사용하여 데이터 조회
...
- [ ] Auto Scaling Rule 추가/편집 모달에서 조건 설정 영역에 Segment 컴포넌트를 추가하여 "단일" / "범위 선택" 두 가지 모드를 전환할 수 있도록 함
...
- [ ] `adminPrometheusQueryPresetResult` API를 호출하여 결과값을 조회 (현재 admin 전용)
- TODO: 유저 전용 쿼리 API가 추가되면 일반 사용자도 미리보기를 사용할 수 있도록 반영
...
- [ ] 모든 필수 필드(`name`, `metricName`, `queryTemplate`, `filterLabels`, `groupLabels`)가 입력되어야 생성/수정이 가능하다
...
- [ ] Segment 컴포넌트로 "단일" / "범위 선택" 모드를 전환할 수 있다
...
| 쿼리 | 설명 |
|---|---|
| `adminPrometheusQueryPresets(filter, orderBy, first, last, before, after)` | Preset 목록 조회 (페이지네이션 지원) |
| `adminPrometheusQueryPreset(id)` | 단일 Preset 조회 |
...
**filter_labels**: 메트릭 범위를 필터링하는 레이블 키. `{labels}` 플레이스홀더에 대입된다.
...
**group_labels**: 결과를 그룹화하는 레이블 키. `{group_by}` 플레이스홀더에 대입된다.
...
- Prometheus Query Preset의 권한별 세분화 (현재는 슈퍼어드민만 관리)
- 일반 사용자 전용 쿼리 결과 미리보기 API (현재 admin 전용만 존재. 백엔드에서 유저 전용 API가 추가되면 별도 반영)
