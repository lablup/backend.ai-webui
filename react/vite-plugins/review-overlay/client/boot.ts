/**
 * The overlay, assembled — and the HOST SEAM it is assembled against
 * (ADR 0008). Everything the dev server provides and another page cannot is
 * an `OverlayHost` field with a default, so the same client boots inside this
 * app, inside a built bundle, and inside a browser extension on any site.
 *
 * This module has NO import-time side effects: `main.ts` is the entry that
 * boots, and a second host imports `bootOverlay` and passes its own answers.
 * What the overlay DOES is `main.ts`'s docblock; this file is how it starts.
 */
...
/**
 * Where the server state comes from. The default host reads the built
 * document's embedded block, then asks `/__review/state`; a host with no such
 * endpoint answers from wherever it does know the PR and the repository.
 */
export interface OverlayStateSource {
  /** Read synchronously at boot. `null` when this document carries none. */
  embedded(): ReviewServerState | null;
  /** Awaited only when `embedded()` gave nothing; resolves `null` on failure. */
  fetch(): Promise<ReviewServerState | null>;
}

/** What a host may answer differently; every field defaults to this app's. */
export interface OverlayHostOptions {
  state?: OverlayStateSource;
  /**
   * The fragment the pins are read from. The entry captures it at module
   * evaluation because the SPA's login redirect drops it, and a host that
   * boots later captured it earlier still.
   */
  bootHash?: string;
  /**
   * Whether the overlay may claim keyboard shortcuts on the page at all: the
   * ⌘C/Ctrl+C pick chord, the ⌘⇧H cards chord, and guided mode's bare `n` /
   * `p` / `v` / `m` / `c` / `[` / `]`. Default `true`. A host that does not
   * own the page answers `false` and drives the overlay through the handle
   * instead — `c` is "create" on the sites this overlay now visits.
   */
  pageChords?: boolean;
  /**
   * `false` refuses to `location.assign` a link's own page: a crafted
   * fragment must not move a reader who never asked. Default `true`; the
   * dock's "go" and guided mode's `›` are user-initiated and unaffected.
   */
   ...
   * state (`host: 'static'` means no), which is what a host that is simply
   * not this app would otherwise have to claim to be.
   */
  expectReactGrab?: boolean | undefined;
}

/** The same host with every default filled in. */
...
/** What the host holds the running overlay by. */
export interface OverlayHandle {
  /** Enter pick mode, as the chord does where there is one. */
  startPick(): void;
  cancelPick(): void;
  /** The dock's cards switch, and its chord. */
  toggleCards(): void;
  /** Pins in the draft set right now. */
  pinCount(): number;
}

function resolveHost(options: OverlayHostOptions): OverlayHost {
  return {
    state: options.state ?? {
      embedded: readEmbeddedState,
      fetch: fetchServerState,
    },
    bootHash: options.bootHash ?? location.hash,
    pageChords: options.pageChords ?? true,
    autoNavigate: options.autoNavigate ?? true,
    palette: options.palette ?? 'inherit',
    marker: options.marker ?? '',
    expectReactGrab: options.expectReactGrab,
  };
}

/** This app's own answers, and the base every other host is stated against. */
export const defaultOverlayHost = (): OverlayHost => resolveHost({});

/**
 * Start the overlay, once per document. `null` when it is already running —
 * the flag is what makes a second `/__review/*.js` entry a no-op.
 */
export function bootOverlay(
  options: OverlayHostOptions = {},
): OverlayHandle | null {
  if (window.__baiReviewOverlay) return null;
  window.__baiReviewOverlay = true;
  return boot(resolveHost(options));
}

function boot(host: OverlayHost): OverlayHandle {
  /** The built document carries its own state; a dev server is asked. */
  let serverState: ReviewServerState | null = host.state.embedded();
  /**
   ...
   */
  const expectReactGrab =
    host.expectReactGrab ?? serverState?.host !== 'static';
  /**
  ...
  let pick: {
    ...
    anchor: AnchorV3;
    stack: string[];
    /** Absent whenever react-grab named no component. */
    component: AnchorComponent | undefined;
  } | null = null;
  /**
  ...
  const ui = createOverlayUI(
    ...
    { marker: host.marker, palette: host.palette },
  );
  ...
  const stateReady = serverState
    ? Promise.resolve()
    : // Total on purpose: every gate below awaits this one, including the
      // source-path privacy gate, so a host whose `fetch()` rejects — or
      // throws outright — must not leave them all pending.
      Promise.resolve()
        .then(() => host.state.fetch())
        .then(
          (state) => {
  ...
  async function applyFragment(hash: string) {
    ...
    if (focusId && !set.some((pin) => !pathNeedsChange(pin.anchor, location))) {
      ...
      // A host that refuses to be moved by a fragment takes the same way out
      // as a guarded loop: pin what is here, and let the list offer the rest.
      if (host.autoNavigate && guard.shouldNavigate(focusId, target)) {
        focus.set(focusId);
  ...
  void applyFragment(host.bootHash);

  return {
    startPick: () => picker.start(),
    cancelPick: () => picker.stop(),
    toggleCards,
    pinCount: () => draft.length,
  };
}
