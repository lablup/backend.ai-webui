...
/**
 * `<id>.<anchor>` — the one grammar a `#bai=v3` link may carry. The anchor is
 * bounded on both ends: a hash is untrusted input, and a 40 MB `s` deflates to
 * well under a GitHub comment's size limit, so an unbounded match is a bomb.
 */
