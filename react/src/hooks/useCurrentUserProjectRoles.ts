...
export const deriveProjectAdminIds = (
  ...
  if (fromPermissions.size > 0) {
    return Array.from(fromPermissions);
  }
  return Array.from(fromRoleNames);
};
  ...
  const assignments: MyRolesAssignmentNode[] =
    data.myRolesResult?.ok === true
      ? (data.myRolesResult.value?.edges
          ?.map(
            (edge) => edge?.node as MyRolesAssignmentNode | null | undefined,
          )
          .filter((node): node is MyRolesAssignmentNode => Boolean(node)) ?? [])
      : [];
