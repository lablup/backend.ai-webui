...
const AgentNodesListPage: React.FC = () => {
  ...
  const [queryParams, setQueryParams] = useQueryStates(
    {
      ...
      status: parseAsStringLiteral(['ALIVE', 'TERMINATED']).withDefault(
        'ALIVE',
      ),
    },
    { history: 'replace' },
  ...
  // Compose filter string (status + property filter)
    ...
    <BAIFlex direction="column" align="stretch" gap="sm">
      <BAIFlex justify="between" wrap="wrap" gap="sm">
        <BAIFlex gap={'sm'} wrap="wrap" style={{ flex: 1 }}>
          <BAIRadioGroup
            options={[
              { label: t('agent.Connected'), value: 'ALIVE' },
              { label: t('agent.Terminated'), value: 'TERMINATED' },
            ]}
