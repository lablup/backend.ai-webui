 ...
 *
 * Every surface that links to a resource — in-app links, notification items, a
 * WebMCP `open_resource` tool, the CLI's `webui_path` — builds its URL here, so
 * the param names and view encodings cannot drift apart. The rules are mirrored
 * by `RESOURCE_PATH_CASES` (`resourcePath.fixture.ts` / `.json`), which the CLI
 * reads for parity.
 *
 ...
 * so non-component callers and plain unit tests can use it.
 */
