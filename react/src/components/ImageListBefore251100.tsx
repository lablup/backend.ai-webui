import {
  ImageListBefore251100Query,
  ImageListBefore251100Query$data,
} from '../__generated__/ImageListBefore251100Query.graphql';
import {
  filterEmptyItem,
  filterNonNullItems,
  getImageFullName,
...
import ManageAppsModalBefore25110 from './ManageAppsModalBefore251100';
import ManageImageResourceLimitModalBefore251100 from './ManageImageResourceLimitModalBefore251100';
...
export type EnvironmentImageBefore251100 = NonNullable<
  NonNullable<ImageListBefore251100Query$data['images']>[number]
>;

const ImageListBefore251100: React.FC<{ style?: React.CSSProperties }> = ({
  style,
}) => {
  const { t } = useTranslation();
  const [selectedRows, setSelectedRows] = useState<
    EnvironmentImageBefore251100[]
  >([]);
  const [, { getBaseVersion, getBaseImages, getBaseImage, tagAlias, getTags }] =
    useBackendAIImageMetaData();
  const { token } = theme.useToken();
  const [managingApp, setManagingApp] =
    useState<EnvironmentImageBefore251100 | null>(null);
  ...
  const { images } = useLazyLoadQuery<ImageListBefore251100Query>(
    graphql`
      query ImageListBefore251100Query {
        images {
          id
          ...
          ...ManageImageResourceLimitModalBefore251100_image
          ...ManageAppsModalBefore251100_image
        }
      }
    `,
    {},
  ...
  const columns: Array<ColumnType<EnvironmentImageBefore251100>> =
    filterEmptyItem([
      {
  ...
  return (
    <>
      ...
      <ManageImageResourceLimitModalBefore251100
...
export default ImageListBefore251100;
