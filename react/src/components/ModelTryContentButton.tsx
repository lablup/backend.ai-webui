...
import { FileItem } from './FolderExplorerModal';
import {
  ServiceCreateType,
...
const ModelTryContentButton: React.FC<ModelTryContentButtonProps> = ({
  ...
  const { vfolder_nodes } =
    useLazyLoadQuery<ModelTryContentButtonVFolderNodeListQuery>(
      ...
      `,
      {
        // TODO: Set domain scope. In the admin's aspect, admin can see all model-store folders as well as their own.
        // scopeId: current user domain
  ...
  const { data: folderFiles } = useSuspenseTanQuery<Array<FileItem>>({
    queryKey: [
      ...
      'folderFiles',
      availableVFolder?.row_id,
    ],
    queryFn: () => {
      const folderId = availableVFolder?.row_id;
      if (!folderId) {
        return [];
      }
