...
const ServiceLauncherPageContent: React.FC<ServiceLauncherPageContentProps> = ({
  ...
  const [
    { model, startService, formValues: formValuesFromQueryParams },
    setQuery,
  ] = useQueryParams({
    model: StringParam,
    startService: StringParam,
  ...
  // Auto-start service when startService query param is 'true'
  useEffect(() => {
    if (startService === 'true' && !hasAutoStarted.current) {
      hasAutoStarted.current = true;
      // Wait for form to be ready before auto-starting
      setTimeout(() => {
        handleOk();
      }, 100);
    }
