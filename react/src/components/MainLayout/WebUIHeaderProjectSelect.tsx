...
/**
 * The header's "Project" label + current-project selector, including the
 * "leaving admin mode" confirm flow bound to it.
 *
...
const WebUIHeaderProjectSelect: React.FC = () => {
  ...
  // The band is a REVERSED surface: its content polarity is the opposite of
  // the app's. Mirrors `WebUIHeader`, which owns the same two values for the
  // rest of the band.
  ...
  return (
    <>
      {/* Same reversed-band context as the rest of the header — see
          `WebUIHeader.tsx`. */}
            ...
            modal.confirm({
              ...
              onOk: () => {
                ...
                const fallbackFeature =
                  currentMenuKey &&
                  MENU_KEY_TO_SCOPE_FEATURE[currentMenuKey]?.scope === 'project'
