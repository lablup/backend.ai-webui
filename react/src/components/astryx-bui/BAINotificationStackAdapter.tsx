/**
 ...
 GAP COMPONENT 5/5 (cn-oss-removal / ticket 10) — the thin adapter that turns
...
*/
import type { NotificationState } from '../../hooks/useBAINotification';
...
export interface ToBAINotificationStackItemsOptions {
  ...
  onNavigate?: (notification: NotificationState) => void;
  /** Renders the deferred `node` / `multiStep` bodies, if the host wires them. */
  renderExtra?: (notification: NotificationState) => React.ReactNode;
}

function resolveStatus(notification: NotificationState): BannerStatus {
 ...
 *   `className` -> the antd-only `.ant-notification-notice-description`
 *   margin patch, which has nothing to target any more (P6).
 */
export function toBAINotificationStackItems(
  notifications: Array<NotificationState>,
  { onNavigate, renderExtra }: ToBAINotificationStackItemsOptions = {},
): Array<BAINotificationStackItem> {
  return notifications
    .filter((notification) => notification.open !== false)
    .map((notification) => {
      const legacyMessage = (notification as { message?: React.ReactNode })
        .message;
      const percent = notification.backgroundTask?.percent;
      const isPending = notification.backgroundTask?.status === 'pending';
      return {
        key: notification.key,
        title: notification.title ?? legacyMessage,
