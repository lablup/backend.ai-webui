...
const FileUploadManager: React.FC = () => {
  ...
  const activeUploads = useRef<Map<string, Array<tus.Upload>>>(new Map());

  const cancelUpload = (uploadId: string) => {
    const uploadItems = activeUploads.current.get(uploadId);
    if (uploadItems) {
      _.forEach(uploadItems, (item) => {
        item.abort();
      });
  ...
  useEffect(() => {
    ...
    _.map(uploadRequests, async (request) => {
      ...
      const isFileUpload = files[0].webkitRelativePath === '';
      let uploadPromises: Array<Promise<tus.Upload>> = [];

      if (isFileUpload) {
        uploadPromises = _.map(files, async (file) => {
          ...
          try {
            ...
            const existingUploads = activeUploads.current.get(uploadId) || [];
            existingUploads.push(upload);
            activeUploads.current.set(uploadId, existingUploads);
            upload.start();
            return upload;
          } catch (err: any) {
            onError?.(err);
        ...
        uploadPromises = _.map(files, async (file, index) => {
          const fullPath = [currentPath, file.webkitRelativePath || file.name]
            .filter(Boolean)
            .join('/');
      ...
      await Promise.allSettled(uploadPromises).then((res) => onSuccess?.());
    });
    setUploadRequests([]);
