...
const ResourceGroupSettingModal: React.FC<ResourceGroupCreateModalProps> = ({
  ...
  const INITIAL_FORM_VALUES = filterEmptyValues({
    ...
    pendingTimeout: _.toNumber(schedulerOpts?.pending_timeout),
    retriesToSkip: _.toNumber(schedulerOpts?.config?.num_retries_to_skip),
    ...
    >
      <Suspense fallback={<Skeleton />}>
        ...
        >
          ...
          {!resourceGroup ? (
            <Form.Item label={t('resourceGroup.Domain')} name="domain">
              <DomainSelector />
            </Form.Item>
          ) : null}
          ...
          <Form.Item label={t('resourceGroup.Scheduler')} name="scheduler">
            <Select
              options={[
                {
                  label: 'FIFO',
