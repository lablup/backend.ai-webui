  ...
  const columns = _.map(
    filterOutEmpty<BAIColumnType<AgentNodeInList>>([
      ...
      },
      {
        ...
        render: (_value, agent) => {
          const parsedOccupiedSlots = JSON.parse(agent?.occupied_slots || '{}');
          const parsedAvailableSlots = JSON.parse(
            agent?.available_slots || '{}',
          );
          return (
            <BAIFlex direction="column" gap="xxs">
              {_.map(parsedAvailableSlots, (_v: any, key: string) => {
                if (key === 'cpu') {
      ...
      },
      {
        ...
        render: (_value, agent) => {
          const parsedDisk =
            JSON.parse(agent?.live_stat || '{}')?.node?.disk ?? {};
      ...
      },
      {
        ...
        render: (value) => {
          const plugins: Record<string, any> = JSON.parse(value || '{}');
      ...
      },
      {
        ...
        render: (value) => {
          const hardwareMetadata: Record<string, any> = JSON.parse(
            value || '{}',
          );
      ...
      },
      {
        ...
        render: (value) => {
          ...
          return <BAIText>{value.join(', ')}</BAIText>;
        },
      },
    ]),
