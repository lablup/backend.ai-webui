...
> = ({ queryRef, isRefetching, ...props }) => {
  ...
  const getResourceValue = (
    ...
    const getCurrentValue = () => {
      if (type === 'usage') {
        return processResourceValue(totalOccupied, resource, 'auto');
      }

      const remaining = totalAvailable - totalOccupied;
      return processResourceValue(remaining, resource, 'auto');
    };

    const getTotalValue = () => {
      return processResourceValue(totalAvailable, resource, 'auto');
    };
