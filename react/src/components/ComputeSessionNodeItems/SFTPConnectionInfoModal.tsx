    ...
    >
      <BAIFlex className={styles.description} direction="column" gap="md">
        <Alert
          showIcon
          type="warning"
          message={t('session.SFTPExtraNotification')}
        />
        ...
        <Descriptions title={t('session.ConnectionExample')} column={1}>
          ...
          <Descriptions.Item>
            <SourceCodeViewer
              wordWrap
              language={'shell'}
            >{`rsync -av -e "ssh -i ./id_container -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p ${directAccessInfo?.sshd_ports[0]}" /path/to/source/ work@${directAccessInfo?.public_host}:~/${session?.vfolder_mounts?.[0]}/`}</SourceCodeViewer>
          </Descriptions.Item>
        </Descriptions>
      </BAIFlex>
    </BAIModal>
