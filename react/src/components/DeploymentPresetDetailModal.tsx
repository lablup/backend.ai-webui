...
import type { DeploymentPresetDetailModalImageQuery } from '../__generated__/DeploymentPresetDetailModalImageQuery.graphql';
...
const ImageCanonicalName: React.FC<{ imageId: string }> = ({ imageId }) => {
  'use memo';
  const data = useLazyLoadQuery<DeploymentPresetDetailModalImageQuery>(
    graphql`
      query DeploymentPresetDetailModalImageQuery($id: ID!) {
        imageV2(id: $id) {
          identity {
            canonicalName
          }
...
interface DeploymentPresetDetailModalProps extends BAIModalProps {
  presetFrgmt: DeploymentPresetDetailModalFragment$key;
}
