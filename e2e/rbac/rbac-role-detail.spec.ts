...
test.describe(
  ...
  () => {
    ...
    test('Drawer shows "Role Assignments" and "Permissions" tabs', async ({
      ...
      // 4. Verify two tabs are visible: "Role Assignments" and "Permissions"
      await expect(
      ...
      // 5. Verify "Role Assignments" tab is active by default
      await expect(
        drawer.getByRole('tab', { name: 'Role Assignments' }),
      ).toHaveAttribute('aria-selected', 'true');

      // 6. Verify the Assignments tab content area is visible
...
test.describe.serial(
  ...
  () => {
    test('Superadmin can add a permission to a role', async ({
      ...
      // 8. Verify the modal has four form fields: Scope Type, Target, Permission Type, Permission
      await expect(addModal.getByLabel('Scope Type')).toBeVisible();

      // 9. Attempt to click OK without filling any fields – verify validation error
      await addModal.getByRole('button', { name: 'Add' }).click();
      await expect(
        addModal.getByText(
          /Please enter Scope Type|Scope Type is required|required/i,
        ),
      ).toBeVisible({ timeout: 5000 });

      // 10. Select "Domain" from the Scope Type dropdown
      // AntD Select renders visible dropdown in .ant-select-dropdown (not :hidden)
      await addModal.getByLabel('Scope Type').click();
      await page
        .locator(
      ...
      // 11. Verify the Target field becomes enabled
      await expect(addModal.getByLabel('Target')).toBeEnabled({
        timeout: 5000,
      });

      // 12. Select a domain from the Target dropdown (e.g., "default")
      await addModal.getByLabel('Target').click();
      await page
        .locator(
          '.ant-select-dropdown:not(.ant-select-dropdown-hidden) .ant-select-item-option',
        )
      ...
      // 13. Verify the Permission Type field becomes enabled
      await expect(addModal.getByLabel('Permission Type')).toBeEnabled({
        timeout: 5000,
      });

      // 14. Select "Project" from the Permission Type dropdown
      // (When scope is Domain, the available entity types are Project and User, not Domain)
      ...
      await expect(
        entityTypeOptions.filter({ hasText: 'Project' }).first(),
      ).toBeVisible({ timeout: 5000 });
      await entityTypeOptions.filter({ hasText: 'Project' }).first().click();

      // 15. Select a permission (operation) from the Permission dropdown
    ...
    test('Superadmin can delete a permission from a role', async ({
      ...
      await expect(
        entityTypeOpts.filter({ hasText: 'Project' }).first(),
      ).toBeVisible({ timeout: 5000 });
      await entityTypeOpts.filter({ hasText: 'Project' }).first().click();
      await expect(addModal.locator('#operation')).toBeEnabled({
        timeout: 5000,
      });
      await addModal.locator('#operation').click();
      ...
      const permissionRow = drawer.locator('.ant-table-row').first();
...
test.describe.serial(
  ...
  () => {
    test('Superadmin can assign a user to a role', async ({
      ...
      // 5. Verify the "Role Assignments" tab is active by default
      await expect(
        drawer.getByRole('tab', { name: 'Role Assignments' }),
      ).toHaveAttribute('aria-selected', 'true');
    ...
    test('Superadmin sees empty state in Role Assignments tab when role has no users', async ({
      ...
      // 5. Verify the "Role Assignments" tab is active (default)
      await expect(
        drawer.getByRole('tab', { name: 'Role Assignments' }),
      ).toHaveAttribute('aria-selected', 'true');
