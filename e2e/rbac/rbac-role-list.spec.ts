...
test.describe(
  ...
  () => {
    ...
    test.fixme('Superadmin can filter roles by Source (SYSTEM or CUSTOM)', async ({
      page,
      request,
    }) => {
      // 1. Login as admin
      ...
      // TODO(needs-backend): Applying Source filter causes a 400 Bad Request from the GraphQL API.
      // The backend does not currently support the source filter query parameter in this format.
      // When the search button is clicked with source=SYSTEM, the page shows an error boundary
      // ("An error has occurred.") instead of filtered results.
      await page.getByRole('button', { name: 'search' }).click();
