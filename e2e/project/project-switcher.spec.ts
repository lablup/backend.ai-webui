...
// scenarios: 1 (Project Switcher Dropdown Contents),
//            2 (Switching Project Scopes Data)
//
// Scope: what the switcher's *dropdown* lists, and that choosing a different
// project actually re-scopes the data. The plan's two other scenarios are
// deliberately not here: bare "the top bar shows the current project name"
// is subsumed by the dropdown tests (which assert the active project carries
// `aria-selected`), and selection persistence across navigation is already
// covered by e2e/admin-scope/admin-header-project-selector.spec.ts.
//
// Project names, project counts, and folder names are all account/cluster-
// specific data -- an earlier version of this file hardcoded them, which
// silently breaks on any account or cluster where that exact data doesn't
// hold. Every test below instead reads the logged-in account's actual
// project membership from the client (`current_group` / `groups`, the same
// source components read via `useCurrentProjectValue()` /
// `baiClient.current_group` -- see feature-gate-util.ts) and asserts the UI
// matches that real data. The data-scope test goes further: instead of
// asserting a specific expected folder count/name per project, it compares
// the *set* of visible folder names before and after switching, which proves
// re-scoping happened without needing to know or create any specific folder.
...
// default 5s expect timeout under parallel load. Apply this explicitly to
// assertions that follow a navigation or a project switch instead of relying
// on the default.
...
test.describe(
  ...
  () => {
    ...
    // -----------------------------------------------------------------------
    // 1. Project Switcher Dropdown Contents
    // -----------------------------------------------------------------------
    ...
    test.describe('Switching Project Scopes Data', () => {
      test(
        ...
        async ({ page, request }) => {
          ...
          // Still the Data page. Only the *page* is asserted, not the whole
          // URL: on project scope `useSwitchProject` (FR-3428) deliberately
          // swaps the `:projectName` segment in place, so the URL does change
          // across a switch -- see
          // `react/src/components/MainLayout/WebUIHeaderProjectSelect.tsx`.
