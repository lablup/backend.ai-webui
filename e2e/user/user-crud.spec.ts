...
test.describe.serial('User CRUD Operations', () => {
  ...
  async function cleanupTestUser(page: Page) {
    ...
    if (isVisible) {
      ...
      // Wait for user to disappear from active list
    ...
    if (isInactiveVisible) {
      ...
      // Wait for user to disappear from inactive list after deletion
  ...
  test('Admin can deactivate a user', async ({ page, request }) => {
    ...
    // 9. Switch to "Inactive" filter and verify the deactivated user appears there
    await page.getByText('Inactive', { exact: true }).click();
    await expect(page.getByRole('cell', { name: EMAIL })).toBeVisible({
      timeout: 10000,
    });
  ...
  test('Admin can reactivate an inactive user', async ({ page, request }) => {
    ...
    // 9. Switch to "Active" filter and verify the activated user appears there
    await page.getByText('Active', { exact: true }).click();
    await expect(page.getByRole('cell', { name: EMAIL })).toBeVisible({
      timeout: 10000,
    });
