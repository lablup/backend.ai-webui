...
test.describe('Session page Visual Regression Test', () => {
    ...
    await expect(page).toHaveScreenshot('session_page.png', {
      mask: [
        page
          ...
          .filter({ hasText: 'Allocated Resources' })
          .first(),
      ],
      fullPage: true,
  ...
  test('Create a new session step by step', async () => {
    ...
    await page.getByText('default', { exact: true }).nth(1).click();
    await page.getByTitle('default').nth(2).click();
    await page.getByText('cpu01-small1Core1GiB').click();
    ...
    await expect(page).toHaveScreenshot('step2.png', {
      ...
      mask: [
        ...
        page
          .locator(
            'div:nth-child(3) > div:nth-child(2) > div > .ant-row > div:nth-child(2) > .ant-form-item-control-input > .ant-form-item-control-input-content > .ant-select > .ant-select-selector',
          )
          .first(),
