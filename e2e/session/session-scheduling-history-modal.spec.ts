...
test.describe(
  ...
  () => {
     ...
     */
    async function openSessionDetailDrawer(page: any) {
     ...
     */
    async function openSchedulingHistoryModal(page: any) {
    ...
    test('Admin can see the scheduling history button in session detail when backend supports the capability', async ({
      ...
      test.fixme(
        true,
        'Requires at least one existing session and backend support for session-scheduling-history capability',
      );
    ...
    test('Admin can open the Session Scheduling History modal and see correct column headers', async ({
      ...
      // 4. Verify the property filter is visible at the top left
      const propertyFilterSelector = modal
        .locator('.ant-space-compact')
        .first();
      await expect(propertyFilterSelector).toBeVisible();
    ...
    test('Admin can filter history records by Phase using the property filter', async ({
      ...
      test.fixme(
        true,
        'Requires at least one scheduling history record to verify filtering behavior',
      );
    ...
    test('Admin can filter history records by Result enum using the property filter', async ({
      ...
      // 3. Select "SUCCESS" from the value options
      const valueCombobox = modal
        .locator('.ant-space-compact .ant-select')
        .last()
        .locator('[role="combobox"]');
    ...
    test('Admin can remove an applied filter to restore the full history list', async ({
      ...
      test.fixme(
        true,
        'Requires at least one scheduling history record to verify filter removal behavior',
      );
      ...
      await filterTag.locator('.ant-tag-close-icon').click();
    ...
    test('Admin can see history records displayed in the scheduling history table', async ({
      ...
      test.fixme(
        true,
        'Requires at least one scheduling history record for the selected session',
      );
    ...
    test('Admin can sort the history table by the CreatedAt column', async ({
      ...
      test.fixme(
        true,
        'Requires multiple history records with different createdAt values to verify sort order changes',
      );
    ...
    test('Admin can sort the history table by the UpdatedAt column', async ({
      ...
      test.fixme(
        true,
        'Requires multiple history records with different updatedAt values to verify sort order changes',
      );
    ...
    test('Admin sees the history table sorted by CreatedAt ascending by default on modal open', async ({
      ...
      test.fixme(
        true,
        'Requires multiple history records with different createdAt values to verify default sort order',
      );
