...
| 5 | ~~Medium~~ RESOLVED (sanctioned) | Admin → Configurations, Statistics | Info `Alert`s paint a cool blue inside a warm brand surface | The blue IS the legacy applied colour: `resources/theme.json` `colorInfo` = `#028DF2` light / `#009BDD`→`#0387bf` dark, whose antd ramp gives `colorInfoBg #e6f9ff` / `#111f27` (measured via `theme.getDesignToken()`). It only looked accidental because nothing declared it | Resolved via semantic theme pinning — `info` is now a first-class brand seed in `backendAiTheme.ts` (options + `themeOptionsFromConfig` + name hash), so the blue is theme-declared and follows a theme.json rebrand. Astryx ships **no** `--color-info*` token (`CoreTokenName` has error/success/warning only), so there is nothing further to pin; the antd-engine `Alert` reads the same seed through the shim. See `POLISH-2.md` §F |
...
| 11 | ~~Low~~ RESOLVED | Sider group titles | Titles sit 16px from the rail edge; item labels sit at 32px — misaligned | Not addressable from `components` (see Defect 2 note) | Fixed in the sider-polish pass — see `SIDER-FIXES.md` §3 (scoped `BAISider.css` rule, justified in-file) |
