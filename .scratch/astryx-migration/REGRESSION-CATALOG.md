...
| `admin/environment` | **R-3** The `Full image path` column renders **empty** for every row — only the copy-to-clipboard icon is present. See `a-environment-light.png`. Not a token or layout issue. **FIXED** — the column is a computed (`dataIndex`-less) one whose `render` read the record off its *first* argument, an rc-table quirk `BAITableAstryx` deliberately does not emulate; the call sites now use `render: (_value, row) => …`. Two more sites carried the same defect (`CustomizedImageList`, and `AdminUserCredentialList`'s `Allocation`, which was throwing rather than blanking). See `issues/approved-2-render-contract.md` and `CONVERSION-IDIOMS.md` §2. | **C** | Med |
