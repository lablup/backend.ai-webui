// Ticket 09 — single-browser before/after capture for the mass theme-shim
// application. Same method as measure-03-pixel-ab.mjs (single chromium
// process, flag-file handshake, frozen animations); adds a third view.
// Handshake: writes .ready-for-swap after the before capture, then waits
// for .swapped (bash swaps the working tree state in between via git stash).
import { chromium } from '@playwright/test';
import { existsSync, readFileSync, writeFileSync, rmSync } from 'node:fs';

// Run from repo root: node .scratch/astryx-migration/shots/measure-09-pixel-ab.mjs
const OUT = process.env.SHOT_DIR ?? '/tmp/shim09-shots';
...
const capture = async (label) => {
  // Warm-up pass: visit every view once in a throwaway context so the dev
  // server has already transformed all modules before the measured loads.
  // Without this, the first post-swap load is slow (vite re-transforms the
  // swapped modules) and the splash weave layer accumulates a different
  // animation/raster history, which shifts its whole-layer rasterization
  // (~11.9% false diff — same signature ticket 03 saw across launches).
  {
