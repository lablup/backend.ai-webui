...
- **Tool Requirements**:
  ...
  - **Walkthrough**: after `dev-server` has advertised the dev server for a PR you implemented, run the `walkthrough` skill (`.claude/skills/walkthrough/`) as the last step. It mints the PR's review stops against that server, upserts one walkthrough comment, and gives you the `[Walkthrough](<set link>) · N stops` line to add to the final message — a markdown link, so chat and PR bodies render it short.
