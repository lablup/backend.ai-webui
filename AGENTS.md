...
- **Tool Requirements**:
  ...
  - **Git/PR**: Use Graphite MCP (`mcp__graphite__run_gt_cmd`) for branch/commit/push
    ...
    - **Open and update PRs with `gt submit` — never `git push` + `gh pr create`.** A PR pushed that way still gets the right GitHub base ref, so it *looks* correct in `gh` and in the GitHub UI, but Graphite never learns the parent and renders it as an isolated PR with no stack. `gt track` alone does not fix this: it only writes local metadata. Only `gt submit` sends the parent relationship to Graphite.
    - **Verify the stack right after submitting** — the failure above is invisible unless you look. Check *both*, because either can be right while the other is wrong:
      - `gt log short` — the local parent chain
      - `gh pr view <N> --json baseRefName` — the GitHub base ref of every PR you touched
    - **Repairing a PR that was already opened with `git push` + `gh pr create`**: `gt track --parent <parent-branch>`, then `gt submit --stack --update-only --no-edit --always`. Run it with `--dry-run` first and read the plan — every branch from trunk up to the current one is listed, and branches that aren't yours must say `(No-op)`. `--update-only` keeps it from opening PRs you did not intend, and `--always` is the documented flag for repairing an inconsistent Graphite stack view. Confirm `git rev-parse <branch>` matches `origin/<branch>` for any branch you do not own before submitting, so the force-push cannot rewrite someone else's PR.
