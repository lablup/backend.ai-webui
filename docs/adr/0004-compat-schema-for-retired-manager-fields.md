# 0004 — Compat schema for retired manager fields
...
| 26.8.3 | 26.9.0 |
|---|---|
| `Role.scopes` connection | `Role.scopeType`, `scopeId`, `scope` |
...
- **First failure site**: `useCurrentUserProjectRoles`가 `Role.scopes`를 select한다. 이 hook을 `useRouteAccess`, `useWebUIMenuItems`, `WebUIHeaderProjectSelect`와 folder component 네 곳(`ProjectSelect`, `VFolderNodes`, `EditableVFolderNameV2`, `VFolderNodeDescriptionV2`)이 부른다. 26.9 매니저가 이 요청을 거부하면 route guard와 사이드바와 헤더가 error boundary로 떨어진다. `/admin/rbac`의 목록 query는 drawer의 tab fragment를 함께 컴파일하므로 같은 이유로 거부된다.
...
- **Location**: `data/compat/rbac-manager-26-8.graphql`이며, 옛 supergraph(backend.ai 태그 26.8.3)에서 복사한 정의만 담는다. 통째로 지워진 type은 그대로 적고, 살아남은 type에서 지워진 field는 `extend type|input <이름> { … }` block으로 적는다.
...
- **Directive pair**: 옛 field에는 `@deprecatedSince(version: V)`, 새 field에는 `@since(version: V)`를 단다. V는 매니저가 그 field를 지우거나 더한 버전이다.
...
- **Reading**: component는 자기 fragment 안에서 새 field 값이 있으면 그것을, 없으면 옛 field를 읽는다. 공유 helper는 없다. `RoleNodes.tsx`의 `readRoleScope`가 이 순서로 `scopeType`과 `scopes.edges[0]`를 고른다.
- **Fallback type**: generated type은 옛 field를 `RBACElementType` union으로, 새 field를 `string`으로 만든다. 두 값을 합치는 표현식은 `string`으로 다룬다.
...
- **Flag**: `client.ts`의 `isManagerVersionCompatibleWith('26.9.0')` block이 `rbac-single-scope-role`을 켠다. client directive는 `on FIELD`라 argument 값은 가르지 못하므로 component가 flag로 값을 고른다.
- **Consumers**: flag를 읽는 component와 그 값이다.

| component | flag가 켜졌을 때 | flag가 꺼졌을 때 |
|---|---|---|
| `RoleFormModal` | `CreateRoleInput.scope` | `CreateRoleInput.scopes`, 항목 하나 |
| `useCurrentUserProjectRoles` | permission entity `scope_admin` | permission entity `PROJECT_ADMIN_PAGE` |
