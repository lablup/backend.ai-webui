...
- **Table cluster waits**: `BAITable`, `tableTypes`, `BAITableSettingModal`, `BAITableColumnCSVExportModal`, `BAIPaginationInfoText`, `BAIBulkErrorModal`은 BUI에 남는다. `BAITableSettingModal`이 drag handle 색으로 읽는 `colorTextTertiary`(verdict `self`)와 `BAIBulkErrorModal`이 경고 icon 색으로 읽는 `colorError`(verdict `brand`)는 Astryx token으로 바꿀 수 없으므로 이 cluster는 theme-shim track을 기다린다. 나머지 read(`paddingSM`, `marginSM`, `marginXS`, `paddingXXS`, `colorText`)는 모두 verdict `astryx`다 (FR-4087).
