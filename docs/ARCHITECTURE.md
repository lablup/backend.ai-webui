...
| [0005 — Compat schema for retired manager fields](adr/0005-compat-schema-for-retired-manager-fields.md) | `data/schema.graphql`은 supergraph 복사본 그대로 두고, 새 매니저가 지운 type과 field는 `data/compat/`에 두어 `relay-base.config.js`가 merged schema에 접어 넣는다. document는 옛 field를 `@deprecatedSince`, 새 field를 `@since`로 함께 select하고, input의 shape는 `backend.ai-client`의 feature flag로 고른다. |
