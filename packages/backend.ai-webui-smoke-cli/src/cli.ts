...
program
  ...
  .addOption(
    new Option(
      "--include <tags>",
      "(deprecated) alias for --also-include.",
    ).hideHelp(),
  )
  .addOption(
  ...
  .option(
    "--headed",
    "Run the browser in headed mode (debugging only). Env: BAI_SMOKE_HEADED=1",
    false,
  )
  .option(
    "--insecure-tls",
    "Accept self-signed TLS certificates. Env: BAI_SMOKE_INSECURE_TLS=1",
    false,
  )
  .action(async (raw: Record<string, unknown>) => {
    ...
    const opts: SmokeRunOptions = {
      ...
      workers:
        typeof raw.workers === "number" && raw.workers > 0
          ? raw.workers
          : undefined,
      timeoutMs,
      outputDir,
      ...
      insecureTls:
        raw.insecureTls === true || envBool("BAI_SMOKE_INSECURE_TLS"),
    };
    ...
    if (opts.include && opts.include.length > 0) {
      const excluded =
        opts.role === "auto"
          ? "the other role"
          : `@smoke-${opts.role === "user" ? "admin" : "user"}`;
      process.stderr.write(
        `[bai-smoke] note: tests tagged ${excluded} stay excluded from this run, ` +
    ...
    process.stdout.write(
      `  role      : ${summary.role} (selection: ${summary.roleSelection})\n`,
    );
    if (summary.results) {
...
async function resolvePassword(
  ...
  // An explicit --password-stdin wins over a BAI_SMOKE_PASSWORD left in the env.
