...
export async function detectRole(opts: SmokeRunOptions): Promise<EffectiveRole> {
  ...
  //
  // Scoped to THIS request: a per-call `undici` dispatcher, not the
  // process-wide `NODE_TLS_REJECT_UNAUTHORIZED` knob. Mutating that env var
  // would disable certificate validation for every other TLS client in the
  // host process for as long as the request is in flight. The same
  // relaxation is forwarded separately to the spawned Playwright process via
  // `buildPlaywrightEnv`, where a whole-process scope is the intent.
  const dispatcher = opts.insecureTls
  ...
  try {
    ...
    try {
      ...
      if (dispatcher) {
        // `@types/node` declares fetch's `dispatcher` against its own bundled
        // `undici-types@6`; ours comes from `undici@7`. Same class at runtime,
