...
const pages = pagesEnv
  ? pagesEnv
      ...
      .filter(Boolean)
  : undefined;
// `*.spec.ts` suffix: `e2e/session/mocking/` holds fixture modules that are not
...
export default defineConfig({
  ...
  use: {
     ...
     * sync with the root `playwright.config.ts`). 30s is 3x the largest
     * explicit per-action timeout in the suite, so it never cuts a
     * legitimately-slow action short.
     */
    actionTimeout: 30000,
  },
  projects: [
