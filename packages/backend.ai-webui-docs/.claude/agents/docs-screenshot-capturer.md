...
```bash
cp .playwright-mcp/./src/{lang}/images/{file}.png \
   ./src/{lang}/images/{file}.png
```
...
- `./SCREENSHOT-GUIDELINES.md` - Naming conventions, capture standards, file locations
- `./DOCUMENTATION-STYLE-GUIDE.md` - How images are referenced in documentation
- `./TERMINOLOGY.md` - Feature names and UI terminology

## Context

- **Documentation images**: `./src/{lang}/images/`
- **All 4 language directories have identical filenames** but each must be captured in its own UI locale
- **Environment variables**: `E2E_USER`, `E2E_PASSWORD`
...
#### Mode A: TODO Markers
```bash
grep -r "TODO.*screenshot\|TODO.*Capture\|TODO.*capture" ./src/en/ --include="*.md"
```
...
1. Open the application URL using `browser_open` (`http://localhost:9081`)
...
```bash
for lang in en ja ko th; do
  cp .playwright-mcp/./src/${lang}/images/*.png \
     ./src/${lang}/images/
done
```
...
| `/environment` | Environments (Admin) |
| `/agent` | Resources (Admin) |
