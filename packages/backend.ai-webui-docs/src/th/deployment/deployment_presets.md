...
หน้าการปรับใช้ของผู้ดูแลระบบมีหลายแท็บ ได้แก่ **การปรับใช้**, **การจัดการร้านค้าโมเดล**, **Prometheus Preset** และ **ค่าตั้งล่วงหน้าการปรับใช้** แท็บที่คุณกำลังเปิดอยู่จะถูกบันทึกไว้ใน URL ของหน้า และตัวกรอง ลำดับการเรียง รวมถึงหมายเลขหน้าของแต่ละแท็บก็ถูกเก็บไว้ใน URL เช่นกัน ซึ่งให้ประโยชน์สองข้อ:

- เมื่อสลับไปแท็บอื่นแล้วกลับมา แท็บพรีเซ็ตการดีพลอยจะกลับสู่สภาพเดิมที่คุณทิ้งไว้ แทนที่จะรีเซ็ตเป็นหน้าแรกที่ไม่มีตัวกรอง
- คุณสามารถบุ๊กมาร์กหรือแชร์ URL ของรายการพรีเซ็ตที่กรองไว้ได้ และผู้รับจะเปิดมาที่แท็บเดียวกันพร้อมตัวกรอง ลำดับการเรียง และหน้าเดียวกัน

สถานะตัวกรองและการแบ่งหน้าของแท็บหนึ่งจะไม่รั่วไหลไปยังอีกแท็บหนึ่ง ดังนั้นตัวกรองที่คุณตั้งไว้ในแท็บ **การปรับใช้** จะไม่ตามไปที่แท็บ **ค่าตั้งล่วงหน้าการปรับใช้**
...
2. กรอกฟิลด์ต่าง ๆ ฟอร์มนี้เป็นตัวช่วยสามขั้นตอน ได้แก่ **ข้อมูลพื้นฐาน**, **โมเดล & การประมวลผล** และ **การทบทวน** โดยมีรายการขั้นตอนอยู่ทางขวาและปุ่ม `ก่อนหน้า` / `ถัดไป` อยู่ด้านล่าง ใช้ `ข้ามไปยังการทบทวน` เพื่อไปยังขั้นตอนสุดท้ายได้ทันที ฟิลด์จัดเป็นส่วนต่าง ๆ ดังนี้:
   ...
   - **การเรียกใช้งาน**: **คำสั่งเตรียมสภาพแวดล้อม (Startup Command)**, environment variables และ bootstrap script ฟิลด์คำสั่งเตรียมสภาพแวดล้อมจะแสดงคำใบ้ไวยากรณ์เชลล์ (`ไวยากรณ์เชลล์: /bin/bash -c "cmd1; cmd2"`) เนื่องจากคำสั่งจะถูกเรียกใช้งานในรูปแบบ `/bin/bash -c <command>` คุณจึงสามารถใช้ตัวดำเนินการของเชลล์ เช่น `;`, `|` และ `&&` ได้โดยตรง
      ...
      :::note[คำสั่งเตรียมสภาพแวดล้อมไม่ใช่คำสั่งเริ่มต้น]
      คำสั่งทั้งสองเป็นคนละอย่างกัน และตอนนี้แต่ละฟิลด์มีคำอธิบายของตัวเอง จึงแยกความแตกต่างได้ง่ายขึ้น
      ...
      | **คำสั่งเริ่มต้น** | ส่วนคำจำกัดความโมเดลของพรีเซ็ต | *"คำสั่ง CLI สำหรับเริ่มต้นกระบวนการให้บริการโมเดล"* | `vllm serve /models --tp 2` |

      ใช้ **คำสั่งเตรียมสภาพแวดล้อม** สำหรับงานเตรียมการ เช่น การติดตั้งแพ็กเกจ การดึงไฟล์ประกอบ หรือการเขียนไฟล์การตั้งค่า และใช้ **คำสั่งเริ่มต้น** สำหรับคำสั่งที่เปิดใช้งานเฟรมเวิร์กให้บริการจริง ข้อความตัวอย่างในแต่ละฟิลด์ก็แสดงตัวอย่างของคำสั่งประเภทที่ถูกต้อง
      :::
   ...
   - **คำจำกัดความโมเดล (Model Definition)** (ตัวเลือกเสริม): เปิดใช้งานด้วย toggle เพื่อกำหนดค่า model definition แบบมีโครงสร้าง ภายในส่วนนี้มีกลุ่ม **การตรวจสอบสุขภาพ (Health Check)** ที่มี toggle **เปิดใช้การตรวจสอบสุขภาพ (Enable Health Check)** ซึ่งตั้งค่าเริ่มต้นเป็น **ปิด (OFF)** เมื่อปิดอยู่ ฟิลด์การตรวจสอบสุขภาพจะถูกซ่อนไว้ เมื่อเปิด toggle นี้ ฟิลด์ทั้งหมดจะปรากฏขึ้น ได้แก่ path, interval, จำนวนครั้งสูงสุดที่ลองใหม่, เวลารอสูงสุด, status code ที่คาดหวัง และ startup grace period
...
:::tip
หากฟิลด์ที่จำเป็นว่างเปล่าหรือมีค่าที่ไม่ถูกต้อง ปุ่มยืนยันในขั้นตอนการทบทวนจะยังคงถูกปิดใช้งานจนกว่าจะแก้ไขข้อผิดพลาด และการ์ดที่มีฟิลด์ดังกล่าวจะถูกตีกรอบสีแดง ฟิลด์ที่จำเป็นจะแสดงข้อความตรวจสอบความถูกต้องในตำแหน่งของฟิลด์ขณะที่คุณพิมพ์
:::
