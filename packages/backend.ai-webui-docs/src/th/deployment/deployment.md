...
- **เปิดใช้งานการตรวจสอบสุขภาพ (Enable Health Check)**: เมื่อเปิดใช้งาน ระบบจะส่งคำขอ HTTP ไปยัง inference server เป็นระยะเพื่อตรวจสอบว่าตอบสนองอย่างถูกต้อง เมื่อปิดใช้งาน (ค่าเริ่มต้นสำหรับ Revision ใหม่) ระบบจะไม่มีการตรวจสอบสุขภาพและ replica ที่ไม่แข็งแรงจะไม่ถูกตรวจพบโดยอัตโนมัติ ควรเปิดใช้งานสำหรับ deployment ที่ใช้งานจริง เมื่อเลือก **เปิดใช้งานการตรวจสอบสุขภาพ** จะมีฟิลด์เพิ่มเติมดังนี้:
   * **URL ตรวจสอบสุขภาพ (Health Check URL)**: เส้นทาง HTTP endpoint ที่เรียกในระหว่างการตรวจสอบสุขภาพของบริการ (ค่าเริ่มต้น: `/health`)
   * **ระยะผ่อนผันการเริ่มต้น (Startup Grace Period)**: ระยะผ่อนผัน (วินาที) หลังจากคอนเทนเนอร์เริ่มทำงานที่การตรวจสอบสุขภาพที่ล้มเหลวยังได้รับการยอมรับ เรพลิกาจะเปิดใช้งานเมื่อการตรวจสอบสำเร็จครั้งแรก (ค่าเริ่มต้น: `60.0`) ให้ตั้งค่าสูงขึ้นสำหรับโมเดลขนาดใหญ่
   * **จำนวนครั้งสูงสุดที่ลองใหม่ (Max Retries)**: จำนวนครั้งสูงสุดที่การตรวจสอบสุขภาพล้มเหลวติดต่อกันก่อนที่เรพลิกาจะถูกทำเครื่องหมายว่า `UNHEALTHY` (ค่าเริ่มต้น: `10`)
   * **ช่วงเวลา (Interval)**: เวลาระหว่างการตรวจสอบสุขภาพที่ต่อเนื่องกัน (วินาที) (ค่าเริ่มต้น: `10.0`)
   * **เวลารอสูงสุด (Max Wait Time)**: เวลาหมดอายุ (วินาที) สำหรับคำขอ HTTP ตรวจสอบสุขภาพแต่ละครั้ง (ค่าเริ่มต้น: `15.0`)
   * **รหัสสถานะที่คาดหวัง (Expected Status Code)**: รหัสสถานะ HTTP response ที่บ่งชี้ว่าบริการมีสุขภาพดี (ค่าเริ่มต้น: `200`)
...
บน backend ที่เก่ากว่า Backend.AI Manager 26.4.4 พารามิเตอร์ทั้งหมดจะถือว่าเป็นตัวเลือกโดยไม่คำนึงถึงการกำหนดค่าของผู้ดูแลระบบ
:::
...
- **URL ตรวจสอบสุขภาพ (Health Check URL)**: เส้นทาง HTTP endpoint ที่ระบบจะเรียกเพื่อตรวจสอบสุขภาพบริการ
- **ระยะผ่อนผันการเริ่มต้น (Startup Grace Period)**: วินาทีที่รอหลังจากคอนเทนเนอร์เริ่มทำงานก่อนที่การตรวจสอบสุขภาพที่ล้มเหลวจะนับต่อเรพลิกา (ค่าเริ่มต้น: `60.0`)
- **จำนวนครั้งสูงสุดที่ลองใหม่ (Max Retries)**: จำนวนครั้งที่ล้มเหลวติดต่อกันที่อนุญาตก่อนที่เรพลิกาจะถูกทำเครื่องหมายว่า `UNHEALTHY` (ค่าเริ่มต้น: `10`)
- **ช่วงเวลา (Interval)**: วินาทีระหว่างการตรวจสอบ (ค่าเริ่มต้น: `10.0`)
- **เวลารอสูงสุด (Max Wait Time)**: เวลาหมดอายุต่อคำขอ (วินาที) (ค่าเริ่มต้น: `15.0`)
- **รหัสสถานะที่คาดหวัง (Expected Status Code)**: รหัสสถานะ HTTP ที่บ่งชี้ว่า response มีสุขภาพดี (ค่าเริ่มต้น: `200`)
...
แท็บนี้มีตัวควบคุมดังต่อไปนี้:

- **แถบตัวกรอง**: กรองรายการล็อกตาม **สถานะ (Status)**, **การดำเนินการ (Operation)**, **ทริกเกอร์โดย (Triggered By)** (ค้นหาด้วย user ID) และตัวเลือกช่วงวันที่ **เวลา (Time)**
- **ปุ่มรีเฟรชอัตโนมัติ**: โหลดล็อกใหม่โดยไม่ต้องออกจากแท็บ
- **การแบ่งหน้า**: ไปยังรายการล็อกทีละหน้า

:::note
แท็บบันทึกการตรวจสอบใช้การโหลดแบบ lazy loading — คิวรีจะถูกส่งเฉพาะเมื่อคุณเปิดใช้งานแท็บเป็นครั้งแรก แท็บที่ใช้งานอยู่จะปรากฏใน URL (`?revisionTab=auditLog`) เพื่อให้คุณแชร์ลิงก์ไปยังมุมมองบันทึกการตรวจสอบได้โดยตรง

การเข้าถึงข้อมูลบันทึกการตรวจสอบถูกบังคับใช้โดย backend และอาจถูกจำกัดเฉพาะ superadmin
:::
...
Deployment preset ช่วยให้ผู้ดูแลระบบสามารถบันทึกค่าการกำหนดค่า Revision ที่สมบูรณ์ ได้แก่ runtime variant, container image, การจัดสรรทรัพยากร, start command และการตั้งค่า health check — แล้วนำมาใช้ซ้ำเมื่อเพิ่ม Revision ผู้ใช้เลือก preset ใน **โหมดพรีเซต** เมื่อเพิ่ม Revision ส่วนผู้ดูแลระบบสร้างและแก้ไข preset ผ่านหน้า Deployment Presets

### การจัดการ Deployment Presets

หน้า Deployment Presets แสดงรายการ preset ทั้งหมดที่ใช้งานได้ในโปรเจกต์ปัจจุบัน ตารางประกอบด้วยคอลัมน์ **ที่แสดงตามค่าเริ่มต้น** ดังนี้:

- **ชื่อ (Name)**: ชื่อ preset
- **Runtime**: runtime variant (เช่น `vLLM`, `SGLang`, `Custom`)
- **อิมเมจ (Image)**: container image ในรูปแบบ `<canonicalName>@<architecture>` (เช่น `cr.backend.ai/stable/pytorch:2.1-cuda12.1@aarch64`) ค่านี้สามารถคัดลอกได้ รูปแบบนี้ช่วยให้แยกแยะ image ตาม CPU architecture บนคลัสเตอร์ที่มีสถาปัตยกรรมแบบผสมได้ง่ายขึ้น
- **เรพลิกา (Replicas)**: จำนวนเรพลิกาที่ต้องการซึ่งบันทึกไว้กับ preset นี้
- **สร้างเมื่อ (Created)**: วันที่สร้าง preset
- **แก้ไขเมื่อ (Modified)**: วันที่อัปเดต preset ล่าสุด (`updatedAt`)

ใช้ปุ่มเฟือง **การมองเห็นคอลัมน์** ที่ด้านขวาของส่วนหัวตารางเพื่อแสดงหรือซ่อนคอลัมน์เพิ่มเติม คอลัมน์ต่อไปนี้ **ซ่อนตามค่าเริ่มต้น** แต่สามารถเปิดใช้งานได้:

- **คำอธิบาย (Description)**: ข้อความคำอธิบาย preset
- **Start Command**: start command ที่กำหนดค่าไว้ โดยแสดงแบบย่อพร้อม tooltip ค่านี้สามารถคัดลอกได้
- **คลัสเตอร์ (Cluster)**: cluster mode ที่บันทึกไว้กับ preset
- **กลยุทธ์ (Strategy)**: scheduling strategy
- **เปิดสาธารณะ (Open to Public)**: แสดงแท็ก `สาธารณะ` หรือ `ส่วนตัว` ตามการตั้งค่าการมองเห็นเริ่มต้นของ preset
- **ขีดจำกัดประวัติ Revision (Revision History Limit)**: จำนวน Revision ที่เก็บไว้
- **อันดับ (Rank)**: อันดับการเรียงลำดับของ preset (คอลัมน์ที่สามารถเรียงได้)

### การสร้าง Deployment Preset

คลิกปุ่ม `สร้าง Preset` เพื่อเปิดฟอร์ม **สร้าง Deployment Preset** ฟอร์มประกอบด้วยฟิลด์เดียวกับฟอร์มโหมดขั้นสูงสำหรับเพิ่ม Revision ซึ่งจัดในส่วนเดียวกัน:

- **โมเดลและ Runtime**: เลือก runtime variant สำหรับ `vLLM` หรือ `SGLang` จะมีส่วน **พารามิเตอร์ Runtime** พร้อมแท็บ (Model Loading, Resource Memory, Serving Performance และอื่น ๆ) ปรากฏขึ้น สำหรับ runtime `Custom` จะแสดงฟิลด์ start command และ health check แทน
- **สภาพแวดล้อม**: เลือก container image ดรอปดาวน์จะแสดง image ในรูปแบบ `<canonicalName>@<architecture>` เพื่อให้แยกแยะ image ตาม CPU architecture ได้ รูปแบบเดียวกันนี้จะปรากฏในขั้นตอน Review ของฟอร์มด้วย
- **คลัสเตอร์และทรัพยากร**: ตั้งค่าการจัดสรร CPU, หน่วยความจำ และ accelerator
- **การตั้งค่าขั้นสูง** *(ย่อ/ขยายได้)*: เมาต์โฟลเดอร์จัดเก็บเพิ่มเติม

:::note[พารามิเตอร์ Runtime ที่จำเป็น]
เมื่อเลือก runtime `vLLM` หรือ `SGLang` ผู้ดูแลระบบสามารถกำหนดให้พารามิเตอร์ runtime แต่ละตัวเป็นแบบจำเป็น (แสดงด้วยเครื่องหมายดอกจัน ★ สีแดง) การส่งฟอร์ม preset โดยมีพารามิเตอร์ที่จำเป็นว่างเปล่าจะถูกปิดกั้นพร้อมข้อผิดพลาดการตรวจสอบแบบ inline กฎการตรวจสอบนี้ยังใช้กับการเพิ่ม Revision จาก preset ด้วย
:::

ค่าพารามิเตอร์ Runtime ที่กำหนดค่าไว้จะถูกบันทึกพร้อมกับ preset และนำไปใช้เมื่อสร้างการปรับใช้จาก preset นี้ พารามิเตอร์ที่ไม่ได้เปลี่ยนแปลงจะคงค่าเริ่มต้นของ runtime ไว้ ค่าที่กำหนดค่าไว้ยังปรากฏในขั้นตอน **Review** ใต้ **ข้อมูลพื้นฐาน** ก่อนที่คุณจะบันทึก preset

### การแก้ไข Deployment Preset

คลิกไอคอนแก้ไขในแถว preset เพื่อเปิดฟอร์ม **แก้ไข Deployment Preset** ซึ่งกรอกค่าปัจจุบันไว้ล่วงหน้า ฟิลด์และกฎการตรวจสอบเหมือนกับการสร้าง preset

### หน้าต่างรายละเอียด Preset

คลิกปุ่ม **ⓘ** ข้างตัวเลือก preset เช่น ใน **โหมดพรีเซต** ของหน้าต่างเพิ่ม Revision เพื่อเปิดหน้าต่าง **รายละเอียด Preset** หน้าต่างนี้แสดงข้อมูลสรุปการกำหนดค่า preset รวมถึงการ์ดต่อไปนี้:

- **ข้อมูลพื้นฐาน (Basic Info)**: ชื่อ, runtime variant, image, จำนวนเรพลิกา, cluster mode, start command และตัวแปรสภาพแวดล้อม
- **ทรัพยากร (Resources)**: การจัดสรร CPU, หน่วยความจำ และ accelerator
- **Health Check**: สถานะการเปิดใช้งาน health check ของ preset นี้
   * เมื่อ **เปิดใช้งาน**: แสดง Path, Interval, Max Retries, Max Wait Time, Expected Status Code และ Startup Grace Period ที่กำหนดค่าไว้
   * เมื่อ **ปิดใช้งาน**: แสดงเป็น `ปิดใช้งาน`
...
### การจัดการ Model Store

Superadmin สามารถสร้าง แก้ไข และลบการ์ดโมเดลได้จากแผงการจัดการ Model Store

เมื่อสร้างหรือแก้ไขการ์ดโมเดล:

- ฟิลด์แท็ก **Label** และ **Framework** รับทั้ง **Enter** หรือ **จุลภาค (,)** เป็นตัวคั่น เพื่อให้คุณพิมพ์ `pytorch, transformers,` เพื่อเพิ่มหลายแท็กในครั้งเดียวโดยไม่ต้องกด Enter ระหว่างแต่ละรายการ
- ฟิลด์ **Domain** จะกรอกโดเมนที่ผู้ดูแลระบบใช้งานอยู่ล่วงหน้าโดยอัตโนมัติเมื่อสร้างการ์ดใหม่ เพื่อประหยัดขั้นตอนการเลือกโดเมนด้วยตนเอง

:::tip
หากต้องการเพิ่มหลาย label หรือแท็ก framework อย่างรวดเร็ว ให้พิมพ์โดยคั่นด้วยจุลภาค (เช่น `nlp, text-generation, llm`) และระบบจะเพิ่มทั้งหมดพร้อมกัน
:::
