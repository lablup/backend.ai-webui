...
- **เปิดใช้งานการตรวจสอบสุขภาพ (Enable Health Check)**: เมื่อเปิดใช้งาน ระบบจะส่งคำขอ HTTP ไปยัง inference server เป็นระยะเพื่อตรวจสอบว่าตอบสนองอย่างถูกต้อง เมื่อปิดใช้งาน (ค่าเริ่มต้นสำหรับ Revision ใหม่) ระบบจะไม่มีการตรวจสอบสุขภาพและ replica ที่ไม่แข็งแรงจะไม่ถูกตรวจพบโดยอัตโนมัติ ควรเปิดใช้งานสำหรับ deployment ที่ใช้งานจริง เมื่อเลือก **เปิดใช้งานการตรวจสอบสุขภาพ** จะมีฟิลด์เพิ่มเติมดังนี้:
   ...
   * **ช่วงเวลา (Interval)**: ช่วงเวลาระหว่างการตรวจสอบ (วินาที) (ค่าเริ่มต้น: `10.0`)
   * **จำนวนครั้งสูงสุด (Max Retries)**: จำนวนความล้มเหลวต่อเนื่องสูงสุด (ค่าเริ่มต้น: `10`)
   * **เวลารอสูงสุด (Max Wait Time)**: หมดเวลา (วินาที) (ค่าเริ่มต้น: `15.0`)
   * **รหัสสถานะที่คาดหวัง (Expected Status Code)**: รหัส HTTP ที่ระบุว่าบริการปกติ (ค่าเริ่มต้น: `200`)
   * **ระยะเวลาผ่อนผัน (Startup Grace Period)**: ระยะเวลาผ่อนผันหลังจากเริ่มต้น container (วินาที) (ค่าเริ่มต้น: `60.0`)
