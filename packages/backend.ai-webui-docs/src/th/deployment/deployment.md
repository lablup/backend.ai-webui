...
- **ตัวแปรสภาพแวดล้อม**: คู่ key/value ที่จะถูกส่งให้คอนเทนเนอร์ของ inference server สำหรับ `vLLM` และ `SGLang` ตัวแปรเฉพาะ runtime ที่ระบุไว้ข้างต้นจะถูกกรอกไว้ล่วงหน้า คุณสามารถเพิ่ม แก้ไข หรือลบรายการได้อย่างอิสระ ขณะที่คุณพิมพ์ชื่อตัวแปร ช่องนี้จะแนะนำชื่อตัวแปรสภาพแวดล้อมที่ใช้บ่อย เช่น `HF_TOKEN`, `AWS_ACCESS_KEY_ID` และ `HTTP_PROXY` คำแนะนำจะจับคู่กับส่วนใดก็ได้ของชื่อ ดังนั้นการพิมพ์ `proxy` จะแสดง `HTTP_PROXY`, `HTTPS_PROXY` และ `NO_PROXY` พร้อมกัน และชื่อที่ถูกใช้ไปแล้วในแถวอื่นจะไม่ถูกแนะนำซ้ำ เมื่อช่องชื่อมีตัวแปรจากรายการแนะนำ ช่องค่าที่อยู่ข้าง ๆ จะแสดงคำใบ้รูปแบบเป็น placeholder (ตัวอย่างเช่น *"โทเค็นการเข้าถึง Hugging Face ของคุณ"* สำหรับ `HF_TOKEN`)
