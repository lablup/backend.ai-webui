...
ปุ่มรีเฟรชที่ด้านบนของรายการยังใช้ควบคุมการรีเฟรชอัตโนมัติได้ด้วย คลิกลูกศรถัดจากปุ่มเพื่อเลือกช่วงเวลา ได้แก่ **ปิด**, `5วิ`, `10วิ`, `15วิ`, `30วิ` หรือ `60วิ` รายการการปรับใช้เริ่มต้นที่ `15วิ` เมื่อคุณเลือกช่วงเวลาแล้ว (รวมถึงการเลือก **ปิด**) ระบบจะจดจำค่านั้นไว้สำหรับรายการนี้แม้จะโหลดหน้าใหม่ ขณะที่การรีเฟรชอัตโนมัติทำงานอยู่ ปุ่มลูกศรจะแสดงช่วงเวลาที่เลือก และเส้นขอบนับถอยหลังจะค่อย ๆ เต็มจนถึงการรีเฟรชครั้งถัดไป การคลิกปุ่มรีเฟรชเองจะเริ่มนับถอยหลังใหม่
...
:::warning[กลุ่มทรัพยากรถูกกำหนดตายตัวตั้งแต่ตอนสร้าง]
ตั้งแต่เปิดหน้าต่างขึ้นมา จะมีคำเตือนแสดงอยู่ใต้ฟิลด์ **กลุ่มทรัพยากร** ว่า *"ไม่สามารถเปลี่ยนกลุ่มทรัพยากรที่เลือกไว้ตอนสร้างได้"* กลุ่มทรัพยากรเป็นสมบัติของการปรับใช้ ไม่ใช่ของ Revision คุณจึงไม่สามารถย้ายการปรับใช้ไปยังกลุ่มทรัพยากรอื่นในภายหลังได้ ไม่ว่าจะด้วยการแก้ไขการปรับใช้หรือการเพิ่ม Revision ใหม่ กรุณาเลือกกลุ่มทรัพยากรให้ถูกต้องก่อนสร้าง หากต้องการกลุ่มทรัพยากรอื่น ให้สร้างการปรับใช้ใหม่

**เปิดสาธารณะ** ก็ถูกกำหนดตายตัวในลักษณะเดียวกัน ในหน้าต่าง **แก้ไขการปรับใช้** ช่องทำเครื่องหมายนี้จะเป็นแบบอ่านอย่างเดียว และคำแนะนำจะระบุว่า *"ไม่สามารถเปลี่ยนแปลงได้หลังจากสร้าง"*
:::
...
หากต้องการเปลี่ยนการตั้งค่าระดับการปรับใช้ (ชื่อ, จำนวนเรพลิกาที่ต้องการ, หรือแท็ก) หลังจากสร้างแล้ว ให้คลิกปุ่ม **แก้ไข** บนการ์ดข้อมูลบริการ หน้าต่าง **แก้ไขการปรับใช้** จะเปิดขึ้นพร้อมค่าปัจจุบันที่กรอกไว้ล่วงหน้า และปุ่มยืนยันจะแสดงข้อความว่า `บันทึก` ฟิลด์ **กลุ่มทรัพยากร** ในหน้าต่างนั้นเป็นแบบอ่านอย่างเดียวและแสดงคำเตือนเดียวกัน ส่วน **เปิดสาธารณะ** จะถูกปิดใช้งาน
...
- **ปลายทางการเมาท์สำหรับโฟลเดอร์โมเดล (Mount Destination For Model Folder)**: เส้นทางภายในคอนเทนเนอร์ที่โฟลเดอร์จัดเก็บโมเดลถูกเมาต์ (ค่าเริ่มต้น: `/models`) คำแนะนำของฟิลด์นี้ระบุข้อความเดียวกันว่า *"เส้นทางที่โฟลเดอร์โมเดลถูกเมานต์ภายในคอนเทนเนอร์"*
...
- **ปลายทางการเมาท์สำหรับโฟลเดอร์โมเดล (Mount Destination For Model Folder)**: เส้นทางภายในคอนเทนเนอร์ที่โฟลเดอร์จัดเก็บโมเดลถูกเมาต์ (ค่าเริ่มต้น: `/models`)
...
:::note
โหมดกำหนดโมเดลทั้งสองโหมดใช้ชื่อฟิลด์เดียวกันคือ **ปลายทางการเมาท์สำหรับโฟลเดอร์โมเดล** และบันทึกค่าเดียวกัน ดังนั้นการสลับระหว่าง **ป้อนคำสั่ง** และ **ใช้ไฟล์การตั้งค่า** จะไม่เปลี่ยนความหมายของฟิลด์นี้
:::
...
ส่วนหัวของการ์ดข้อมูลบริการมีปุ่ม **แก้ไข** ควบคู่ไปกับเมนู **เพิ่มเติม** ปุ่ม **แก้ไข** จะเปิดหน้าต่าง **แก้ไขการปรับใช้** ตามที่อธิบายไว้ใน [หน้าต่างสร้างการปรับใช้](#create-deployment) และปุ่มยืนยันจะแสดงข้อความว่า `บันทึก` ปัจจุบันเมนูเพิ่มเติมมีการกระทำ **ลบการปรับใช้**
...
- **ขยายทั้งหมด**: เปิดทุกแถวที่ขยายได้
- **ยุบทั้งหมด**: ปิดทุกแถว
- **ขยายเฉพาะข้อผิดพลาด**: เปิดเฉพาะแถวที่ผลลัพธ์ไม่ใช่ความสำเร็จ นี่คือค่าเริ่มต้น คุณจึงเห็นรายการที่ล้มเหลว ลองใหม่ และหมดอายุได้ทันทีที่เปิดโมดอล

   ตัวเลือกการแสดงผลกำหนดเพียงว่า **แถวใดจะเริ่มต้นในสถานะขยาย** เท่านั้น และจะไม่เปลี่ยนสิ่งที่แสดงอยู่ในแถวที่ขยายแล้ว แถวที่ขยายจะแสดงตารางขั้นตอนย่อย **ทั้งหมด** ของเหตุการณ์นั้นเสมอ รวมถึงขั้นตอนย่อยที่สำเร็จ คุณจึงอ่านขั้นตอนที่ล้มเหลวควบคู่กับขั้นตอนที่ทำงานก่อนหน้าได้ แถวที่คุณเปิดหรือปิดเองจะคงสถานะนั้นไว้จนกว่าคุณจะเปลี่ยนตัวเลือกการแสดงผลหรือข้อมูลถูกโหลดใหม่

โมดอล **ประวัติการจัดตารางเวลาของ Replica** ที่เปิดจากแท็บเรพลิกาก็ใช้ตารางและตัวควบคุมการขยายชุดเดียวกัน ดูที่ [เรพลิกา](#replicas-tab-history)
...
ปุ่มรีเฟรชของการ์ดเรพลิกามีตัวเลือกช่วงเวลารีเฟรชอัตโนมัติชุดเดียวกับรายการการปรับใช้ เนื่องจากสถานะของเรพลิกาเปลี่ยนแปลงเร็ว การ์ดนี้จึงเริ่มต้นที่ `10วิ` และช่วงเวลาที่คุณเลือกจะถูกจดจำแยกจากค่าของรายการการปรับใช้
...
2. เลือก **วันหมดอายุ** ของโทเค็น
   ...
   * **ไม่มีวันหมดอายุ**: สร้างโทเค็นที่ไม่หมดอายุ
...
เมื่อออกโทเค็นแล้ว หน้าต่าง **โทเค็น** จะแสดงค่าโทเค็นเพียงครั้งเดียว พร้อมปุ่มคัดลอกและวันหมดอายุที่คุณเลือก (หรือ *ไม่มีวันหมดอายุ*) คัดลอกโทเค็นจากหน้าต่างนี้ หรือคัดลอกจากตารางในภายหลังก็ได้
...
- **วันหมดอายุ**: เวลาที่โทเค็นจะหมดอายุ หรือ *ไม่มีวันหมดอายุ*
...
- แต่ละตัวเลือกจะแสดง **อักขระ 6 ตัวสุดท้าย** ของโทเค็นในรูปแบบโค้ด พร้อมวันหมดอายุ (เช่น `~ 26 ส.ค. 2026`) หรือ *ไม่มีวันหมดอายุ* เนื่องจากโทเค็นทุกตัวขึ้นต้นด้วยส่วนหัว JWT เหมือนกัน ส่วนท้ายจึงเป็นสิ่งที่ใช้แยกความแตกต่างได้จริง วางเมาส์เหนือตัวเลือกเพื่อดูเวลาที่ออกโทเค็น → เวลาหมดอายุแบบเต็ม
- **โทเค็นที่หมดอายุแล้วจะไม่ถูกแสดง** รายการจะมีเฉพาะโทเค็นที่ยังใช้ได้ รวมถึงโทเค็นที่ออกแบบ **ไม่มีวันหมดอายุ**
