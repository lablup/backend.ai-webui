...
คุณสามารถตรวจสอบการทำงานของตัวจัดกำหนดงานได้ในลักษณะง่าย ๆ จากผู้ใช้ Web-UI เมื่อโฮสต์ GPU สามารถจัดสรร fGPU ได้สูงสุด 2 ตัว ให้เราสร้างเซสชันการคำนวณ 3 เซสชันพร้อมกันโดยขอจัดสรร fGPU 1 ตัวตามลำดับ ในส่วน **การจัดสรรแบบกำหนดเอง** ของกล่องโต้ตอบการเปิดเซสชัน ให้ตั้งค่าจำนวน GPU เป็น 1 และตั้งค่า **จำนวนเซสชัน** เป็น 3 หากคุณระบุค่าที่มากกว่า 1 ใน **จำนวนเซสชัน** และคลิกปุ่ม **LAUNCH** จำนวนเซสชันดังกล่าวจะถูกขอพร้อมกัน (**จำนวนเซสชัน** เป็นแนวคิดที่แตกต่างจาก **ขนาดคลัสเตอร์** ซึ่งควบคุมจำนวนโหนดที่ประกอบกันเป็นเซสชันหลายโหนดเดียว) นี่คือสถานการณ์ที่มีเซสชัน 3 เซสชันที่ขอ fGPU ทั้งหมด 3 ตัว ในขณะที่มี fGPU เพียง 2 ตัวเท่านั้น
...
ไปที่หน้า เซสชัน และเปิดกล่องสนทนาเริ่มต้นเซสชัน อาจมีภาพเคอร์เนลหลายประเภทขึ้นอยู่กับการตั้งค่าการติดตั้ง เฟรมเวิร์กและเวอร์ชันที่ใช้ได้ขึ้นอยู่กับสิ่งที่ผู้ดูแลระบบได้ลงทะเบียนไว้ ดังนั้นรายการในสภาพแวดล้อมของคุณอาจแตกต่างจากภาพด้านล่าง
...
ที่นี่ ให้เลือกเวอร์ชัน TensorFlow เวอร์ชันใดเวอร์ชันหนึ่งและสร้างเซสชัน

![](../images/session_launch_dialog_tf23.png)

เปิดเว็บเทอร์มินัลของเซสชันที่สร้างขึ้น และรันคำสั่ง Python ต่อไปนี้ คุณจะเห็นว่ามีการติดตั้ง TensorFlow เวอร์ชันที่เลือกไว้อยู่

![](../images/tf23_version_print.png)

คราวนี้ ให้เลือก TensorFlow อีกเวอร์ชันหนึ่งเพื่อสร้างเซสชันการคำนวณ หากทรัพยากรไม่เพียงพอ ให้ลบเซสชันก่อนหน้าออก

![](../images/session_launch_dialog_tf115.png)

เปิดเว็บเทอร์มินัลของเซสชันที่สร้างขึ้นแล้วรันคำสั่ง Python เดียวกันกับที่ทำก่อนหน้านี้ คุณจะเห็นว่ามีการติดตั้ง TensorFlow อีกเวอร์ชันหนึ่งอยู่

![](../images/tf115_version_print.png)

สุดท้าย ให้สร้างเซสชันการคำนวณโดยใช้อิมเมจ PyTorch

![](../images/session_launch_dialog_pytorch17.png)

เปิดเว็บเทอร์มินัลของเซสชันที่สร้างขึ้น และรันคำสั่ง Python ต่อไปนี้ คุณจะเห็นว่ามีการติดตั้ง PyTorch เวอร์ชันที่เลือกไว้อยู่
...
หากคุณต้องการแปลงเซสชันการประมวลผลที่กำลังทำงาน (คอนเทนเนอร์) เป็นภาพ Docker ใหม่ที่คุณสามารถนำกลับมาใช้ในภายหลังเพื่อสร้างเซสชันการประมวลผลใหม่ Backend.AI มีฟีเจอร์ "แปลงเซสชันเป็นอิมเมจ" แบบบริการตนเอง คุณสามารถคอมมิตเซสชันที่กำลังทำงานได้โดยตรงจาก WebUI โดยไม่ต้องขอให้ผู้ดูแลระบบ และอิมเมจที่ได้จะกลายเป็นอิมเมจที่กำหนดเองแบบส่วนตัวที่มองเห็นได้เฉพาะคุณเท่านั้น
...
- เมื่อเซสชันการคอมพิวเตอร์ของคุณเตรียมพร้อมแล้ว ให้เปิดแผงรายละเอียดเซสชันและเลือก **ผลักดันเซสชันไปยังอิมเมจที่กำหนดเอง** (การกระทำ **คอมมิตเซสชัน**) ป้อนชื่อสำหรับอิมเมจและยืนยัน สำหรับขั้นตอนแบบละเอียดทั้งหมด โปรดดูส่วน [บันทึกการคอมมิตเซสชัน](#save-session-commit)
- อิมเมจที่คอมมิตแล้วจะถูกสร้างเป็นอิมเมจที่กำหนดเองแบบส่วนตัว คุณสามารถดูและจัดการอิมเมจนั้นได้จากหน้า [สภาพแวดล้อมของฉัน](#my-environments) และเลือกได้จากรายการสภาพแวดล้อมในครั้งถัดไปที่คุณเปิดเซสชัน

:::note
ฟีเจอร์ "แปลงเซสชันเป็นอิมเมจ" ใช้ได้ตั้งแต่เวอร์ชัน 24.03 และใช้ได้เฉพาะเมื่อเซสชันอยู่ในโหมด `INTERACTIVE` เท่านั้น ฟีเจอร์นี้ต้องเปิดใช้งานในการติดตั้งของคุณ (`enableContainerCommit`) หากคุณไม่เห็นการกระทำ **ผลักดันเซสชันไปยังอิมเมจที่กำหนดเอง** แสดงว่าการคอมมิตคอนเทนเนอร์ถูกปิดใช้งานในการติดตั้งของคุณ ในกรณีนี้ ให้ขอให้ผู้ดูแลระบบแปลงเซสชันของคุณเป็นอิมเมจใหม่แทน โดยแจ้งชื่อเซสชันหรือ ID และที่อยู่อีเมลของคุณ จากนั้นผู้ดูแลระบบจะส่งชื่ออิมเมจเต็มและแท็กให้คุณ คุณสามารถป้อนชื่ออิมเมจนั้นในกล่องโต้ตอบเปิดเซสชันด้วยตนเอง อิมเมจนี้เป็นส่วนตัวและจะไม่แสดงให้ผู้ใช้อื่นเห็น
