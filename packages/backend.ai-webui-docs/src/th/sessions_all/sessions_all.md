...
สำหรับวิธีการจัดกลุ่มและกรองรายการเซสชัน โปรดดูที่
[การแสดงผลรายการเซสชัน](#session-list-view-and-refresh)

คลิกปุ่ม `เริ่มเซสชัน` เพื่อเริ่มเซสชันการคำนวณใหม่
...
หากผู้ใช้สร้างเซสชันด้วยบัญชี `superadmin`
...
:::note
AI Accelerator บางตัวใช้ **หน่วยความจำแบบรวม (unified memory)** โดยไม่มีหน่วยความจำ
เฉพาะของตัวเอง แต่ใช้พูลหน่วยความจำของโฮสต์ร่วมกัน เมื่อคุณเลือก AI Accelerator
ประเภทนี้ จะไม่สามารถตั้งค่าหน่วยความจำของ AI Accelerator แยกต่างหากได้ โดยจะใช้
หน่วยความจำจากพูลหน่วยความจำของโฮสต์โดยอัตโนมัติ
:::
...
:::
  ...
  * โหมดคลัสเตอร์ (Cluster Mode): เลือกวิธีจัดวางเซสชันการคำนวณหนึ่งเซสชันบนเอเจนต์ การเลือก **โหนดเดี่ยว (Single Node)** จะวางโหนดหลักและโหนดเวิร์กเกอร์ทั้งหมดไว้บนเอเจนต์เดียว (โหนดกายภาพหรือเครื่องเสมือน) ในขณะที่ **หลายโหนด (Multi Node)** จะแยกโหนดหลักหนึ่งโหนดและโหนดเวิร์กเกอร์หนึ่งโหนดขึ้นไปไว้บนเอเจนต์หลายตัว วางเมาส์เหนือไอคอน (`?`) ข้างแต่ละตัวเลือกเพื่อดูคำอธิบายสั้น ๆ หลังจากเลือกโหมดแล้ว ให้กำหนด**ขนาดคลัสเตอร์ (Cluster Size)** ด้วยแถบเลื่อนหรือช่องป้อนตัวเลข ในโหมดโหนดเดี่ยว ขนาดจะนับจำนวน **คอนเทนเนอร์** บนเอเจนต์เดียว และในโหมดหลายโหนดจะนับจำนวน **โหนด**ที่กระจายอยู่บนเอเจนต์หลายตัว เครื่องหมาย *ทรัพยากรที่เหลือ* บนแถบเลื่อนจะแสดงขนาดสูงสุดที่สามารถเริ่มต้นได้ทันทีด้วยทรัพยากรที่จัดสรรไว้ในปัจจุบัน คุณสามารถเลือกขนาดที่ใหญ่กว่าได้ แต่เซสชันอาจอยู่ในสถานะรอดำเนินการจนกว่าจะมีทรัพยากรเพียงพอ เซสชันหลายโหนดที่มีขนาดเท่ากับ 1 จะถูกสร้างเป็นเซสชันโหนดเดี่ยว สำหรับข้อมูลเพิ่มเติม โปรดดูที่ [ภาพรวมของเซสชันการคำนวณแบบคลัสเตอร์ของ Backend.AI](#backendai-cluster-compute-session)
...
ผู้ใช้สามารถระบุโฟลเดอร์จัดเก็บที่จะเมานต์ในเซสชันการคำนวณได้จากดรอปดาวน์ `เลือกโฟลเดอร์`
แต่ละรายการในดรอปดาวน์จะแสดงชื่อโฟลเดอร์พร้อมกับเจ้าของ (ผู้ใช้หรือโปรเจกต์) โหมดการใช้งาน
โฮสต์ วันที่สร้าง และสิทธิ์ที่คุณมีต่อโฟลเดอร์นั้น (`R`, `W`, `D`)
พิมพ์ข้อความในดรอปดาวน์เพื่อกรองรายการตามชื่อโฟลเดอร์
โฟลเดอร์ที่ถูก mount อัตโนมัติและโฟลเดอร์ที่ไม่สามารถเมานต์ได้ในโปรเจกต์ปัจจุบันจะไม่ปรากฏในรายการ

ถัดจากดรอปดาวน์มีปุ่มสองปุ่ม:

- `+` (สร้างโฟลเดอร์จัดเก็บใหม่): เปิดไดอะล็อกสร้างโฟลเดอร์ โฟลเดอร์ที่สร้างขึ้นจะถูกเพิ่มเข้าในรายการ
   และถูกเลือกเป็นโฟลเดอร์ที่จะเมานต์ สำหรับข้อมูลเพิ่มเติม
   โปรดดูที่ส่วน [สร้างโฟลเดอร์จัดเก็บ](#create-storage-folder)
- `รีเฟรช`: โหลดรายการโฟลเดอร์ใหม่ เช่น หลังจากสร้างหรือได้รับการแชร์โฟลเดอร์จากหน้าอื่น

![](../images/folder_create_modal.png)

โฟลเดอร์ที่เลือกแต่ละรายการจะแสดงเป็นแถวที่มีช่องพาธสองช่อง:

- **พาธในโฟลเดอร์ (source)**: โฟลเดอร์ย่อยภายในโฟลเดอร์จัดเก็บที่จะเมานต์
   คลิกที่ช่องนี้เพื่อเรียกดูโฟลเดอร์และเลือกไดเรกทอรี หากเว้นว่างไว้จะเมานต์ที่ root ของโฟลเดอร์
- **พาธในคอนเทนเนอร์ (destination)**: พาธภายในคอนเทนเนอร์ที่โฟลเดอร์จะถูกเมานต์
   หากเว้นว่างไว้ โฟลเดอร์จะถูกเมานต์ที่ `/home/work/<ชื่อโฟลเดอร์>` หากระบุพาธแบบสัมบูรณ์
   จะเมานต์ที่พาธนั้น และหากระบุพาธแบบสัมพัทธ์จะเมานต์ภายใต้ `/home/work/`
   พาธที่ซ้ำซ้อนกับโฟลเดอร์ที่เมานต์อื่นหรือโฟลเดอร์ที่ mount อัตโนมัติจะถูกรายงานเป็นข้อผิดพลาด
   และไม่สามารถเริ่มเซสชันได้จนกว่าจะแก้ไขความซ้ำซ้อนนั้น

คลิกปุ่มลบ (`X`) ที่ท้ายแถวเพื่อนำโฟลเดอร์นั้นออกจากรายการที่จะเมานต์
...
โฟลเดอร์ที่ถูกเมานต์โดยอัตโนมัติจะแสดงอยู่ใต้แถวเหล่านี้ในชื่อ `โฟลเดอร์ที่ mount อัตโนมัติ`
โดยไม่จำเป็นต้องเลือกเอง

:::note
หากโฟลเดอร์ที่เลือกไว้ไม่สามารถเมานต์ได้อีกต่อไป เช่น เมื่อการแชร์ถูกยกเลิก
โฟลเดอร์นั้นจะถูกนำออกจากรายการที่เลือกและจะมีการแจ้งเตือนแสดงขึ้น
:::
...
หน้า 'Session Launcher' มีชุดตัวเลือกสำหรับการสร้างเซสชัน
ฟีเจอร์ `Recent History` จะจดจำข้อมูลเกี่ยวกับเซสชันที่สร้างไว้ก่อนหน้า
...
## การแสดงผลรายการเซสชัน

รายการเซสชันแบ่งเป็นแท็บตามประเภทของเซสชัน ได้แก่ **ทั้งหมด**, **แบบโต้ตอบ**, **แบบแบตช์**,
**การอนุมาน** และ **เซสชันอัปโหลด** โดยแต่ละแท็บจะมีปุ่มสลับระหว่าง **กำลังทำงาน** /
**เสร็จสิ้น** และตัวกรองคุณสมบัติสำหรับ **ชื่อเซสชัน**, **กลุ่มทรัพยากร** และ **ตัวแทน**

<a id="session-resource-grid"></a>

### มุมมองกริดทรัพยากร

เมื่อเปิดใช้งานคุณสมบัติการทดลอง **Session resource grid view** ในการตั้งค่าผู้ใช้
ปุ่มสลับ **View mode** จะปรากฏเหนือรายการถัดจากปุ่มรีเฟรช และใช้สลับรายการระหว่าง
**Table** กับ **Grid** ได้ ดูวิธีเปิดใช้งานคุณสมบัตินี้ได้ที่หัวข้อ
[คุณสมบัติการทดลอง](#experimental-features) มุมมองกริดใช้แท็บประเภทเซสชัน ปุ่มสลับ
**กำลังทำงาน** / **เสร็จสิ้น** และตัวกรองคุณสมบัติชุดเดียวกับตาราง
คุณจึงสามารถกรองรายการก่อนแล้วค่อยสลับมุมมองได้

![](../images/session_resource_grid.png)

ในมุมมองกริด แต่ละเซสชันจะแสดงเป็นแผ่นที่ประกอบด้วยเซลล์ขนาดเล็ก
โดยสีของเซลล์แสดงอัตราการใช้ทรัพยากรแบบเรียลไทม์ของเซสชันนั้น
หากต่ำกว่า 50% จะใช้สีปกติ ระหว่าง 50% ถึง 80% จะใช้สีเตือนที่เข้มขึ้นตามลำดับ
และตั้งแต่ 80% ขึ้นไปจะใช้สีแดง เซสชันที่ยังไม่มีข้อมูลแบบเรียลไทม์ เช่น
เซสชันที่ยังไม่เริ่มทำงาน จะคงสี **No data** ไว้ ส่วนเซสชันที่ทรัพยากรยังอยู่ในสถานะร้องขอ
จะแสดงด้วยเส้นขอบแบบประ ความหมายของแต่ละสีดูได้จากคำอธิบายสัญลักษณ์เหนือกริด

เมื่อวางเมาส์บนแผ่นของเซสชัน จะเห็นข้อมูลสรุปของเซสชันนั้น ได้แก่ สถานะ ประเภท
โหมดคลัสเตอร์ กลุ่มทรัพยากร อิมเมจ เวลาเริ่มต้นและเวลาที่ผ่านไป
อัตราการใช้ทรัพยากรที่จัดสรรแต่ละรายการ และดิสก์ I/O เมื่อคลิกที่แผ่นดังกล่าว
แผงรายละเอียดเซสชันจะเปิดขึ้น

ตัวควบคุมเหนือกริดใช้กำหนดว่าหนึ่งเซลล์แทนอะไร

- **Grid mode**: เลือก **Resource** เพื่อแสดงหนึ่งเซลล์ต่อหนึ่งหน่วยของทรัพยากรที่เลือก
  หรือเลือก **Kernel** เพื่อแสดงหนึ่งเซลล์ต่อหนึ่งเคอร์เนลของเซสชัน
- **Resource** (เฉพาะโหมด Resource): เลือกทรัพยากรที่ต้องการแสดงผล ได้แก่ **CPU**,
  **หน่วยความจำ** หรือตัวเร่งความเร็วที่เซสชันในรายการใช้งานอยู่
  หนึ่งเซลล์เท่ากับ CPU หนึ่งคอร์ หน่วยความจำหนึ่งหน่วย หรือตัวเร่งความเร็วหนึ่งตัว
  และหากใช้ตัวเร่งความเร็วแบบเศษส่วนจะแสดงเป็นเซลล์ที่เติมสีเพียงบางส่วน
  ตัวเร่งความเร็วที่รายงานหน่วยความจำของอุปกรณ์จะมีสองรายการ คือ
  อัตราการใช้งาน (`util`) และหน่วยความจำของอุปกรณ์ (`mem`)
- **Memory unit** (เฉพาะโหมด Resource เมื่อเลือกทรัพยากรประเภทหน่วยความจำ):
  กำหนดว่าหนึ่งเซลล์แทนหน่วยความจำเท่าใด โดยเลือกได้ 1, 2, 4 หรือ 8 GiB
- **Metric** (เฉพาะโหมด Kernel): เลือกสถิติแบบเรียลไทม์ที่ใช้กำหนดสีของเซลล์เคอร์เนล
  หากไม่มีเซสชันในรายการที่รายงานเมตริกสด ตัวเลือกนี้จะถูกปิดใช้งานและแสดงข้อความ
  **ไม่มีเมตริกสด**
- **Layout**: **Serpentine** จะเรียงแถวสลับทิศทางกัน
  เพื่อให้เซลล์ของเซสชันที่ขึ้นบรรทัดใหม่ยังต่อเนื่องกัน ส่วน **Word-wrap**
  จะเริ่มทุกแถวจากขอบด้านซ้าย

เหนือกริดจะมีข้อความแจ้งเกี่ยวกับเซสชันที่ไม่ได้แสดงด้วย

- เมื่อตัวกรองปัจจุบันตรงกับเซสชันมากกว่าจำนวนที่กริดแสดงได้ ระบบจะแสดงเพียง 100
  เซสชันแรก พร้อมข้อความแจ้งจำนวนทั้งหมด
- ในโหมด Resource เซสชันที่ไม่ได้รับการจัดสรรทรัพยากรที่เลือกจะไม่ถูกแสดง
  พร้อมข้อความแจ้งจำนวนเซสชันที่ถูกซ่อน
...
ปุ่มไอคอนนาฬิกาจะปรากฏข้างแท็กสถานะเซสชันในแผงรายละเอียดเซสชัน การคลิกไอคอนนี้จะเปิดโมดัลประวัติการจัดตารางเซสชัน ซึ่งแสดงบันทึกรายละเอียดของการตัดสินใจในการจัดตารางทั้งหมดที่ระบบได้ทำสำหรับเซสชันนั้น สำหรับข้อมูลเพิ่มเติม โปรดดูที่ส่วน[ประวัติการจัดตารางเซสชัน](#session-scheduling-history)

![](../images/session_scheduling_history_button.png)

<a id="preemption-statuses"></a>

### สถานะที่เกี่ยวข้องกับการเบียดออก

เซสชันอาจปรากฏในสถานะ `RESERVED`, `PREEMPTED` และ `RESCHEDULING` ได้ด้วย ตัวจัดกำหนดการจะใช้สถานะเหล่านี้เมื่อจำเป็นต้องปลดปล่อยทรัพยากรให้กับงานที่มีลำดับความสำคัญสูงกว่า

- `RESERVED`: เซสชันถือครองการจองทรัพยากรและกำลังรอให้ทรัพยากรที่ต้องการถูกปลดปล่อย
- `PREEMPTED`: เซสชันถูกเลือกให้ถูกเบียดออก เพื่อให้งานที่มีลำดับความสำคัญสูงกว่าใช้ทรัพยากรได้
- `RESCHEDULING`: เซสชันถูกเบียดออกและถูกส่งกลับเข้าคิว โดยจะถูกจัดตารางใหม่โดยอัตโนมัติ

เซสชันในทั้งสามสถานะนี้ยังคงใช้ทรัพยากรอยู่ จึงแสดงอยู่ภายใต้ **กำลังทำงาน** ไม่ใช่ **เสร็จสิ้น**

เมื่อตัวจัดกำหนดการบันทึกเหตุผลของการเบียดออกไว้กับเซสชัน การวางเมาส์เหนือแท็กสถานะเซสชันจะแสดงเหตุผลนั้นเป็นคำอธิบายแทนค่าภายในแบบดิบ

| เหตุผล | คำอธิบาย |
|--------|----------|
| `PREEMPTED_BY_SCHEDULER` | เซสชันนี้ถูกยุติโดยตัวจัดกำหนดการเพื่อปลดปล่อยทรัพยากรให้กับงานที่มีลำดับความสำคัญสูงกว่า |
| `preempted-by-reservation` | เซสชันนี้ถูกเลือกให้ถูกเบียดออกเพื่อปลดปล่อยทรัพยากรให้กับงานที่มีลำดับความสำคัญสูงกว่า |
| `preemption-reservation` | เซสชันนี้ถือครองการจองทรัพยากรและกำลังรอให้ทรัพยากรถูกปลดปล่อย |
| `RESCHEDULED` | เซสชันนี้ถูกเบียดออกและถูกส่งกลับเข้าคิว โดยจะถูกจัดตารางใหม่โดยอัตโนมัติ |
...
- **สถานะต้นทาง**: กรองตามสถานะเซสชันก่อนขั้นตอนการจัดตาราง
- **สถานะปลายทาง**: กรองตามสถานะเซสชันหลังขั้นตอนการจัดตาราง
- **รหัสข้อผิดพลาด**: กรองตามรหัสข้อผิดพลาด
- **ข้อความ**: กรองตามเนื้อหาข้อความ
- **สร้างเมื่อ**: กรองตามเวลาที่สร้างบันทึก
- **อัปเดตเมื่อ**: กรองตามเวลาที่อัปเดตบันทึก
...
- **สถานะต้นทาง**: สถานะเซสชันก่อนขั้นตอนการจัดตารางนี้
- **สถานะปลายทาง**: สถานะเซสชันหลังขั้นตอนการจัดตารางนี้
- **จำนวนครั้งที่ลอง**: จำนวนครั้งที่ขั้นตอนการจัดตารางนี้ถูกลอง
- **ข้อความ**: ข้อความสรุปสำหรับขั้นตอนการจัดตารางนี้

คุณสามารถเรียงลำดับตารางตามคอลัมน์**สร้างเมื่อ**หรือ**อัปเดตเมื่อ**เพื่อดูบันทึกในลำดับที่ต้องการ

#### การแบ่งหน้าตารางประวัติ

ตารางประวัติการจัดตารางใช้การแบ่งหน้าแบบ offset โดยมีขนาดหน้าเริ่มต้น 10 รายการต่อหน้า การเปลี่ยนตัวกรองหรือลำดับการเรียงจะรีเซ็ตมุมมองกลับไปที่หน้า 1 โดยอัตโนมัติ
...
### บันทึกการตรวจสอบเซสชัน

แท็บ **Audit Log** จะปรากฏที่ด้านล่างของแผงรายละเอียดเซสชันควบคู่กับแท็บ **Kernels** แท็บนี้ให้บันทึกตามลำดับเวลาของทุกการกระทำที่ดำเนินการกับเซสชัน เช่น การเปลี่ยนแปลงสถานะ การแก้ไขทรัพยากร และการดำเนินการทางการดูแลระบบ พร้อมด้วยข้อมูลว่าใครเป็นผู้ดำเนินการและเมื่อใด

![](../images/session_audit_log.png)
<!-- TODO: Capture screenshot showing the Client IP column (FR-3661) -->

:::note
Audit Log ถูกควบคุมโดย backend และอาจมองเห็นได้เฉพาะ superadmin เท่านั้น ผู้ใช้ทั่วไปอาจเห็นผลลัพธ์ว่างเปล่าแม้ว่าแท็บจะสามารถเข้าถึงได้
:::

#### คอลัมน์ Audit Log

ตาราง Audit Log ประกอบด้วยคอลัมน์ต่อไปนี้:

- **Triggered By**: ที่อยู่อีเมลและ ID บัญชีของผู้ดำเนินการที่เริ่มต้นการกระทำ
- **Client IP**: ที่อยู่ IP ที่ใช้ร้องขอการกระทำ แสดงตามที่เซิร์ฟเวอร์รายงานทุกประการ อาจถูกปิดบังบางส่วนตามนโยบายการปิดบัง IP ของไคลเอ็นต์ที่ผู้ดูแลระบบกำหนด และจะแสดงเป็น `-` หากไม่มีการบันทึก
- **Operation**: ประเภทของการกระทำที่ดำเนินการ (ตัวอย่างเช่น การสร้างเซสชัน การยุติ หรือการเปลี่ยนแปลงทรัพยากร)
- **Status**: ผลลัพธ์ของการกระทำ (`SUCCESS`, `ERROR`, `RUNNING`, `DENIED` หรือ `UNKNOWN`) โดย `DENIED` หมายถึงการกระทำถูกปฏิเสธโดยเจตนาตามนโยบาย ไม่ใช่ความล้มเหลวจากข้อผิดพลาด
- **Time**: เวลาที่เกิดการกระทำ

#### ตัวกรอง Audit Log

ใช้แถบตัวกรองด้านบนตารางเพื่อจำกัดรายการบันทึก:

- **Status**: กรองตามผลลัพธ์ของการกระทำ (`SUCCESS`, `ERROR`, `RUNNING`, `UNKNOWN`)
- **Operation**: ค้นหาตามชื่อหรือคำสำคัญของการดำเนินการ
- **Triggered By**: ค้นหาตาม ID ของผู้ดำเนินการ
- **Time**: กรองด้วยตัวเลือกช่วงวันที่

ปุ่ม **Refresh** จะโหลดบันทึกใหม่ การค้นหา Audit Log จะถูกเลื่อนออกไปและส่งเฉพาะครั้งแรกที่คุณเปิดใช้งานแท็บ **Audit Log**
...
ผู้ใช้สามารถดูบันทึกของเซสชันการคำนวณได้โดยคลิกไอคอน `ดูบันทึกคอนเทนเนอร์`
ในแผงรายละเอียดเซสชัน

![](../images/session_log.png)

หน้าต่างบันทึกมีตัวควบคุมต่อไปนี้อยู่เหนือพื้นที่แสดงบันทึก:

- **ตัวเลือกเคอร์เนล**: เลือกเคอร์เนลของเซสชันที่ต้องการอ่านบันทึก สำหรับเซสชันคอนเทนเนอร์เดียว
  จะมีเพียงรายการเดียว ส่วนเซสชันแบบคลัสเตอร์จะแสดงแต่ละคอนเทนเนอร์ตามชื่อโฮสต์
  พร้อมอักขระตัวแรก ๆ ของ ID
- **ดาวน์โหลด**: บันทึกล็อกที่แสดงอยู่ในขณะนั้นเป็นไฟล์ข้อความ
...
เกณฑ์ที่ใช้กับเซสชันที่กำลังทำงานจะแสดงอยู่ในแถว **สถานะการเรียกคืน** ของแผงรายละเอียดเซสชัน
พร้อมกับเวลาที่เหลือก่อนที่แต่ละเกณฑ์จะยุติเซสชัน แถวนี้จะปรากฏขึ้นขณะที่เซสชันกำลังทำงาน
และมีการตรวจสอบสถานะไม่ใช้งานอย่างน้อยหนึ่งรายการที่แสดงเวลาที่เหลือ
ไม่ว่าจะเหลืออีกหลายวันหรือเพียงไม่กี่นาทีก็ตาม
...
:::note
ขึ้นอยู่กับการตั้งค่าของสภาพแวดล้อม ตัวตรวจสอบที่ไม่ทำงานและประเภทของทรัพยากรที่แสดงในตัวตรวจสอบการใช้งานอาจแตกต่างกันได้
:::
...
รายการเซสชันมีคอลัมน์ **สถานะการเรียกคืน** ที่บอกให้คุณทราบได้ในทันทีว่าแต่ละเซสชัน
ใกล้จะถูกเรียกคืนโดยตัวตรวจสอบการใช้งานมากเพียงใด ป้ายเดียวกันนี้ยังแทนที่ชื่อของรายการ
ตัวตรวจสอบการใช้งานในแถว **สถานะการเรียกคืน** ของแผงรายละเอียดเซสชันด้วย

ป้ายมีสามระดับ ตามที่ระบุไว้ในคำอธิบายสัญลักษณ์ของป๊อปโอเวอร์:

- **ปลอดภัย**: การใช้งานเฉลี่ยในปัจจุบันสูงกว่าเกณฑ์อย่างเพียงพอ
- **คำเตือน**: การใช้งานเฉลี่ยในปัจจุบันสูงกว่าเกณฑ์ แต่ใกล้เคียงกับเกณฑ์
- **มีความเสี่ยง**: การใช้งานเฉลี่ยในปัจจุบันต่ำกว่าเกณฑ์

เนื่องจากป้ายจะเปลี่ยนระดับก่อนที่ค่าเฉลี่ยจะลดลงต่ำกว่าเกณฑ์จริง ๆ เซสชันจึงอาจแสดงเป็น
**มีความเสี่ยง** ได้ทั้งที่ค่าเฉลี่ยปัจจุบันยังสูงกว่าเกณฑ์อยู่ ให้ถือว่าเป็นคำเตือนล่วงหน้า
ที่เผื่อเวลาให้คุณรับมือ ไม่ใช่การแจ้งว่าค่าเฉลี่ยข้ามเกณฑ์ไปแล้ว

เซสชันที่ตัวตรวจสอบการใช้งานไม่มีผลบังคับใช้ และเซสชันที่ยังไม่มีค่าที่วัดได้ จะแสดง `-`
ในคอลัมน์นี้ ส่วนในแผงรายละเอียดเซสชัน รายการที่ยังไม่มีการตัดสินใจเรียกคืนครั้งแรกจะแสดงข้อความ
**กำลังตรวจสอบการเรียกคืน** แทนแท็กเวลาที่เหลือ

วางเมาส์เหนือไอคอนข้อมูล (i) ถัดจากป้ายเพื่อเปิดป๊อปโอเวอร์ที่แสดงรายละเอียดเบื้องหลังระดับนั้น:

- ประโยคเงื่อนไขที่ระบุว่าทรัพยากรจะถูกเรียกคืนเมื่อเงื่อนไข **ใดเงื่อนไขหนึ่ง** ที่แสดงไว้
  คงอยู่ในระดับที่มีความเสี่ยง หรือเมื่อเงื่อนไข **ทั้งหมด** อยู่ในระดับนั้นเท่านั้น
  ประโยคที่ปรากฏขึ้นอยู่กับว่าผู้ดูแลระบบตั้งค่าตัวตรวจสอบไว้อย่างไร
- หนึ่งแถวต่อหนึ่งทรัพยากรที่วัดได้ แต่ละแถวมีป้ายระดับของตนเอง พร้อมค่าเฉลี่ยปัจจุบันเทียบกับ
  เกณฑ์ที่กำหนดไว้ ในรูปแบบ `(ปัจจุบัน 12% | เกณฑ์ 5%)`
- คำอธิบายสัญลักษณ์ที่ระบุเกณฑ์ของแต่ละระดับทั้งสาม

ทรัพยากรที่ไม่มีค่าที่วัดได้จะแสดงด้วยเครื่องหมายขีดและไม่มีผลต่อการตัดสินระดับโดยรวม

เมื่อเวลาที่เหลือของรายการตัวตรวจสอบการใช้งานใกล้จะหมดลง แท็กเวลาที่เหลือถัดจากรายการนั้น
จะใช้สีเดียวกับป้าย เซสชันที่การนับถอยหลังเปลี่ยนเป็นสีแดงจึงเป็นเซสชันที่ใกล้จะถูกเรียกคืน
ในทางกลับกัน ขณะที่ยังเหลือเวลาอีกมาก แท็กจะไม่มีสีแม้ว่าป้ายจะแสดงระดับความเสี่ยงแล้วก็ตาม
...
Backend.AI รองรับฟีเจอร์ \"Convert Session to Image\" การคอมมิตเซสชันที่อยู่ในสถานะ `RUNNING` จะบันทึก
