...
สำหรับวิธีการจัดกลุ่ม กรอง และรีเฟรชรายการเซสชัน โปรดดูที่
[การแสดงผลและการรีเฟรชรายการเซสชัน](#session-list-view-and-refresh)
...
## การแสดงผลและการรีเฟรชรายการเซสชัน

รายการเซสชันถูกจัดกลุ่มตามประเภทของเซสชันเป็นแท็บ **ทั้งหมด**, **แบบโต้ตอบ**, **แบบแบตช์**,
...
แต่ละแท็บจะเก็บตัวกรอง ลำดับการเรียง และหมายเลขหน้าของตนเองไว้ตราบใดที่คุณยังอยู่บนหน้านี้
ดังนั้นเมื่อคุณสลับไปยังแท็บอื่นแล้วกลับมา มุมมองเดิมจะถูกกู้คืนแทนที่จะถูกรีเซ็ต
นอกจากนี้สถานะการแสดงผลของแท็บที่คุณกำลังใช้งานอยู่จะถูกเก็บไว้ใน URL ของหน้าด้วย
การโหลดเบราว์เซอร์ใหม่จึงแสดงมุมมองนั้นได้เหมือนเดิม และคุณสามารถบุ๊กมาร์กหรือแชร์ URL
เพื่อเปิดรายการในสถานะเดียวกันได้ อย่างไรก็ตาม URL จะเก็บเฉพาะสถานะของแท็บที่ใช้งานอยู่เท่านั้น
หลังจากโหลดใหม่ แท็บอื่นจะเริ่มต้นจากค่าเริ่มต้น

<a id="auto-refresh-interval"></a>

### ช่วงเวลารีเฟรชอัตโนมัติ

ปุ่มรีเฟรชทางด้านขวาของแถบเครื่องมือรายการจะโหลดรายการเซสชันใหม่ทันที ปุ่มนี้ยังมีตัวเลือก
ช่วงเวลารีเฟรชอัตโนมัติด้วย คลิกลูกศรถัดจากปุ่มเพื่อเลือกช่วงเวลา — **ปิด** หรือหนึ่งใน
ค่าที่กำหนดไว้ล่วงหน้าตั้งแต่ 5 วินาทีถึง 60 วินาที ขณะที่การรีเฟรชอัตโนมัติเปิดอยู่
เส้นขอบนับถอยหลังจะแสดงบนปุ่มเพื่อบอกเวลาที่เหลือจนถึงการโหลดครั้งถัดไป
และช่วงเวลาที่เลือกจะแสดงเป็นป้ายกำกับ (เช่น `15วิ`) บนตัวเลือก

การรีเฟรชอัตโนมัติเปิดใช้งานตั้งแต่แรกบนหน้าจอของเซสชันการคำนวณ:

- **รายการเซสชัน** เริ่มต้นที่ช่วงเวลา 15 วินาที
- **แผงรายละเอียดเซสชัน** และหน้าต่าง **บันทึกคอนเทนเนอร์** เริ่มต้นที่ช่วงเวลา 10 วินาที
...
ปุ่มรีเฟรชในส่วนหัวของแผงจะโหลดข้อมูลเซสชันใหม่ และมีตัวเลือกช่วงเวลารีเฟรชอัตโนมัติเช่นเดียวกับ
แถบเครื่องมือของรายการ โปรดดูที่[ช่วงเวลารีเฟรชอัตโนมัติ](#auto-refresh-interval)
สำหรับวิธีการทำงานของช่วงเวลา
...
:::note
โหมด **ขยายเฉพาะข้อผิดพลาด** (ที่อธิบายด้านล่าง) จะใช้เฉพาะกับรายการที่มองเห็นได้ในหน้าปัจจุบันเท่านั้น ไม่ใช่กับประวัติทั้งหมด
:::
...
#### การควบคุมการขยาย / ยุบ

การคลิกเมนู kebab (⋮) ในส่วนหัวของคอลัมน์ขยายจะเปิดเมนู **ตัวเลือกการขยาย** ซึ่งช่วยให้คุณควบคุมวิธีการขยายแถวทั่วทั้งตารางได้ มีโหมดการแสดงผลสามแบบ:

- **ขยายทั้งหมด**: ขยายทุกแถวเพื่อให้มองเห็นขั้นตอนย่อยทั้งหมดได้ทันที
- **ยุบทั้งหมด**: ยุบทุกแถวเพื่อแสดงเฉพาะบันทึกประวัติระดับบนสุด
- **ขยายเฉพาะข้อผิดพลาด** (ค่าเริ่มต้น): ขยายแถวที่มีผลลัพธ์ไม่ใช่ SUCCESS โดยอัตโนมัติ และซ่อนขั้นตอนย่อยที่เป็น SUCCESS ภายในแถวที่ขยายแล้วด้วย โหมดนี้มีประโยชน์มากที่สุดสำหรับการระบุอย่างรวดเร็วว่าเกิดอะไรขึ้นผิดพลาดในวงจรชีวิตการจัดตารางของเซสชัน

โหมดที่เลือกไว้จะถูกจำไว้ต่อผู้ใช้และยังคงอยู่ข้ามการเปิดโมดัล คุณยังสามารถขยายหรือยุบแถวแต่ละแถวด้วยตนเองได้ตลอดเวลา โดยไม่คำนึงถึงโหมดที่ใช้งานอยู่
...
- **Status**: ผลลัพธ์ของการกระทำ (`SUCCESS`, `ERROR`, `RUNNING` หรือ `UNKNOWN`)
...
:::note
ผู้ดูแลระบบสามารถซ่อนแอปภายในคอนเทนเนอร์สำหรับเซสชันแบบแบตช์ได้โดยเปิดใช้งาน
`hideAppsOnBatchSession` ใน `resources/app_template.json` เมื่อเปิดใช้งานตัวเลือกนี้
ตัวเปิดแอปและทางลัดเทอร์มินัลจะไม่ปรากฏสำหรับเซสชัน **แบบแบตช์** ทั้งในการดำเนินการของแถวใน
รายการเซสชัน ในการแจ้งเตือนการสร้างเซสชัน และในแผงรายละเอียดเซสชัน เซสชันแบบโต้ตอบจะไม่ได้รับ
ผลกระทบ ตัวเลือกนี้ถูกปิดไว้เป็นค่าเริ่มต้น
:::
...
- **รีเฟรช**: โหลดบันทึกใหม่ ปุ่มนี้ยังมีตัวเลือกช่วงเวลารีเฟรชอัตโนมัติตามที่อธิบายไว้ใน
  [ช่วงเวลารีเฟรชอัตโนมัติ](#auto-refresh-interval)

เนื่องจากหน้าต่างบันทึกมักถูกเปิดขึ้นเพื่อเฝ้าดูบรรทัดใหม่ที่เข้ามา การรีเฟรชอัตโนมัติจึงเริ่มต้น
ที่ช่วงเวลา 10 วินาที คุณสามารถเลือกช่วงเวลาอื่น หรือเลือก **ปิด** จากตัวเลือกถัดจากปุ่มรีเฟรช
และค่าที่เลือกจะถูกจดจำไว้สำหรับครั้งถัดไปที่คุณเปิดหน้าต่างบันทึก มุมมองบันทึกจะเลื่อนไปยัง
บรรทัดล่าสุดหลังการโหลดใหม่แต่ละครั้ง และบรรทัดที่เพิ่มเข้ามาตั้งแต่การโหลดครั้งก่อนจะถูกเน้นสี
...
และมีการตรวจสอบสถานะไม่ใช้งานอย่างน้อยหนึ่งรายการรายงานเวลาที่เหลือ ไม่ว่าจะเหลือเวลาอีกมากเพียงใด
การตรวจสอบที่ยังเหลือเวลาอีกนานก็จะแสดงในลักษณะเดียวกับการตรวจสอบที่ใกล้หมดอายุ
...
ป้ายมีสามระดับ ซึ่งคำอธิบายสัญลักษณ์ในป๊อปโอเวอร์ก็ระบุไว้เช่นเดียวกัน:
...
ระดับเหล่านี้ถูกจำแนกโดยเผื่อระยะห่างไว้ ไม่ใช่การเปรียบเทียบกับเกณฑ์ที่กำหนดไว้โดยตรง
ดังนั้นเซสชันอาจแสดงเป็น **มีความเสี่ยง** ได้ทั้งที่ค่าเฉลี่ยปัจจุบันยังสูงกว่าเกณฑ์อยู่
ให้อ่านป้ายนี้เป็นระดับคำเตือนเชิงเปรียบเทียบที่เผื่อเวลาให้คุณรับมือ
ไม่ใช่คำบอกโดยตรงว่าค่าเฉลี่ยข้ามเกณฑ์ไปแล้วหรือยัง
...
- ประโยคเงื่อนไข ซึ่งระบุว่าทรัพยากรจะถูกเรียกคืนเมื่อเงื่อนไข **ใดเงื่อนไขหนึ่ง** ที่แสดงไว้
...
เมื่อเวลาที่เหลือของรายการตัวตรวจสอบการใช้งานใกล้จะหมดลง แท็กเวลาที่เหลือซึ่งแสดงถัดจากรายการนั้น
จะใช้สีเดียวกับป้าย ดังนั้นเซสชันที่การนับถอยหลังเปลี่ยนเป็นสีแดงก็คือเซสชันที่มีความเสี่ยงที่จะถูกเรียกคืน
ในทางกลับกัน ขณะที่ยังเหลือเวลาอีกมาก แท็กจะไม่มีสีแม้ว่าป้ายจะแสดงระดับความเสี่ยงแล้วก็ตาม
กล่าวคือ แท็กเวลาที่เหลือที่มีสีเป็นสัญญาณว่าความเสี่ยงนั้นใกล้เข้ามาแล้ว ไม่ใช่เพียงแค่มีอยู่
