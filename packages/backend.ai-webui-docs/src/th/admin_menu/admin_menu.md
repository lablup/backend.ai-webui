...
- **บันทึกการตรวจสอบ (Audit Log)**: แสดงประวัติการดำเนินการที่กระทำกับการปรับใช้นี้ แต่ละรายการจะแสดง **การดำเนินการ (Operation)** ที่เกิดขึ้น, **ผู้ใช้ (User)** ที่เรียกใช้, **ตราเวลา (Timestamp)** และ **รายละเอียด (Details)** ของการดำเนินการ แท็บนี้รองรับตัวกรองเพื่อจำกัดรายการให้แคบลง และโหลดข้อมูลแบบ lazy loading เมื่อคุณเลื่อนดู

:::note[ข้อกำหนดเวอร์ชัน]
แถว **พารามิเตอร์ Runtime (Runtime Parameters)** กำหนดให้ใช้ Backend.AI Manager เวอร์ชัน 26.4.4 ขึ้นไป
:::
...
ฟิลด์ **อิมเมจ (Image)** บนหน้ารายละเอียดจะแสดงในรูปแบบ `canonicalName@architecture` ตัวอย่างเช่น `cr.backend.ai/stable/pytorch:2.1-cuda12.1@aarch64` ซึ่งระบุทั้งชื่อเต็มของอิมเมจและสถาปัตยกรรมที่ใช้
...
คอลัมน์ **แก้ไขเมื่อ (Modified)** จะแสดงโดยค่าเริ่มต้น คอลัมน์เพิ่มเติม ได้แก่ **คำอธิบาย (Description)**, **คำสั่งเริ่มต้น (Startup Command)**, **เปิดให้สาธารณะ (Open to Public)** และ **อันดับ (Rank)** จะถูกซ่อนไว้โดยค่าเริ่มต้น คุณสามารถแสดงหรือซ่อนคอลัมน์เหล่านี้ได้โดยคลิกไอคอนรูปเฟือง (gear) ที่ส่วนหัวของตารางเพื่อเปิดตัวจัดการคอลัมน์
...
2. กรอกฟิลด์ในไดอะล็อก *สร้างค่าตั้งล่วงหน้า* ซึ่งจัดเป็นส่วนต่าง ๆ ดังนี้:
   ...
   - **พารามิเตอร์ Runtime (Runtime Parameters)**: ส่วนนี้จะปรากฏ **เฉพาะเมื่อเลือก runtime variant เป็น vLLM หรือ SGLang เท่านั้น** พารามิเตอร์จะถูกจัดเป็นหลายแท็บ ได้แก่ **Model Loading**, **Resource Memory** และ **Serving Performance** เพื่อให้กำหนดค่าได้ง่ายขึ้น พารามิเตอร์ที่จำเป็นจะมีเครื่องหมายดอกจันสีแดง (`*`) กำกับ และฟอร์มจะไม่อนุญาตให้ส่งหากฟิลด์ที่จำเป็นเหล่านั้นว่างเปล่า
      ...
      :::note[ข้อกำหนดเวอร์ชัน]
      การตรวจสอบความถูกต้องของพารามิเตอร์ runtime ที่จำเป็น (เครื่องหมายดอกจันสีแดงและการบล็อกการส่ง) กำหนดให้ใช้ Backend.AI Manager เวอร์ชัน 26.4.4 ขึ้นไป
      :::
   ...
   - **การเรียกใช้งาน**: startup command, environment variables และ bootstrap script ฟิลด์ **คำสั่งเริ่มต้น (Startup Command)** จะแสดงคำใบ้ไวยากรณ์เชลล์ คำสั่งจะถูกเรียกใช้งานในรูปแบบ `/bin/bash -c <command>` คุณจึงสามารถใช้ไวยากรณ์ของเชลล์ เช่น การคั่นคำสั่งหลายคำสั่งด้วย `;` ได้
   ...
   - **คำจำกัดความโมเดล (Model Definition)** (ตัวเลือกเสริม): เปิดใช้งานด้วย toggle เพื่อกำหนดค่า model definition แบบมีโครงสร้าง เมื่อเปิดใช้งานจะสามารถตั้งค่าได้ดังนี้:
      ...
      * **การตรวจสอบสุขภาพ (Health Check)** (ตัวเลือกเสริม): ส่วนนี้มี toggle **เปิดใช้การตรวจสอบสุขภาพ (Enable Health Check)** ซึ่งตั้งค่าเริ่มต้นเป็น **ปิด (OFF)** เมื่อปิดอยู่ ฟิลด์การตรวจสอบสุขภาพจะถูกซ่อนไว้ เมื่อเปิด toggle นี้ ฟิลด์ทั้งหมดจะปรากฏขึ้น ได้แก่ path, interval, จำนวนครั้งสูงสุดที่ลองใหม่, เวลารอสูงสุด, status code ที่คาดหวัง และ startup grace period
      * **Metadata** (ตัวเลือกเสริม): ผู้แต่ง, ชื่อ, เวอร์ชัน, task, หมวดหมู่ และข้อมูลอื่น ๆ

      :::note[ข้อกำหนดเวอร์ชัน]
      toggle **เปิดใช้การตรวจสอบสุขภาพ (Enable Health Check)** กำหนดให้ใช้ Backend.AI Manager เวอร์ชัน 26.4.4 ขึ้นไป toggle เดียวกันนี้ยังใช้กับ Advanced Mode ของ runtime variant vLLM และ SGLang ด้วย
      :::
...
#### สิทธิ์โฟลเดอร์โปรเจกต์

Drawer รายละเอียด storage host มีแท็บ **สิทธิ์โฟลเดอร์โปรเจกต์** ซึ่งผู้ดูแลระบบสามารถดูและจัดการว่าโปรเจกต์ใดมีสิทธิ์เข้าถึง storage host ที่เลือกได้ หากต้องการเปิด drawer ให้คลิกชื่อ storage host ในรายการ Storages

:::tip
ตัวเลือกโดเมนในแท็บ **สิทธิ์โฟลเดอร์โปรเจกต์** จะตั้งค่าเริ่มต้นเป็นโดเมนที่ใช้งานอยู่ในปัจจุบันเมื่อ drawer เปิดขึ้น ทำให้สิทธิ์โฟลเดอร์ของโดเมนนั้นแสดงขึ้นทันที คุณสามารถสลับไปยังโดเมนอื่นหรือล้างการเลือกเพื่อดูสิทธิ์ในโดเมนอื่นได้
:::
