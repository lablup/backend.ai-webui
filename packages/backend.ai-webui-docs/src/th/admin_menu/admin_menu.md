...
ในรายการผู้ใช้ คอลัมน์ที่มีหลายค่า ได้แก่ **IP ไคลเอนต์ที่อนุญาต** และ **GID เสริม** จะแสดงค่าแรกไว้ในบรรทัด และย่อค่าที่เหลือไว้หลังแท็ก `+N` วางเมาส์เหนือแท็กเพื่อดูรายการทั้งหมดโดยไม่ต้องขยายความกว้างของคอลัมน์

คอลัมน์ **Projects** แสดงโปรเจกต์ที่ผู้ใช้แต่ละคนสังกัด โดยใช้รูปแบบเดียวกัน คือแสดงบางค่าไว้ในบรรทัดและย่อค่าที่เหลือไว้หลังแท็ก `+N` คอลัมน์นี้ถูกซ่อนไว้โดยค่าเริ่มต้น และแสดงได้ด้วยปุ่มตั้งค่าคอลัมน์ (⚙) ที่อยู่ใต้ตาราง ถัดจากตัวควบคุมการแบ่งหน้า

ปุ่ม **คล่องแคล่ว** และ **ไม่ทำงาน** ด้านบนรายการจะเลือกว่าจะแสดงบัญชีกลุ่มใด และตัวกรองคุณสมบัติที่อยู่ข้างกันจะจำกัดรายการให้แคบลงอีก คุณสมบัติที่รองรับได้แก่ **อีเมล**, **ID**, **ชื่อ**, **โปรเจกต์**, **Project Is Active**, **โดเมน**, **Domain Is Active**, **Integration Name**, **วันที่สร้าง** และ **บทบาท** สำหรับแมเนเจอร์ที่รองรับตัวกรองผู้ใช้แบบขยาย จะมี **ชื่อเต็ม**, **นโยบายทรัพยากร**, **คำอธิบาย**, **ข้อมูลสถานะ**, **ต้องเปลี่ยนรหัสผ่าน**, **เปิดใช้งาน 2FA แล้ว**, **เปิดใช้งานเซสชัน sudo**, **UID คอนเทนเนอร์**, **GID คอนเทนเนอร์** และ **GID เสริม** ให้เลือกใช้เพิ่มเติม

การส่งออกรายการเป็นไฟล์ CSV จะใช้ตัวกรองคุณสมบัติที่มีผลอยู่ในขณะนั้น ดังนั้นคุณสามารถจำกัดรายการก่อนแล้วส่งออกเฉพาะผู้ใช้ที่ต้องการได้ เงื่อนไขตัวกรองที่การส่งออกไม่รองรับจะถูกละเว้น โดยจะมีคำเตือนแสดงขึ้นเมื่อเกิดกรณีนี้ และไฟล์ที่ส่งออกอาจมีผู้ใช้มากกว่าที่ตารางแสดง แต่จะไม่น้อยกว่า
...
บางส่วน ข้อความจะแสดงจำนวนผู้ใช้ที่สร้างสำเร็จและจำนวนที่ล้มเหลว
พร้อมทั้งแสดงรายการผู้ใช้ที่ไม่สามารถสร้างได้และเหตุผลของแต่ละรายการ
:::
...
ไฟล์ที่ขาดคอลัมน์ใดคอลัมน์หนึ่งในสองคอลัมน์นี้จะถูกปฏิเสธ และไดอะล็อกจะระบุว่าขาดคอลัมน์ใด

**คอลัมน์ที่เลือกได้:**

- **password**: รหัสผ่านเริ่มต้น กฎรหัสผ่านเดียวกันกับการสร้างผู้ใช้แบบเดี่ยวจะถูกนำมาใช้ (อย่างน้อย 8 ตัวอักษร ประกอบด้วยตัวอักษร อักขระพิเศษ และตัวเลขอย่างน้อยอย่างละ 1 ตัว) ทุกบัญชียังคงต้องมีรหัสผ่าน แต่ไม่จำเป็นต้องมาจากไฟล์ ดูหมายเหตุด้านล่าง
...
ด้านซ้ายของไดอะล็อกมีแผง **ค่าเริ่มต้น** ค่าที่คุณกำหนดไว้ที่นั่นจะถูกเติมลงในฟิลด์ที่เว้นว่างไว้ในทุกแถว จึงไม่ต้องพิมพ์ค่าเดิมซ้ำในทุกเซลล์ของ CSV แผงนี้ครอบคลุม **โดเมน**, **โปรเจกต์**, **นโยบายทรัพยากรผู้ใช้**, **รหัสผ่าน**, **ต้องเปลี่ยนรหัสผ่าน** และ **คำอธิบาย** ฟิลด์ที่คุณไม่ได้กำหนดจะแสดงว่า `ไม่มีค่าเริ่มต้น`
...
:::note
ไฟล์ CSV ที่ไม่มีคอลัมน์ `password` เช่น ไฟล์ที่ส่งออกจากรายการผู้ใช้ จะโหลดได้ตามปกติ แถวเหล่านั้นจะแสดงข้อผิดพลาดรายแถวว่า `จำเป็นต้องระบุรหัสผ่าน` และปุ่มสร้างจะยังคงถูกปิดใช้งานตราบใดที่ยังมีแถวที่มีข้อผิดพลาดนี้ เมื่อกรอกรหัสผ่านในแผง **ค่าเริ่มต้น** ตัวอย่างจะอัปเดตทันที ข้อผิดพลาดจะหายไปและปุ่มจะใช้งานได้ โดยไม่ต้องอัปโหลดไฟล์ใหม่
:::

#### การสร้างผู้ใช้

เมื่อตรวจสอบตัวอย่างแล้วและยืนยันว่าทุกแถวถูกต้อง ให้คลิก **สร้างผู้ใช้ N คน** เพื่อส่ง หากบางแถวล้มเหลวฝั่งเซิร์ฟเวอร์ (เช่น เนื่องจากอีเมลหรือชื่อผู้ใช้มีอยู่แล้ว) ไดอะล็อกจะยังคงเปิดอยู่และแสดงรายการข้อผิดพลาดแต่ละแถวเพื่อให้คุณระบุและแก้ไขข้อขัดแย้งได้

หากมีเพียงบางแถวที่สำเร็จ ระบบจะแสดงข้อความ `N ผู้ใช้ถูกสร้างสำเร็จ. M ผู้ใช้สร้างไม่สำเร็จ.` และเปิดรายการที่ล้มเหลวขึ้นเหนือไดอะล็อก โดยแสดงอีเมล ชื่อผู้ใช้ และข้อผิดพลาดที่ได้รับของผู้ใช้ทุกรายที่ไม่สามารถสร้างได้ หากมีบัญชีบางส่วนที่ถูกสร้างขึ้น รายการคีย์แพร์ที่สร้างขึ้นจะแสดงก่อน แล้วรายการที่ล้มเหลวจะแสดงต่อเมื่อคุณปิดรายการดังกล่าว บัญชีที่สร้างสำเร็จแล้วจะยังคงอยู่ ดังนั้นคุณเพียงแก้ไขแถวที่ถูกรายงานเท่านั้น
...
เมื่อต้องเปลี่ยนค่าเดียวกันให้กับหลายบัญชี เช่น ย้ายไปยังโดเมนหรือโปรเจกต์อื่น เปลี่ยนนโยบายทรัพยากร หรือกำหนด UID/GID ของคอนเทนเนอร์ คุณสามารถแก้ไขทั้งหมดในครั้งเดียวแทนที่จะเปิดผู้ใช้ทีละราย

1. เลือกผู้ใช้ที่ต้องการเปลี่ยนด้วยช่องทำเครื่องหมายในรายการผู้ใช้
2. จำนวนที่เลือกจะปรากฏบนแถบเครื่องมือ คลิกปุ่มแก้ไข (ดินสอ) ที่อยู่ถัดไป
...
ไดอะล็อกมีฟิลด์ต่อไปนี้ ฟิลด์ที่คุณไม่ได้แก้ไขจะไม่ถูกเปลี่ยนแปลงในผู้ใช้ที่เลือกไว้:

- **โดเมน**: ย้ายผู้ใช้ที่เลือกไปยังโดเมนอื่น การเปลี่ยนค่านี้จะล้างการเลือก **โปรเจกต์**
- **โปรเจกต์**: กำหนดผู้ใช้ที่เลือกให้กับโปรเจกต์หนึ่งรายการขึ้นไป ต้องเลือกโดเมนก่อน
- **สถานะผู้ใช้**: ตั้งค่าบัญชีเป็นใช้งาน ไม่ใช้งาน ไม่ใช้งาน (รวมคีย์แพร์) หรือก่อนการตรวจสอบ
- **นโยบายทรัพยากร**: ใช้นโยบายทรัพยากรผู้ใช้กับทุกบัญชีที่เลือก
- **UID คอนเทนเนอร์**, **GID คอนเทนเนอร์**, **GID เสริม**: กำหนด ID ตัวเลขที่ให้กับกระบวนการภายในคอนเทนเนอร์

เมื่อดำเนินการเสร็จ ระบบจะแจ้งว่ามีผู้ใช้กี่รายจากที่เลือกไว้ที่ได้รับการอัปเดต บัญชีที่เปลี่ยนแปลงไม่สำเร็จจะถูกรายงานแยกพร้อมข้อผิดพลาดของตัวเอง
...
ป๊อปโอเวอร์สำหรับยืนยันจะปรากฏขึ้น คลิกปุ่ม **ปิดใช้งาน** เพื่อปิดใช้งานผู้ใช้
...
จะปรากฏขึ้น คลิกปุ่ม **เปิดใช้งาน** เพื่อเปิดใช้งานผู้ใช้อีกครั้ง
...
- **ต้องการลบ deployment ที่สร้างไว้ด้วยหรือไม่?**: เมื่อเลือก deployment ที่ผู้ใช้
  ที่ถูกลบถาวรสร้างไว้จะถูกลบด้วย เมื่อไม่เลือก deployment เหล่านั้นจะไม่ถูกลบ
...
ตัวกรองคุณสมบัติด้านบนรายการจะจำกัดคีย์แพร์ตาม **รหัสผู้ใช้**, **คีย์การเข้าถึง**, **สิทธิ์**, **ชื่อเต็ม**, **นโยบายทรัพยากร**, **โปรเจกต์**, **ขีดจำกัดอัตรา**, **จำนวนคำขอ** หรือ **สร้างเมื่อ** นอกจากคีย์การเข้าถึงและเจ้าของแล้ว รายการยังแสดงคอลัมน์ **ชื่อเต็ม** ซึ่งเป็นชื่อที่แสดงของเจ้าของ และคอลัมน์ **ใช้ครั้งล่าสุด** ซึ่งเป็นเวลาที่คีย์แพร์ยืนยันตัวตนคำขอครั้งล่าสุด คีย์แพร์ที่ยังไม่เคยถูกใช้จะแสดงเป็น `-`
...
คุณสามารถเปลี่ยนนโยบายทรัพยากรและขีดจำกัดอัตราของคีย์แพร์ได้โดยคลิกปุ่ม **แก้ไข** (ดินสอ) ในแถวของคีย์แพร์
ไดอะล็อกมีชื่อว่า **อัปเดตนโยบายทรัพยากรคู่คีย์** และคลิก **บันทึก** เพื่อใช้การเปลี่ยนแปลง
...
### เปิดหรือปิดการใช้งานข้อมูลประจำตัวหลายรายการพร้อมกัน

แทนที่จะดำเนินการทีละแถว คุณสามารถเปลี่ยนสถานะของคีย์แพร์หลายรายการได้ในขั้นตอนเดียว:

1. เลือกคีย์แพร์ด้วยช่องทำเครื่องหมายของแถว
2. จำนวนที่เลือกจะปรากฏบนแถบเครื่องมือ ตามด้วยปุ่มเปลี่ยนสถานะ — **ปิดการใช้งาน** ในแท็บที่ใช้งานอยู่
   และ **เปิดใช้งาน** ในแท็บไม่ทำงาน
3. คลิกปุ่มนั้น ไดอะล็อกยืนยันจะระบุจำนวนข้อมูลประจำตัวที่ได้รับผลกระทบ และความหมายของการเปลี่ยนแปลงต่อเจ้าของ
...
- **ผู้ใช้**: รายการตัวเลือกที่ค้นหาได้ซึ่งแสดงบัญชีผู้ใช้ที่มีอยู่ เริ่มพิมพ์เพื่อจำกัดรายการ แล้วเลือกบัญชี
  ที่จะเป็นเจ้าของคีย์แพร์ใหม่ เนื่องจากเลือกบัญชีจากรายการ จึงไม่สามารถส่งที่อยู่ที่ไม่มีผู้ใช้จริงได้อีกต่อไป
- **นโยบายทรัพยากร**: นโยบายทรัพยากรคู่คีย์ที่จะใช้กับคีย์แพร์ใหม่
- **ขีดจำกัดอัตรา (ต่อ 15 นาที)**: จำนวนคำขอสูงสุดที่สามารถส่งไปยังเซิร์ฟเวอร์
  ...
  ตามรูปแบบของผู้ใช้
   ...
   ค่าสูงสุดที่ยอมรับคือ 50000 การป้อนค่าที่มากกว่านั้นจะแสดงข้อความตรวจสอบ
   `ขีดจำกัดอัตราควรมากกว่าศูนย์และต่ำกว่า 50000` และบล็อกการส่ง ดังนั้นค่าที่คุณพิมพ์จะไม่ถูกลดลงเงียบ ๆ
   ค่าที่ไม่เกิน 100 สามารถป้อนได้แต่จะแสดงคำเตือน เนื่องจากขีดจำกัดอัตราที่ต่ำอาจบล็อกการดำเนินการ เช่น
   การเข้าสู่ระบบ
...
หน้าข้อมูลสำหรับผู้ดูแลระบบ ให้คลิก 'สร้างโฟลเดอร์' เพื่อเปิดกล่องโต้ตอบการสร้างโฟลเดอร์
เนื่องจากหน้าข้อมูลสำหรับผู้ดูแลระบบสามารถสร้างได้เฉพาะโฟลเดอร์โปรเจกต์ กล่องโต้ตอบนี้จึงไม่แสดงตัวเลือกประเภทโฟลเดอร์
ให้เลือกโปรเจกต์ที่จะเป็นเจ้าของโฟลเดอร์ในช่อง **โครงการปลายทาง**
ที่ด้านบนของกล่องโต้ตอบ จากนั้นป้อนชื่อโฟลเดอร์และตั้งค่า **การอนุญาตเมานต์**
...
- **การปรับใช้**: แสดงรายการการปรับใช้ข้ามทุกโปรเจกต์ พร้อมตัวกรองวงจรชีวิตและคุณสมบัติเดียวกับหน้าการปรับใช้สำหรับผู้ใช้ มุมมองข้ามโปรเจกต์นี้ยังมีคุณสมบัติตัวกรอง **โปรเจกต์** เพิ่มเติม
...
:::note
การปรับใช้สำหรับผู้ดูแลระบบแต่ละรายการมีที่อยู่ของตัวเองภายใต้หน้า Admin Deployments คือ `/admin/deployments/<รหัสการปรับใช้>` เมื่อคุณเปิดการปรับใช้จากหน้า Admin Deployments URL จะเปลี่ยนเป็นเส้นทางนี้ จึงสามารถเชื่อมโยงไปยังหน้ารายละเอียดการปรับใช้โดยตรงหรือบันทึกเป็นบุ๊กมาร์กได้
:::
...
- **IP ของไคลเอ็นต์ (Client IP)**: ที่อยู่ IP ที่ใช้ร้องขอการดำเนินการ แสดงตามที่เซิร์ฟเวอร์รายงานทุกประการ อาจถูกปิดบังบางส่วนตามนโยบายการปิดบัง IP ของไคลเอ็นต์ที่ผู้ดูแลระบบกำหนด และจะแสดงเป็น `-` หากไม่มีการบันทึก
...
- **โดเมน (Domain)**: กรองตามโดเมนที่เป็นเจ้าของ (จับคู่สตริง)
- **โปรเจกต์ (Project)**: กรองตามโปรเจกต์ที่เป็นเจ้าของ แทนที่จะพิมพ์ตัวระบุโปรเจกต์เอง คุณสามารถเลือกจากดรอปดาวน์ที่ค้นหาได้ซึ่งแสดงรายการโปรเจกต์ Model Store
- **Storage Host**: กรองตาม storage host ของโฟลเดอร์ที่เชื่อมโยง แทนที่จะพิมพ์ค่าเอง ให้เลือกโฮสต์จากดรอปดาวน์ของโฮสต์ที่ลงทะเบียนไว้ในคลัสเตอร์นี้ และใช้ได้ทั้งตัวดำเนินการเท่ากับและไม่เท่ากับ
...
คลิกปุ่ม `สร้างการ์ดโมเดล` เพื่อเปิดโมดอลการสร้าง กรอกฟิลด์ต่อไปนี้:
...
คุณสามารถค้นหาและจำกัดรายการได้ด้วยตัวกรองคุณสมบัติด้านบนของตาราง และคลิกหัวคอลัมน์ใด ๆ เพื่อเปลี่ยนลำดับการเรียง คุณสมบัติ **หมวดหมู่** สามารถเลือกได้จากรายการหมวดหมู่ที่กำหนดไว้ในคลัสเตอร์นี้ แทนที่จะพิมพ์รหัสหมวดหมู่เอง
...
- **หมวดหมู่**: หมวดหมู่ที่เลือกได้สำหรับจัดกลุ่มพรีเซตที่เกี่ยวข้อง ฟิลด์นี้เป็นดรอปดาวน์ที่แสดงหมวดหมู่ซึ่งกำหนดไว้บนเซิร์ฟเวอร์ ไม่ใช่ช่องพิมพ์อิสระ ปล่อยว่างเพื่อให้เป็น **ไม่มีหมวดหมู่** ขณะที่รายการหมวดหมู่กำลังโหลด ฟิลด์นี้จะแสดงตัวบ่งชี้การโหลด และฟิลด์อื่น ๆ ยังคงใช้งานได้ตามปกติ
...
:::note
ไดอะล็อกนี้จะเริ่มต้นจากสถานะว่างเปล่าทุกครั้งที่เปิด ค่าที่คุณพิมพ์ไว้ในครั้งก่อน หรือค่าของพรีเซตที่คุณเพิ่งแก้ไข จะไม่ถูกนำมาแสดงในไดอะล็อก **สร้างพรีเซต** หรือ **แก้ไขพรีเซต** ครั้งถัดไป
:::
...
ตารางนโยบายทรัพยากรคีย์แพร์มีคอลัมน์ต่อไปนี้ สองคอลัมน์ในนั้นอธิบายวิธีการนำนโยบายไปใช้ มากกว่าจะเป็นค่าขีดจำกัด:

- **ชื่อ**: ชื่อของนโยบาย เซลล์นี้ยังมีการกระทำแบบอินไลน์ **ข้อมูล**, **แก้ไข** และ **ลบ**
- **ค่าเริ่มต้นสำหรับการไม่ระบุ**: พฤติกรรมของนโยบายเมื่อเซสชันไม่ได้ระบุ CPU หรือหน่วยความจำอย่างชัดเจน
  `UNLIMITED` อนุญาตให้ใช้งานได้ถึงความจุของโหนดเอเจนต์ ส่วน `LIMITED` จะไม่จัดสรรทรัพยากรเว้นแต่จะระบุอย่างชัดเจน
  ในกรณีส่วนใหญ่แนะนำ `UNLIMITED` วางเมาส์เหนือไอคอนเครื่องหมายคำถามในหัวคอลัมน์เพื่อดูคำอธิบายเดียวกันใน UI
- **นโยบายทรัพยากร**: ช่องทรัพยากรที่นโยบายอนุญาต
- **เซสชันที่ทำงานพร้อมกัน**, **ขนาดคลัสเตอร์**, **เวลาว่างสูงสุด**, **อายุเซสชันสูงสุด**, **โหนดที่เก็บข้อมูล**,
  **จำนวนเซสชันที่รอสูงสุด**, **จำนวนเซสชัน SFTP พร้อมกันสูงสุด**: ค่าขีดจำกัดแต่ละรายการที่อธิบายด้านล่าง
- **ช่องทรัพยากรเซสชันที่รออยู่สูงสุด**: ช่องทรัพยากรที่นโยบายอนุญาตสำหรับเซสชันที่รออยู่ในสถานะ `PENDING`
  โดยแสดงแยกตามชนิดทรัพยากร (แสดงเป็น `∞` เมื่อไม่จำกัด) คอลัมน์นี้เป็นแบบอ่านอย่างเดียวใน WebUI
  เนื่องจากไดอะล็อกนโยบายทรัพยากรไม่มีฟิลด์นี้
- **สร้างเมื่อ**: เวลาที่สร้างนโยบาย

คุณสามารถใช้ตัวกรองคุณสมบัติ การเรียงลำดับ และตัวแบ่งหน้าของตารางเพื่อค้นหานโยบายที่ต้องการได้

หากต้องการเปลี่ยนนโยบายทรัพยากร ให้คลิกการกระทำ **แก้ไข** (ดินสอ) ในคอลัมน์ชื่อของนโยบายนั้น
ในไดอะล็อก **อัปเดตนโยบายทรัพยากรคู่คีย์** ทุกตัวเลือก
สามารถแก้ไขได้ยกเว้นชื่อนโยบาย ซึ่งทำหน้าที่เป็นคีย์หลักสำหรับ
...
- Sessions
  ...
  - เซสชันที่ทำงานพร้อมกัน: จำนวนเซสชันการคำนวณพร้อมกันสูงสุดต่อคีย์แพร์
...
ข้อมูลประจำตัวของหน้าผู้ใช้ คลิกการกระทำ **แก้ไข** (ดินสอ) ที่อยู่ใน
...
ในคอลัมน์ชื่อ เมื่อคุณคลิกไอคอน ไดอะล็อกยืนยันจะปรากฏขึ้น
...
ตารางแสดงคอลัมน์ต่อไปนี้: **ID**, **ชื่อ** (พร้อมการกระทำแบบอินไลน์ **แก้ไข** และ **ลบ**),
**จำนวนโฟลเดอร์สูงสุด**, **ขนาดขอบเขตโควตาสูงสุด**, **จำนวนอิมเมจที่กำหนดเองสูงสุด** และ **สร้างเมื่อ**
ใช้ตัวควบคุมการตั้งค่าคอลัมน์เพื่อซ่อนคอลัมน์ที่ไม่ต้องการ
ตัวเลือกของคุณจะถูกเก็บไว้ต่อเบราว์เซอร์

การเรียงลำดับและการแบ่งหน้าทั้งหมดประมวลผลบนเซิร์ฟเวอร์ แท็บนี้จึงยังตอบสนองได้ดีในคลัสเตอร์ที่มีนโยบายจำนวนมาก
คลิกหัวคอลัมน์เพื่อเปลี่ยนลำดับการเรียง และใช้ตัวแบ่งหน้าด้านล่างตารางเพื่อเลื่อนดูผลลัพธ์

หากต้องการสร้างนโยบายทรัพยากรผู้ใช้ใหม่ ให้คลิกปุ่ม **สร้าง**
...
- Max Folder Size: ขนาดสูงสุดของพื้นที่จัดเก็บของผู้ใช้ หาก
  ...
  จัดเก็บใหม่ได้ หากตั้งค่าเป็น Unlimited จะแสดงเป็น "∞" ในรายการ ขีดจำกัดนี้จะแสดงในคอลัมน์
  **ขนาดขอบเขตโควตาสูงสุด** พร้อมหน่วยที่เหมาะกับค่าที่ตั้งไว้
- Max Concurrent Logins: จำนวนเซสชันสูงสุดที่ผู้ใช้สามารถเข้าสู่ระบบพร้อมกันได้
  หากตั้งค่าเป็น Unlimited จะแสดงเป็น "∞"
...
หากต้องการเปลี่ยนนโยบายที่มีอยู่ ให้คลิกการกระทำ **แก้ไข** (ดินสอ) ในคอลัมน์ชื่อ ไดอะล็อกมีชื่อว่า
**อัปเดตนโยบายทรัพยากรผู้ใช้** และปุ่มส่งคือ **บันทึก** หากต้องการลบ ให้คลิกการกระทำ **ลบ** (ถังขยะ)
...
หากต้องการเปลี่ยนแปลง ให้คลิกการกระทำ **แก้ไข** (ดินสอ) ในคอลัมน์ชื่อ ไดอะล็อกมีชื่อว่า **อัปเดตนโยบายทรัพยากรโครงการ** และปุ่มส่งคือ **บันทึก** ชื่อนโยบายทรัพยากรไม่สามารถแก้ไขได้ การลบสามารถทำได้โดยคลิกการกระทำ **ลบ** (ถังขยะ)
...
หน้า Admin Session มีสองแท็บ แท็บ **เซสชัน** แสดงเซสชันการคำนวณทั้งหมดในคลัสเตอร์ และแท็บ
**เซสชันที่รอดำเนินการ** จะแสดงมุมมองรวมของเซสชันที่รอดำเนินการทั้งหมดภายใน
...
ใช้ตัวเลือก **กลุ่มทรัพยากร** ที่อยู่เหนือรายการเพื่อเลือกว่าจะแสดงคิวของกลุ่มทรัพยากรใด ตัวเลือกนี้ค้นหาได้ และแสดงกลุ่มทรัพยากรที่ใช้งานอยู่ทั้งหมดในคลัสเตอร์โดยเรียงตามชื่อ ค่าที่คุณเลือกจะถูกเก็บไว้ใน URL ของหน้าเป็นพารามิเตอร์คิวรี `resourceGroup` คุณจึงบุ๊กมาร์กหรือแชร์แท็บนี้แล้วกลับมาที่กลุ่มทรัพยากรเดิมได้ หากกลุ่มทรัพยากรที่ระบุใน URL ไม่มีอยู่แล้วหรือไม่ได้ใช้งานอยู่ ระบบจะแสดงกลุ่มทรัพยากรที่ใช้งานอยู่กลุ่มแรกแทน

เช่นเดียวกับหน้าเซสชัน คุณสามารถคลิกชื่อเซสชันเพื่อเปิด drawer ที่แสดง
ข้อมูลรายละเอียดเกี่ยวกับเซสชันได้

รายการเซสชันที่รอดำเนินการยังแสดง **ลำดับความสำคัญ** ของแต่ละเซสชัน ซึ่งเป็นตัวกำหนดลำดับของคิว หากต้องการเปลี่ยนค่านี้ ให้คลิกการกระทำ **การตั้งค่า** (รูปเฟือง) ในแถวของเซสชัน หรือเลือกเซสชันที่รอดำเนินการอย่างน้อยหนึ่งรายการด้วยช่องทำเครื่องหมาย แล้วคลิกปุ่ม **การตั้งค่า** ที่อยู่เหนือรายการ ทั้งสองวิธีจะเปิดไดอะล็อก **การตั้งค่าเซสชัน** จากนั้นกำหนด **ลำดับความสำคัญ** เป็นค่าระหว่าง 0 ถึง 100 แล้วคลิก **บันทึก** เมื่อดำเนินการสำเร็จจะมีข้อความยืนยันว่าลำดับความสำคัญของเซสชันได้รับการอัปเดตเรียบร้อยแล้ว ลำดับความสำคัญสามารถเปลี่ยนได้เฉพาะขณะที่เซสชันอยู่ในสถานะ `PENDING` เท่านั้น

:::note
คอลัมน์ **ลำดับความสำคัญ** และการกระทำสำหรับแก้ไขลำดับความสำคัญจะแสดงเฉพาะเมื่อ Backend.AI Manager เป็นเวอร์ชัน 26.4.0 ขึ้นไป
:::

ในแท็บ **เซสชัน** คุณสามารถใช้ตัวกรองคุณสมบัติเพื่อจำกัดรายการให้แคบลงได้ และหากจำเป็นก็สามารถค้นหาเซสชันเฉพาะด้วย **รหัสเซสชัน** ได้ นอกจากนี้ยังกรองตามเจ้าของเซสชัน (อีเมลหรือชื่อเต็ม) โปรเจกต์และโดเมน รวมถึงคุณสมบัติของเซสชัน เช่น ผลลัพธ์ โหมดคลัสเตอร์ ลำดับความสำคัญ และเวลาที่สร้างหรือสิ้นสุดได้ด้วย

การส่งออกรายการเป็นไฟล์ CSV จะใช้ตัวกรองคุณสมบัติที่กำลังใช้งานอยู่ด้วย คุณจึงจำกัดรายการก่อนแล้วส่งออกเฉพาะเซสชันที่ต้องการได้ เงื่อนไขตัวกรองที่การส่งออกไม่รองรับจะถูกละเว้น ดังนั้นไฟล์ที่ส่งออกอาจมีเซสชันมากกว่าที่แสดงในตาราง แต่จะไม่น้อยกว่า

เมื่อเปิดใช้งานฟีเจอร์ทดลอง **Session resource grid view** ในการตั้งค่าผู้ใช้ (ดูที่หัวข้อ [คุณสมบัติการทดลอง](#experimental-features)) แท็บ **เซสชัน** จะแสดงตัวควบคุม **View mode** ถัดจากปุ่มรีเฟรช สำหรับสลับระหว่าง **Table** และ **Grid** มุมมองกริดจะแสดงหนึ่งเซลล์ต่อหนึ่งเซสชัน โดยแต่ละเซลล์มีสีตามการใช้ทรัพยากรแบบเรียลไทม์ของเซสชันนั้น สำหรับรายละเอียดของตัวควบคุมภายในมุมมองกริด ให้ดูที่หัวข้อ [การแสดงผลรายการเซสชัน](#session-list-view-and-refresh)
...
หน้านี้ยังเตือนเมื่อรายการที่คุณกำลังดูอยู่ไม่สามารถใช้กลุ่มทรัพยากรที่คุณเจาะลงมาได้จริง เนื่องจากการตรวจสอบทำโดยอ้างอิงโดเมนหรือโปรเจกต์ที่คุณเลือกระหว่างเจาะลงมา คำเตือนจึงสะท้อนตำแหน่งปัจจุบันของคุณในลำดับชั้น ไม่ใช่การเลือกก่อนหน้า:

- ในขั้นตอน **โดเมน** ไอคอนคำเตือนจะปรากฏถัดจากโดเมนที่ไม่ได้รับอนุญาตให้ใช้กลุ่มทรัพยากรที่เลือก วางเมาส์เหนือไอคอนเพื่อดูคำอธิบายว่ากลุ่มทรัพยากรนั้นอาจยังได้รับอนุญาตในโปรเจกต์ย่อยของโดเมน
- ในขั้นตอน **โปรเจกต์** ไอคอนเดียวกันจะปรากฏบนโปรเจกต์ที่ไม่ได้รับอนุญาตให้ใช้กลุ่มทรัพยากรที่เลือก
- ในขั้นตอน **ผู้ใช้** จะมีคำเตือนว่าผู้ใช้ภายใต้โปรเจกต์ที่เลือกไม่สามารถใช้กลุ่มทรัพยากรที่เลือกได้

การแจ้งเตือนเดียวกันจะแสดงซ้ำภายในโมดัลการตั้งค่าน้ำหนักด้วย จึงเห็นความไม่สอดคล้องได้ในขณะที่แก้ไขน้ำหนัก ไม่ใช่เฉพาะในตารางเท่านั้น

ในแต่ละขั้นตอน ฟีเจอร์ทั่วไปต่อไปนี้พร้อมใช้งาน:

- **ตัวกรอง**: ตัวกรองคุณสมบัติเหนือตารางช่วยจำกัดแถวที่แสดง ขั้นตอนกลุ่มทรัพยากรกรองได้ด้วย **ชื่อ**, **คำอธิบาย**, **สถานะใช้งาน** และ **สถานะสาธารณะ** ส่วนขั้นตอนโดเมนและโปรเจกต์กรองได้ด้วย **ชื่อ** และ **สถานะการใช้งาน** บนแมเนเจอร์ที่ไม่รองรับการรวมเงื่อนไข จะมีเฉพาะ **ชื่อ** และใช้ได้ครั้งละหนึ่งเงื่อนไขเท่านั้น
...
- **สถานะ**: แสดงว่ากลุ่มทรัพยากร **คล่องแคล่ว** หรือ **ไม่ทำงาน**
- **วันที่สร้าง**: timestamp การสร้าง
...
- **สถานะ**: แสดงว่าโดเมน **คล่องแคล่ว** หรือ **ไม่ทำงาน**
...
- **Image Status**: สถานะวงจรชีวิตของอิมเมจ ได้แก่ `ALIVE`, `DELETED`, `PURGING` หรือ `PURGE_ERROR`
- **ประเภท**: ประเภทของอิมเมจ ได้แก่ `COMPUTE`, `SERVICE` หรือ `SYSTEM`
- **ท้องถิ่น**: ระบุว่าอิมเมจมาจาก Docker daemon ในเครื่องของเอเจนต์แทนที่จะมาจากรีจิสทรีหรือไม่
- **Size**: ขนาดของอิมเมจ แสดงในหน่วยฐานสอง
- **Aliases**: นามแฝงที่ลงทะเบียนไว้กับอิมเมจ
- **Supported Accelerators**: ชนิดของตัวเร่งความเร็วที่อิมเมจใช้งานได้

คอลัมน์ **Image Status**, **ประเภท**, **ท้องถิ่น**, **Aliases** และ **Supported Accelerators** ถูกซ่อนไว้โดยค่าเริ่มต้น และแสดงได้ด้วยปุ่มตั้งค่าคอลัมน์ (⚙) ที่อยู่ใต้ตาราง ถัดจากตัวควบคุมการแบ่งหน้า คอลัมน์ส่วนใหญ่ยังเรียงลำดับได้ด้วยการคลิกที่หัวคอลัมน์

ตัวกรองคุณสมบัติด้านบนรายการจะจำกัดอิมเมจตาม **รหัส**, **อิมเมจ**, **ชื่อ**, **ทะเบียน**, **สถาปัตยกรรม**, **เนมสเปซ**, **ชื่อภาพฐาน**, **แท็ก**, **Image Status**, **ประเภท**, **ท้องถิ่น**, **Registry Project**, **Registry ID**, **วันที่สร้าง** หรือ **Supported Accelerators**

:::note
รายการจะแสดงเฉพาะอิมเมจที่ยังใช้งานอยู่ (ALIVE) จนกว่าคุณจะเพิ่มเงื่อนไข **Image Status** ลงในตัวกรองคุณสมบัติ เมื่อเพิ่มเงื่อนไขแล้ว ระบบจะค้นหาทุกสถานะ ทำให้เห็นอิมเมจที่ถูกลบหรือกำลังถูกล้างด้วย
:::

คุณสามารถเลือกอิมเมจที่ยังไม่ได้ติดตั้งหลายรายการ แล้วคลิกปุ่ม **ติดตั้งอิมเมจ** เพื่อติดตั้งพร้อมกันได้ การติดตั้งอิมเมจจะเพิ่มเซสชันระยะสั้นเข้าคิวเพื่อดึงอิมเมจ ดังนั้นไดอะล็อกจึงให้คุณระบุว่าเซสชันดังกล่าวจะทำงานที่ใด

- **โปรเจกต์ของเซสชันการติดตั้ง**: โปรเจกต์ที่เซสชันการติดตั้งจะทำงาน เซสชันจะถูกสร้างในโปรเจกต์นี้ในฐานะเซสชันของคุณเอง
  จึงแสดงเฉพาะโปรเจกต์ที่คุณเป็นสมาชิกอยู่เท่านั้น
- **กลุ่มทรัพยากรเป้าหมาย**: กลุ่มทรัพยากรที่จะดึงอิมเมจไปไว้ โดยจะแสดงเฉพาะกลุ่มทรัพยากรที่เข้าถึงได้จากโปรเจกต์
  ที่เลือกเท่านั้น

ทั้งสองช่องเป็นช่องที่ต้องระบุ และปุ่ม **ติดตั้ง** ในไดอะล็อกจะยังคงถูกปิดใช้งานจนกว่าคุณจะเลือกโปรเจกต์และกลุ่มทรัพยากร อิมเมจที่เลือกไว้ซึ่งติดตั้งอยู่แล้วจะถูกยกเว้นจากคำขอ

![](../images/image_install_modal.png)

คุณสามารถเปลี่ยนข้อกำหนดทรัพยากรขั้นต่ำสำหรับแต่ละอิมเมจได้โดยคลิกการกระทำ
**แก้ไข** (ดินสอ) ในคอลัมน์ 'การควบคุม' ซึ่งจะเปิดไดอะล็อก **แก้ไขขีด จำกัด ทรัพยากรภาพขั้นต่ำ** แต่ละอิมเมจมีข้อกำหนดฮาร์ดแวร์และทรัพยากร
...
นอกจากรายละเอียดการเชื่อมต่อแล้ว รายการรีจิสทรียังแสดงคอลัมน์ **โปรเจกต์ที่ได้รับอนุญาต** ซึ่งระบุโปรเจกต์ที่ใช้รีจิสทรีนั้นได้ โดยรีจิสทรีที่แชร์กับทุกโปรเจกต์จะแสดงเป็น **ทุกโปรเจกต์** และคอลัมน์ **Global** จะบอกว่ารีจิสทรีถูกแชร์แบบนั้นหรือไม่ คอลัมน์ **Global** ถูกซ่อนไว้โดยค่าเริ่มต้น และแสดงได้ด้วยปุ่มตั้งค่าคอลัมน์ (⚙) ที่อยู่ใต้ตาราง ถัดจากตัวควบคุมการแบ่งหน้า

คลิกปุ่ม **เพิ่มทะเบียน** เพื่อเพิ่ม Docker registry ส่วนตัวของคุณ กล่องโต้ตอบสร้าง registry มีฟิลด์ต่อไปนี้:
...
คุณยังสามารถเปลี่ยนการตั้งค่าของ registry ที่มีอยู่ได้จากไดอะล็อก **แก้ไขทะเบียน** ยกเว้นชื่อรีจิสทรี
...
ตัวกรองคุณสมบัติด้านบนรายการจะจำกัด preset ตาม **ชื่อ** หรือ **กลุ่มทรัพยากร** และทั้งสองคอลัมน์เรียงลำดับได้ด้วยการคลิกที่หัวคอลัมน์

คุณสามารถตั้งค่าทรัพยากรเช่น CPU, RAM, fGPU เป็นต้น ที่จะให้บริการโดย resource preset ได้โดยคลิกการกระทำ **แก้ไข** (ดินสอ) ในคอลัมน์ชื่อ โมดัล **สร้างค่าที่กำหนดไว้ล่วงหน้าของทรัพยากร** / **แก้ไขค่าที่กำหนดไว้ล่วงหน้าของทรัพยากร** จะแสดงฟิลด์ของทรัพยากรที่มีอยู่ในปัจจุบัน ขึ้นอยู่กับการตั้งค่าเซิร์ฟเวอร์ ทรัพยากรบางอย่างอาจไม่แสดง หลังจากตั้งค่าทรัพยากรตามต้องการ ให้คลิก **บันทึก** และตรวจสอบว่า preset ที่สอดคล้องกันจะแสดงเมื่อสร้างเซสชันการคำนวณ หากทรัพยากรที่มีน้อยกว่าปริมาณทรัพยากรที่กำหนดใน preset ที่สอดคล้องกัน preset จะไม่แสดง
...
Superadmin สามารถดูรายการโหนดเอเจนต์ที่เชื่อมต่อกับ Backend.AI อยู่ในปัจจุบันได้โดยไปที่หน้าทรัพยากร คุณสามารถตรวจสอบ IP ของโหนดเอเจนต์ เวลาที่เชื่อมต่อ ทรัพยากรที่ใช้งานจริงอยู่ในปัจจุบัน เป็นต้น จากแผงรายละเอียดของเอเจนต์ คุณยังสามารถเปลี่ยนสถานะ schedulable และควบคุมบริการเอเจนต์เองได้
...
ใช้ตัวเลือก **เชื่อมต่อแล้ว** / **ยุติแล้ว** เพื่อสลับระหว่างเอเจนต์ที่เชื่อมต่ออยู่ในปัจจุบัน กับเอเจนต์ที่เคยเชื่อมต่อแล้วถูกยกเลิกหรือตัดการเชื่อมต่อ รายการยุติแล้วสามารถใช้เป็นข้อมูลอ้างอิงสำหรับการจัดการโหนด หากรายการว่างเปล่า หมายความว่าไม่มีการตัดการเชื่อมต่อหรือการยกเลิกเกิดขึ้น

![](../images/terminated_agent_list.png)

คุณสามารถจำกัดรายการด้วยตัวกรองคุณสมบัติ ซึ่งรองรับ **ID**, **จุดเชื่อมต่อ** และ **จัดตารางได้**

คลิก ID ของเอเจนต์เพื่อเปิดแผง **ข้อมูลเอเจนต์** ซึ่งแสดงการใช้ทรัพยากรที่แน่นอนของโหนดนั้น แผงนี้มีสองแท็บ ได้แก่ แท็บ **ทรัพยากร** ที่แสดงการใช้งานแยกตามสล็อตทรัพยากร และแท็บ **เซสชัน** ที่แสดงรายการเซสชันการคำนวณซึ่งถูกกำหนดให้กับเอเจนต์นี้ ทำให้คุณเห็นว่ามีอะไรกำลังทำงานอยู่บนโหนดก่อนที่จะหยุดหรือรีสตาร์ทเอเจนต์ ในแท็บ **เซสชัน** ให้ใช้ตัวเลือก **กำลังทำงาน** / **เสร็จสิ้น** เพื่อสลับระหว่างเซสชันที่ยังใช้ทรัพยากรของโหนดอยู่กับเซสชันที่เสร็จสิ้นแล้ว และคลิกชื่อเซสชันเพื่อแสดงรายละเอียดของเซสชันนั้น

![](../images/detailed_agent_node_usage_information.png)

เมื่อเปิดใช้งานฟีเจอร์ทดลอง **Session resource grid view** ในการตั้งค่าผู้ใช้ (ดูที่หัวข้อ [คุณสมบัติการทดลอง](#experimental-features)) แท็บ **เซสชัน** จะแสดงตัวควบคุม **View mode** ถัดจากปุ่มรีเฟรชด้วย สำหรับสลับระหว่าง **Table** และ **Grid** มุมมองกริดจะแทนที่ตารางเซสชันด้วยหนึ่งเซลล์ต่อหนึ่งเซสชันบนเอเจนต์นี้ โดยแต่ละเซลล์มีสีตามการใช้ทรัพยากรแบบเรียลไทม์ของเซสชันนั้น และเป็นไปตามตัวเลือก **กำลังทำงาน** / **เสร็จสิ้น** ที่เลือกอยู่ สำหรับรายละเอียดของตัวควบคุมภายในมุมมองกริด ให้ดูที่หัวข้อ [การแสดงผลรายการเซสชัน](#session-list-view-and-refresh)

![](../images/agent_info_sessions_view_mode.png)

<a id="control-agent-service"></a>

#### เริ่ม หยุด หรือรีสตาร์ทเอเจนต์

แผงข้อมูลเอเจนต์มีกลุ่มปุ่มสำหรับควบคุมบริการเอเจนต์เอง:

- **เริ่มตัวแทน**: เริ่มบริการเอเจนต์ขึ้นมาใหม่ ใช้ได้เฉพาะเมื่อเอเจนต์ไม่ได้อยู่ในสถานะ `ALIVE`
- **หยุดตัวแทน**: ปิดบริการเอเจนต์ ใช้ได้เฉพาะเมื่อเอเจนต์อยู่ในสถานะ `ALIVE`
...
:::warning
การกระทำเหล่านี้มีผลกับบริการเอเจนต์บนโหนด ไม่ใช่กับเซสชันใดเซสชันหนึ่ง การหยุดหรือรีสตาร์ทเอเจนต์จะส่งผลต่อเซสชันการคำนวณทุกเซสชันที่กำลังทำงานอยู่บนเอเจนต์นั้น โปรดตรวจสอบว่าโหนดถูกถ่ายงานออกแล้ว หรือคุณยอมรับผลกระทบนั้นได้ ก่อนยืนยัน
:::

   คำขอจะถูกส่งต่อไปยัง watcher daemon ของเอเจนต์ หาก watcher ไม่สามารถเข้าถึงได้ หรือโหนดถูกจัดการในลักษณะที่ไม่รองรับการดำเนินการนั้น WebUI จะแสดงข้อผิดพลาดที่เอเจนต์ส่งกลับมาแทนข้อความสำเร็จ

<a id="set-schedulable-status-of-agent-nodes"></a>

#### ตั้งค่าสถานะ Schedulable ของโหนดเอเจนต์

คุณอาจต้องการป้องกันไม่ให้เซสชันการคำนวณใหม่ถูกจัดตารางไปยังบริการ Agent โดยไม่ต้องหยุดบริการ ในกรณีนี้ คุณสามารถปิดใช้งานสถานะ Schedulable ของ Agent ได้จากปุ่ม **การตั้งค่า** ในแผงข้อมูลเอเจนต์ จากนั้นคุณสามารถบล็อกการสร้างเซสชันใหม่ในขณะที่รักษาเซสชันที่มีอยู่บน Agent
...
คอลัมน์ **ค่าเริ่มต้น** ระบุกลุ่มทรัพยากรเริ่มต้น ซึ่งมีได้เพียงกลุ่มเดียวในแต่ละครั้ง และเอเจนต์ที่ลงทะเบียนโดยไม่มีชื่อกลุ่มทรัพยากรที่ระบุได้จะถูกจัดเข้ากลุ่มนี้
...
- **พร็อกซีที่เก็บข้อมูล SFTP**: พร็อกซีที่เก็บข้อมูลที่ใช้เมื่อผู้ใช้ในกลุ่มทรัพยากรนี้ถ่ายโอนไฟล์ไปยังโฟลเดอร์ของตนผ่าน SFTP ฟิลด์นี้มีให้สำหรับ superadmin และถูกเติมไว้ล่วงหน้าด้วยพร็อกซีที่ให้บริการกลุ่มทรัพยากรนี้อยู่ในปัจจุบัน การบันทึกจะซิงค์เฉพาะการเป็นสมาชิกของกลุ่มนี้เท่านั้น กล่าวคือกลุ่มจะถูกเพิ่มเข้าไปในพร็อกซีที่คุณเลือกใหม่และถูกนำออกจากพร็อกซีที่คุณยกเลิกการเลือก ส่วนกลุ่มทรัพยากรอื่น ๆ บนพร็อกซีเหล่านั้นจะไม่ถูกแตะต้อง หากไม่ได้เลือกพร็อกซีใดเลย กลุ่มทรัพยากรนั้นจะไม่ถูกจำกัดให้ใช้พร็อกซีใดพร็อกซีหนึ่ง
- **ใช้งาน**: เปิด/ปิดสถานะการใช้งานของกลุ่มทรัพยากร
- **สาธารณะ**: เมื่อปิดใช้งาน กลุ่มทรัพยากรนี้จะถูกสงวนไว้สำหรับเซสชันของระบบ เช่น การอัปโหลดผ่าน SFTP กลุ่มนี้จะถูกซ่อนจากรายการกลุ่มทรัพยากรของผู้ใช้ทั่วไป
  และการสร้างเซสชันทั่วไปจะถูกปฏิเสธ แต่ผู้ดูแลระบบยังคงมองเห็นได้ การเปิดใช้งานไม่ได้ขยายสิทธิ์การเข้าถึงด้วยตัวเอง โดเมน โปรเจกต์
  และคู่กุญแจใดที่สามารถใช้กลุ่มทรัพยากรนี้ได้ จะถูกกำหนดโดยการตั้งค่าการเชื่อมโยงของแต่ละรายการ
...
คุณสามารถสร้างกลุ่มทรัพยากรใหม่ได้โดยคลิกปุ่ม **สร้างกลุ่มทรัพยากร** เช่นเดียวกับตัวเลือกการสร้างอื่นๆ คุณไม่สามารถสร้างกลุ่มทรัพยากรที่มีชื่อเดียวกันกับที่มีอยู่แล้วได้ เนื่องจากชื่อเป็นค่าคีย์

![](../images/create_resource_group.png)

<a id="bulk-edit-resource-groups"></a>

### แก้ไขกลุ่มทรัพยากรหลายกลุ่มพร้อมกัน

การตั้งค่าที่ใช้เหมือนกันกับกลุ่มทรัพยากรหลายกลุ่มสามารถเปลี่ยนได้ในครั้งเดียว:

1. เลือกกลุ่มทรัพยากรด้วยช่องทำเครื่องหมายของแถว (ช่องทำเครื่องหมายมีให้สำหรับ superadmin)
2. จำนวนที่เลือกจะปรากฏบนแถบเครื่องมือ คลิกปุ่ม **แก้ไขเป็นชุด** (เฟือง) ที่อยู่ถัดไป
...
- การเลือกพร็อกซีจะทำให้กลุ่มทรัพยากร **ทุกกลุ่ม** ที่เลือกไว้ถูกเพิ่มเข้าไปในพร็อกซีนั้น
- การยกเลิกการเลือกพร็อกซีจะนำกลุ่มทรัพยากรที่เลือกออกจากพร็อกซีนั้น เนื่องจากเป็นการตัดการเข้าถึง SFTP ไดอะล็อกจะขอให้คุณยืนยันก่อนนำการเปลี่ยนแปลงไปใช้ พร้อมระบุชื่อพร็อกซีที่เกี่ยวข้อง

ในทั้งสองกรณี กลุ่มทรัพยากรที่คุณไม่ได้เลือกจะยังคงการกำหนดพร็อกซีเดิมไว้
...
สิทธิ์โฟลเดอร์ผู้ใช้คือสิทธิ์ที่กำหนดไว้ในนโยบายทรัพยากรคีย์แพร์ หากต้องการจำกัดรายการให้เหลือผู้ใช้คนเดียว ให้เพิ่มเงื่อนไข **ผู้ใช้** ในแถบตัวกรองเหนือตาราง ช่องค่าเป็นดรอปดาวน์ที่ค้นหาบัญชีผู้ใช้ได้ ไม่ใช่ช่องพิมพ์อิสระ คุณจึงเลือกบัญชีแทนการพิมพ์ที่อยู่ จากนั้นระบบจะแสดงเฉพาะนโยบายทรัพยากรคีย์แพร์ที่กำหนดให้กับคีย์แพร์ของผู้ใช้นั้น และแท็กตัวกรองที่ได้จะแสดงอีเมลของผู้ใช้ คุณสามารถดูคีย์การเข้าถึงหลักของผู้ใช้ได้จากคอลัมน์ **คีย์แพร์ ที่กำหนด** ซึ่งจะแสดงเฉพาะเมื่อเลือกผู้ใช้แล้วเท่านั้น การเลือกผู้ใช้คนอื่นจะแทนที่เงื่อนไขเดิมแทนที่จะเพิ่มเงื่อนไขใหม่
...
คลิกปุ่ม `แก้ไข` ในคอลัมน์รหัสขอบเขตโควตา เพื่อเปิดโมดอลสำหรับกำหนดค่าโควตา
...
- **ปรับฐานข้อมูลการใช้งานให้ตรงกับสถานะปัจจุบัน**: บางครั้ง เนื่องจากการเชื่อมต่อเครือข่ายที่ไม่เสถียรหรือ
  ...
  ในกรณีนี้ ให้คลิก **คำนวณการใช้งานใหม่** เพื่อแก้ไข
  การครอบครองทรัพยากรด้วยตนเอง

**อิมเมจ / สภาพแวดล้อม**

- **สแกนรายการอิมเมจใหม่จากพื้นที่เก็บข้อมูล**: คลิก **สแกนอิมเมจใหม่** เพื่ออัปเดตข้อมูลเมตาของอิมเมจจาก
  Docker registry ที่ลงทะเบียนทั้งหมด สามารถใช้ได้เมื่อมีการ push อิมเมจใหม่ไปยัง
  docker registry ที่เชื่อมต่อกับ Backend.AI การสแกนอาจใช้เวลานาน ความคืบหน้าจะรายงานในพื้นที่การแจ้งเตือน

**ประกาศ**

- **ประกาศระบบ**: คลิก **แก้ไขประกาศ** เพื่อสร้างหรือเปลี่ยนแบนเนอร์ประกาศที่แสดงต่อผู้ใช้ทุกคนที่ด้านบนของทุกหน้า
  ดูส่วน[ประกาศระบบ](#system-announcement)ด้านล่าง
...
Superadmin สามารถเผยแพร่ข้อความสั้น ๆ ที่ผู้ใช้ทุกคนจะเห็นเป็นแบนเนอร์ที่ด้านบนของทุกหน้า เช่น การแจ้งช่วงเวลาบำรุงรักษาที่วางแผนไว้ ประกาศเขียนด้วย Markdown และจัดการได้ทั้งหมดจาก WebUI ไม่ว่าจะเคยเผยแพร่ประกาศมาก่อนหรือไม่ก็ตาม

เปิดตัวแก้ไขได้สองวิธี:

- จากหน้าการบำรุงรักษา คลิก **แก้ไขประกาศ** ในแถว **ประกาศระบบ** วิธีนี้ใช้ได้แม้ยังไม่มีประกาศใด ๆ จึงเป็นวิธีสำหรับเผยแพร่ประกาศแรก
- จากแบนเนอร์ประกาศโดยตรง เมื่อมีประกาศที่เผยแพร่อยู่แล้ว เนื่องจากแบนเนอร์แสดงที่ด้านบนของทุกหน้า superadmin จึงเปิดตัวแก้ไขได้จากทุกที่ใน WebUI

![](../images/announcement_edit_modal.png)

ไดอะล็อก **แก้ไขประกาศ** แบ่งเป็นสองส่วน:

- **ข้อความ**: ตัวแก้ไข Markdown พร้อมแถบเครื่องมือจัดรูปแบบ แถบเครื่องมือมีหัวข้อ (H1–H3), ตัวหนา, ตัวเอียง, ขีดฆ่า, อ้างอิง, โค้ด, ลิงก์, รูปภาพ, รายการหัวข้อย่อย และรายการลำดับเลข ปุ่มแต่ละปุ่มจะใช้กับส่วนที่เลือกอยู่ หรือแทรกข้อความตัวอย่างที่คุณพิมพ์ทับได้
- **ตัวอย่าง**: แสดงข้อความแบบเรียลไทม์ตรงตามที่ผู้ใช้จะเห็น

ส่วนท้ายมีการกระทำสามอย่าง:

- **เผยแพร่**: บันทึกข้อความและแสดงแบนเนอร์ให้ผู้ใช้ทุกคนเห็น ปุ่มจะถูกปิดใช้งานขณะที่ข้อความว่างเปล่า เนื่องจากไม่สามารถเปิดใช้งานประกาศที่ว่างเปล่าได้
- **ยกเลิก**: ปิดไดอะล็อกโดยไม่บันทึก
- **ลบ**: นำประกาศที่เผยแพร่ออก ทำให้แบนเนอร์หายไปสำหรับทุกคน จะมีไดอะล็อกยืนยันปรากฏขึ้นก่อน หากยังไม่มีประกาศที่เผยแพร่อยู่ การกระทำนี้จะใช้ไม่ได้
...
แทนที่จะต้องค้นหาบทบาทของโปรเจกต์ในหน้าการจัดการ RBAC คุณสามารถจัดการผู้ดูแลของโปรเจกต์ได้โดยตรงจากรายการโปรเจกต์

1. ค้นหาโปรเจกต์ในรายการโปรเจกต์
2. คลิกการกระทำ **ตั้งค่าผู้ดูแลโปรเจกต์** (โล่) ในแถวนั้น การกระทำนี้ใช้ไม่ได้กับโปรเจกต์ประเภทคลังโมเดล
...
- การแจ้งเตือนข้อมูลที่อธิบายว่าการให้สิทธิ์ผู้ดูแลโปรเจกต์จะเพิ่มโปรเจกต์นั้นเข้าไปในรายการโปรเจกต์ที่ได้รับอนุญาตของผู้ใช้โดยอัตโนมัติ ผู้ใช้จึงเข้าถึงโปรเจกต์นั้นได้หลังจากนั้น
- ฟิลด์ **ค้นหาและเลือกผู้ใช้** สำหรับเลือกบัญชีหนึ่งบัญชีขึ้นไป ตามด้วยปุ่ม **เพิ่ม** ที่ให้สิทธิ์ผู้ดูแลโปรเจกต์แก่บัญชีเหล่านั้น
- ตารางแสดงผู้ใช้ที่ถือสิทธิ์นี้อยู่ในปัจจุบัน โดยแสดงอีเมลและ ID ของผู้ใช้แต่ละคน พร้อมการกระทำ **เพิกถอนสิทธิ์ผู้ดูแลระบบ** ที่ท้ายแถว

ไดอะล็อกจะยังคงเปิดอยู่หลังการเปลี่ยนแปลงแต่ละครั้ง คุณจึงให้และเพิกถอนสิทธิ์แก่หลายคนได้ในคราวเดียว หากสำเร็จเพียงบางส่วน ระบบจะรายงานข้อผิดพลาดแยกตามผู้ใช้

:::warning
การเพิกถอนสิทธิ์ผู้ดูแลโปรเจกต์ **จะไม่** ลบโปรเจกต์นั้นออกจากรายการโปรเจกต์ที่ได้รับอนุญาตของผู้ใช้ หากต้องการเพิกถอนการเข้าถึงโปรเจกต์ด้วย โปรดลบออกจากรายการนั้นด้วยตนเอง ดูส่วน[เรียกดูและจัดการผู้ใช้](#create-and-update-users)
:::

สำหรับแบบจำลองบทบาทและสิทธิ์ทั้งหมดที่อยู่เบื้องหลังทางลัดนี้ และขั้นตอนที่เทียบเท่าในหน้าการจัดการ RBAC โปรดดูส่วน[การให้สิทธิ์ผู้ดูแลโปรเจกต์](#grant-project-admin)
...
ผู้ดูแลระบบสูงสุด (superadmin) สามารถเปิดหน้า **การวินิจฉัย** จากแถบด้านข้างของผู้ดูแลระบบเพื่อเรียกใช้การตรวจสอบสถานะอัตโนมัติของการ deploy WebUI หน้านี้จะประเมินการกำหนดค่าระหว่างเบราว์เซอร์กับเซิร์ฟเวอร์ และรายงานการตั้งค่าที่ไม่ถูกต้อง เพื่อให้สามารถระบุปัญหาการเชื่อมต่อหรือความปลอดภัยได้ก่อนที่จะส่งผลกระทบต่อผู้ใช้

![](../images/diagnostics_page.png)

แถบเครื่องมือที่ด้านบนของหน้ามีตัวควบคุมต่อไปนี้

- **แสดงเฉพาะรายการที่ล้มเหลว**: ปุ่มสลับที่ซ่อนการตรวจสอบที่ผ่านทั้งหมด และแสดงเฉพาะส่วนที่มีคำเตือนหรือข้อผิดพลาด
- `เรียกใช้การวินิจฉัยอีกครั้ง`: ประเมินการตรวจสอบทั้งหมดอีกครั้งและรีเฟรชผลลัพธ์
- `ส่งออก CSV`: ใช้งานได้จากเมนูดรอปดาวน์การดำเนินการ โดยจะส่งออกผลการวินิจฉัยทั้งหมดเป็นไฟล์ชื่อ `diagnostics-YYYY-MM-DD.csv`
...
การวินิจฉัยจัดเป็นส่วนที่ยุบได้ดังต่อไปนี้ โดยแต่ละส่วนจะแสดงผลการตรวจสอบแต่ละรายการด้วยระดับความรุนแรงแบบผ่าน คำเตือน หรือวิกฤต

- **นโยบายความปลอดภัยของเนื้อหา**: ตรวจสอบส่วนหัวของนโยบายความปลอดภัยของเนื้อหา (เช่น `connect-src`, `frame-src`, `script-src`, `style-src`) เพื่อให้แอปพลิเคชันเข้าถึงทรัพยากรที่ต้องการได้
- **พร็อกซีที่เก็บข้อมูล**: ตรวจสอบรายการสตอเรจวอลุ่มพร้อมกับความสามารถในการเข้าถึงและตัววัดการใช้งานของพร็อกซีที่เก็บข้อมูล
- **การเชื่อมต่อจุดปลายทาง**: ตรวจสอบว่าจุดปลายทาง API เข้าถึงได้และสามารถสร้างการเชื่อมต่อได้
- **การกำหนดค่าเว็บเซิร์ฟเวอร์**: ตรวจสอบการตั้งค่า `config.toml` รวมถึงรูปแบบ URL ความสอดคล้องของ SSL/โปรโตคอล และโหมดการเชื่อมต่อ
...
หน้า **การสร้างแบรนด์** ช่วยให้ผู้ดูแลระบบสูงสุดปรับแต่งเอกลักษณ์ทางภาพของ WebUI ได้แก่ สี โลโก้ และแบบอักษร ให้ตรงกับเอกลักษณ์ขององค์กร โดยสามารถเข้าถึงได้จากแถบด้านข้างของผู้ดูแลระบบ

![](../images/branding_page.png)

:::warning
ธีมที่แก้ไขแล้วไม่สามารถนำไปใช้ได้ทันทีจากหน้านี้ หากต้องการใช้ธีมที่ปรับแต่ง ให้ส่งออกการกำหนดค่าเป็นไฟล์ JSON แล้วส่งให้ทีมสนับสนุน
:::

แถบเครื่องมือมีดังนี้

- `ดูตัวอย่าง`: เปิดหน้าต่างใหม่เพื่อดูตัวอย่างการเปลี่ยนแปลงธีมปัจจุบัน
- `การตั้งค่า JSON`: เปิดหน้าต่างโมดอลที่สามารถแก้ไขการกำหนดค่าธีมเป็น JSON ได้โดยตรงและส่งออก
- ตัวควบคุมรีเซ็ตที่คืนค่าการตั้งค่าการสร้างแบรนด์ทั้งหมดเป็นค่าเริ่มต้น
- แถบค้นหาสำหรับกรองการตั้งค่า

การตั้งค่าแบ่งเป็นกลุ่มดังนี้

- **ธีม**: ตัวเลือกสีสำหรับสีหลัก สีพื้นหลังส่วนหัว สีลิงก์ สีข้อมูล สีข้อผิดพลาด สีสำเร็จ และสีข้อความ แต่ละสีสามารถตั้งค่าแยกกันสำหรับโหมดสว่างและมืด และรีเซ็ตได้ทีละรายการ
- **โลโก้ CI**: อัปโหลดโลโก้แถบด้านข้างหลักสำหรับโหมดสว่างและมืด พร้อมกับโลโก้แถบด้านข้างแบบยุบ และกำหนดขนาดการแสดงผล
- **โลโก้ CI แบบละเอียด**: อัปโหลดโลโก้ที่แสดงบนหน้าเข้าสู่ระบบและในโมดอล About สำหรับทั้งโหมดสว่างและมืด พร้อมกำหนดขนาด
- **แบบอักษร**: เลือกแบบอักษร (font family) ที่ใช้ทั่วทั้งอินเทอร์เฟซ

:::note
หลังจากอัปโหลดรูปภาพหรือเปลี่ยนสี ให้ใช้ `ดูตัวอย่าง` เพื่อดูผลลัพธ์ เนื่องจากการเปลี่ยนแปลงจะไม่ถูกนำไปใช้กับบริการที่ใช้งานจริงโดยอัตโนมัติ ให้ส่งออกการกำหนดค่า JSON และติดต่อทีมสนับสนุนเพื่อนำไปใช้
:::
