...
ใช้แถบตัวกรองคุณสมบัติเหนือตารางเพื่อจำกัดรายการให้แคบลง คุณสมบัติพื้นฐานได้แก่ User ID (อีเมล), ID, ชื่อ (username), โปรเจกต์ และบทบาท คุณสามารถจัดเรียงรายการโดยคลิกที่ส่วนหัวของคอลัมน์ และสถานะของตัวกรองและการจัดเรียงจะถูกนำไปใช้กับข้อมูลทั้งหมด ไม่ใช่เฉพาะหน้าปัจจุบัน

นอกจากคอลัมน์พื้นฐานแล้ว รายการผู้ใช้ของ super-admin ยังแสดงคอลัมน์เพิ่มเติมหลายคอลัมน์ที่ดึงมาจากเรกคอร์ดของผู้ใช้:

- **คีย์การเข้าถึงหลัก (Main Access Key)**: คีย์การเข้าถึงหลักของผู้ใช้ที่ใช้สำหรับการตรวจสอบสิทธิ์ API ค่านี้สามารถคัดลอกได้
- **เวลาที่เปิดใช้งาน 2FA**: เวลาที่เปิดใช้งานการตรวจสอบสิทธิ์สองปัจจัยสำหรับผู้ใช้
- **IP ไคลเอนต์ที่อนุญาต**: ที่อยู่ IP หรือช่วง CIDR ที่ผู้ใช้ได้รับอนุญาตให้เข้าถึงระบบ
- **ต้องเปลี่ยนรหัสผ่าน**: ผู้ใช้ถูกตั้งค่าให้ต้องเปลี่ยนรหัสผ่านในการเข้าสู่ระบบครั้งถัดไปหรือไม่
- **UID คอนเทนเนอร์ / GID หลักของคอนเทนเนอร์ / GIDs คอนเทนเนอร์**: หมายเลข user ID, group ID หลัก และ group ID เสริมที่กำหนดให้กับโปรเซสภายในคอนเทนเนอร์ของผู้ใช้

:::note
เมื่อ Backend.AI Manager ที่เชื่อมต่ออยู่เป็นเวอร์ชัน 26.4.4 ขึ้นไป จะมีคุณสมบัติตัวกรองเพิ่มเติมในแถบตัวกรองคุณสมบัติ ได้แก่ ชื่อเต็ม, คำอธิบาย, ข้อมูลสถานะ และนโยบายทรัพยากร (จับคู่สตริง); ต้องเปลี่ยนรหัสผ่าน, เปิดใช้งาน 2FA และเปิดใช้งานเซสชัน sudo (boolean); UID คอนเทนเนอร์ และ GID หลักของคอนเทนเนอร์ (จำนวนเต็ม); และ GIDs คอนเทนเนอร์ (การตรวจสอบการมีอยู่ของ GID เดี่ยว) ตัวกรองเหล่านี้จะแสดงเฉพาะเมื่อ Manager รองรับเท่านั้น
:::
...
เมื่อคุณต้องการสร้างบัญชีผู้ใช้หลายบัญชีจากรายชื่อที่มีอยู่หรือข้อมูลที่ส่งออกมา คุณสามารถอัปโหลดไฟล์ CSV ได้โดยตรงแทนการระบุ prefix แบบลำดับ คลิกปุ่มดรอปดาวน์จุดไข่ปลา (`...`) ถัดจากปุ่ม **สร้างผู้ใช้** และเลือก **สร้างผู้ใช้แบบกลุ่มจาก CSV** เพื่อเปิดไดอะล็อกอัปโหลด CSV
...
คลิกไอคอนลบในคอลัมน์ 'การควบคุม' ป๊อปโอเวอร์สำหรับยืนยันจะปรากฏขึ้น และคุณสามารถ
...
:::note
การลบถาวรแบบกลุ่มพร้อมตัวเลือกช่องทำเครื่องหมายต้องใช้ Backend.AI Manager
เวอร์ชัน 26.3.0 ขึ้นไป สำหรับ Manager เวอร์ชันก่อนหน้า การลบถาวรจะทำงานโดย
ลบผู้ใช้ที่เลือกทีละรายตามลำดับ
:::
...
สำหรับคำอธิบายรูปแบบไฟล์กำหนดโมเดลอย่างละเอียด โปรดดูที่ [คู่มือการกำหนดโมเดล](../deployment/deployment.md#model-definition-guide) ในเอกสารการให้บริการโมเดล

:::note
หากต้องการเปิดใช้งานปุ่ม **Deploy** บนการ์ดโมเดลใน Model Store โฟลเดอร์ที่เชื่อมโยงต้องมี `service-definition.toml` ส่วน `model-definition.yaml` จำเป็นเฉพาะเมื่อใช้ runtime variant `Custom` เท่านั้น โดย runtime variant แบบ preset (เช่น `vLLM`, `SGLang`, `NVIDIA NIM`, `Modular MAX`) ไม่จำเป็นต้องมี สำหรับรายละเอียดเกี่ยวกับไฟล์กำหนดบริการ โปรดดูที่ส่วน [ไฟล์กำหนดบริการ](../deployment/deployment.md#service-definition-file) ในเอกสารการให้บริการโมเดล
:::
...
ผู้ดูแลระบบและผู้ดูแลระบบขั้นสูงสามารถเข้าถึงหน้า Admin Deployments ที่ `/admin-deployments` ซึ่งให้มุมมองข้ามโปรเจกต์ของการปรับใช้ทั้งหมดในคลัสเตอร์ หน้านี้แสดงคอลัมน์ **โปรเจกต์** เพิ่มเติมจากคอลัมน์รายการการปรับใช้มาตรฐาน ช่วยให้ผู้ดูแลระบบจัดการการปรับใช้ข้ามทุกโปรเจกต์

![](../images/admin_serving_page.png)

หน้า Admin Deployments มีได้สูงสุดสี่แท็บ:

- **การปรับใช้**: แสดงเสมอ แสดงรายการการปรับใช้ข้ามทุกโปรเจกต์ พร้อมตัวกรองวงจรชีวิตและคุณสมบัติเดียวกับหน้าการปรับใช้สำหรับผู้ใช้
- **การจัดการร้านค้าโมเดล**: แสดงเสมอ มีให้เฉพาะผู้ดูแลระบบขั้นสูง ดูส่วน[การจัดการคลังโมเดลสำหรับผู้ดูแลระบบ](#admin-model-store-management)ด้านล่าง
...
**Keypair ที่กำหนด** ซึ่งแสดงว่าคีย์แพร์ใดของผู้ใช้ที่กำลังผูกกับแต่ละนโยบายในขณะนี้
...
หากต้องการบันทึกรายการนโยบายทรัพยากรปัจจุบันเป็นไฟล์ CSV ให้ใช้การดำเนินการ **ส่งออก CSV (Export CSV)** ที่ **มุมล่างขวาของตาราง** ซึ่งใช้ได้กับแท็บนโยบายทรัพยากร Keypair, User และ Project เหมือนกัน ตัวควบคุมการส่งออก CSV ถูกย้ายจากบริเวณส่วนหัวไปยังแถบเครื่องมือที่มุมล่างขวาของตาราง เพื่อให้สอดคล้องกับรายการอื่น ๆ ของผู้ดูแลระบบ
...
การคลิกที่ส่วนหัวของคอลัมน์ที่จัดเรียงได้จะจัดเรียงรายการอิมเมจทั้งหมดที่ฝั่งเซิร์ฟเวอร์ (ครอบคลุมทุกหน้า ไม่ใช่เฉพาะหน้าที่แสดงอยู่ในปัจจุบัน) และสถานะการจัดเรียงที่ใช้งานอยู่จะถูกเก็บไว้ใน URL ของหน้า จึงคงอยู่เมื่อคุณโหลดซ้ำหรือแชร์ลิงก์
...
- **ตั้งเป็นรีจิสทรีระดับระบบ (Set as Global Registry)**: ปุ่มสลับที่เมื่อเปิดใช้งานจะอนุญาตให้เข้าถึง registry จากทุกโครงการ ฟิลด์นี้ใช้ได้ตั้งแต่เวอร์ชัน 24.09.0
- **โปรเจกต์ที่ได้รับอนุญาต (Allowed Projects)**: เมื่อปิดปุ่มสลับ **ตั้งเป็นรีจิสทรีระดับระบบ** ให้ใช้ฟิลด์นี้เพื่อเลือกโครงการเฉพาะที่ได้รับอนุญาตให้ใช้ registry ฟิลด์นี้ใช้ได้ตั้งแต่เวอร์ชัน 25.3.0
...
ในการจัดการ storage host ให้คลิกชื่อ storage host (หรือ 'การตั้งค่า' ในคอลัมน์ 'การควบคุม')
...
ในแท็บ Capacity ได้โดยไม่คำนึงถึงประเภทของพื้นที่จัดเก็บ แต่คุณไม่สามารถกำหนดค่าโควตา
...
โดยจะเปิดจากรายการ Storages และแสดงชื่อโฮสต์กับเส้นทาง mount ที่ด้านบน สรุปทรัพยากร
และชุดแท็บ หน้าการตั้งค่าพื้นที่จัดเก็บแบบแยกที่เคยมีอยู่ก่อนหน้านี้ได้ถูกรวมเข้ากับ
แท็บ **Capacity** ของ drawer นี้แล้ว
...
- **Capacity**: กำหนดค่าโควตาพื้นที่จัดเก็บต่อผู้ใช้และต่อโครงการ และดูการใช้งานกับ
  ความสามารถของโฮสต์ ดูส่วน [แผงการตั้งค่าโควตา](#quota-setting-panel) ด้านล่าง
...
แท็บ **Capacity** ของ Storage Host Detail Drawer มีสองแผง
...
ก่อนอื่น ในแท็บ Capacity ให้ตรวจสอบให้แน่ใจว่าแท็บย่อยที่ใช้งานของแผง Quota Settings คือ `For User` จากนั้น ให้เลือกผู้ใช้ที่คุณต้องการ
...
แท็บ **สิทธิ์โฟลเดอร์โครงการ** ของ [Storage Host Detail Drawer](#storage-host-detail-drawer) ช่วยให้ผู้ดูแลระบบดูและจัดการว่าโดเมนและโครงการใดสามารถเข้าถึงโฟลเดอร์โครงการของ storage host ที่เลือกได้
...
แท็บนี้มีสองส่วน ได้แก่ ตาราง **Domains** สำหรับโดเมนที่เลือก และตาราง **Projects** สำหรับโครงการในโดเมนนั้น

:::tip
ตัวเลือกโดเมนในแท็บ **สิทธิ์โฟลเดอร์โครงการ** จะตั้งค่าเริ่มต้นเป็นโดเมนที่ใช้งานอยู่ในปัจจุบันเมื่อ drawer เปิดขึ้น ทำให้สิทธิ์โฟลเดอร์ของโดเมนนั้นแสดงขึ้นทันที คุณสามารถสลับไปยังโดเมนอื่นหรือล้างการเลือกเพื่อดูสิทธิ์ในโดเมนอื่นได้
:::

สิทธิ์ที่มีผลสำหรับโฟลเดอร์โครงการคือ **ผลรวม (union) ของสิทธิ์ที่ให้กับโดเมนที่เลือกและสิทธิ์ของโครงการเอง** กล่าวคือ สิทธิ์ที่ให้ไว้ในระดับโดเมนหรือระดับโครงการระดับใดระดับหนึ่งจะมีผลกับโฟลเดอร์ของโครงการนั้น เพื่อให้ชัดเจน ตาราง Projects ใช้ตัวบ่งชี้สามสถานะ (tri-state) สำหรับแต่ละสิทธิ์:

- **กำหนดในโครงการ**: สิทธิ์ถูกให้โดยตรงกับโครงการ
- **รับสืบทอดจากโดเมน**: สิทธิ์ถูกให้ในระดับโดเมน จึงสืบทอดมายังโครงการ
- ไม่ใช่ทั้งสองอย่าง: ไม่มีการให้สิทธิ์ในระดับใดเลย

คุณสามารถเลือกหลายแถว (โดยใช้ช่องทำเครื่องหมายของแถว) ในตาราง Domains, Projects หรือสิทธิ์โฟลเดอร์ผู้ใช้ เพื่อเปรียบเทียบชุดสิทธิ์เคียงข้างกันได้ เมื่อเลือกแถวแล้ว ให้คลิก **แก้ไขสิทธิ์** เพื่อเปิดโมดัลแก้ไขแบบกลุ่ม ซึ่งจะนำชุดสิทธิ์ที่เลือกไปใช้กับเป้าหมายที่เลือกทั้งหมดพร้อมกัน โมดัลจะเปิดขึ้นโดยมีสิทธิ์ทั้งหมดถูกเลือกไว้เป็นค่าเริ่มต้น และเมื่อบันทึก จะเขียนทับสิทธิ์ของเป้าหมายที่เลือกด้วยชุดที่คุณเลือกไว้อย่างถูกต้อง
