...
このダイアログには、接続に使用する**ユーザー**、**ホスト**、**ポート**の値と、
そのまま使用できる`sftp`、`scp`、`rsync`のコマンド例、そして
ローカルマシンに`id_container`ファイルを保存する**SSHキーをダウンロード**ボタンが表示されます。
...
接続先のアドレスは、必ずこのダイアログの**ホスト**と**ポート**の値を使用してください。
これらの値は、Backend.AIの利用方法によって異なります。
WebUIデスクトップアプリは自分のマシン上で動作するプロキシを経由して接続を中継するため、
ホストはローカルアドレスになります。一方、ブラウザはセッションが動作している
リソースグループのApp Proxyに直接接続するため、ホストはそのプロキシのアドレスになります。
ダイアログに仮のアドレスが表示されることはありません。
接続情報を解決できない場合、ダイアログは開かず、エラー通知でその理由が示されます。
[接続エラー](#connection-errors)を参照してください。

ダウンロードしたSSH秘密鍵でコンピュートセッションにSSH接続するには、
シェル環境で次のコマンドを実行します。
`-i`オプションの後にダウンロードした`id_container`ファイルのパスを、
`-p`オプションの後にダイアログに表示されたポート番号を記述し、
ダイアログに表示されたホストを接続先アドレスとして使用します。
コンピュートセッション内のユーザーは通常`work`に設定されていますが、
セッションで他のアカウントを使用している場合は、`work@<ホスト>`の`work`の部分を
実際のセッションアカウントに変更する必要があります。
コマンドを正しく実行すると、コンピュートセッションへのSSH接続が確立され、
コンテナのシェル環境が表示されます。
以下の例は、ホストがローカルアドレスとなるWebUIデスクトップアプリで実行したものです。
実際のダイアログに表示されたホストとポートに置き換えて使用してください。
...
SSH/SFTP接続情報は、セッションが動作しているリソースグループのApp Proxyを通じて解決されます。
この処理に失敗した場合、接続ダイアログは開かず、代わりにその理由を伝える通知が表示されます。
メッセージから、どの段階で失敗したかを判断できます。

- **プロキシはまだ準備ができていません。詳細については、プロキシ設定を確認してください。**:
  WebUIがApp Proxyに到達できなかったか、プロキシが接続情報を返さなかった場合です。
  プロキシがまだ起動中の可能性があるため、数秒待ってからSSH/SFTPアイコンを再度クリックしてください。
  メッセージが繰り返し表示される場合は、自分のマシンからApp Proxyに到達できない状態です。
  たとえば、ファイアウォールによってブロックされている、
  あるいはWebUI自体とは異なるプロトコルで提供されている、といった原因が考えられます。
  管理者に該当リソースグループの**App Proxy サーバアドレス**の確認を依頼してください。
- **プロキシ直接 TCP 接続はまだサポートされていません**: App Proxyは応答しましたが、
  SSH/SFTPに必要な直接TCP接続を提供していない場合です。
  同じセッションでも、Jupyter Notebookやターミナルなどのウェブベースのアプリは引き続き利用できます。
  管理者に該当リソースグループでTCPに対応したApp Proxyを有効化するよう依頼してください。
  それまでの間は、[Backend.AIクライアントパッケージ](#establish-ssh-connection-with-backendai-client-package)を
  使用してSSHでセッションに接続できます。
- **無効なURLのため、アプリを起動できません。**: App ProxyがWebUIで解釈できないアドレスを返した場合です。
  SSH/SFTPアイコンを再度クリックし、それでもメッセージが表示される場合は、
  セッションが属するリソースグループ名とあわせて管理者に報告してください。

ダイアログは実際のアドレスが解決された後にのみ開くため、
表示されているホストとポートは常に実際に接続できる値です。
ダイアログが開かない場合は、接続先アドレスを推測せず、
通知に表示されたエラーを先に解決してください。

接続確立後に発生する問題（WebUIアプリを終了して転送が中断される場合など）については、
トラブルシューティングの章のSFTP切断の項目を参照してください。
...
SSH/SFTPダイアログと同様に、このダイアログも接続情報が解決された場合にのみ開きます。
開かない場合は、[接続エラー](#connection-errors)を参照してください。
...
`Hostname`と`Port`には、ダイアログに表示されたホストとポート番号を入力します。
ホストがローカルアドレスになるのは、WebUIデスクトップアプリを使用する場合のみです。
便宜上、ホスト名を`bai-vscode`に設定します。任意のエイリアスに変更可能です。

```
Host bai-vscode
User work
# 前に控えておいたホストとポート番号を入力する
Hostname 127.0.0.1
Port 49335
