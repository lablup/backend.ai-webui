...
Backend.AIは、作成したコンピュートセッション（コンテナ）へのSSH/SFTP接続をサポートしています。このセクションでは、その方法を学びます。
   ...
   .. note::
      ...
      a panel from the サマリー page. Using this panel, the compatible version will be downloaded automatically.
...
To SSH接続 to the compute session with the downloaded SSH private key, you
...
assigned port number after `-p` option. The ユーザー inside the compute session is
...
you run the command correctly, you can see that SSH接続ion is made to the
...
name `id_container.ppk__PROTECTED_22__id_container.ppk__PROTECTED_23__id_container__PROTECTED_24__/home/work/__PROTECTED_25__~/.ssh/config__PROTECTED_26__C:\Users\[ユーザー name]\.ssh\config` (for Windows) and add the following block.
...
will see the message `ログイン succeeded`.
