...
最近の main ブランチを基準として、モデルサービスを作成するフローは次の 2 ステップに分かれています。
...
- **アプリを外部公開 (Open To Public)**: 有効にすると、アクセストークンなしでエンドポイントにアクセスできます。無効にすると、すべてのリクエストにトークンを含める必要があります。詳細は [トークンの生成](#generating-tokens) を参照してください。
...
- **Runtime**: モデルを実行するサービングランタイムです（例: `vLLM`、`SGLang`、`NVIDIA NIM`、`Modular MAX`、`Custom`）。独自の起動コマンドを定義する場合は `Custom` を選択します。利用可能なランタイムバリアントはバックエンドから動的にロードされます。
...
:::note
Revision の **名前** フィールドは削除されました。各 Revision は自動付与される Revision 番号で識別されます。下記の [Revisions タブ](#revisions-tab) セクションを参照してください。
:::
...
プロジェクトに互換性のあるプリセットがない場合、モーダルは手動モードにフォールバックし、各フィールドを直接入力します。プリセットの作成および管理方法については [デプロイメントプリセット](deployment_presets.md) を参照してください。
...
- **ライフサイクルステータス**: レプリカがライフサイクルのどの段階にあるかを示します（例: *Provisioning*、*Running*、*Terminating*）。
- **ヘルスステータス**: レプリカプロセスの現在の健全性です（例: *Healthy*、*Unhealthy*）。
...
- **ステータス**: Revision の現在の状態です（例: *Active*、*Inactive*、*Applying*）。
- **Runtime**、**イメージ**、**リソースプリセット**: Revision の構成サマリです。
- **作成日時**
...
:::note
Revision 関連の UI（行アクション、モーダルでの確認、アラートテキスト）におけるアクション名は **適用** に統一されました。以前使用されていた *アクティブ化*、*プロモート* といった表現はすべて **適用** に一本化されています。
:::
