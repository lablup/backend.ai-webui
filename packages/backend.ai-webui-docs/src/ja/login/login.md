...
プロジェクト管理者向けの項目は、アカウント全体ではなく**現在選択されているプロジェクト**を基準に判定されます。作業中のプロジェクトはページアドレスの一部であるため、ヘッダーのプロジェクトセレクターでプロジェクトを切り替えるとロールが再評価されます。つまり、同じアカウントでもあるプロジェクトではプロジェクト管理者、別のプロジェクトでは一般ユーザーということがあり、それに応じてプロジェクト管理者向けの項目がサイドバーに表示されたり消えたりします。特定のプロジェクトを指すブックマークや共有リンクを開いた場合も同様で、ページが読み込まれた時点でそのプロジェクトにおけるロールがサイドバーに反映されます。
