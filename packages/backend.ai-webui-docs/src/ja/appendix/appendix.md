...
ジョブスケジューラの動作は、ユーザーWeb-UIから簡単に確認することができます。GPUホストが最大2つのfGPUを割り当てることができる場合、それぞれ1つのfGPUの割り当てを要求する3つのコンピュートセッションを同時に作成してみましょう。セッション起動ダイアログの**カスタム割り当て**セクションで、GPUの値を1に設定し、**セッション数**の値を3に設定します。**セッション数**に1より大きい値を指定して**LAUNCH**ボタンをクリックすると、その数だけのセッションが同時に要求されます。（**セッション数**は、1つのマルチノードセッションを構成するノード数を制御する**クラスターサイズ**とは異なる概念です。）これは、合計3つのfGPUを要求する3つのセッションが、2つのfGPUしか存在しないときに作成される状況です。
...
セッションページに移動し、セッション起動ダイアログを開きます。インストール設定によっては、さまざまなカーネルイメージがあるかもしれません。利用可能なフレームワークとバージョンは管理者が登録した内容によって異なるため、お使いの環境の一覧は以下の図と異なる場合があります。
...
ここでは、特定のTensorFlowバージョンを選択してセッションを作成してみましょう。

![](../images/session_launch_dialog_tf23.png)

作成したセッションのWebターミナルを開き、以下のPythonコマンドを実行します。選択したTensorFlowバージョンがインストールされていることを確認できます。

![](../images/tf23_version_print.png)

今度は、別のTensorFlowバージョンを選択してコンピュートセッションを作成しましょう。リソースが不足している場合は、先に作成したセッションを削除してください。

![](../images/session_launch_dialog_tf115.png)

作成したセッションのWebターミナルを開き、先ほどと同じPythonコマンドを実行します。別のTensorFlowバージョンがインストールされていることを確認できます。

![](../images/tf115_version_print.png)

最後に、PyTorchイメージを使用してコンピュートセッションを作成します。

![](../images/session_launch_dialog_pytorch17.png)

作成したセッションのWebターミナルを開き、以下のPythonコマンドを実行します。選択したPyTorchバージョンがインストールされていることを確認できます。
...
実行中のコンピュートセッション（コンテナ）を、後で新しいコンピュートセッションを作成するために再利用できる新しいDockerイメージに変換したい場合、Backend.AIはセルフサービス方式の「セッションを画像に変換」機能を提供しています。管理者に依頼することなく、WebUIから実行中のセッションを直接コミットでき、作成されたイメージは自分だけに表示されるプライベートなカスタマイズイメージになります。
...
- コンピュートセッションの準備ができたら、セッション詳細パネルを開き、**セッションをカスタマイズされたイメージにプッシュする**（**セッションを画像でコミットする**操作）を選択します。イメージ名を入力して確定します。詳細な手順については、[セッションコミットの保存](#save-session-commit)セクションを参照してください。
- コミットされたイメージは、プライベートなカスタマイズイメージとして作成されます。[マイ環境](#my-environments)ページからこのイメージを確認・管理でき、次回セッションを起動する際に環境一覧から選択できます。

:::note
「セッションを画像に変換」機能はバージョン24.03から利用でき、セッションが`INTERACTIVE`モードの場合にのみ使用できます。この機能はデプロイメントで有効になっている必要があります（`enableContainerCommit`）。**セッションをカスタマイズされたイメージにプッシュする**操作が表示されない場合は、お使いのインストール環境でコンテナコミットが無効になっています。その場合は、代わりに管理者にセッションを新しいイメージに変換するよう依頼してください。セッション名またはIDとプラットフォームに登録されているメールアドレスを知らせると、管理者が完全なイメージ名とタグを通知します。その後、セッション起動ダイアログでそのイメージ名を手動で入力できます。このイメージはプライベートであり、他のユーザーには表示されません。
