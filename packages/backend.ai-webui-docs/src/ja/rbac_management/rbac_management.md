...
RBAC管理ページにアクセスするには、サイドバーメニューのAdministrationセクションで**RBAC管理**をクリックします。
...
- **システム**: Backend.AIが提供する事前定義のロールです。名前や説明は編集できませんが、ユーザーの割り当てと権限を管理できます。
...
2. 作成モーダルで以下のフィールドを入力します:
   ...
   - **プロジェクト**（任意）: ロールを特定のプロジェクトにスコープするプロジェクトを選択します。グローバルロールの場合は空のままにします
...
- **権限**: リソースに対して許可される操作です。操作は2つのカテゴリに分類されます:
