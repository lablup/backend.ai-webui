...
:::note
**割り当てられたユーザー**フィルターはこれをサポートするマネージャーが必要で、**スコープタイプ**と**スコープID**フィルターは**マネージャー26.8.0以降**が必要です。それより古いマネージャーではこれらのフィルターオプションは表示されません。
:::
...
:::note
**自動割り当て**のメタデータ行は**マネージャー26.4.4以降**でのみ表示されます。それより古いマネージャーでは自動割り当ての行は表示されません。
:::
...
:::note
この統合された権限タブには**マネージャー26.8.0以降**が必要です。それより古いマネージャーでは、ロール詳細パネルに**スコープ**タブと**権限**タブが別々に表示されます。**スコープ**タブはロールのスコープエントリ（スコープタイプ、対象、スコープID）を読み取り専用の行として一覧表示し、**権限**タブは各権限を個別に一覧表示して行ごとの追加・削除コントロールを提供します。以下で説明するカードベースのビューとグリッド編集モーダルは、それらのマネージャーでは利用できません。
:::
...
:::note
この制限には**マネージャー26.8.0以降**が必要です。それより古いマネージャーでは、システムの`project-<project_id>-admin`ロールへの直接割り当てが可能であり、そこでプロジェクト管理者権限を付与する唯一の方法であるため、警告アラートと読み取り専用の状態は適用されません。
:::
...
3. ユーザーを割り当てるには[ロールにユーザーを追加](#add-users-to-a-role)の手順を、ユーザーを外すには[ロールからユーザーを解除](#revoke-users-from-a-role)の手順を実行します。
:::
