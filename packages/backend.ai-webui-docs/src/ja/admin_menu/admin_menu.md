...
**Projects** 列は各ユーザーが所属するプロジェクトを、同じく一部をそのまま表示して残りを `+N` タグに
まとめる形式で示します。この列は既定で非表示になっており、テーブル下部のページ送りコントロールの横にある
列設定（⚙）ボタンで表示できます。

一覧の上にある **アクティブ** / **非アクティブ** ボタンでどのアカウントを一覧表示するかを選び、その横の
プロパティフィルターでさらに絞り込めます。利用できるプロパティは **Eメール**、**ID**、**名前**、
**プロジェクト**、**プロジェクト有効**、**ドメイン**、**ドメイン有効**、**連携名**、**作成日**、**役割** です。
拡張ユーザーフィルターに対応したマネージャーでは、**フルネーム**、**リソースポリシー**、**説明**、
**ステータス情報**、**パスワードの変更が必要です**、**2要素認証が有効**、**sudoセッションを有効にする**、
**コンテナUID**、**コンテナGID**、**補助GID** も利用できます。

一覧をCSVファイルとしてエクスポートすると、現在適用中のプロパティフィルターがそのまま適用されるため、先に
一覧を絞り込んでから必要なユーザーだけをエクスポートできます。エクスポートが対応していないフィルター条件は
無視されます。その場合は警告が表示され、エクスポートされたファイルには表に表示されているよりも多くの
ユーザーが含まれることがあります（少なくなることはありません）。
...
一覧の上にあるプロパティフィルターでは、**ユーザーID**、**アクセスキー**、**許可**、**フルネーム**、
**リソースポリシー**、**プロジェクト**、**レート制限**、**クエリの数**、**作成日** でキーペアを絞り込めます。
アクセスキーとその所有者に加えて、所有者の表示名を示す **フルネーム** 列と、キーペアが最後にリクエストを
認証した時刻を示す **最後に使用した** 列が表示されます。一度も使用されていないキーペアは `-` と表示されます。
...
まず、管理者アカウントでログインし、プロジェクトフォルダを作成します。管理者用データページに
移動してから、「フォルダー作成」をクリックしてフォルダ作成ダイアログを開きます。
管理者用データページではプロジェクトフォルダのみ作成できるため、このダイアログにはフォルダの種類を選択する項目は
表示されません。
...
- **デプロイ**: すべてのプロジェクトのデプロイメント一覧を表示し、ユーザー向けデプロイページと同じライフサイクルおよびプロパティフィルターを提供します。このプロジェクト横断のビューには **プロジェクト** フィルタープロパティも用意されています。
...
- **クライアントIP（Client IP）**: アクションを要求したIPアドレスです。サーバーが報告したまま表示され、管理者のクライアントIPマスキングポリシーによっては一部がマスクされ、記録されていない場合は`-`と表示されます。
...
- **プロジェクト（Project）**: 所有プロジェクトで検索します。プロジェクトの識別子を直接入力するのではなく、モデルストア（Model Store）プロジェクトが並ぶ検索可能なドロップダウンから選択できます。
...
テーブル上部のプロパティフィルタで一覧を検索・絞り込みでき、任意の列見出しをクリックすることで並び順を変更できます。**カテゴリ** プロパティはカテゴリ ID を直接入力するのではなく、このクラスターに定義されたカテゴリから選択できます。
...
検索に対応しており、クラスター内のすべての有効なリソースグループを名前順に表示します。選択した値はページURLの `resourceGroup` クエリパラメータとして保持されるため、タブを
ブックマークしたり共有したりして同じリソースグループに戻れます。URLに指定されたリソースグループがすでに存在しない、
または無効になっている場合は、代わりに最初の有効なリソースグループが表示されます。
...
**セッション** タブでは、プロパティフィルターを使って一覧を絞り込めます。必要に応じて **セッションID** で
特定のセッションを探せるほか、セッションの所有者（Eメールまたはフルネーム）、プロジェクトやドメイン、
結果・クラスターモード・優先度・作成時刻や終了時刻といったセッションの属性でも絞り込めます。

一覧をCSVファイルとしてエクスポートすると、その時点で適用されているプロパティフィルターも反映されるため、
先に一覧を絞り込んでから必要なセッションだけをエクスポートできます。エクスポートが対応していないフィルター条件は
無視されるため、エクスポートされたファイルに表よりも多くのセッションが含まれることはあっても、少なくなることは
ありません。
...
- **フィルター**: テーブル上部のプロパティフィルターで行を絞り込めます。リソースグループステップでは
  **名前**、**説明**、**アクティブステータス**、**公開状況** で、ドメインおよびプロジェクトステップでは
  **名前** と **アクティブ状態** で絞り込めます。条件の組み合わせに対応していないマネージャでは **名前**
  のみが提供され、一度に1つの条件のみ適用できます。
...
- **状態**: リソースグループが **アクティブ** か **非アクティブ** かを表示します。
- **作成日**: 作成のタイムスタンプです。
...
- **状態**: ドメインが **アクティブ** か **非アクティブ** かを表示します。
- **更新日時**: 最終更新のタイムスタンプです。
- **作成日時**: 作成のタイムスタンプです。
...
管理者は環境設定ページのイメージタブで、コンピュートセッションの作成に使用されるイメージを管理できます。このタブでは、現在Backend.AIサーバーにあるすべてのイメージのメタ情報が表示されます。レジストリ、アーキテクチャ、ネームスペース、イメージ名、ダイジェスト、各イメージに必要な最小リソースなどの情報を確認できます。1つ以上のエージェントノードにダウンロードされたイメージの場合、状態カラムに`installed`タグが表示されます。非公開イメージの場合は、状態カラムに`非公開`タグも表示されます。非公開イメージはインストール済みであってもセッションランチャーで選択できません。
...
- **Image Status**: イメージのライフサイクル状態で、`ALIVE`、`DELETED`、`PURGING`、`PURGE_ERROR` のいずれかです。
- **タイプ**: イメージの種類で、`COMPUTE`、`SERVICE`、`SYSTEM` のいずれかです。
- **ローカル**: レジストリではなくエージェントのローカル Docker デーモンから取得したイメージかどうかを示します。
- **Size**: イメージのサイズで、2進接頭辞の単位で表示されます。
- **Aliases**: イメージに登録されたエイリアスです。
- **Supported Accelerators**: イメージが利用できるアクセラレーターの種類です。

**Image Status**、**タイプ**、**ローカル**、**Aliases**、**Supported Accelerators** の各列は既定で非表示に
なっており、テーブル下部のページ送りコントロールの横にある列設定（⚙）ボタンで表示できます。ほとんどの列は
ヘッダーをクリックして並べ替えることもできます。

一覧の上にあるプロパティフィルターでは、**ID**、**イメージ**、**名前**、**レジストリ**、**アーキテクチャ**、
**名前空間**、**ベースイメージ名**、**タグ**、**Image Status**、**タイプ**、**ローカル**、
**Registry Project**、**Registry ID**、**作成日**、**Supported Accelerators** でイメージを絞り込めます。

:::note
プロパティフィルターに **Image Status** の条件を追加するまで、一覧には稼働中（ALIVE）のイメージだけが
表示されます。条件を追加するとすべての状態が対象になり、削除済みや消去中のイメージも表示されます。
:::
...
接続情報のほかに、レジストリ一覧にはそのレジストリを利用できるプロジェクトを示す **許可されたプロジェクト**
列が表示されます。すべてのプロジェクトと共有されているレジストリは **すべてのプロジェクト** と表示されます。
そのように共有されているかどうかは **Global** 列で確認できます。**Global** 列は既定で非表示になっており、
テーブル下部のページ送りコントロールの横にある列設定（⚙）ボタンで表示できます。
...
一覧の上にあるプロパティフィルターでは、**名前** または **リソースグループ** でプリセットを絞り込めます。
どちらの列もヘッダーをクリックして並べ替えられます。
...
表示し、各セルはそのセッションのリアルタイムなリソース使用率に応じて色分けされ、現在の
...
対応していないマネージャでは、一度に1つの条件のみ適用できます。

**デフォルト** 列は既定のリソースグループを示します。既定のリソースグループは同時に1つだけ存在し、解決可能な
リソースグループ名を持たずに登録されたエージェントはこのリソースグループに割り当てられます。
...
- **公開**: 無効にすると、このリソースグループはSFTPアップロードなどのシステムセッション専用になります。
  一般ユーザーのリソースグループ一覧からは非表示になり、通常のセッション作成も拒否されますが、管理者には
  引き続き表示されます。有効にしても利用範囲が広がるわけではありません。どのドメイン・プロジェクト・
  キーペアがこのリソースグループを使用できるかは、それぞれの関連付け設定で決まります。
