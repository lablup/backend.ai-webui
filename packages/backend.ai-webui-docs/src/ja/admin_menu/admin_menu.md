...
ユーザーページはテーブルの状態（フィルター、並び順、ページ、ステータスタブ）をページURLに保持し、タブごとに
別々に記憶します。**ユーザー** タブと **資格情報** タブを行き来しても元の表示が復元され、URLをブックマークしたり
共有したりすれば、まったく同じ一覧を再び開けます。
...
- **レート制限（15分あたり）**: 15分以内にBackend.AIサーバーに送信できるリクエストの
  ...
  値を増やすことを推奨します。
   ...
   `レート制限は0より大きく50000未満である必要があります。` という検証メッセージが表示されて送信がブロックされる
...
各タブはテーブルの状態（フィルター、並び順、ページ）をページURLに個別に保持します。別のタブへ移動して戻っても
元の表示が復元され、コピーしたURLを開けばまったく同じ表示が再現されます。

デプロイメント一覧の右上にある更新ボタンには、自動更新の間隔ドロップダウンが付いています。一覧は15秒ごとに
再読み込みされ、ドロップダウンから別の間隔や **オフ** を選べます。自動更新が有効な間はボタンの枠にカウントダウンが
表示され、次の再読み込みまでの時間がわかります。選んだ間隔はこのブラウザの当該一覧について記憶されます。
...
Prometheus クエリは評価コストが高くなる場合があるため、このタブの更新ボタンは他の管理者向け一覧よりも長い自動更新
間隔（30秒、1分、5分、10分）を提供します。間隔を選ぶまで自動更新は **オフ** で、選んだ間隔はこのブラウザの
当該タブについて記憶されます。
...
3つのタブはそれぞれテーブルの状態（フィルター、並び順、ページ）をページURLに個別に保持します。別のタブへ移動して
戻っても元の表示が復元され、コピーしたURLを開けばまったく同じ表示が再現されます。
...
**ポリシーを作成** ボタンをクリックして新しいリソースポリシーを作成できます。各設定値は上記の
説明と同じです。
...
できます。アイコンをクリックすると、確認ポップアップが表示されます。「削除」ボタンを
クリックすると削除されます。
...
**カスタマイズされた画像の最大数**、**作成日** の各列が表示されます。列設定コントロールで不要な列を非表示にでき、
選択した設定はブラウザごとに保持されます。

フィルタリング、並べ替え、ページ送りはすべてサーバー側で処理されるため、ポリシー数の多いクラスターでもタブは
軽快に動作します。テーブル上部のプロパティフィルタで **名前**、**作成日**、**最大フォルダ数**、
**最大同時ログイン数**、**モデルセッションごとの最大セッション数**、**カスタマイズされた画像の最大数** の条件を
追加して一覧を絞り込み、列見出しをクリックして並び順を変更し、テーブル下部のページャで結果を移動できます。
...
新しいプロジェクトリソースポリシーを作成するには、テーブルの右上にある **ポリシーを作成** ボタンをクリックします。
...
各タブはテーブルの状態（フィルター、並び順、ページ）をページURLに個別に保持するため、タブを切り替えて戻っても
元の表示が復元されます。
...
どちらのタブも自動的に更新されます。**セッション** タブは15秒ごと、**ペンディングセッション** タブは10秒ごとです。
更新ボタン横の間隔ドロップダウンから別の間隔を選ぶか、**オフ** を選んで自動更新を止められます。自動更新が有効な間は
ボタンの枠にカウントダウンが表示され、次の再読み込みまでの時間がわかります。各タブは選んだ間隔を個別に記憶します。
...
- **自動リフレッシュ**: 更新ボタンには間隔ドロップダウンが付いています。フェアシェアのクエリはコストが高いため、
  他の画面より長い間隔（30秒、1分、5分、10分）が用意されており、間隔を選ぶまで自動更新は **オフ** です。
  自動更新が有効な間はボタンの枠にカウントダウンが表示され、次の再読み込みまでの時間がわかります。選んだ間隔は
  このブラウザに記憶されます。このボタンはいつでも手動更新として使用できます。
...
未インストールのイメージを複数選択し、**イメージをインストール** ボタンをクリックすると、利用可能なエージェントノードに一括インストールできます。確認ダイアログのボタンは **インストール** です。
...
プロパティフィルターでは **ID**、**エンドポイント**、**スケジュール可能** の条件で一覧を絞り込めます。一覧は
15秒ごとに再読み込みされ、更新ボタン横の間隔ドロップダウンから30秒、1分、5分を選ぶか、**オフ** を選んで自動更新を
止められます。クラスター全体を対象とする重いクエリのため、ここで提供される最短間隔は15秒です。
...
一覧上部の更新ボタンには、自動更新の間隔ドロップダウンが付いています。間隔を選ぶまで自動更新は **オフ** で、
有効な間はボタンの枠にカウントダウンが表示され、次の再読み込みまでの時間がわかります。選んだ間隔はこのブラウザの
当該タブについて記憶されます。
...
- **システムのお知らせ**: **発表を編集** をクリックすると、すべてのユーザーにページ上部で表示されるお知らせバナーを
  作成または変更できます。下記の[システムのお知らせ](#system-announcement)セクションを参照してください。
...
スーパー管理者は、予定されているメンテナンスの案内などの短いメッセージを、すべてのユーザーにページ上部のバナーとして
表示できます。お知らせはマークダウンで記述し、これまでに公開したものがあるかどうかにかかわらず、すべて WebUI から
管理できます。
...
プロジェクトページにはクラスター内のすべてのプロジェクトが一覧表示され、管理者はプロジェクトの作成、編集、
無効化、有効化、完全削除を行えます。各行には、プロジェクト管理者権限を付与するためのショートカットもあります。
