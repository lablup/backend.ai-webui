...
テーブル上部のプロパティフィルターでリストを検索して絞り込むことができ、**名前**、**作成日**、
**セッション寿命**、**同時実行セッション数**、**クラスターサイズ**、**アイドルタイムアウト**、
**最大同時SFTPセッション数**、**最大ペンディングセッション数** に対応しています。並べ替えとページ送りは
すべてサーバー側で処理されるため、ポリシー数の多いクラスターでもタブは軽快に動作します。列見出しを
クリックして並び順を変更し、テーブル下部のページャで結果を移動できます。
...
プロジェクトリソースポリシーのテーブルには **名前**（インラインの **編集** と **削除** アクション付き）、
**最大フォルダ数**、**最大フォルダサイズ**、**最大ネットワーク数**、**ID**、**作成日** の各列が表示されます。

テーブル上部のプロパティフィルターでリストを検索して絞り込むことができ、**名前**、**作成日**、
**最大フォルダ数**、**最大ネットワーク数** に対応しています。並べ替えとページ送りはすべてサーバー側で
処理されるため、ポリシー数の多いクラスターでもタブは軽快に動作します。列見出しをクリックして並び順を
変更し、テーブル下部のページャで結果を移動できます。
