...
- **デプロイ**: すべてのプロジェクトのデプロイメント一覧を表示し、ユーザー向けデプロイページと同じライフサイクルおよびプロパティフィルターを提供します。このプロジェクト横断のビューには **プロジェクト** フィルタープロパティも用意されており、プロジェクト ID を直接入力するのではなく検索可能なプロジェクトのドロップダウンから選択します。
...
- **プロジェクト（Project）**: 所有プロジェクトで検索します。プロジェクトの識別子を直接入力するのではなく、モデルストア（Model Store）プロジェクトが並ぶ検索可能なドロップダウンから選択し、フィルタータグにはプロジェクト名が表示されます。
...
テーブル上部のプロパティフィルタで一覧を検索・絞り込みでき、任意の列見出しをクリックすることで並び順を変更できます。**カテゴリ** プロパティはカテゴリ ID を直接入力するのではなく、このクラスターに定義されたカテゴリから選択し、フィルタータグにはカテゴリ名が表示されます。
