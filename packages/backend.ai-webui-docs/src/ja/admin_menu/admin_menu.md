...
ユーザー管理ページで、リスト上部の**アクティブ** / **非アクティブ**ステータスセレクターを
使って**非アクティブ**タブに切り替えます。次の2つの方法でユーザーを完全削除できます。
...
- **共有された仮想フォルダも削除しますか？**: 完全削除されるユーザーの仮想フォルダのうち、
  他のユーザーと共有されているものの扱いを決めます。チェックすると、それらのフォルダは
  完全削除を実行する管理者に所有権が移り、削除されずに残ります（名前が管理者の既存フォルダと
  重複する場合はランダムな接尾辞が付きます）。チェックしない場合、それらのフォルダは
  ユーザーの他の仮想フォルダと併せて削除されます。
- **作成されたデプロイメントも削除しますか？**: チェックすると、完全削除されるユーザーが
  作成したデプロイメントも併せて削除されます。チェックしない場合、それらのデプロイメントは
  削除されず、完全削除を実行する管理者に所有権が委任されます。

:::warning
最初のオプションのラベルは、チェックすると共有フォルダが削除されるように読めますが、
実際の動作は逆です。**チェックするとフォルダは管理者の所有として残り**、
チェックしない場合はユーザーの他のデータと併せて削除されます。
:::
