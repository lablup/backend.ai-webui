...
テーブル上部のプロパティフィルターバーを使用して一覧を絞り込めます。基本プロパティはユーザーID（メールアドレス）、ID、名前（ユーザー名）、プロジェクト、ロールです。カラムヘッダーをクリックして一覧をソートでき、フィルターおよびソートの状態は現在のページだけでなくデータ全体に適用されます。

基本カラムに加えて、スーパー管理者のユーザー一覧にはユーザーレコードから取得した以下の追加カラムが表示されます。

- **メインアクセスキー（Main Access Key）**: API認証に使用されるユーザーのメインアクセスキーです。値はコピーできます。
- **2FA有効化日時**: ユーザーに対して二要素認証が有効化された日時です。
- **許可されたクライアントIP**: ユーザーがシステムにアクセスできるIPアドレスまたはCIDR範囲です。
- **パスワード変更要否**: 次回ログイン時にユーザーがパスワードを変更するようフラグが設定されているかどうかです。
- **コンテナUID / コンテナメインGID / コンテナGIDs**: ユーザーのコンテナ内部のプロセスに割り当てられる数値ユーザーID、メイングループID、補助グループIDです。

:::note
接続されているBackend.AI Managerがバージョン26.4.4以降の場合、プロパティフィルターバーで追加のフィルタープロパティを使用できます。フルネーム、説明、ステータス情報、リソースポリシー（文字列一致）、パスワード変更要否、2FA有効化、sudoセッション有効化（boolean）、コンテナUIDおよびコンテナメインGID（整数）、コンテナGIDs（単一GIDの包含判定）が該当します。これらのフィルターは、Managerが該当機能をサポートしている場合にのみ表示されます。
:::
...
既存の名簿やエクスポートしたデータから複数のユーザーアカウントを作成する必要がある場合、連番プレフィックスを指定する代わりに CSV ファイルを直接アップロードできます。**ユーザーを作成** ボタン横の省略記号（`...`）ドロップダウンをクリックし、**CSVからユーザーを一括作成** を選択すると、CSVアップロードダイアログが開きます。
...
ブロックします。コントロールパネルの削除アイコンをクリックしてください。確認を求める
ポップオーバーが表示され、「無効化」ボタンをクリックすることでユーザーを無効化できます。
...
完全削除は**Inactive**タブにあるユーザーに対してのみ可能で、アクティブなユーザーは
先に無効化する必要があります。無効化とは異なり、完全削除は元に戻すことができず、
ユーザーの関連データも併せて削除されます。

Usersページで**Inactive**タブに切り替えます（ステータスセレクターには、完全削除が
ユーザーのキーペアにも影響することを示すために**非アクティブ（キーペアを含む）**と
表示されます）。次の2つの方法でユーザーを完全削除できます。

- **個別の完全削除**: 非アクティブなユーザー1人の行で、コントロール列のゴミ箱
  （完全削除）アイコンをクリックします。
- **一括完全削除**: 行のチェックボックスで1人以上の非アクティブなユーザーを選択し、
  選択件数の横に表示される**ユーザーを完全に削除**ボタン（赤いゴミ箱ボタン）を
  クリックします。
...
- **共有された仮想フォルダも削除しますか？**: チェックすると、完全削除されるユーザーが
  ...
  そのまま残ります。
- **作成されたモデルサービスも削除しますか？**: チェックすると、完全削除されるユーザーが
  作成したモデルサービスも併せて削除されます。チェックしない場合、それらのサービスは
  削除されず、所有権が委任されます。
...
:::note
オプションのチェックボックスを使用する一括完全削除には、Backend.AI Manager
バージョン26.3.0以降が必要です。それ以前のManagerでは、選択したユーザーを1人ずつ
順に削除する方式で動作します。
:::
...
管理者およびスーパー管理者は、クラスター内のすべてのデプロイメントを横断的に表示できる管理者デプロイページ（`/admin-deployments`）にアクセスできます。このページには、標準のデプロイメント一覧列に加えて **プロジェクト** 列が表示され、管理者がすべてのプロジェクトのデプロイメントを管理できます。

![](../images/admin_serving_page.png)

管理者デプロイページには最大 4 つのタブがあります:

- **デプロイ**: 常に表示されます。すべてのプロジェクトのデプロイメント一覧を表示し、ユーザー向けデプロイページと同じライフサイクルおよびプロパティフィルターを提供します。
- **モデルストア管理**: 常に表示されます。スーパー管理者のみ利用可能です。下記の[管理者モデルストア管理](#admin-model-store-management)セクションを参照してください。
...
上部のプロパティフィルターバーを使用して一覧を絞り込むことができ、次のプロパティでフィルタリングできます：

- **名前（Name）**: モデルカードの名前でフィルタリングします（文字列一致）。
- **ドメイン（Domain）**: 所有ドメインでフィルタリングします（文字列一致）。
- **プロジェクト（Project）**: 所有プロジェクトの UUID でフィルタリングします。
- **ストレージホスト（Storage Host）**: 紐づくフォルダーのストレージホストでフィルタリングします。等しい、または等しくない演算子と、入力した値を使用します。
...
ソート可能なカラムヘッダーをクリックすると、現在表示されているページだけでなくイメージ一覧全体がサーバー側でソートされ、適用されたソート状態はページURLに保持されるため、再読み込みやリンク共有の際にも維持されます。
...
- **グローバルレジストリとして設定（Set as Global Registry）**: 有効にすると、すべてのプロジェクトから当該レジストリへのアクセスを許可するトグルです。このフィールドはバージョン24.09.0から利用できます。
- **許可されたプロジェクト（Allowed Projects）**: **グローバルレジストリとして設定**トグルをオフにした場合、このフィールドで当該レジストリを使用できる特定のプロジェクトを選択します。このフィールドはバージョン25.3.0から利用できます。
...
ストレージホストを管理するには、Storages一覧でストレージホスト名（または「コントロール」列の
「設定（歯車）」）をクリックします。すると、容量（クォータ）とフォルダ権限を構成する
**ストレージホスト詳細Drawer**が開きます。
...
Storages一覧から開かれ、上部にホスト名とマウントパス、リソースの概要、そして複数のタブが
表示されます。以前存在していた独立したストレージ設定ページは、このDrawerの**Capacity**タブに
統合されました。
...
- **Capacity**: ユーザーごと・プロジェクトごとのストレージクォータを構成し、ホストの使用量と
  機能を確認します。以下の[クォータ設定パネル](#quota-setting-panel)セクションを参照して
  ください。クォータ設定をサポートしていないストレージホストでは、このタブは利用できません。

<a id="quota-setting-panel"></a>

#### クォータ設定パネル

ストレージホスト詳細Drawerの**Capacity**タブには、2つのパネルがあります。
...
[ストレージホスト詳細Drawer](#storage-host-detail-drawer)の **プロジェクトフォルダの権限** タブで、管理者は選択したストレージホストのプロジェクトフォルダにどのドメインとプロジェクトがアクセスできるかを確認・管理できます。
...
このタブは、選択したドメインに対する **Domains** テーブルと、そのドメインに属するプロジェクトに対する **Projects** テーブルの2つのセクションで構成されます。

:::tip
**プロジェクトフォルダの権限** タブのドメインセレクタは、ドロワーが開いたとき現在のアクティブドメインがデフォルトで選択されるため、そのドメインのフォルダ権限がすぐに表示されます。別のドメインに切り替えたり、選択を解除して他のドメインの権限を確認したりすることもできます。
:::

プロジェクトフォルダに適用される有効権限は、**選択したドメインの権限とプロジェクト自身の権限の和集合（union）** です。つまり、ドメインレベルまたはプロジェクトレベルのいずれかで付与された権限は、そのプロジェクトのフォルダに適用されます。これを明確に示すため、Projectsテーブルは各権限について3つの状態（tri-state）のインジケーターを使用します。

- **プロジェクトに設定済み**: 権限がプロジェクトに直接付与されています。
- **ドメインから継承**: 権限がドメインレベルで付与され、プロジェクトがそれを継承しています。
- どちらでもない: いずれのレベルでも権限が付与されていません。
...
:::tip
Backend.AI からエクスポートされるすべての CSV ファイルには、ファイル先頭に UTF-8 BOM（バイト順マーク）が含まれています。これにより、UTF-8 以外のシステム（例：CP949 を使用する韓国語 Windows）の Microsoft Excel でも文字コードが正しく認識され、韓国語・日本語・中国語などのマルチバイト文字が文字化けせずに表示されます。
:::
