...
デプロイメント一覧の右上にある更新ボタンには、共通の自動更新の間隔ドロップダウンが付いており
（[自動更新](#common-auto-refresh)を参照）、この一覧は15秒間隔から始まります。
...
間隔（30秒、1分、5分、10分）を提供し、間隔を選ぶまで自動更新は **オフ** です
（[自動更新](#common-auto-refresh)を参照）。
...
どちらのタブも自動的に更新されます。**セッション** タブは15秒間隔、**ペンディングセッション** タブは10秒間隔で
始まり、各タブは選んだ間隔を個別に記憶します（[自動更新](#common-auto-refresh)を参照）。
...
- **自動更新**: フェアシェアのクエリはコストが高いため、更新ボタンには他の画面より長い間隔（30秒、1分、5分、
  10分）が用意されており、間隔を選ぶまで自動更新は **オフ** です（[自動更新](#common-auto-refresh)を参照）。
...
15秒の自動更新間隔から始まり、クラスター全体を対象とする重いクエリのため、ここで提供される最短間隔も15秒です
（[自動更新](#common-auto-refresh)を参照）。
...
一覧上部の更新ボタンには、共通の自動更新の間隔ドロップダウンが付いています
（[自動更新](#common-auto-refresh)を参照）。このタブでは間隔を選ぶまで自動更新は **オフ** です。
