...
- **デプロイ**：レプリカ数、リビジョン履歴の保持数、*Open to Public*（公開可否）の既定値。
...
テーブルヘッダー右側の列表示設定（歯車）ボタン（⚙）を使用して、次の列を表示または非表示にできます：**説明**、**起動コマンド**（長い値はツールチップ付きで省略表示され、コピー可能）、**クラスター**、**戦略（Strategy）**、**Open to Public**（Public／Private タグで表示）、**リビジョン履歴の保持数**。
...
- **デプロイ**：
   ...
   * **Open to Public**：このプリセットから生成されたデプロイのエンドポイントを、アクセストークンなしで到達可能にするかの既定値（チェックボックス）。
...
:::note[バージョン要件]
**ヘルスチェックを有効化** トグル、および必須ランタイムパラメータの検証には、Backend.AI Manager バージョン 26.4.4 以降が必要です。古いバックエンドではすべてのランタイムパラメータが任意として扱われます。
:::
...
5. **OK** をクリックすると、選択したプリセットの値を使用してデプロイメントが作成されます。
