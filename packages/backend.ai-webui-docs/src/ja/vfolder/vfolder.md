...
このフォルダに対するユーザーの権限に書き込み権限または削除権限が含まれていない場合、たとえば**読み取り専用**で共有されたフォルダの場合、**削除**、**名前を変更**、**フォルダ作成**のボタンは無効になります。この場合はフォルダの閲覧と内容のダウンロードのみ行えます。
...
フォルダ自体に対する書き込み権限は、フォルダの所有者であるか、共有時に付与された権限によって
決まります。
:::
...
この状態では、**名前**列にある該当フォルダの行の復元ボタンをクリックしてフォルダを復元できます。フォルダを完全に削除する場合は、同じ行の「ゴミ箱」ボタンをクリックしてください。
