...
<!-- TODO: Re-capture data_page.png — フォルダ一覧の上にある更新ボタンが、間隔選択のシェブロンを備えた分割コントロールに変更されたため -->
...
フォルダ一覧の上にある更新ボタンをクリックすると、ページ全体を読み込み直すことなく
フォルダ一覧だけを再読み込みできます。その隣のシェブロンをクリックすると
**自動更新** メニューが開き、一覧が自動的に再読み込みされる間隔を `オフ`、`5秒`、
`10秒`、`15秒`、`30秒`、`1分` から選択できます。現在有効な間隔にはチェックマークが
表示されます。

![](../images/vfolder_auto_refresh_menu.png)
<!-- TODO: Capture screenshot of vfolder_auto_refresh_menu.png — データページの更新ボタンで自動更新メニューを開いた状態。オフ / 5秒 / 10秒 / 15秒 / 30秒 / 1分 の各項目と、有効な項目のチェックマークが見えること -->

フォルダの内容が自動的に変化することはまれなため、フォルダ一覧の自動更新は `オフ` の
状態で始まります。一度間隔を選択すると、その選択はこの一覧について記憶され、次に
ページを開いたときに再適用されます。再び `オフ` を選んだ場合も同様です。

自動更新が有効な間は、選択した間隔がシェブロンの隣に表示され、次の再読み込みまで
ボタンの周囲にカウントダウンの枠線が満ちていきます。手動で更新すると、カウントダウンは
最初からやり直しになります。ページが表示されていない間（たとえば別のブラウザタブに
切り替えた場合）は自動更新が一時停止し、戻ると再開します。
...
- **権限**: フォルダがコンピュートセッションにマウントされるときに適用されるマウント権限。**読み取り・書き込み**はセッション内のフォルダへの書き込みを許可し、**読み取り専用**はそれを防止します。これはユーザーフォルダとプロジェクトフォルダの両方に適用されます。プロジェクトフォルダの場合、すべてのプロジェクトメンバーのアクセスを管理します。
...
このタブにも、[フォルダ一覧を更新](#refresh-folder-list)で説明したものと同じ
**自動更新** 間隔メニュー付きの更新ボタンがあります。こちらの間隔も `オフ` の状態で
始まり、フォルダ一覧とは別に記憶されます。
...
<!-- TODO: Capture screenshot of vfolder_bulk_move_to_trash.png — 削除できるフォルダとできないフォルダが混在した選択から開いたゴミ箱移動モーダル。除外フォルダのアラートとその一覧が確認メッセージの上に表示されていること -->

選択したフォルダの中に削除権限のないものが含まれている場合、モーダルには
*「以下の削除権限のないフォルダーは除外されます。」* というタイトルのアラートが表示され、
該当するフォルダ名が一覧されます。確定する前に必ずこの一覧を確認してください。ゴミ箱
タブに移動するのは残りのフォルダだけであり、アラートの下の確認メッセージも実際に移動
するフォルダのみを数えています。除外されるフォルダが多い場合、一覧はモーダルを広げる
のではなくアラート内でスクロールします。
