...
- **マウント許可**: フォルダがコンピュートセッションにマウントされるときに適用されるマウント権限です。**読み書き可能**はセッション内のフォルダへの書き込みを許可し、**読み取り専用**はそれを防止します。
- **クローン可能**: 使用モードで「モデル」を選択した場合のみ表示されます。作成するバーチャルフォルダ（vfolder）をクローン可能にするかどうかを選択します。

:::note
データページから作成したフォルダは、常に**ユーザーフォルダー**です。**プロジェクトフォルダー**は、管理者が管理者用データページから作成します。
:::

ここで作成したフォルダは、コンピュートセッション作成時に[マウント](#session-mounts)できます。フォルダはユーザーのデフォルト作業ディレクトリ `/home/work/` にマウントされ、マウントされたディレクトリに保存されたファイルはコンピュートセッションが終了しても削除されません。
...
左パネル内ではフォルダ内のすべてのディレクトリとファイルを確認できます。**名前**列でディレクトリ名をクリックすると、そのディレクトリに移動します。各行の操作ボタンは、**名前**列の項目名の隣にあります。**ダウンロード**をクリックするとファイルやディレクトリをダウンロードし、**消去**をクリックすると削除し、名前の隣にある鉛筆（**名前を変更**）ボタンをクリックするとその場で名前を変更できます。列の幅が狭くすべてのボタンを表示できない場合、残りの操作は同じ行の**その他の操作**（⋮）メニューに移動します。より詳細なファイル操作については、コンピュートセッションを作成する際にこのフォルダをマウントし、TerminalやJupyter Notebookなどのサービスを使用して行うことができます。
...
:::note
エディタから保存すると変更したファイルがアップロードされるため、**ファイルを編集**の操作には、対象のストレージフォルダに対する `write_content` 権限（フォルダの共有権限、またはフォルダに付与されたロールを通じて付与されます）と、フォルダがホストされているストレージホストに対する `upload-file` 権限の **両方** が必要です。いずれかが欠けている場合、この操作は使用できません。ファイルの読み込みに失敗した場合、エラーメッセージが表示されます。
:::
...
該当するフォルダ名が一覧されます。削除を進める前にこの一覧を確認してください。権限の
ないフォルダを除いた残りのフォルダだけがゴミ箱タブに移動し、アラートの下の確認
メッセージにも実際に移動するフォルダの数のみが表示されます。

### フォルダの復元と完全削除

この状態では、**名前**列にある該当フォルダの行の復元ボタンをクリックしてフォルダを復元できます。フォルダを完全に削除する場合は、同じ行の「ゴミ箱」ボタンをクリックしてください。完全削除が実際に開始された後は、フォルダを復元することも再度削除することもできず、その行の両方のボタンは *「このフォルダの削除は既に開始されています。」* という理由とともに無効になります。フォルダに対する削除権限がない場合も、その行の「ゴミ箱」ボタンは使用できません。たとえば削除が許可されていない共有フォルダやプロジェクトフォルダがこれに当たり、この場合ボタンには *「削除許可なし」* という理由が表示されます。
...
Backend.AIは[FileBrowser](https://filebrowser.org)をサポートしています。FileBrowserは、Webブラウザを通じてリモートサーバー上のファイルを管理できるプログラムです。ユーザーのローカルマシンからディレクトリをアップロードする際に特に便利です。
...
### SFTPセッションを作成できなかった場合

SFTPセッションを開始できない場合、SSH / SFTP接続ダイアログの代わりに「SFTPセッションの作成に失敗しました」ダイアログが表示されます。このダイアログには、サーバーが報告した失敗理由に続けて、HTTPステータスとエラーコードが括弧内に表示されます（例: `... (HTTP 500, BAI_E0001)`）。そのため、障害の内容をそのまま管理者に報告できます。理由の下には、使用していないアップロードセッションを閉じるとSFTPホストのリソースを解放できるというヒントが表示されます。

ダイアログには次の2つのボタンがあります。

- 「アップロードセッションへ移動」: 「アップロードセッション」タイプで絞り込んだセッションページを開きます。ここで不要になったアップロードセッションを終了できます。
- 「閉じる」: 何も実行せずにダイアログを閉じます。

このエラーの最も一般的な原因は、キーペアリソースポリシーで許可されている同時アップロード（SFTP）セッション数の上限に達していることです。
