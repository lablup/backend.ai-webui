...
- **クライアントIP**: 操作を要求したIPアドレスで、サーバーが報告したまま表示されます。管理者のクライアントIPマスキングポリシーによっては一部がマスクされ、記録されていない場合は`-`と表示されます。
...
選択したフォルダの一部を削除できなかった場合、削除できたフォルダはそのままゴミ箱タブ
に移動し、失敗したフォルダは **エラーメッセージ** 列に理由を添えて失敗ダイアログに
一覧表示されます。
