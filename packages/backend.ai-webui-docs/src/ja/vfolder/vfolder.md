...
この状態では、**名前**列にある該当フォルダの行の復元ボタンをクリックしてフォルダを復元できます。フォルダを完全に削除する場合は、同じ行の「ゴミ箱」ボタンをクリックしてください。完全削除が実際に開始された後は、フォルダを復元することも再度削除することもできず、その行の両方のボタンは *「このフォルダの削除は既に開始されています。」* という理由とともに無効になります。
