...
ページ右上の**編集**ボタンをクリックすると、ボードの右側にカスタムパネルのサイドバーが開き、**閉じる**をクリックすると再び非表示になります。サイドバーにはカスタムパネルの一覧が表示されます。カスタムパネルは、選択したリソースを指定した条件で絞り込んで表示するテーブルです。パネルにはタイトルとテーブルのみが表示されるため、条件がわかるタイトルを付けることをおすすめします。
