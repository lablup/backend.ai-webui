...
更新中もボードは画面に表示されたままです。それまで見ていた値は表示され続け、新しいデータを取得している間もパネルの移動、サイズ変更、スクロールを続けられます。各パネルは自身の進行状況を個別に表示します。

- パネルのデータを取得している間、そのパネルのヘッダーにある更新ボタンにローディング表示が現れます。
- パネルがコンテンツを再構築する必要がある場合は、そのパネルの領域内にのみローディングスケルトンが表示されます。ボードの他の部分は表示されたままで操作できます。

次の自動更新を待たずに、パネルヘッダーの更新ボタンをクリックして個別のパネルをいつでも更新することもできます。

:::note
[スーパー管理者ダッシュボード](#superadmin-dashboard)は**30秒**間隔で更新されます。
:::
