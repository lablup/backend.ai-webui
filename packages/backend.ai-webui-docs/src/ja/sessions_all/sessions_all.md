...
各タブはそれぞれのフィルター、ソート順、ページ番号を保持し、この表示状態はページのURLに
保存されます。別のタブに移動して戻っても状態がリセットされることはなく、直前に見ていた表示が
そのまま復元されます。ブラウザを再読み込みしても同じ状態が再現され、URLをブックマークしたり
共有したりすれば、まったく同じ状態のリストを開けます。
...
- 測定されたリソースごとの行。各行にはそのリソースのレベルバッジと、現在の平均使用率および
  設定されたしきい値が `(現在 12% | しきい値 5%)` の形式で表示されます。
