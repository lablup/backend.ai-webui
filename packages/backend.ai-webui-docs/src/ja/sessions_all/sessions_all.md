...
セッションリストの構成と絞り込みの方法については、
[セッションリストの表示](#session-list-view-and-refresh)を参照してください。

新しいコンピュートセッションを開始するには、「セッション開始」ボタンをクリックします。
...
持たず、ホストメモリプールを共用します。このようなアクセラレータを選択した場合、
アクセラレータメモリを個別に設定することはできず、ホストメモリプールから自動的に
メモリが使用されます。
:::
...
次のページに進むには、下部の「次のページ」ボタンをクリックするか、右側の「データストレージ選択」メニューをクリックします。
...
ユーザーは、「フォルダを選択」ドロップダウンで、コンピュートセッションにマウントする
ストレージフォルダを指定できます。一覧の各項目には、フォルダ名とともに所有者
(ユーザーまたはプロジェクト)、使用モード、ホスト、作成日、そのフォルダに対する権限
(`R`、`W`、`D`) が表示されます。ドロップダウンに文字を入力すると、フォルダ名で一覧を
絞り込めます。自動的にマウントされるフォルダと、現在のプロジェクトでマウントできない
フォルダは一覧に表示されません。

ドロップダウンの横には 2 つのボタンがあります。

- 「+」(新規ストレージフォルダー作成): フォルダ作成ダイアログを開きます。作成した
   フォルダは一覧に追加され、マウントするフォルダとして選択されます。詳細については、
   [ストレージフォルダの作成](#create-storage-folder)セクションを参照してください。
- 「更新」: フォルダ一覧を再読み込みします。別の画面でフォルダを作成したり共有された
   場合などに使用します。

![](../images/folder_create_modal.png)

選択したフォルダは、2 つのパス入力欄を持つ行として表示されます。

- **フォルダ内のパス (source)**: マウントするストレージフォルダ内のサブフォルダです。
   入力欄をクリックするとフォルダを参照してディレクトリを選択でき、空のままにすると
   フォルダのルートがマウントされます。
- **コンテナのパス (destination)**: フォルダがマウントされるコンテナ内のパスです。
   空のままにすると `/home/work/<フォルダ名>` にマウントされ、絶対パスを入力すると
   そのパスに、相対パスを入力すると `/home/work/` の下にマウントされます。他のマウント
   フォルダや自動マウントフォルダとパスが重複する場合はエラーとして表示され、重複を
   解消するまでセッションを開始できません。

行末の削除 (`X`) ボタンをクリックすると、そのフォルダをマウント一覧から除外できます。
フォルダ名をクリックすると、フォルダエクスプローラーを使用できます。詳細については、
[フォルダの探索](#explore-folder)セクションを参照してください。

![](../images/folder_explorer.png)

自動的にマウントされるフォルダは、行の下に「自動マウントフォルダ」として表示されます。
これらを選択する必要はありません。

:::note
共有が解除されるなどの理由で、選択したフォルダをマウントできなくなった場合、
そのフォルダは選択から除外され、通知が表示されます。
:::
...
`最近の履歴`機能は、以前に作成したセッションの情報を記憶します。
...
セッションリストはセッションタイプごとに **全体**、**インタラクティブ**、**バッチ**、
**INFERENCE**、**アップロードセッション** のタブに分かれており、各タブには
**実行中** / **終了セッション** の切り替えと、**セッション名**、**リソースグループ**、
**エージェント** のプロパティフィルターが用意されています。

<a id="session-resource-grid"></a>

### リソースグリッド表示

ユーザー設定で実験的機能の **Session resource grid view** を有効にすると、リスト上部の
更新ボタンの隣に **View mode** の切り替えが表示され、リストを **Table** と **Grid** で
切り替えられます。機能を有効にする方法については、[実験的特徴](#experimental-features)
セクションを参照してください。グリッド表示はテーブルと同じタイプタブ、**実行中** /
**終了セッション** の切り替え、プロパティフィルターを使用するため、リストを絞り込んでから
表示を切り替えることができます。

![](../images/session_resource_grid.png)

グリッドでは、各セッションが小さなセルの集まった1枚のプレートとして表示され、セルの色は
そのセッションのリアルタイムの利用率を表します。利用率が50%未満の場合は通常の色、50%以上
80%未満の範囲では段階的に濃くなる警告色、80%以上の場合は赤色で表示されます。まだ実行されて
いないセッションなどリアルタイムデータがないセッションは **No data** の色のままとなり、
リソースがまだ要求中のセッションは破線の枠線で表示されます。各色の意味は、グリッド上部の
凡例で確認できます。

プレートにマウスを重ねると、そのセッションのステータス、タイプ、クラスターモード、
リソースグループ、実行環境、開始時刻と経過時間、割り当てられた各リソースの利用率、
ディスクI/Oの概要が表示されます。プレートをクリックすると、セッション詳細パネルが開きます。

グリッド上部のコントロールで、セル1つが何を表すかを指定します。

- **Grid mode**: **Resource** を選択すると、選択したリソース1単位につきセル1つを、
  **Kernel** を選択すると、セッションのカーネル1つにつきセル1つを表示します。
- **Resource**（Resource モードのみ）: 可視化するリソースを選択します。**CPU**、
  **メモリ**、またはリスト内のセッションが使用しているアクセラレータを選択できます。
  セル1つはCPUコア1つ、メモリ1単位、アクセラレータ1台に相当し、アクセラレータを分割して
  使用している場合は一部だけが塗られたセルで表示されます。デバイスメモリを報告する
  アクセラレータには、利用率（`util`）とデバイスメモリ（`mem`）の2つの項目が用意されます。
- **Memory unit**（Resource モードでメモリ系リソースを選択した場合のみ）: セル1つが表す
  メモリ容量を 1、2、4、8 GiB から選択します。
- **Metric**（Kernel モードのみ）: カーネルセルの色付けに使用するリアルタイム統計項目を
  選択します。リストに表示されているセッションのいずれもライブメトリクスを報告していない
  場合は、この選択欄は無効になり、**ライブメトリクスなし** と表示されます。
- **Layout**: **Serpentine** は1行ごとに向きを交互にして、折り返したセッションのセルが
  つながって見えるように配置します。**Word-wrap** はすべての行を左端から開始します。

グリッド上部には、表示されていないセッションに関する案内も表示されます。

- 現在のフィルターに一致するセッションがグリッドに表示できる数を超える場合は、最初の100件
  のみが表示され、全体の件数を知らせる案内が表示されます。
- Resource モードでは、選択したリソースを割り当てられていないセッションは表示されず、
  非表示にした件数を知らせる案内が表示されます。
...
セッション詳細パネルのセッションステータスタグの横に時計アイコンボタンが表示されます。このアイコンをクリックすると、セッションのスケジュール履歴モーダルが開き、そのセッションに対してシステムが行ったすべてのスケジューリング決定の詳細ログを確認できます。詳細については、[セッションのスケジュール履歴](#session-scheduling-history)セクションを参照してください。
...
セッションは`RESERVED`、`PREEMPTED`、`RESCHEDULING`のステータスでも表示されることがあります。これらは、スケジューラーが優先度の高いジョブのためにリソースを解放する必要があるときに使用されます。

- `RESERVED`: リソースの予約を保持し、必要なリソースが解放されるのを待っているセッションです。
- `PREEMPTED`: 優先度の高いジョブがリソースを使用できるように、プリエンプションの対象として選択されたセッションです。
- `RESCHEDULING`: プリエンプションによりキューに戻されたセッションで、自動的に再スケジュールされます。

これら3つのステータスのセッションはいずれもリソースを占有しているため、**終了セッション** ではなく **実行中** の一覧に表示されます。

スケジューラーがセッションにプリエンプションの理由を記録している場合、セッションステータスタグにマウスを合わせると、内部の値ではなく次のような説明が表示されます。

| 理由 | 説明 |
|------|------|
| `PREEMPTED_BY_SCHEDULER` | このセッションは、優先度の高いジョブのためにリソースを解放する目的で、スケジューラーによって終了されました。 |
| `preempted-by-reservation` | このセッションは、優先度の高いジョブのためにリソースを解放する目的で、プリエンプション対象として選択されました。 |
| `preemption-reservation` | このセッションはリソースの予約を保持しており、リソースが解放されるのを待っています。 |
| `RESCHEDULED` | このセッションはプリエンプションによりキューに戻されました。自動的に再スケジュールされます。 |
...
- **遷移元ステータス**: スケジューリングステップ前のセッションステータスでフィルタリング
- **遷移先ステータス**: スケジューリングステップ後のセッションステータスでフィルタリング
- **エラーコード**: エラーコードでフィルタリング
- **メッセージ**: メッセージ内容でフィルタリング
- **作成日時**: レコードの作成日時でフィルタリング
- **更新日時**: レコードの更新日時でフィルタリング
...
- **遷移元ステータス**: このスケジューリングステップ前のセッションステータス
- **遷移先ステータス**: このスケジューリングステップ後のセッションステータス
- **試行回数**: このスケジューリングステップが試行された回数
- **メッセージ**: このスケジューリングステップの概要メッセージ

**作成日時**または**更新日時**カラムでテーブルをソートして、お好みの順序でレコードを表示できます。

#### 履歴テーブルのページネーション

スケジュール履歴テーブルはオフセットベースのページネーションを使用しており、既定のページサイズは1ページあたり10件です。フィルターまたはソート順を変更すると、表示は自動的に1ページ目にリセットされます。
...
:::note
監査ログはバックエンドによって管理されており、スーパー管理者のみに表示される場合があります。一般ユーザーは、タブにアクセスできる場合でも空の結果が表示されることがあります。
:::

#### 監査ログのカラム

監査ログテーブルには以下のカラムが含まれます：

- **トリガー元**: アクションを開始したアクターのメールアドレスとアカウントID
- **クライアントIP**: アクションを要求したIPアドレス。サーバーが報告したまま表示され、管理者のクライアントIPマスキングポリシーによっては一部がマスクされ、記録されていない場合は`-`と表示されます
- **操作**: 実行されたアクションの種類（例：セッション作成、終了、リソース変更）
- **ステータス**: アクションの結果（`SUCCESS`、`ERROR`、`RUNNING`、`DENIED`、または`UNKNOWN`）。`DENIED`は、エラーで失敗したのではなく、ポリシーによって意図的に拒否されたことを意味します
- **時刻**: アクションが発生した日時

#### 監査ログのフィルター

テーブル上部のフィルターバーを使用して、ログエントリを絞り込むことができます：

- **ステータス**: アクションの結果でフィルタリング（`SUCCESS`、`ERROR`、`RUNNING`、`UNKNOWN`）
- **操作**: 操作名またはキーワードで検索
- **トリガー元**: アクターIDで検索
- **時刻**: 日付範囲ピッカーでフィルタリング

**更新**ボタンでログを再読み込みできます。監査ログのクエリは遅延実行され、**監査ログ**タブを初めてアクティブにしたときにのみ送信されます。
...
セッション詳細パネルで`コンテナログを見る`アイコンをクリックすると、
コンピュートセッションのログを確認できます。

![](../images/session_log.png)

ログウィンドウのログ表示領域の上には、次のコントロールが用意されています。

- **カーネルセレクター**: ログを確認するセッションのカーネルを選択します。単一コンテナの
  セッションでは項目は1つだけですが、クラスターセッションでは各コンテナがホスト名とIDの
  先頭数文字とともに一覧表示されます。
- **ダウンロード**: 現在表示されているログをテキストファイルとして保存します。
...
実行中のセッションに適用される基準は、セッション詳細パネルの **リクレイム状態** 項目に、
それぞれの基準がセッションを終了するまでの残り時間とともに表示されます。この項目は、
...
:::note
環境設定によっては、適用されるアイドルチェッカーや、使用量ベースの資源回収に表示される
リソース種別が異なる場合があります。
:::
...
セッションリストには、各セッションが使用量ベースの資源回収の対象にどれだけ近づいているかを
一目で把握できる **リクレイム状態** カラムがあります。セッション詳細パネルの
**リクレイム状態** 項目でも、使用量ベースの資源回収の項目名に代わって同じバッジが表示されます。

バッジには次の3つのレベルがあり、ポップオーバーの凡例にも同じ説明が表示されます。

- **安全**: 現在の平均使用率がしきい値を十分に上回っています。
- **警告**: 現在の平均使用率はしきい値を上回っていますが、しきい値に近づいています。
- **リスクあり**: 現在の平均使用率がしきい値を下回っています。

バッジは、平均使用率が実際にしきい値を下回る前にレベルが上がるため、現在の平均使用率が
まだしきい値を上回っている状態でも **リスクあり** と表示されることがあります。しきい値を
超えたことを知らせる表示ではなく、対応する時間を残して前もって知らせる警告と考えてください。

使用量ベースの資源回収が適用されないセッションや、まだ測定値がないセッションでは、カラムに
`-` が表示されます。セッション詳細パネルでは、最初のリクレイム判定がまだ行われていない項目に、
残り時間のタグの代わりに **回収するかどうかを確認中** と表示されます。

バッジの横にある情報（i）アイコンにマウスを合わせると、レベル判定の根拠を示すポップオーバーが
開きます。

- 列挙された条件の **いずれか** がリスクあり状態のままのときにリソースが回収されるのか、
  それとも **すべて** がリスクあり状態のときにのみ回収されるのかを示す条件文。どちらの文が
  表示されるかは、管理者がチェッカーをどう設定したかによって異なります。
- 測定されたリソースごとの行。各行にはそのリソースのレベルバッジと、現在の平均使用率および
  設定されたしきい値が `(現在 12% | 閾値 5%)` の形式で表示されます。
- 3つのレベルそれぞれの判定基準を説明する凡例。

測定値のないリソースはダッシュで表示され、全体のレベル判定には影響しません。

使用量ベースの資源回収の項目は、残り時間が少なくなると、その横に表示される残り時間のタグが
バッジと同じ色になります。残り時間のタグが赤いセッションが、まさに回収の対象です。一方、残り時間に十分な余裕がある間は、バッジがすでにリスクのある
レベルを示していてもタグには色が付きません。
...
Backend.AIは「セッションをイメージに変換」機能をサポートしています。
