...
ユーザーは、コンピュートセッションにマウントするストレージフォルダを指定できます。
...
Backend.AI Managerのバージョンが26.8.0以降の場合、セッションは`RESERVED`、`PREEMPTED`、`RESCHEDULING`のステータスでも表示されることがあります。これらは、スケジューラーが優先度の高いジョブのためにリソースを解放する必要があるときに使用されます。
...
:::note
これらのステータスと理由の説明は、Backend.AI Managerのバージョンが26.8.0以降の場合にのみ表示されます。それより古いサーバーでは、セッションステータスタグに元のステータス情報の値がそのまま表示されます。
:::
