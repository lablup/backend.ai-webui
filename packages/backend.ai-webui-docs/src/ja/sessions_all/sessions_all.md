...
リストツールバー右側の更新ボタンをクリックすると、セッションリストをその場で再読み込み
でき、共通の自動更新の間隔セレクターも付いています（[自動更新](#common-auto-refresh)を
参照）。コンピュートセッションの各画面では自動更新が既定で有効になっています。
**セッションリスト** は15秒間隔で、**セッション詳細パネル** と **コンテナログ**
ウィンドウは10秒間隔で開始し、選択した間隔は画面ごとに個別に記憶されます。
...
展開カラムのヘッダーにあるケバブメニュー（⋮）をクリックすると**展開オプション**メニューが開き、すべての行を展開する、すべての行を折りたたむ、結果がSUCCESSでない行だけを展開する、のいずれかを選択できます。既定値は**エラーのみ展開**で、このモードでは展開された行のSUCCESSサブステップも非表示になるため、セッションのスケジューリングライフサイクルで何が問題だったかがすぐにわかります。
...
開始します。再読み込みのたびにログ表示は最新の行までスクロールし、前回の再読み込み以降に
追加された行がハイライトされます。
