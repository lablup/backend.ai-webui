...
セッションリストの構成、絞り込み、更新の方法については、
[セッションリストの表示と更新](#session-list-view-and-refresh)を参照してください。
...
各タブは、ページを開いている間はそれぞれのフィルター、ソート順、ページ番号を保持します。
そのため、別のタブに移動して戻っても状態がリセットされることはなく、直前に見ていた表示が
そのまま復元されます。現在表示しているタブの表示状態はページのURLにも保存されるため、
ブラウザを再読み込みしてもその表示が再現され、URLをブックマークしたり共有したりすれば
同じ状態のリストを開けます。ただし、URLに保存されるのはアクティブなタブの状態だけで、
再読み込み後は他のタブは既定値から始まります。

<a id="auto-refresh-interval"></a>

### 自動更新の間隔

リストツールバー右側の更新ボタンをクリックすると、セッションリストをその場で再読み込みできます。
このボタンには自動更新の間隔セレクターも付いています。ボタンの横にある矢印をクリックすると、
**オフ**、または5秒から60秒までのプリセット間隔を選択できます。自動更新が有効な間は、次の
再読み込みまでの残り時間を示すカウントダウンの枠線がコントロールに表示され、選択中の間隔が
セレクターにラベル（例: `15秒`）として表示されます。

コンピュートセッションの各画面では、自動更新が既定で有効になっています。

- **セッションリスト** は15秒間隔で開始します。
- **セッション詳細パネル** と **コンテナログ** ウィンドウは10秒間隔で開始します。
...
パネルヘッダーの更新ボタンをクリックするとセッション情報を再読み込みでき、リストツールバーと
同様に自動更新の間隔セレクターも利用できます。間隔の仕組みについては
[自動更新の間隔](#auto-refresh-interval)を参照してください。
...
:::note
以下で説明する**エラーのみ展開**モードは、現在のページに表示されているレコードにのみ適用され、履歴全体には適用されません。
:::
...
展開カラムのヘッダーにあるケバブメニュー（⋮）をクリックすると**展開オプション**メニューが開き、テーブル全体の行の展開方法を制御できます。3つの表示モードが利用可能です：

- **すべて展開**: すべての行を展開して、すべてのサブステップをすぐに表示します。
- **すべて折りたたむ**: すべての行を折りたたんで、トップレベルの履歴レコードのみを表示します。
- **エラーのみ展開**（既定）: 結果がSUCCESSでない行を自動的に展開し、展開された行のSUCCESSサブステップも非表示にします。これは、セッションのスケジューリングライフサイクルで何が問題だったかをすばやく特定するのに最も役立つモードです。

選択されたモードはユーザーごとに記憶され、モーダルを開き直しても保持されます。アクティブなモードに関わらず、個々の行をいつでも手動で展開または折りたたむことができます。
...
:::note
管理者は `resources/app_template.json` の `hideAppsOnBatchSession` を有効にすることで、
バッチセッションのコンテナ内アプリを非表示にできます。このオプションが有効な場合、
**バッチ** セッションではセッションリストの行アクション、セッション作成通知、
セッション詳細パネルのいずれにもアプリランチャーとターミナルのショートカットが表示されません。
インタラクティブセッションは影響を受けません。このオプションは既定で無効です。
:::
...
- **更新**: ログを再読み込みします。このボタンには
  [自動更新の間隔](#auto-refresh-interval)で説明した自動更新の間隔セレクターも付いています。

ログウィンドウは新しい行が追加される様子を見るために開く画面であるため、自動更新は10秒間隔で
開始します。更新ボタンの横のセレクターから別の間隔、または **オフ** を選択でき、その選択は
次にログウィンドウを開いたときにも維持されます。再読み込みのたびにログ表示は最新の行までスク
ロールし、前回の再読み込み以降に追加された行がハイライトされます。
...
セッションが実行中で、いずれかのアイドルチェックが残り時間を返していれば表示され、残り時間の
長さは問いません。期限までまだ十分に余裕があるチェックも、期限が近いチェックと同じように
表示されます。
...
**リクレイム状態** 項目でも、使用量ベースの資源回収の項目名の代わりに同じバッジが表示されます。
...
レベルはしきい値との単純な比較ではなく、余裕を持たせて判定されます。そのため、現在の平均
使用率がまだしきい値を上回っている状態でも **リスクあり** と表示されることがあります。バッジは
平均使用率がしきい値を超えたかどうかをそのまま示す値ではなく、対応の余地を残した相対的な
警告レベルとして読み取ってください。
...
バッジと同じ色になります。そのため、残り時間のタグが赤くなっているセッションが、まさに回収の
対象となるセッションです。一方、残り時間に十分な余裕がある間は、バッジがすでにリスクのある
レベルを示していてもタグには色が付きません。つまり、色の付いた残り時間のタグは、リスクが
存在することにとどまらず、差し迫っていることを示すサインです。
