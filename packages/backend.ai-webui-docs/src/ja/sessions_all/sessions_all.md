...
同じ状態のリストを開けます。ただし、URLに保存されるのは現在表示しているタブだけなので、
...
バッジは、平均使用率が実際にしきい値を下回る前にレベルが上がります。そのため、現在の平均
使用率がまだしきい値を上回っている状態でも **リスクあり** と表示されることがあります。しきい値を
...
レベルを示していてもタグには色が付きません。つまり、タグに色が付いたら回収が間近だという
意味です。
