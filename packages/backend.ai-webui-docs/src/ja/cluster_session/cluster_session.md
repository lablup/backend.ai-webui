...
- クラスタセッションには、2つのモード/タイプがあります。
   ...
   * 1 つのコンテナしかないコンピュートセッションは、クラスタセッションではなく通常のコンピュートセッションとして分類されます。上の図では、これがセッション Y です。
...
セッションページで、セッション作成ダイアログを開き、通常のコンピュートセッションを作成するのと同じ方法で設定します。このときに設定するリソース量は、**1つのコンテナ**に割り当てられる量です。例えば、4つのCPUを設定した場合、クラスタセッションの下で各コンテナに4コアが割り当てられます。これは、クラスタコンピュートセッション全体に割り当てられるリソース量ではないことに注意してください。クラスタコンピュートセッションを作成するには、ここで設定したリソース量のN倍に相当するサーバーリソースが必要です（Nはクラスタのサイズです）。データの安全保存のためにストレージフォルダをマウントすることを忘れないでください。
...
その下の「クラスターサイズ」を設定します。3に設定すると、メインコンテナを含む合計3つのコンテナが作成されます。これらの3つのコンテナは、プライベートネットワークで結合され、1つのコンピュートセッションを形成します。
...
このように、Backend.AI はクラスタコンピュートセッションの作成を容易にします。クラスタコンピュートセッションを通じて分散学習と計算を実行するには、TensorFlow/PyTorch などの ML ライブラリが提供する分散学習モジュールや、Horovod、NNI、MLFlow などの追加のサポートソフトウェアが必要であり、そのソフトウェアを利用できる方法でコードを記述する必要があります。慎重に書く必要があります。Backend.AI は分散学習に必要なソフトウェアを含むカーネルイメージを提供しているため、そのイメージを使用して優れた分散学習アルゴリズムを作成することができます。
