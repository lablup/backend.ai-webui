...
- Web `wsproxy`: Web上で提供されるWebUIの場合、ブラウザの特性上、組み込みサーバーを使用することはできません。この場合、`wsproxy`サーバーを別のWebサーバーとして設定し、WebUIアプリがWeb `wsproxy`を参照できるようにすることで、Jupyter NotebookやターミナルなどのサービスをWeb環境で使用できます。
