...
確認できます。タブ上部の情報バナーには、ここに表示される使用履歴が終了したコンピュートセッションに
基づいていることが示されます。バナーの下にある左上の**期間**ドロップダウンを使用して、表示期間を
...
:::info
IO-ReadおよびIO-Writeカードは、ユーザーメトリクスが利用できない場合、つまり
[ユーザーセッション履歴](#user-session-history)タブが表示されない場合にのみ表示されます。
ユーザーメトリクスが利用可能な場合は、セッションごとのI/Oアクティビティがユーザーセッション履歴タブを
通じて表示されるため、これら2つのカードはここでは省略されます。
:::
...
- **Capacity**: 選択した期間における、そのメトリクスに割り当てられた、または利用可能な総量。
...
:::note
選択した範囲や環境で利用可能なメトリクスがない場合、メトリクスカードには表示するメトリクスがないことを
示す空の状態が表示されます。
:::
