...
:::tip
必須フィールドが未入力または不正な場合、レビューステップの送信ボタンは無効のままになり、該当するフィールドを含むカードの枠が赤く表示され、カードヘッダーの **編集** リンクの横にエラーアイコンが表示されます。右側のステップ一覧にも、エラーを含むステップがすべて示されます。すでに離れたステップのフィールドで発生したエラーも対象です。必須フィールドには入力中にインラインの検証メッセージが表示されます。
:::
