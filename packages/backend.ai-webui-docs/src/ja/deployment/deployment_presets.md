...
:::note
API などで作成され、互いに適合しないコントロールと値の型が保存されているプリセットを開くと、保存された値の型を保存された UI タイプでは表示できないという警告が表示され、該当のフィールドに **選択した UI タイプではこの値の型を表示できません。** というメッセージが表示されます。保存する前にどちらか一方を変更してください。

**UIタイプ** のフィールドが表示されないサーバーでは、逆方向に制約がかかります。つまり、プリセットにすでに保存されているコントロールによって、選択できる **値の型** が制限されます。
:::
