...
管理者向けデプロイページには **デプロイ**、**モデルストア管理**、**Prometheusプリセット**、**デプロイメントプリセット** といった複数のタブがあります。各タブのフィルター、並び順、ページ番号は URL に保存されるため、次の 2 つの利点があります。

- 別のタブへ移動して戻ってきても、デプロイメントプリセットタブは離れたときの状態のまま復元されます。
- フィルターを適用したプリセット一覧の URL をブックマークしたり共有したりすると、受け取った相手も同じフィルター・並び順・ページで開くことができます。
