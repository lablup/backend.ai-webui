...
管理者向けデプロイページには **デプロイ**、**モデルストア管理**、**Prometheusプリセット**、**デプロイメントプリセット** といった複数のタブがあります。各タブはそれぞれのフィルター、並び順、ページ番号を保持するため、別のタブへ移動して戻ってきても、デプロイメントプリセットタブは離れたときの状態のまま復元されます。
