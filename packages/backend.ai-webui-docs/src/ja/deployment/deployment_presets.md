...
管理者向けデプロイページには **デプロイ**、**モデルストア管理**、**Prometheusプリセット**、**デプロイメントプリセット** といった複数のタブがあります。現在開いているタブはページの URL に記録され、各タブのフィルター、並び順、ページ番号も同じく URL に保存されます。これには次の 2 つの利点があります。

- 別のタブへ移動して戻ってきても、デプロイメントプリセットタブはフィルターが解除された 1 ページ目に戻ることなく、離れたときの状態のまま復元されます。
- フィルターを適用したプリセット一覧の URL をブックマークしたり共有したりすると、受け取った相手も同じタブ・同じフィルター・並び順・ページで開くことができます。

あるタブのフィルターやページ状態が別のタブに引き継がれることはありません。したがって **デプロイ** タブで設定したフィルターが **デプロイメントプリセット** タブに反映されることはありません。
...
2. 各フィールドに入力します。この画面は **基本情報**、**モデル & 実行**、**レビュー** の 3 ステップからなるウィザードで、右側にステップ一覧、下部に `前へ` / `次のページ` のナビゲーションが表示されます。`レビューへスキップ` をクリックすると最後のステップへ直接移動できます。フィールドは次のセクションに分かれています。
   ...
   - **実行**：**スタートアップコマンド**、環境変数、ブートストラップスクリプト。スタートアップコマンドのフィールドにはシェル構文のヒント（`シェル構文: /bin/bash -c "cmd1; cmd2"`）が表示されます。コマンドは `/bin/bash -c <コマンド>` として実行されるため、`;`、`|`、`&&` などのシェル演算子をそのまま使用できます。
      ...
      :::note[スタートアップコマンドは起動コマンドとは別のものです]
      2 つのコマンドは別物であり、それぞれのフィールドに専用の説明が表示されるようになったため区別しやすくなりました。

      | フィールド | 場所 | 役割 | 例 |
      |---|---|---|---|
      | **スタートアップコマンド** | プリセットの実行セクション | *「モデルフレームワークを起動する前に環境を準備するコマンドです（例: vllm のインストール）。」* | `pip install vllm` |
      | **起動コマンド** | プリセットのモデル定義セクション | *「モデルサービングプロセスを開始するための CLI コマンドです。」* | `vllm serve /models --tp 2` |

      パッケージのインストール、アセットの取得、設定ファイルの作成といった準備作業には **スタートアップコマンド** を、サービングフレームワークを実際に起動するコマンドには **起動コマンド** を使用します。各フィールドのプレースホルダーにも、それぞれに適したコマンドの例が表示されます。
      :::
   - **デプロイ既定値**：
