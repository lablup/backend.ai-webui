...
- **ヘルスチェックを有効にする**: 有効にすると、システムは推論サーバへ定期的に HTTP リクエストを送信し、正しく応答しているか確認します。無効（新規 Revision のデフォルト）の場合、ヘルスチェックは設定されず、異常なレプリカは自動的に検出されません。本番デプロイでは有効にすることを推奨します。**ヘルスチェックを有効にする** をオンにすると、以下のフィールドが追加で表示されます。
   ...
   * **起動猶予期間**: コンテナ起動後にヘルスチェックの失敗を無視する猶予期間（秒）で、最初のヘルスチェックが成功するとレプリカがアクティブになります（デフォルト: `60.0`）。ロード時間が長い大規模モデルの場合は、この値を大きく設定してください。
   ...
   * **期待ステータスコード**: サービスが正常であることを示す HTTP レスポンスステータスコードです（デフォルト: `200`）。
...
- **ヘルスチェック URL**: システムがサービスの健全性を確認するために呼び出す HTTP エンドポイントパスです。
- **起動猶予期間**: コンテナ起動後にヘルスチェックの失敗をレプリカに対してカウントしない期間（秒）です（デフォルト: `60.0`）。
- **最大再試行回数**: レプリカが `UNHEALTHY` とマークされるまでに許容される連続失敗回数です（デフォルト: `10`）。
- **間隔**: チェック間隔（秒）です（デフォルト: `10.0`）。
- **最大待機時間**: 各リクエストのタイムアウト時間（秒）です（デフォルト: `15.0`）。
- **期待ステータスコード**: 正常な応答を示す HTTP ステータスコードです（デフォルト: `200`）。
...
- **モデル名**、**モデルパス**、**開始コマンド**、**ポート**、**ヘルスチェック URL**、**起動猶予期間**、**最大再試行回数**、**インターバル (s)**、**最大待機時間 (s)**
...
- **フィルターバー**: **ステータス**、**オペレーション**、**トリガーしたユーザー**（ユーザー ID で検索）、および **時間** の日付範囲ピッカーでログエントリを絞り込みます。
- **自動更新ボタン**: タブを離れることなくログを再読み込みします。
- **ページネーション**: ログエントリをページごとに閲覧できます。

:::note
監査ログタブは遅延読み込みを使用しています。クエリは最初にタブをアクティブにしたときのみ送信されます。アクティブなタブは URL に反映されます（`?revisionTab=auditLog`）。このリンクを共有することで、監査ログビューに直接アクセスできます。

監査ログデータへのアクセスはバックエンドによって制限されており、スーパー管理者のみに制限される場合があります。
:::
...
デプロイメントプリセットを使用すると、管理者がリビジョン構成値（ランタイムバリアント、コンテナイメージ、リソース割り当て、起動コマンド、ヘルスチェック設定など）を事前に保存し、リビジョンを追加する際に再利用できます。ユーザーはリビジョン追加時の **プリセットモード** でプリセットを選択し、管理者はデプロイメントプリセットページでプリセットを作成・編集します。

### デプロイメントプリセットの管理

デプロイメントプリセットページには、現在のプロジェクトで使用可能なすべてのプリセットが一覧表示されます。テーブルには次の**デフォルト表示**カラムが含まれます：

- **名前**: プリセット名です。
- **ランタイム**: ランタイムバリアントです（例: `vLLM`、`SGLang`、`Custom`）。
- **イメージ**: `<canonicalName>@<architecture>` 形式のコンテナイメージです（例: `cr.backend.ai/stable/pytorch:2.1-cuda12.1@aarch64`）。値はコピー可能です。この形式により、混合アーキテクチャクラスターでCPUアーキテクチャ別にイメージを区別しやすくなります。
- **レプリカ**: このプリセットに保存された希望レプリカ数です。
- **作成日時**: プリセットが作成された日時です。
- **更新日時**: プリセットが最後に更新された日時です（`updatedAt`）。

テーブルヘッダー右側の**カラム表示**ギアボタンで追加カラムの表示/非表示を切り替えられます。次のカラムは**デフォルト非表示**ですが有効化できます：

- **説明**: プリセットの説明テキストです。
- **起動コマンド**: 設定された起動コマンドで、省略表示されツールチップで全文を確認できます。値はコピー可能です。
- **クラスター**: プリセットに保存されたクラスターモードです。
- **ストラテジー**: スケジューリングストラテジーです。
- **公開設定**: プリセットのデフォルト公開範囲設定に応じて `パブリック` または `プライベート` タグを表示します。
- **リビジョン履歴の上限**: 保持するリビジョン数です。
- **ランク**: プリセットの並び順のランクです（ソート可能なカラム）。

### デプロイメントプリセットの作成

`プリセットを作成` ボタンをクリックして **デプロイメントプリセットを作成** フォームを開きます。フォームは Revision 追加の高度な設定モードフォームと同じセクションで構成されます：

- **モデルとランタイム**: ランタイムバリアントを選択します。`vLLM` または `SGLang` を選択すると、タブで区切られた **ランタイムパラメータ** セクション（Model Loading、Resource Memory、Serving Performance など）が表示されます。`Custom` ランタイムの場合は起動コマンドとヘルスチェックのフィールドが表示されます。
- **環境**: コンテナイメージを選択します。ドロップダウンではイメージが `<canonicalName>@<architecture>` 形式で表示されるため、CPUアーキテクチャ別にイメージを区別できます。この形式はフォームのレビューステップにも同様に表示されます。
- **クラスターとリソース**: CPU、メモリ、アクセラレータの割り当てを設定します。
- **詳細設定** *（折りたたみ可能）*: 追加のストレージフォルダーをマウントします。

:::note[必須ランタイムパラメータ]
`vLLM` または `SGLang` ランタイムが選択されている場合、管理者は個々のランタイムパラメータを必須としてマークできます（赤いアスタリスク ★ で示されます）。必須パラメータが空のままプリセットフォームを送信しようとすると、インラインバリデーションエラーとともにブロックされます。この検証ルールはプリセットからリビジョンを追加する際にも同様に適用されます。
:::

設定されたランタイムパラメータの値はプリセットとともに保存され、このプリセットからデプロイメントを作成する際に適用されます。変更されていないパラメータはランタイムのデフォルト値が維持されます。設定された値はプリセットを保存する前の **レビュー** ステップの **基本情報** でも確認できます。

### デプロイメントプリセットの編集

プリセット行の編集アイコンをクリックすると、現在の値が事前入力された **デプロイメントプリセットを編集** フォームが開きます。フィールドと検証ルールはプリセット作成と同じです。

### プリセット詳細モーダル

Revision 追加モーダルの **プリセットモード** でプリセットセレクタ横の **ⓘ** ボタンをクリックすると、**プリセット詳細** モーダルが開きます。このモーダルでは、次のカードを含むプリセット構成の概要を確認できます：

- **基本情報**: 名前、ランタイムバリアント、イメージ、レプリカ数、クラスターモード、起動コマンド、環境変数。
- **リソース**: CPU、メモリ、アクセラレータの割り当て。
- **ヘルスチェック**: このプリセットのヘルスチェックの有効状態です。
   * **有効**の場合：設定されたパス（Path）、間隔（Interval）、最大再試行回数（Max Retries）、最大待機時間（Max Wait Time）、期待ステータスコード（Expected Status Code）、起動猶予期間（Startup Grace Period）が表示されます。
   * **無効**の場合：`無効` と表示されます。
...
スーパー管理者は、モデルストア管理パネルからモデルカードの作成、編集、削除を行うことができます。

モデルカードを作成または編集する際：

- **ラベル（Label）** および **フレームワーク（Framework）** タグフィールドでは、**Enter** キーまたは **カンマ（,）** をトークン区切り文字として使用できます。例えば `pytorch, transformers,` と入力すると、各項目の間に Enter を押すことなく複数のタグを一括追加できます。
- **ドメイン（Domain）** フィールドは、新しいカードを作成する際に管理者の現在アクティブなドメインで自動入力されます。これにより、ドメインを手動で選択する手間が省けます。

:::tip
複数のラベルやフレームワークタグをすばやく追加するには、カンマ区切りで入力してください（例: `nlp, text-generation, llm`）。すべてのタグが一括で追加されます。
:::
