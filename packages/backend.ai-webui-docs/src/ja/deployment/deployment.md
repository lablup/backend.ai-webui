...
一覧上部の更新ボタンでは自動更新も設定でき（[自動更新](#common-auto-refresh)を参照）、デプロイ一覧は `15秒` 間隔から始まります。
...
サブステップを持つイベントは、行の先頭にある `+` アイコンをクリックすると展開でき、そのイベントのサブステップ表が表示されます。展開列のヘッダーにあるメニューでは、すべての行を展開する、すべての行を折りたたむ、エラーのある行だけを展開する、の 3 つの表示方法を選択できます。既定値は **エラーのみ展開** のため、モーダルを開いた直後から失敗・再試行・期限切れを確認できます。
...
レプリカカードの更新ボタンにも自動更新の間隔選択があります（[自動更新](#common-auto-refresh)を参照）。レプリカの状態は短時間で変化するため、このカードは `10秒` から始まり、選択した間隔はデプロイ一覧の設定とは別に保存されます。
