...
- **環境変数**: 推論サーバコンテナに渡されるキー/値のペアです。`vLLM` および `SGLang` の場合は、上記に示したランタイム専用の変数が事前入力されています。エントリは自由に追加、編集、削除できます。変数名を入力すると、`HF_TOKEN`、`AWS_ACCESS_KEY_ID`、`HTTP_PROXY` などのよく使われる環境変数名が候補として表示されます。候補は名前のどの位置でも一致するため、`proxy` と入力すると `HTTP_PROXY`、`HTTPS_PROXY`、`NO_PROXY` がまとめて表示されます。他の行ですでに使用されている名前は候補に含まれません。名前フィールドに候補の変数が入力されると、隣の値フィールドのプレースホルダーに書式のヒントが表示されます（例: `HF_TOKEN` なら *「あなたのHugging Faceアクセストークン」*）。
