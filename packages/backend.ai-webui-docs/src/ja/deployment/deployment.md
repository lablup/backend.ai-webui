...
**公開** も同様に作成時点で確定します。**デプロイを編集** モーダルではこのチェックボックスは読み取り専用となり、ツールチップには *「作成時に選択した公開設定は変更できません。」* と表示されます。
:::
...
仮想フォルダを作成する必要があります。仮想フォルダを作成する際は、
