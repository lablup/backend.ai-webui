...
- **サブパス**: フォルダーのルートの代わりにマウントする、モデルフォルダー内のサブフォルダーです。パスを直接入力することはできません。まず **モデルフォルダー** を選択し(選択するまでこのフィールドは無効です)、フィールドをクリックしてフォルダー内のディレクトリを参照し、**この場所を選択** をクリックして指定します。選択したパスは先頭に `/` が付いて表示されるため、フォルダーのルートは `/` と表示されます。**モデルフォルダー** を変更すると、すでに選択したサブパスは解除されます。空のままにするとフォルダーのルートがマウントされます。
