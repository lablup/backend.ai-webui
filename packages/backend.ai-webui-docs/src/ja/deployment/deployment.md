...
一覧上部の更新ボタンでは自動更新も設定できます。ボタン横のシェブロンをクリックすると、**オフ**、`5秒`、`10秒`、`15秒`、`30秒`、`60秒` から間隔を選択できます。デプロイ一覧は `15秒` から始まり、間隔を一度選択すると（**オフ** を選んだ場合も含めて）その選択はページを再読み込みしてもこの一覧に対して保持されます。自動更新が有効な間はシェブロンのボタンに選択中の間隔が表示され、次の更新までの残り時間がコントロールの枠線として満ちていきます。更新ボタンを手動でクリックすると、このカウントダウンも最初からやり直します。
...
:::warning[リソースグループは作成時に確定します]
モーダルを開いた時点から、**リソースグループ** フィールドの下に *「作成時に選択したリソースグループは変更できません。」* という警告が表示されます。リソースグループは Revision ではなくデプロイメントに属する値であるため、デプロイメントを編集しても、新しい Revision を追加しても、後から別のリソースグループへ移すことはできません。デプロイメントを作成する前に適切なリソースグループを選択してください。別のリソースグループが必要な場合は、新しいデプロイメントを作成します。

**公開** も同様に作成時点で確定します。**デプロイを編集** モーダルではこのチェックボックスは読み取り専用となり、ツールチップには *「作成後は変更できません。」* と表示されます。
:::

`作成` をクリックするとデプロイメントが作成され、デプロイ詳細ページに移動します。最初の Revision を追加するまで、**デプロイされたリビジョンがありません** 警告が表示されます。

作成後にデプロイレベルの設定（名前、希望レプリカ数、タグ）を変更するには、サービス情報カードの **編集** ボタンをクリックします。現在の値が入力された状態で **デプロイを編集** モーダルが開き、送信ボタンは `保存` と表示されます。このモーダルの **リソースグループ** フィールドは読み取り専用で同じ警告を表示し、**公開** チェックボックスは無効化されます。
...
- **モデルフォルダーのマウント先**: コンテナ内でモデルストレージフォルダーがマウントされるパスです（デフォルト: `/models`）。フィールドのツールチップにも *「コンテナ内にモデルフォルダがマウントされるパスです。」* と同じ説明が表示されます。
...
- **ヘルスチェックを有効にする**: 有効にすると、システムは推論サーバへ定期的に HTTP リクエストを送信し、正しく応答しているか確認します。無効（新規 Revision のデフォルト）の場合、ヘルスチェックは設定されず、異常なレプリカは自動的に検出されません。本番デプロイでは有効にすることを推奨します。**ヘルスチェックを有効にする** をオンにすると、以下のフィールドが追加で表示されます。
   ...
   * **最大リトライ数**: 最大連続失敗回数
   * **最大待機時間**: タイムアウト（秒）
   * **予想ステータスコード**: 正常を示すHTTPステータスコード
   * **初期ヘルスチェック猶予時間**: コンテナ起動後の猶予時間（秒）
...
:::note
どちらのモデル定義モードでも、このフィールドのラベルは **モデルフォルダーのマウント先** で統一されており、保存される値も同じです。そのため **コマンドを入力** と **設定ファイルを使用** を切り替えても、このフィールドの意味は変わりません。
:::
...
:::note
このファイルは以前 `service-definition.toml`（TOML 形式）という名前でした。レガシーの `service-definition.toml` はフォールバックとして読み取られますが、**非推奨** です。新しいモデルフォルダーには `deployment-config.yaml` を使用することをお勧めします。
:::
...
サービス情報カードのヘッダーには、**編集** ボタンに加えて **その他** メニューが表示されます。**編集** ボタンは [デプロイメント作成モーダル](#create-deployment) で説明した **デプロイを編集** モーダルを開き、送信ボタンは `保存` と表示されます。その他メニューには現在 **デプロイを削除** アクションが含まれます。
...
- **すべて展開**: 展開可能なすべての行を開きます。
- **すべて折りたたむ**: すべての行を閉じます。
- **エラーのみ展開**: 結果が成功以外の行だけを開きます。これが既定値のため、モーダルを開いた直後から失敗・再試行・期限切れを確認できます。

   表示方法が決めるのは **どの行を展開した状態で開始するか** だけであり、展開された行の内容を変えることはありません。展開された行には、成功したサブステップも含めてそのイベントの **すべて** のサブステップ表が常に表示されるため、失敗した手順をその前に実行された手順と併せて読むことができます。手動で開閉した行は、表示方法を変更するかデータが再読み込みされるまでその状態を保ちます。

レプリカタブから開く **レプリカのスケジュール履歴** モーダルも、同じ表と同じ展開コントロールを使用します。[レプリカ](#replicas-tab-history) を参照してください。
...
レプリカカードの更新ボタンにも、デプロイ一覧と同じ自動更新の間隔選択があります。レプリカの状態は短時間で変化するため、このカードは `10秒` から始まり、選択した間隔はデプロイ一覧の設定とは別に保存されます。
...
トークンが発行されると **トークン** ダイアログが開き、トークンの値とコピーボタン、選択した有効期限（または *有効期限なし*）が一度だけ表示されます。このダイアログから、または後で一覧からトークンをコピーできます。
...
- **作成日時**: トークンが発行された日時です。
- **有効期限**: トークンが失効する日時です。失効しないトークンは *有効期限なし* と表示されます。
...
- 各選択肢にはトークンの **末尾 6 文字** がコードチップとして表示され、その横に有効期限（例: `~ 2026年8月26日`）または *有効期限なし* が表示されます。すべてのトークンは同じ JWT ヘッダーで始まるため、実際に区別できるのは末尾の部分です。選択肢にカーソルを合わせると、発行日時 → 有効期限の全体を確認できます。
- **期限切れのトークンは表示されません。** 有効なトークンのみが一覧に表示され、**有効期限なし** で発行したトークンは常に含まれます。
