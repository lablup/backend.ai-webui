...
- **サブパス**: フォルダーのルートの代わりにマウントする、モデルフォルダー内のサブフォルダーです。空のままにするとフォルダーのルートがマウントされます。このフィールドは、サーバがマウントサブパスに対応している場合にのみ表示されます。
...
:::
   ...
   実行方式の設定に対応していないサーバでは、**実行方式** と **シェル** のフィールドは表示されず、**開始コマンド** フィールドと **ポート** のみが提供されます。この場合コマンドはシェルなしで実行されるため、シェル演算子を使うには引き続き `/bin/bash -c "..."` で明示的に囲む必要があります。
