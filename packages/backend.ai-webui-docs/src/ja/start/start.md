...
1. **Hugging Face モデルの URL または ID**フィールドにモデルを入力します。
   ...
   タグ（ある場合）、**サイズ**、更新日時が表示されます。サイズにマウスを
   合わせると、すべてのリビジョンを含むリポジトリ全体のサイズであり、実際の
   ダウンロード容量はこれより小さい場合があるという注記が表示されます

   プレビューは、そのままではダウンロードできないモデルについても警告します。
   モデルが Hugging Face で無効化されていてダウンロードできない場合は赤色の
   エラーが、ゲート付きモデルの場合は黄色の警告が表示されます。ゲート付き
   モデルは、Hugging Face でライセンスに同意し、アクセストークンを入力しないと
   （手順3を参照）ダウンロードに失敗します。入力したアドレスでモデルが
   見つからない場合は、モデルが存在しないか非公開であると表示されるため、
   モデルIDを確認するか、アクセストークンを入力してください

   この照会はあくまで参考情報です。たとえばブラウザが`huggingface.co`に
   接続できずモデル情報を取得できない場合でも、インポートはそのまま実行できる
   というメッセージが表示されるだけで、インポートは妨げられません
