...
:::note
ここで説明する権限は個々のストレージフォルダに適用され、フォルダの所有者が管理します。ストレージホスト全体に適用されるフォルダ権限は管理者専用の設定で、ストレージホスト詳細Drawerの**プロジェクトフォルダーの権限**タブおよび**ユーザーフォルダーの権限**タブで構成します。その際、対象のドメインやユーザーは名前を入力するのではなく、セレクターから選択します。詳細については、[ユーザーフォルダーの権限](#user-folder-permission)を参照してください。
:::
