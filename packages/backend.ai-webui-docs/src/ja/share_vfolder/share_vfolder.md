...
:::note
以前のセッションランチャーを使用する場合は、[ユーザー設定](#general-tab)セクションを参照してください。使用方法については、次の[リンク](https://webui.docs.backend.ai/en/23.09_a/sessions_all/sessions_all.html)を参照してください。NEOセッションランチャーの詳細については、[セッション作成](#start-a-new-session)を参照してください。
:::
