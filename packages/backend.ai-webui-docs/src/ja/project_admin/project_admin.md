...
スーパー管理者は、**プロジェクト**管理者ページ（**ユーザー → プロジェクト**）から起動する**プロジェクト管理者を設定**モーダルを通じて、1か所で1人以上のユーザーにプロジェクト管理者権限を付与・取り消しできます。**マネージャー26.8.0以降**では、これがプロジェクト管理者を管理する主要な方法です。
...
モーダルタイトルの横にある盾チェック（**RBAC権限を表示**）アイコンは、プロジェクトの`project-<project_id>-admin`ロールがあらかじめフィルターされ、詳細パネルが開いた状態でRBAC管理ページを開き、基盤となるロールを確認できるようにします。このロールの詳細については、RBAC管理章の[プロジェクト管理者権限の付与](#grant-project-admin)を参照してください。
...
4つのプロジェクト管理者ページはいずれも、テーブル上部のツールバー右端に同じ更新コントロールを備えています。更新ボタンをクリックすると、一覧をすぐに再読み込みします。
...
- 各ページはそれぞれの間隔を記憶し、選択した値はページを再読み込みしても保持されます。**オフ** を選択すると、そのページの自動更新だけが停止します。

更新ボタンを手動でクリックした場合も残り時間はリセットされるため、次の自動更新までは選択した間隔がまるごと残ります。
...
- **環境**: セッションのコンテナイメージを、イメージのアイコン、ベース名、バージョン、アーキテクチャの順で表示します。アイコンと表示名は、インストールされたBackend.AIに登録されたイメージメタデータから取得され、メタデータが登録されていないイメージにはデフォルトのアイコンが表示されます。
- **セッションタイプ**: セッションの種類を色付きタグで表示します — `INTERACTIVE`、`BATCH`、`INFERENCE`。
- **クラスターモード**: **シングル** または **マルチ** を表示し、括弧内にセッションのクラスターサイズを併記します。

表示する列を変更するには、テーブル下部のページネーションコントロールの横にある設定（歯車）アイコンをクリックし、**テーブル設定** モーダルで選択を調整します。既定で非表示の **セッションID** と **プロジェクト** の列も、ここから表示できます。
