...
モーダルタイトルの横にある**RBAC権限を表示**アイコンは、プロジェクトの`project-<project_id>-admin`ロールがあらかじめフィルターされ、詳細パネルが開いた状態でRBAC管理ページを開き、基盤となるロールを確認できるようにします。このロールの詳細については、RBAC管理章の[プロジェクト管理者権限の付与](#grant-project-admin)を参照してください。
