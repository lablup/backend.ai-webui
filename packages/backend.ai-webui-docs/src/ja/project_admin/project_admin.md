...
:::note
本章で説明するプロジェクト管理者機能は **Backend.AI WebUI 26.4.0** 以降でサポートされます。
:::
...
プロジェクト管理者ロールを持つプロジェクトを選択すると、サイドバーの **管理** セクションに、そのプロジェクトを管理するための4つの項目が表示されます:
...
これら4つのページのいずれにも、現在選択されているプロジェクトに表示内容が限定されていることを知らせる情報バナーが上部に表示されます。
...
1. 最も左側の列のチェックボックスを使用して、終了するセッションを選択します。単一のセッションを終了する場合は、その行の終了アクションを使用することもできます。
2. テーブルヘッダーの電源オフアイコンをクリックして確認モーダルを開きます。
3. モーダルに表示された対象セッションのリストを確認し、**OK** をクリックします。
...
- **ゴミ箱アイコン**をクリックすると、確認後にデプロイメントを削除できます。
...
- **アクティブ / 削除済み** タブ: 現在アクティブなフォルダとソフト削除されたフォルダを切り替えます。
- **モードフィルター**: フォルダの使用モードでフィルタリングします — **すべて**、**一般**、**データ**、**自動マウント**、**モデル**。
...
**削除済み** タブに切り替えると、ソフト削除されたフォルダを確認できます。各行で次の2つのアクションを使用できます:

- **復元**: フォルダをアクティブタブに戻します。
- **完全削除**: フォルダを完全に削除します。この操作は取り消せず、確認のためにフォルダ名を入力する必要があります。
