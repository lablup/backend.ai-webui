...
いずれかのプロジェクト管理者ページを表示している間は、現在選択されているプロジェクトに属する項目のみが表示されていることを知らせる単一の情報バナーが、ページコンテンツの上部に表示されます。このバナーは各ページごとに個別に表示されるのではなく、レイアウトレベルで一度だけレンダリングされます。
...
**リビジョン** 列には、デプロイメントの現在のリビジョンがクリック可能な `#N` リンクとして表示されます。これをクリックすると、現在のリビジョンの詳細を表示するパネルが開きます。
   ...
   現在のリビジョン情報は Backend.AI Manager 26.4.3 以降で利用できます。
