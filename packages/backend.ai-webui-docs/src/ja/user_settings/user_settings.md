...
の下部にある`生む`ボタンをクリックすると、SSHキーペアを更新できます。SSH
...
サーバーがキーを拒否した場合（例: キーの形式が正しくない場合）は、サーバーが返した
エラーメッセージが表示され、入力内容を修正できるようにダイアログは開いたままに
なります。
...
- **コマンドパレット**: ヘッダーに**検索**ボタンを追加し、`Ctrl+K`
  ...
  ボタンとショートカットは、本機能を有効にしている間のみ利用できます。
