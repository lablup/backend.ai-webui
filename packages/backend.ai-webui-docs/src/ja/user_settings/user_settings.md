...
テーブル上部の更新ボタンをクリックすると、一覧をその場で再読み込みします。隣の
矢印をクリックすると**自動更新**メニューが開き、一覧が自動的に再読み込みされる
間隔を選択できます:

- **オフ**（初期設定）
- **5秒**、**10秒**、**15秒**、**30秒**、**60秒**

自動更新がオンの間は、選択した間隔がボタンに表示され、次の再読み込みが近づくに
つれてボタンの枠線が満たされていきます。選択した間隔はこの一覧に対して保存され、
次にタブを開いたときにも適用されます。WebUIの各一覧はそれぞれ独自の間隔を保持
するため、ここで自動更新をオンにしても他のページには影響しません。
...
更新ボタンと自動更新の間隔メニューは、ログインセッションタブと同じように動作
します。詳しくは[一覧の更新](#auto-refresh-interval)を参照してください。この
一覧は、ログインセッションの一覧とは別に独自の間隔を保持します。
