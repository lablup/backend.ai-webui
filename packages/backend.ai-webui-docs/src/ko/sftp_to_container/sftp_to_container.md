...
먼저 연산 세션을 하나 생성한 후 Control의 앱 아이콘(첫 번째 버튼)을 클릭하고 SSH / SFTP 아이콘을 클릭합니다. 그러면 해당 컨테이너 내부에서 SSH/SFTP 접속을 할 수 있도록 해주는 데몬(daemon)이 뜨게 되고, 로컬 proxy를 통해 Web-UI 앱과 컨테이너 내부의 데몬이 연결됩니다.
...
assigned port number after `-p` option. The 일반 사용자 inside the compute session is
...
name `id_container.ppk__PROTECTED_22__id_container.ppk__PROTECTED_23__id_container__PROTECTED_24__/home/work/__PROTECTED_25__~/.ssh/config__PROTECTED_26__C:\Users\[일반 사용자 name]\.ssh\config` (for Windows) and add the following block.
