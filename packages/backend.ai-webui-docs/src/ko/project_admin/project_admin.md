...
모달 제목 옆의 **RBAC 권한 확인** 아이콘은 프로젝트의 `project-<project_id>-admin` 역할이 미리 필터링되고 상세 패널이 열린 상태로 RBAC 관리 페이지를 열어 기반 역할을 확인할 수 있게 합니다. 이 역할에 대한 자세한 내용은 RBAC 관리 장의 [프로젝트 관리자 권한 부여](#grant-project-admin)를 참고하세요.
