...
프로젝트 관리자 페이지 중 하나를 보고 있을 때는 페이지 콘텐츠 위에 현재 선택한 프로젝트에 속한 항목만 표시된다는 것을 알리는 단일 정보 배너가 표시됩니다. 이 배너는 각 페이지마다 별도로 표시되는 것이 아니라 레이아웃 수준에서 한 번만 렌더링됩니다.
...
**리비전** 열에는 배포의 현재 리비전이 클릭 가능한 `#N` 링크로 표시됩니다. 이 링크를 클릭하면 현재 리비전의 상세 정보를 보여 주는 패널이 열립니다.
   ...
   현재 리비전 정보는 Backend.AI Manager 26.4.3 이상에서 사용할 수 있습니다.
