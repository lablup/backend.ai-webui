...
1. **Hugging Face 모델 URL 또는 ID** 필드에 모델을 입력합니다.
   ...
   조회하여 — 확인하는 동안 로딩 표시가 나타납니다 — 가져오기를 시작하기 전에
   필드 아래에 미리보기 카드를 표시합니다. 카드에는 Hugging Face 페이지로
   연결되는 모델 ID 링크, 모델의 작업 및 라이브러리 태그(있는 경우), **크기**,
   수정일이 표시됩니다. 크기 위에 마우스를 올리면 모든 리비전을 포함한 저장소
   전체 크기이므로 실제 다운로드 용량은 더 작을 수 있다는 안내가 표시됩니다.

   미리보기는 그대로 다운로드할 수 없는 모델도 알려줍니다. 모델이 Hugging
   Face에서 비활성화되어 다운로드할 수 없는 경우에는 빨간색 오류가, 접근이
   제한된(gated) 모델인 경우에는 노란색 경고가 표시됩니다. 접근이 제한된
   모델은 Hugging Face에서 라이선스에 동의하고 액세스 토큰을 입력하지
   않으면(3단계 참고) 다운로드에 실패합니다. 입력한 주소로 모델을 찾을 수
   없으면 모델이 없거나 비공개라는 안내가 표시되므로, 모델 ID를 확인하거나
   액세스 토큰을 입력하세요.

   이 조회는 참고용입니다. 예를 들어 브라우저가 `huggingface.co`에 접속할 수
   없어 모델 정보를 불러오지 못하더라도 가져오기를 그대로 진행할 수 있다는
   안내만 표시될 뿐 가져오기는 차단되지 않습니다.
