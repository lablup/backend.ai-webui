...
새로고침이 진행되는 동안에도 보드는 화면에 그대로 유지됩니다. 패널은 콘텐츠를 다시 구성해야 할 때만 해당 영역에 로딩 스켈레톤을 표시하며, 나머지 보드에는 영향을 주지 않습니다. 헤더에 새로고침 버튼이 있는 패널은 다음 자동 새로고침을 기다리지 않고 개별적으로 새로고침할 수도 있습니다.
