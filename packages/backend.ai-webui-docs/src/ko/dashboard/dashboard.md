...
새로고침이 진행되는 동안에도 보드는 화면에 그대로 유지됩니다. 이전에 보고 있던 값이 계속 표시되며, 새 데이터를 불러오는 중에도 패널을 이동하거나 크기를 조정하고 스크롤할 수 있습니다. 각 패널은 자신의 진행 상태를 개별적으로 표시합니다.

- 해당 패널의 데이터를 불러오는 동안 패널 헤더의 새로고침 버튼에 로딩 표시가 나타납니다.
- 패널이 콘텐츠를 다시 구성해야 하는 경우, 해당 패널 영역에만 로딩 스켈레톤이 표시됩니다. 나머지 보드는 계속 표시되고 조작할 수 있습니다.

다음 자동 새로고침을 기다리지 않고, 패널 헤더의 새로고침 버튼을 눌러 개별 패널을 언제든지 새로고침할 수도 있습니다.

:::note
[슈퍼 관리자 대시보드](#superadmin-dashboard)는 **30초** 주기로 새로고침됩니다.
:::

## 대시보드 레이아웃 사용자화

패널을 재배치하거나 크기를 조정하여 대시보드 레이아웃을 원하는 대로 사용자화할 수 있습니다.
...
사용자화된 레이아웃은 자동으로 저장되어 브라우저 세션 간에 유지됩니다. 레이아웃은 사용자별로 저장되므로, 각 사용자가 자신만의 배치를 설정할 수 있습니다.
