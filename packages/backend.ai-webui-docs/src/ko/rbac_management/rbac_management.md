...
- **속성 필터**: 목록을 좁히는 속성 필터입니다. 필터 입력은 선택한 속성에 따라 달라집니다. 다음 속성을 사용할 수 있습니다:
   ...
   * **적용 범위 ID**: 정확한 적용 범위 UUID로 목록을 필터링합니다.
...
- **적용 범위**: 역할에 설정된 적용 범위의 타입이며, 적용 범위 이름과 함께 표시됩니다.
- **적용 범위 ID**: 역할에 설정된 적용 범위의 원시 ID입니다.
...
역할에는 하나의 적용 범위만 설정됩니다. 역할이 여러 적용 범위를 가질 수 있었던 26.9 이전 버전의 매니저에 연결된 경우에는 **적용 범위**와 **적용 범위 ID** 열에 첫 번째 적용 범위가 표시되고 나머지 개수가 `+N`으로 함께 표시됩니다.
...
- **사용자 정의**: 슈퍼 관리자가 생성한 역할입니다. 이름, 설명, 사용자 할당, 세부 권한을 모두 편집할 수 있습니다.

## 역할 생성

역할을 생성할 때는 **적용 범위**를 먼저 정의합니다. 적용 범위는 역할을 특정 리소스 엔티티(도메인, 프로젝트, 사용자 등)에 바인딩하여, 이후 추가되는 모든 세부 권한이 여기에서 정의한 적용 범위 내에서만 작동하도록 제한합니다. 하나의 역할에는 적용 범위를 하나만 설정할 수 있습니다.
...
2. 생성 모달에서 다음 필드를 입력합니다:
   ...
   - **적용 범위** (필수): 역할이 적용될 적용 범위 타입을 선택합니다(예: 도메인, 프로젝트, 사용자)
   - **적용 대상** (필수): 선택한 적용 범위 타입 안에서 구체적인 대상을 선택합니다. 역할에는 하나의 적용 범위만 설정됩니다.
3. **확인**을 클릭하여 역할을 생성합니다

![](../images/rbac_create_role_modal.png)

:::info
역할 생성 시점에 설정하는 **적용 범위**와 **적용 대상**은 그 자체로 어떤 권한을 부여하지 않습니다. 대신 이후 이 역할에 [세부 권한](#manage-permissions)을 추가할 때 사용할 수 있는 **적용 범위 / 적용 대상**을 미리 정의하는 역할을 합니다. 즉, 역할 생성 단계에서는 이 역할의 세부 권한이 가질 수 있는 적용 범위와 적용 대상을 미리 한정하는 것이며, 세부 권한은 여기에서 정의한 적용 범위와 적용 대상 안에서만 설정할 수 있습니다.
:::

:::warning
적용 범위는 역할 생성 시점에 정의되며 이후 역할 상세 패널에서 수정할 수 없습니다. 역할을 생성하기 전에 적용 범위를 신중하게 결정하세요.
:::
...
:::info
각 세부 권한의 **적용 범위 / 적용 대상** 조합은 역할의 적용 범위에서 상속됩니다. 역할 생성 시에 정의된 적용 범위에 대해서만 세부 권한을 부여할 수 있습니다. 역할의 범위를 넓히려면 필요한 적용 범위를 가진 새 역할을 추가로 생성해야 합니다.
:::
