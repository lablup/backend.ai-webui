...
**역할 이름**, **생성일**, **수정일** 열 헤더를 클릭하여 테이블을 정렬할 수 있습니다. 테이블 헤더 오른쪽의 열 표시 설정 버튼을 사용하여 개별 열을 표시하거나 숨길 수 있습니다. **역할 생성** 버튼 옆의 새로고침 버튼을 클릭하면 최신 데이터로 목록을 다시 불러옵니다. 새로고침 버튼은 역할 상세 패널과 각 탭에서도 제공됩니다.

:::note
**자동 할당** 열이 표시되려면 Backend.AI Manager 26.4.4 이상 버전이 필요합니다.
:::
...
역할은 **활성** 또는 **비활성** 상태일 수 있습니다. 역할을 비활성화하는 것은 되돌릴 수 있는 소프트 삭제 방식의 작업입니다. 비활성화된 역할은 활성 목록에서 숨겨지지만 그대로 보존되며, 언제든지 다시 활성화하여 복구할 수 있습니다. 역할 목록 상단의 **상태 필터**를 사용하여 활성 역할과 비활성 역할을 전환할 수 있습니다.

생명주기 관련 동작은 **역할 이름** 셀의 호버 동작으로 제공됩니다. 역할 행 위에 마우스를 올리면 역할 이름 옆에 동작 아이콘이 나타납니다.
...
1. **상태 필터**가 **활성**으로 설정된 상태에서 비활성화할 역할 행 위에 마우스를 올립니다.
2. 역할 이름 옆의 **비활성화** 동작(금지 아이콘)을 클릭합니다.
3. 확인 팝업에서 **비활성화**를 클릭하여 확정하거나 **취소**를 눌러 닫습니다.

해당 역할은 비활성 목록으로 이동합니다. 다시 확인하려면 **상태 필터**를 **비활성**으로 설정합니다.

**상태 필터**가 **비활성**으로 설정되어 있으면 각 역할 행에 **활성화**(되돌리기 아이콘)와 **역할 영구 삭제**(삭제 아이콘) 두 가지 호버 동작이 표시됩니다.
...
1. **상태 필터**를 **비활성**으로 설정하고 역할 행 위에 마우스를 올립니다.
2. 역할 이름 옆의 **활성화** 동작(되돌리기 아이콘)을 클릭합니다.
3. 확인 팝업에서 **활성화**를 클릭하여 확정하거나 **취소**를 눌러 닫습니다.

해당 역할은 적용 범위, 세부 권한, 사용자 할당이 그대로 유지된 채 활성 목록으로 돌아갑니다.

## 역할 영구 삭제

영구 삭제는 시스템에서 역할을 완전히 제거합니다. 비활성화와 달리 영구 삭제는 되돌릴 수 없습니다.

:::danger
역할 영구 삭제는 **되돌릴 수 없습니다**. 역할과 그 구성이 영구적으로 삭제되어 복구할 수 없습니다. 일시적으로만 비활성화하려는 경우에는 영구 삭제 대신 비활성화를 사용하세요.
:::

**역할 영구 삭제** 동작은 **비활성** 역할에서만 사용할 수 있습니다. 현재 활성 상태인 역할은 먼저 비활성화해야 합니다([역할 활성화 및 비활성화](#activate-and-deactivate-roles) 참고).

비활성 역할을 영구 삭제하려면:

1. **상태 필터**를 **비활성**으로 설정하고 역할 행 위에 마우스를 올립니다.
2. 역할 이름 옆의 **역할 영구 삭제** 동작(삭제 아이콘)을 클릭합니다.
3. 확인 모달의 입력란에 역할 이름을 정확히 입력하여 확인합니다. 입력한 이름이 일치할 때까지 **역할 영구 삭제** 버튼은 비활성화 상태로 유지됩니다.
4. **역할 영구 삭제**를 클릭하여 역할을 영구적으로 삭제하거나 **취소**를 눌러 닫습니다.
...
:::warning
역할은 모든 사용자 할당과 세부 권한이 제거된 후에만 영구 삭제할 수 있습니다. 하나라도 남아 있으면 영구 삭제는 *"이 역할을 제거할 수 없습니다. 먼저 모든 할당 및 세부 권한을 제거하세요."* 메시지와 함께 실패합니다. 영구 삭제하기 전에 역할의 [세부 권한](#manage-permissions)과 [사용자 할당](#manage-user-assignments)을 제거하세요.
:::
...
적용 범위 타입은 역할을 어떤 종류의 리소스 엔티티에 바인딩할지를 결정합니다. 배포 환경에 따라 역할 생성 시 다음과 같은 적용 범위를 선택할 수 있습니다:
...
- **세션**: 연산 세션 선택
- **모델 서비스**: 모델 서비스 선택
- **컨테이너 레지스트리**: 컨테이너 레지스트리 선택
- **스토리지 호스트**: 스토리지 호스트 선택
- **키페어**: 키페어 선택

:::note
사용 가능한 적용 범위 타입 목록은 Backend.AI Manager가 결정하며, 실제로 동작을 수행할 수 있는 엔티티가 하나 이상 존재하는 적용 범위 타입만 표시됩니다. 따라서 선택 가능한 적용 범위 타입의 정확한 집합은 버전과 배포 환경에 따라 달라질 수 있으며, 위 목록과 다를 수 있습니다.
:::
...
3. 편집 모달에서 다음 필드를 수정합니다:
   - **역할 이름**: 역할의 이름
   - **설명**: 역할의 목적에 대한 설명
   - **자동 할당**: 활성화하면 역할이 등록된 적용 범위에 사용자가 추가될 때 자동으로 역할이 부여됩니다. 이 필드는 Backend.AI Manager 26.4.4 이상에서만 사용할 수 있습니다.
...
- **권한 타입**: 세부 권한이 제어하는 리소스 카테고리로, 선택한 적용 범위에 따라 필터링됩니다. 사용 가능한 권한 타입에는 **도메인**, **프로젝트**, **사용자**, **세션**, **폴더**, **모델 서비스**, **자원 그룹**, **이미지** 등이 포함됩니다. 정확한 집합은 선택한 적용 범위와 배포 환경에 따라 달라집니다.
...
| 도메인 내 모든 세션 조회 허용             | 도메인 / default      | 세션        | 조회             |
| 모델 서비스 관리 허용                     | 도메인 / default      | 모델 서비스 | 생성, 조회, 수정 |
| 컨테이너 이미지 삭제 허용                 | 도메인 / default      | 이미지      | 삭제             |
...
기존 세부 권한을 제거하고 다시 추가하지 않고도 수정할 수 있습니다.

1. **세부 권한** 탭에서 변경할 세부 권한 행 위에 마우스를 올린 후 **수정** 동작(연필 아이콘)을 클릭합니다.
2. 세부 권한 추가에 사용하는 것과 동일한 모달이 현재 값으로 미리 채워진 상태로 열립니다. 필요에 따라 **적용 범위 / 적용 대상**, **권한 타입**, **동작**을 조정합니다.
...
수정할 수 있는 필드는 [세부 권한 추가](#add-a-permission)의 필드와 동일하며, 모달 제목(**권한 수정**)과 확인 버튼(**저장**)만 다릅니다.
...
:::note
선택한 사용자 중 일부를 할당할 수 없는 경우에도 나머지 사용자에 대한 할당은 계속 진행됩니다. 몇 명의 사용자가 실패했는지 요약 메시지로 알려주며, 실패한 각 사용자에 대해 그 이유를 설명하는 별도의 오류 알림이 표시됩니다.
:::
...
:::note
선택한 사용자 중 일부를 해제할 수 없는 경우에도 나머지 사용자에 대한 작업은 계속 진행됩니다. 몇 명의 사용자가 실패했는지 요약 메시지로 알려주며, 실패한 각 사용자에 대해 그 이유를 설명하는 별도의 오류 알림이 표시됩니다.
:::
...
프로젝트를 생성하면 `project-<project_id>-admin`이라는 전용 역할이 함께 생성되며, 여기서 `<project_id>`는 해당 프로젝트의 UUID입니다. 이 역할에 사용자를 할당하면 해당 사용자에게 그 프로젝트에 대한 [프로젝트 관리자](#project-admin-features) 권한이 부여됩니다. 즉, 시스템 전반의 슈퍼관리자 권한을 갖지 않고도 해당 프로젝트의 사용자, 세션, 배포, 스토리지 폴더를 관리할 수 있습니다.

![](../images/rbac_project_admin_role_in_list.png)

사용자에게 프로젝트 관리자 권한을 부여하려면:

1. [역할 목록](#role-list)을 열고 대상 프로젝트의 `project-<project_id>-admin` 역할을 찾습니다. 속성 필터를 사용하여 역할 이름으로 검색할 수 있습니다(예: `project-`를 입력하여 목록을 좁힙니다).
2. 역할 이름을 클릭하여 역할 상세 화면을 엽니다.
3. 해당 역할에 대해 [역할에 사용자 추가](#add-users-to-a-role)의 절차를 따라 사용자를 할당합니다.

![](../images/rbac_project_admin_role_detail.png)

사용자에게는 즉시 프로젝트 관리자 권한이 부여됩니다. 다음에 헤더의 프로젝트 드롭다운을 열면 해당 프로젝트 옆에 프로젝트 관리자 배지가 표시되며, [프로젝트 관리자 기능](#project-admin-features) 챕터에서 설명한 프로젝트 관리자 전용 사이드바 항목도 함께 나타납니다.

프로젝트 관리자 권한을 회수하려면, 동일한 `project-<project_id>-admin` 역할에 대해 [역할에서 사용자 해제](#revoke-users-from-a-role)의 절차를 따릅니다.
