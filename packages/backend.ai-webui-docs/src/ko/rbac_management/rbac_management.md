...
RBAC(역할 기반 접근 제어) 관리를 통해 슈퍼어드민은 세분화된 권한이 포함된 역할을 정의하고 사용자에게 할당할 수 있습니다. RBAC를 사용하면 Backend.AI 시스템 전반에서 특정 사용자가 다양한 리소스에 대해 수행할 수 있는 작업을 제어할 수 있습니다.

:::note
RBAC 관리는 슈퍼어드민만 사용할 수 있으며, Backend.AI Manager 25.4.0 이상 버전이 필요합니다.
:::

RBAC 관리 페이지에 접근하려면 사이드바 메뉴의 Administration 섹션에서 **RBAC 관리**를 클릭합니다.
...
- **사용자 정의**: 슈퍼어드민이 생성한 권한입니다. 이름, 설명, 할당, 세부 권한 등 모든 항목을 편집할 수 있습니다.
...
2. 생성 모달에서 다음 필드를 입력합니다:
   ...
   - **프로젝트** (선택): 권한이 적용될 프로젝트를 선택합니다. 특정 프로젝트에 권한 범위를 지정하는 경우에만 필요합니다.
3. **확인**을 클릭하여 권한을 생성합니다

:::info
향후 버전에서는 권한 생성 모달에서 프로젝트 선택기가 제거될 예정입니다. 프로젝트 범위 권한은 권한 생성 시 프로젝트를 선택하지 않아도 세부 권한 설정에서 직접 구성할 수 있게 됩니다.
:::
...
메타데이터 아래에는 **권한 할당**과 **세부 권한** 두 개의 탭이 있습니다.
...
:::note
편집 버튼은 사용자 정의 권한에서만 사용할 수 있습니다. 시스템 권한의 이름이나 설명은 수정할 수 없습니다.
:::
...
**활성** 권한을 볼 때 권한 이름 위에 마우스를 올리면 **비활성화** 작업이 표시됩니다. **비활성** 권한을 볼 때 권한 이름 위에 마우스를 올리면 **활성화** 및 **영구 삭제** 작업이 표시됩니다.
...
- **적용 범위 타입**: 세부 권한이 대상으로 하는 리소스 유형(예: 도메인, 프로젝트, 사용자, 폴더, 자원 그룹, 세션, 모델 서비스, 컨테이너 레지스트리, 스토리지 호스트, 키페어)
- **적용 대상**: 적용 범위 타입 내의 특정 엔티티(예: 특정 도메인 이름, 특정 프로젝트)
- **권한 타입**: 세부 권한이 제어하는 리소스 카테고리로, 선택한 적용 범위 타입에 따라 필터링됩니다
...
다음은 네 가지 구성 요소가 어떻게 함께 작동하는지 이해하는 데 도움이 되는 일반적인 세부 권한 설정 예시입니다:

| 시나리오 | 적용 범위 타입 | 적용 대상 | 권한 타입 | 권한 |
|---------|-------------|---------|---------|------|
...
3. 모달에서 다음 필드를 입력합니다:
   - **적용 범위 타입**: 리소스 유형을 선택합니다(예: 도메인, 프로젝트, 사용자, 폴더, 자원 그룹)
   - **적용 대상**: 특정 엔티티를 선택하거나 검색합니다. 입력 방식은 선택한 적용 범위 타입에 따라 동적으로 변경됩니다:
      * 도메인: 활성 도메인 목록에서 선택
      * 프로젝트: 프로젝트 선택(도메인 필터링 가능)
      * 사용자: 이메일이나 이름으로 사용자 검색
      * 자원 그룹: 사용 가능한 자원 그룹에서 선택
      * 기타 유형: 해당 엔티티 목록에서 선택
   - **권한 타입**: 엔티티 유형을 선택합니다. 선택한 적용 범위 타입에 유효한 유형만 표시됩니다
   - **권한**: 작업을 선택합니다(예: 생성, 조회, 수정, 삭제, 영구 삭제 또는 위임 작업)
4. **확인**을 클릭하여 세부 권한을 생성합니다
