...
:::note
**할당된 사용자** 필터는 이를 지원하는 매니저가 필요하며, **적용 범위**와 **적용 범위 ID** 필터는 **매니저 26.8.0 이상**이 필요합니다. 이전 버전의 매니저에서는 이 필터 옵션이 표시되지 않습니다.
:::
...
:::note
**자동 할당** 메타데이터 행은 **매니저 26.4.4 이상**에서만 표시됩니다. 이전 버전의 매니저에서는 자동 할당 행이 표시되지 않습니다.
:::
...
:::note
이 통합 세부 권한 탭은 **매니저 26.8.0 이상**이 필요합니다. 이전 버전의 매니저에서는 역할 상세 패널에 **역할 범위**와 **세부 권한** 탭이 별도로 표시됩니다. **역할 범위** 탭은 역할의 적용 범위 항목(적용 범위, 적용 대상, 적용 대상 ID)을 읽기 전용 행으로 나열하고, **세부 권한** 탭은 각 세부 권한을 개별적으로 나열하며 행별 추가 및 제거 컨트롤을 제공합니다. 아래에서 설명하는 카드 기반 보기와 그리드 편집 모달은 해당 매니저에서는 사용할 수 없습니다.
:::

### 적용 범위 타입 카드

세부 권한 탭에는 역할이 사용하는 적용 범위 타입(예: 도메인, 프로젝트, 사용자)마다 하나의 카드가 표시됩니다. 역할이 사용하지 않는 적용 범위 타입은 숨겨지며, 카드 제목은 현지화된 적용 범위 타입 이름입니다. 각 카드는 다음을 포함합니다:
...
- **적용 범위 테이블**: 이 타입에 속하는 역할의 적용 범위를 다음 열과 함께 나열합니다:
   ...
   * **ID**: 적용 범위 UUID입니다.
   * **세부 권한**: 권한 타입마다 하나의 태그가 부여 상태에 따라 색상으로 표시됩니다(아래 참조). 적용 범위 타입에 설정 가능한 엔티티가 없으면 이 열은 `-`로 표시됩니다.
...
**세부 권한** 열에서 각 권한 타입 태그는 해당 적용 범위에 대해 그 타입의 동작이 얼마나 부여되었는지에 따라 색상으로 표시됩니다:
...
1. **세부 권한** 탭에서 적용 범위 타입 카드를 열고 변경할 적용 범위 행의 **편집** 동작을 클릭합니다.
...
:::note
이 제한은 **매니저 26.8.0 이상**이 필요합니다. 이전 버전의 매니저에서는 시스템 `project-<project_id>-admin` 역할에 대한 직접 할당이 가능하며, 그곳에서 프로젝트 관리자 권한을 부여하는 유일한 방법이므로 경고 알림과 읽기 전용 상태가 적용되지 않습니다.
:::
...
프로젝트를 생성하면 `project-<project_id>-admin`이라는 전용 역할이 함께 생성되며, 여기서 `<project_id>`는 해당 프로젝트의 UUID입니다. 이 역할에 할당된 사용자는 그 프로젝트에 대한 [프로젝트 관리자](#project-admin-features) 권한을 얻습니다. 즉, 시스템 전반의 슈퍼 관리자 권한을 갖지 않고도 해당 프로젝트의 사용자, 세션, 배포, 스토리지 폴더를 관리할 수 있습니다.
...
3. 사용자를 할당하려면 [역할에 사용자 추가](#add-users-to-a-role)의 절차를, 사용자를 제거하려면 [역할에서 사용자 해제](#revoke-users-from-a-role)의 절차를 따릅니다.
:::
