...
end-일반 사용자s (such as AI-based mobile apps and web service backends) to make
...
The official 모델 서비스 feature in version 23.09 resolves these
limitations. Therefore, starting from version 23.09, it is recommended
to create/manage 모델 서비스s through the model serving menu whenever
...
1. 모델 정의 파일 생성하기
2. 모델 정의 파일을 모델 타입 폴더에 업로드하기
3. 모델 서비스 생성하기/유효성 검사하기
4. (모델 서비스가 공개되지 않은 경우) 토큰 발급하기
5. (엔드 유저 전용) 모델 서비스에 대응하는 엔드포인트에 접속하여 서비스 확인하기
6. (필요한 경우) 모델 서비스 수정하기
7. (필요한 경우) 모델 서비스 종료하기
