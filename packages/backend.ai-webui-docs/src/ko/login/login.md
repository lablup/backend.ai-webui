...
로그인 화면은 브라우저의 언어로 표시됩니다. 먼저 로그인하거나 별도의 설정을 변경할 필요가 없습니다. 처음 방문한 경우나, 저장된 정보가 없는 시크릿 모드(사생활 보호 모드) 창에서도 WebUI가 브라우저가 알려주는 언어를 읽어 로그인 대화창과 입력 필드의 안내 문구, 버튼을 해당 언어로 표시합니다.

언어는 다음 순서로 결정됩니다.

1. 이전에 사용자 설정에서 언어를 선택한 적이 있다면 그 언어가 사용됩니다. 이 선택은 브라우저에 저장되며, 로그인 이후뿐 아니라 로그인 화면에도 적용됩니다.
2. 브라우저의 전체 로캘에 해당하는 번역이 제공되는 경우 그 언어가 사용됩니다(예: `pt-BR`).
3. 전체 로캘과 정확히 일치하는 번역이 없으면 로캘의 기본 언어 부분이 사용됩니다(예: 브라우저가 `de-AT`로 설정된 경우 `de`).
4. 위 어느 것도 제공되는 언어와 일치하지 않으면 영어가 사용됩니다.

:::note
인터페이스 언어를 직접 선택하려면 로그인 후 사용자 설정의 [일반](#general-tab) 탭을 엽니다. 여기서 선택한 언어는 이후 브라우저 언어 감지를 대체하며, 애플리케이션 내부는 물론 로그인 화면에도 적용됩니다.
:::
...
로그인하면 왼쪽 사이드바가 기본 탐색 수단이 됩니다. 사이드바의 너비를 변경할 수 있으며, 표시되는 항목은 현재 열려 있는 프로젝트에서 사용자가 가진 권한에 따라 달라집니다.
...
프로젝트 관리자용 항목은 계정 전체가 아니라 **현재 열려 있는 프로젝트**를 기준으로 판단됩니다. 작업 중인 프로젝트는 페이지 주소의 일부이므로, 상단 바의 프로젝트 선택기에서 프로젝트를 전환하면 권한이 다시 계산됩니다. 즉, 같은 계정이라도 어떤 프로젝트에서는 프로젝트 관리자이고 다른 프로젝트에서는 일반 사용자일 수 있으며, 그에 따라 프로젝트 관리자용 항목이 사이드바에 나타나거나 사라집니다. 특정 프로젝트를 가리키는 즐겨찾기나 공유된 링크를 열었을 때도 마찬가지로, 페이지가 로드되는 즉시 해당 프로젝트에서의 권한이 사이드바에 반영됩니다.

프로젝트 관리자에게 제공되는 항목과 각 항목의 내용은 프로젝트 관리자 기능 장의 [프로젝트 관리자 사이드바](#the-project-admin-sidebar) 섹션을 참고하세요.
