...
:::tip
필수 필드가 누락되었거나 값이 유효하지 않으면 검토 단계의 저장 버튼이 비활성화 상태로 유지되며, 해당 필드가 포함된 카드의 테두리가 빨간색으로 표시되고 카드 헤더의 **수정** 링크 옆에 오류 아이콘이 나타납니다. 또한 오른쪽 단계 목록에도 오류가 있는 단계가 모두 표시되며, 이미 지나쳐 온 단계의 필드에서 발생한 오류도 함께 표시됩니다. 필수 필드에는 입력 즉시 인라인 검증 메시지가 표시됩니다.
:::
