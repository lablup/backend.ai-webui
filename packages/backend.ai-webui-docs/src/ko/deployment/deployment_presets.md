...
:::note
API 등으로 서로 맞지 않는 컨트롤과 값 유형이 저장된 프리셋을 열면, 저장된 값 유형을 저장된 UI 유형으로는 표시할 수 없다는 경고가 나타나고 해당 필드에 **선택한 UI 유형은 이 값 유형을 표시할 수 없습니다.** 라는 메시지가 표시됩니다. 저장하기 전에 둘 중 하나를 변경해 주세요.

**UI 유형** 필드를 제공하지 않는 서버에서는 반대 방향으로 제약이 적용됩니다. 즉, 프리셋에 이미 저장된 컨트롤에 따라 선택할 수 있는 **값 유형** 이 제한됩니다.
:::
