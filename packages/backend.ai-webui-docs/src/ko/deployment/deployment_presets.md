...
- **모델 정의**(선택 사항): 모델 이름, 모델 경로, 서비스 구성, 헬스 체크 설정, 메타데이터.
...
관리자 배포 페이지에는 **배포**, **모델 스토어 관리**, **프로메테우스 프리셋**, **배포 프리셋** 등 여러 탭이 있습니다. 현재 보고 있는 탭은 페이지 URL에 기록되며, 각 탭의 필터, 정렬 순서, 페이지 번호도 함께 URL에 저장됩니다. 이로 인해 다음 두 가지 이점이 있습니다.

- 다른 탭으로 이동했다가 돌아와도 배포 프리셋 탭이 필터가 해제된 첫 페이지로 초기화되지 않고, 떠날 때의 상태 그대로 복원됩니다.
- 필터를 적용한 프리셋 목록의 URL을 북마크하거나 공유하면, 받는 사람도 동일한 탭에서 동일한 필터, 정렬, 페이지 상태로 화면을 열게 됩니다.

한 탭의 필터와 페이지 상태는 다른 탭으로 넘어가지 않습니다. 따라서 **배포** 탭에서 설정한 필터가 **배포 프리셋** 탭에 그대로 적용되는 일은 없습니다.
...
2. 각 필드를 입력합니다. 이 화면은 **기본 정보**, **모델 & 실행**, **검토** 세 단계로 구성된 마법사이며, 오른쪽에 단계 목록이, 아래쪽에 `이전` / `다음` 버튼이 표시됩니다. `검토로 건너뛰기`를 클릭하면 마지막 단계로 바로 이동합니다. 필드는 다음 섹션으로 구성됩니다.
   ...
   - **실행**: **실행 준비 명령어**, 환경 변수, 부트스트랩 스크립트. **실행 준비 명령어** 필드에는 셸 문법 안내(`쉘 문법: /bin/bash -c "cmd1; cmd2"`)가 함께 표시됩니다. 입력한 명령은 `/bin/bash -c <명령>` 형태로 실행되므로, 여러 명령을 세미콜론(`;`)으로 연결하는 등 셸 문법을 사용할 수 있습니다.
      ...
      :::note[실행 준비 명령어와 실행 명령어는 다릅니다]
      두 명령어는 서로 다른 값이며, 이제 각 필드마다 별도의 설명이 표시되어 구분하기 쉬워졌습니다.

      | 필드 | 위치 | 역할 | 예시 |
      |---|---|---|---|
      | **실행 준비 명령어** | 프리셋의 실행 섹션 | *"모델 프레임워크를 시작하기 전에 환경을 준비하는 명령어입니다 (예: vllm 설치)."* | `pip install vllm` |
      | **실행 명령어** | 프리셋의 모델 정의 섹션 | *"모델 서빙 프로세스를 시작하는 CLI 명령어입니다."* | `vllm serve /models --tp 2` |

      패키지 설치, 자산 내려받기, 설정 파일 생성처럼 준비 작업에는 **실행 준비 명령어**를, 서빙 프레임워크를 실제로 구동하는 명령에는 **실행 명령어**를 사용하세요. 각 필드의 플레이스홀더에도 해당 유형에 맞는 예시가 표시됩니다.
      :::
   - **배포 기본값**:
   ...
   - **모델 정의**(선택 사항): 토글을 활성화하면 모델 이름, 모델 경로, 서비스 구성, 헬스 체크, 메타데이터를 설정할 수 있습니다. 헬스 체크는 **헬스 체크 활성화(Enable Health Check)** 토글로 제어합니다 (기본값: 끔). 토글이 꺼져 있으면 헬스 체크 필드가 숨겨지고, 켜면 모든 필드가 나타납니다.
...
:::tip
필수 필드가 누락되었거나 값이 유효하지 않으면 검토 단계의 저장 버튼이 비활성화 상태로 유지되며, 해당 필드가 포함된 카드의 테두리가 빨간색으로 표시됩니다. 필수 필드에는 입력 즉시 인라인 검증 메시지가 표시됩니다.
:::
...
2. `/admin/deployments/deployment-presets/{presetId}/edit` 주소의 프리셋 작성 화면이 현재 값으로 채워진 상태로 열립니다. 단계와 섹션 구성은 생성 흐름과 동일하며, vLLM 및 SGLang 런타임의 **런타임 파라미터** 섹션도 그대로 제공됩니다.
