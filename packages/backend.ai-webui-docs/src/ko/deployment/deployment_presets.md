...
**관리자 배포** 페이지(`/admin/deployments`)의 **런타임 파라미터** 탭에서는 런타임이 제공하는 개별 파라미터를 정의합니다. 각 항목은 파라미터 하나를 설명하며, 컨테이너에 전달될 키, 값 유형, 기본값, 그리고 화면에 표시되는 방식을 담습니다. 이렇게 정의한 항목들이 모여, 사용자가 배포 리비전을 추가할 때 입력하는 **런타임 파라미터** 탭을 구성합니다(배포 페이지의 [런타임 파라미터](#runtime-parameters) 참고).
...
- **파라미터 대상**: 값이 컨테이너에 전달되는 방식으로, **환경 변수** 또는 **커맨드라인 인자** 입니다.
- **값 유형**: **문자열**, **정수**, **실수**, **bool 자료형(Boolean)**, **플래그** 중 하나입니다.
- **UI 유형**: 배포 폼에서 이 파라미터를 표시할 때 사용하는 컨트롤로, **텍스트 입력**, **숫자 입력**, **체크박스**, **셀렉트**, **슬라이더** 중 하나입니다. 현재 버전의 WebUI가 인식하지 못하는 컨트롤은 저장된 값이 그대로 표시되며, `-` 는 지정된 컨트롤이 없다는 뜻입니다. 배포 폼에서는 지정된 컨트롤이 해당 값 유형을 표시할 수 없는 경우 일반 텍스트 입력으로 대체됩니다.
- **키**: 값이 전달되는 환경 변수 이름 또는 커맨드라인 인자입니다.
- **기본값**: 사용자가 파라미터를 변경하지 않았을 때 런타임이 사용하는 값입니다.
- **필수**: 해당 런타임으로 리비전을 만들 때 이 파라미터를 반드시 입력해야 하는지 여부입니다.
- **순위**: 같은 런타임의 파라미터 간 표시 순서입니다. 값이 낮을수록 먼저 표시됩니다.
- **생성일**: 파라미터가 생성된 시각입니다.
...
표 위의 **파라미터 생성** 버튼을 클릭하면 **파라미터 생성** 모달이 열리고, 행의 편집 버튼을 클릭하면 현재 값이 채워진 **파라미터 편집** 모달이 열립니다. 이미 생성된 파라미터의 런타임은 변경할 수 없습니다.
...
- **파라미터 대상**: **환경 변수** 또는 **커맨드라인 인자** 입니다. 필수 항목입니다.
- **값 유형**: **문자열**, **정수**, **실수**, **bool 자료형(Boolean)**, **플래그** 중 하나입니다. 필수 항목입니다. 아래의 **UI 유형** 보다 먼저 선택하세요. 값 유형을 변경하면 이미 선택한 컨트롤이 해제되는데, 이전 유형에 맞춰 고른 컨트롤이 새 유형을 표시하지 못할 수 있기 때문입니다.
- **UI 유형**: 배포 폼에서 이 파라미터를 표시할 때 사용할 컨트롤입니다. 비워 두면 일반 텍스트 입력으로 표시됩니다. 선택한 **값 유형** 을 표시할 수 있는 컨트롤만 선택할 수 있으며, 나머지는 비활성화됩니다. 유형을 선택하면 해당 유형의 설정이 함께 나타납니다.
   * **텍스트 입력**(문자열, 정수, 실수): **입력 플레이스홀더** — 필드가 비어 있을 때 표시되는 안내 문구입니다.
   * **숫자 입력**(정수, 실수): **최솟값** 과 **최댓값**. 범위를 벗어난 값은 배포 폼에서 검증 오류로 표시됩니다.
   * **체크박스**(Boolean, 플래그): 추가 설정이 없습니다.
   * **셀렉트**(문자열): **선택지** — 옵션마다 **값** / **라벨** 한 행씩 입력합니다. **선택지 추가** 버튼으로 행을 추가하고 휴지통 버튼으로 삭제하며, 최소 하나 이상의 선택지가 필요합니다.
   * **슬라이더**(정수, 실수): **최솟값** 과 **최댓값**(둘 다 필수이며 최댓값이 최솟값보다 커야 합니다), 그리고 슬라이더가 움직이는 증가 단위인 **단위(Step)** 입니다(기본값 `1`).
...
- **필수 여부**: **필수** 체크박스를 선택하면 사용자가 리비전을 만들 때 이 파라미터를 반드시 입력해야 합니다. 필수 파라미터는 배포 폼에서 빨간색 별표(★)로 표시됩니다.
- **순위** *(편집 시에만 표시)*: 같은 런타임의 파라미터 간 표시 순서입니다. 값이 낮을수록 먼저 표시됩니다.

:::note
API 등으로 서로 맞지 않는 컨트롤과 값 유형이 저장된 파라미터를 열면, 저장된 값 유형을 저장된 UI 유형으로는 표시할 수 없다는 경고가 나타나고 해당 필드에 **선택한 UI 유형은 이 값 유형을 표시할 수 없습니다.** 라는 메시지가 표시됩니다. 저장하기 전에 둘 중 하나를 변경해 주세요.

**UI 유형** 필드를 제공하지 않는 서버에서는 반대 방향으로 제약이 적용됩니다. 즉, 파라미터에 이미 저장된 컨트롤에 따라 선택할 수 있는 **값 유형** 이 제한됩니다.
:::

`생성`(편집 시에는 `저장`) 버튼을 클릭하면 저장됩니다. 런타임 파라미터가 생성 또는 업데이트되었다는 알림이 표시되고 목록이 새로 고쳐집니다.

### 런타임 파라미터 삭제

파라미터 행의 삭제 버튼을 클릭합니다. 파라미터 이름을 직접 입력해 확인하는 다이얼로그가 열리며, 입력한 값이 정확히 일치할 때까지 **삭제** 버튼은 비활성화 상태로 유지됩니다.

:::danger
런타임 파라미터 삭제는 **되돌릴 수 없습니다**. 해당 런타임을 사용하는 모든 배포 폼의 **런타임 파라미터** 섹션에서 이 파라미터가 사라집니다.
:::
