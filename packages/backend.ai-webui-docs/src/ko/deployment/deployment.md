...
목록 상단의 새로고침 버튼에서 자동 새로고침도 함께 설정할 수 있습니다. 버튼 옆의 화살표를 클릭하면 **끄기**, `5초`, `10초`, `15초`, `30초`, `60초` 중에서 주기를 선택할 수 있습니다. 배포 목록은 `15초`로 시작하며, 주기를 한 번 선택하면(**끄기**를 선택한 경우도 포함) 그 선택은 이 목록에 대해 페이지를 새로 고쳐도 유지됩니다. 자동 새로고침이 켜져 있는 동안에는 화살표 버튼에 선택한 주기가 표시되고, 다음 새로고침까지 남은 시간이 버튼 테두리에 채워집니다. 새로고침 버튼을 직접 클릭하면 이 카운트다운도 다시 시작됩니다.
...
:::warning[자원 그룹은 생성 시점에 확정됩니다]
모달이 열리는 순간부터 **자원 그룹** 필드 아래에는 *"생성 시 선택한 자원 그룹은 변경할 수 없습니다."* 라는 경고 문구가 표시됩니다. 자원 그룹은 리비전이 아니라 배포에 속하는 값이므로, 배포를 수정하거나 새 리비전을 추가하는 방법으로도 다른 자원 그룹으로 옮길 수 없습니다. 배포를 생성하기 전에 올바른 자원 그룹을 선택하고, 다른 자원 그룹이 필요하다면 배포를 새로 생성하세요.

**외부에 공개** 역시 같은 방식으로 확정됩니다. **배포 수정** 모달에서 이 체크박스는 읽기 전용이며, 툴팁에는 *"생성한 이후에는 변경할 수 없습니다."* 라고 표시됩니다.
:::
...
- **시작 명령어**: 추론 서버를 실행하는 명령어입니다. 예를 들어 `python -m http.server 8000`.

:::tip[셸 연산자를 사용하려면 셸을 명시적으로 호출해야 합니다]
...
- **모델 폴더 마운트할 경로**: 컨테이너 내부에 모델 폴더가 마운트되는 경로입니다 (기본값: `/models`).
...
:::note
두 모델 정의 모드 모두 이 필드의 이름은 **모델 폴더 마운트할 경로**로 동일하며 같은 값을 저장합니다. 따라서 **명령어 입력**과 **설정 파일 사용** 사이를 오가더라도 이 필드의 의미는 달라지지 않습니다.
:::
...
고급 모드에서는 모든 런타임 변형의 폼 하단에 **헬스 체크 활성화** 섹션이 포함됩니다. 새 리비전의 경우 기본적으로 비활성화되어 있습니다. **헬스 체크 활성화**를 선택하면 다음 필드가 표시되며, 모두 필수입니다:

- **경로**: 서비스 정상 여부를 확인하기 위해 시스템이 호출하는 HTTP 엔드포인트 경로입니다.
- **간격**: 헬스 체크 사이의 시간(초)입니다.
- **최대 재시도 횟수**: 복제본이 `UNHEALTHY`로 표시되기 전까지 허용되는 연속 실패 횟수입니다.
- **최대 대기 시간**: 각 요청의 타임아웃 시간(초)입니다.
...
- **모두 열기**: 펼칠 수 있는 모든 행을 엽니다.
- **모두 닫기**: 모든 행을 닫습니다.
- **오류만 펼치기**: 결과가 성공이 아닌 행만 엽니다. 기본값이므로, 모달을 열자마자 실패·재시도·만료 항목을 바로 확인할 수 있습니다.

   보기 방식은 **어떤 행을 펼친 상태로 시작할지**만 결정하며, 펼쳐진 행에 표시되는 내용은 바꾸지 않습니다. 펼쳐진 행에는 성공한 하위 단계를 포함하여 해당 이벤트의 **전체** 하위 단계 표가 항상 표시되므로, 실패한 단계를 그 앞에 실행된 단계들과 함께 확인할 수 있습니다. 직접 열거나 닫은 행은 보기 방식을 바꾸거나 데이터를 다시 불러오기 전까지 그 상태를 유지합니다.

복제본 탭에서 여는 **복제본 스케줄링 기록** 모달도 동일한 표와 동일한 펼침 컨트롤을 사용합니다. [복제본](#replicas-tab-history)을 참고하세요.
...
- **모델 폴더**: 각 복제본에 마운트된 모델 폴더로, 링크와 함께 **모델 폴더 마운트 대상**이 표시됩니다.
...
복제본 카드의 새로고침 버튼에도 배포 목록과 동일한 자동 새로고침 주기 선택 기능이 있습니다. 복제본 상태는 빠르게 변하므로 이 카드는 `10초`로 시작하며, 선택한 주기는 배포 목록의 설정과 별도로 저장됩니다.
...
토큰이 발급되면 **토큰** 대화 상자가 열려 토큰 값과 복사 버튼, 그리고 선택한 만료일(또는 *만료 없음*)을 한 번 보여줍니다. 이 대화 상자에서 토큰을 복사하거나, 이후 목록에서 복사할 수 있습니다.
...
- **생성일**: 토큰이 발급된 시점입니다.
- **만료일**: 토큰이 만료되는 시점이며, 만료되지 않는 토큰은 *만료 없음*으로 표시됩니다.
...
- 각 항목에는 토큰의 **마지막 6자리**가 코드 형태로 표시되고, 그 옆에 만료일(예: `~ 2026년 8월 26일`) 또는 *만료 없음*이 함께 표시됩니다. 모든 토큰은 동일한 JWT 헤더로 시작하므로, 실제로 서로를 구분해 주는 부분은 끝부분입니다. 항목에 마우스를 올리면 생성 시각 → 만료 시각 전체를 확인할 수 있습니다.
- **만료된 토큰은 표시되지 않습니다.** 아직 유효한 토큰만 목록에 나타나며, **만료 없음**으로 발급한 토큰도 항상 포함됩니다.
