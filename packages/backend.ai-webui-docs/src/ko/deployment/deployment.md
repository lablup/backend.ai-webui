...
각 복제본 행에는 세 가지 독립적인 상태 필드가 표시됩니다.

- **라이프사이클 상태**: 복제본가 라이프사이클의 어느 단계에 있는지를 나타냅니다(예: *프로비저닝 중*, *실행 중*, *종료 중*).
- **헬스 상태**: 복제본 프로세스의 현재 헬스 상태입니다(예: *정상*, *비정상*).
- **트래픽 상태**: 복제본가 현재 요청을 처리 중인지 여부입니다.
