...
**배포** 카드 헤더 오른쪽 위의 `배포 생성` 버튼을 클릭하여 **배포 생성** 모달을 엽니다.
...
배포 페이지에서 **배포** 카드 헤더 오른쪽 위의 `배포 생성` 버튼을 클릭하여 **배포 생성** 모달을 엽니다. 이 모달에서는 배포 단위의 메타데이터만 입력하며, 이 단계에서 리비전이 함께 생성되지는 않습니다.
