...
- **서브경로**: 폴더 루트 대신 마운트할 모델 폴더 내부의 하위 폴더입니다. 필드를 클릭하면 디렉터리 선택 창이 열리며, 선택한 모델 폴더의 디렉터리 트리에서 하위 폴더를 고릅니다. 비워 두면 폴더 루트가 마운트됩니다.
...
- **명령어**: 추론 서버를 실행하는 명령어입니다. 예를 들어 `python -m http.server 8000`. **Shell** 방식에서는 여러 줄 입력 상자이며, **Exec** 방식에서는 **명령어 (argv)** 라는 이름의 한 줄 입력 필드로 바뀝니다. JSON 문자열을 하나의 인수로 전달하려면 JSON 전체를 작은따옴표로 감싸고 내부의 큰따옴표는 그대로 둡니다. 예: `--override-generation-config '{"temperature": 0.7}'`.
...
배포 상세 페이지의 **리비전** 카드에는 **현재 리비전**, **리비전 히스토리**, **감사 로그** 세 개의 탭이 있습니다. 카드 상단의 `리비전 추가` 버튼은 모든 탭에서 사용할 수 있으며, 리비전 추가 모달을 엽니다([리비전 추가](#add-revision) 참고).
...
테이블 위에는 속성 필터 바와 새로 고침 버튼이 있습니다. **리비전 번호**, **생성일**, **클러스터 모드**, **이미지**, **모델 폴더** 로 목록을 좁힐 수 있습니다. **이미지** 와 **모델 폴더** 값은 식별자를 직접 입력하는 대신 드롭다운에서 선택할 수 있습니다. **모델 폴더** 드롭다운은 해당 배포가 속한 프로젝트로 범위가 한정되고 삭제된 폴더를 제외한 모델 용도 폴더만 표시하며, 이는 리비전 추가 모달의 폴더 선택기와 동일한 범위입니다.
...
`규칙 추가` 버튼을 클릭하면 **오토스케일링 규칙 추가** 편집기가 열립니다.
