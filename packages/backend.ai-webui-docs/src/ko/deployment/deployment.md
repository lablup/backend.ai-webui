...
`배포 생성`을 클릭하면 배포가 생성되며, 배포 상세 페이지로 이동합니다. 첫 번째 리비전을 추가하기 전까지는 **배포된 리비전이 없습니다** 경고가 표시됩니다. 생성 후 배포 수준 설정(이름, 공개 여부, 목표 복제본 수, 태그)을 변경하려면 서비스 정보 카드의 **수정** 버튼을 클릭합니다.
...
마법사의 검토 단계(4단계)에서 시작 명령어와 부트스트랩 스크립트가 코드 블록으로 렌더링되어 확인 전에 쉽게 검토할 수 있습니다.
:::

- **모델 마운트 대상**: 컨테이너 내부에 모델 스토리지 폴더가 마운트되는 경로입니다 (기본값: `/models`).
- **포트(Port)**: 추론 서버가 수신하는 컨테이너 포트입니다 (기본값: `8000`).
- **헬스 체크 활성화**: 활성화하면 시스템이 주기적으로 추론 서버에 HTTP 요청을 보내 정상 응답 여부를 확인합니다. 비활성화 시(새 리비전의 기본값) 헬스 체크가 구성되지 않으며, 비정상 복제본이 자동으로 감지되지 않습니다. 프로덕션 배포에서는 활성화를 권장합니다. **헬스 체크 활성화**를 선택하면 다음 추가 필드가 표시됩니다:
   * **헬스 체크 URL**: 서비스 헬스 체크 시 호출되는 HTTP 엔드포인트 경로입니다 (기본값: `/health`).
   * **초기 헬스 체크 유예 시간**: 컨테이너 시작 후 헬스 체크 실패가 허용되는 유예 시간(초)이며, 첫 번째 헬스 체크가 성공하면 복제본이 활성화됩니다 (기본값: `60.0`). 로딩 시간이 긴 대형 모델의 경우 이 값을 높게 설정합니다.
   * **최대 재시도 횟수**: 복제본이 `UNHEALTHY`로 표시되기 전까지 허용되는 연속 헬스 체크 실패 횟수입니다 (기본값: `10`).
   * **간격**: 연속 헬스 체크 사이의 시간(초)입니다 (기본값: `10.0`).
   * **최대 대기 시간**: 각 헬스 체크 HTTP 요청의 타임아웃 시간(초)입니다 (기본값: `15.0`).
   * **예상 상태 코드**: 서비스가 정상임을 나타내는 HTTP 응답 상태 코드입니다 (기본값: `200`).
...
:::note[필수 파라미터]
관리자는 개별 파라미터를 필수로 지정할 수 있습니다. 필수 파라미터는 라벨 옆에 빨간 별표(★)로 표시됩니다. 필수 파라미터가 비어 있는 상태로 폼을 제출하면 차단됩니다. 방문하지 않은 탭에 있는 파라미터도 인라인 유효성 검사 오류가 표시됩니다.

Backend.AI Manager 26.4.4 이전 버전의 백엔드에서는 관리자 설정과 관계없이 모든 파라미터가 선택 사항으로 처리됩니다.
:::

**헬스 체크 활성화**

고급 모드에서는 `vLLM`과 `SGLang`을 포함한 모든 런타임 변형의 런타임 파라미터 영역 하단에 **헬스 체크 활성화** 섹션이 포함됩니다. 새 리비전의 경우 기본적으로 비활성화되어 있습니다. **헬스 체크 활성화**를 선택하면 다음 필드가 표시되며, 모두 필수입니다:

- **헬스 체크 URL**: 서비스 정상 여부를 확인하기 위해 시스템이 호출하는 HTTP 엔드포인트 경로입니다.
- **초기 헬스 체크 유예 시간**: 컨테이너 시작 후 헬스 체크 실패가 복제본에 적용되기 전까지 대기하는 시간(초)입니다 (기본값: `60.0`).
- **최대 재시도 횟수**: 복제본이 `UNHEALTHY`로 표시되기 전까지 허용되는 연속 실패 횟수입니다 (기본값: `10`).
- **간격**: 헬스 체크 사이의 시간(초)입니다 (기본값: `10.0`).
- **최대 대기 시간**: 각 요청의 타임아웃 시간(초)입니다 (기본값: `15.0`).
- **예상 상태 코드**: 정상 응답을 나타내는 HTTP 상태 코드입니다 (기본값: `200`).
...
- **필터 바**: **상태(Status)**, **작업(Operation)**, **실행자(Triggered By)**(사용자 ID로 검색), **시간(Time)** 날짜 범위 선택기로 로그 항목을 필터링합니다.
- **자동 새로 고침 버튼**: 탭을 이탈하지 않고 로그를 다시 로드합니다.
- **페이지네이션**: 로그 항목을 페이지 단위로 탐색합니다.

:::note
감사 로그 탭은 지연 로딩을 사용합니다. 쿼리는 탭을 처음 활성화할 때만 전송됩니다. 활성 탭은 URL에 반영되므로(`?revisionTab=auditLog`), 감사 로그 뷰로 직접 연결되는 링크를 공유할 수 있습니다.

감사 로그 데이터 접근은 백엔드에서 권한이 적용되며, 슈퍼관리자에게만 허용될 수 있습니다.
:::
...
배포 프리셋은 관리자가 리비전 구성 값(런타임 변형, 컨테이너 이미지, 자원 할당, 시작 명령어, 헬스 체크 설정 등)을 미리 저장해 두고, 리비전을 추가할 때 재사용할 수 있도록 합니다. 사용자는 리비전 추가 시 **프리셋 모드**에서 프리셋을 선택하며, 관리자는 배포 프리셋 페이지에서 프리셋을 생성 및 편집합니다.

### 배포 프리셋 관리

배포 프리셋 페이지에는 현재 프로젝트에서 사용 가능한 모든 프리셋이 목록으로 표시됩니다. 테이블에는 다음과 같은 **기본 표시** 컬럼이 포함됩니다:

- **이름**: 프리셋 이름입니다.
- **런타임**: 런타임 변형입니다 (예: `vLLM`, `SGLang`, `Custom`).
- **이미지**: `<canonicalName>@<architecture>` 형식의 컨테이너 이미지입니다 (예: `cr.backend.ai/stable/pytorch:2.1-cuda12.1@aarch64`). 값은 복사 가능합니다. 이 형식을 사용하면 혼합 아키텍처 클러스터에서 CPU 아키텍처별로 이미지를 쉽게 구분할 수 있습니다.
- **복제본 수**: 이 프리셋에 저장된 목표 복제본 수입니다.
- **생성일**: 프리셋이 생성된 날짜입니다.
- **수정일**: 프리셋이 마지막으로 업데이트된 날짜입니다 (`updatedAt`).

테이블 헤더 오른쪽의 **컬럼 표시** 기어 버튼으로 추가 컬럼을 표시하거나 숨길 수 있습니다. 다음 컬럼은 **기본적으로 숨겨져 있지만** 활성화할 수 있습니다:

- **설명**: 프리셋의 설명 텍스트입니다.
- **시작 명령어**: 구성된 시작 명령어로, 말줄임표 처리되어 툴팁으로 전체 내용을 확인할 수 있습니다. 값은 복사 가능합니다.
- **클러스터**: 프리셋에 저장된 클러스터 모드입니다.
- **전략**: 스케줄링 전략입니다.
- **외부에 공개**: 프리셋의 기본 공개 범위 설정에 따라 `공개` 또는 `비공개` 태그로 표시됩니다.
- **리비전 기록 제한**: 유지하는 리비전 수입니다.
- **순위**: 프리셋의 정렬 순위입니다 (정렬 가능한 컬럼).

### 배포 프리셋 생성

`프리셋 생성` 버튼을 클릭하여 **배포 프리셋 생성** 폼을 엽니다. 폼에는 리비전 추가 고급 모드 폼과 동일한 섹션으로 구성된 필드가 포함됩니다:

- **모델 및 런타임**: 런타임 변형을 선택합니다. `vLLM` 또는 `SGLang`을 선택하면 탭으로 구성된 **런타임 파라미터** 섹션(Model Loading, Resource Memory, Serving Performance 등)이 나타납니다. `Custom` 런타임의 경우 시작 명령어 및 헬스 체크 필드가 표시됩니다.
- **실행 환경**: 컨테이너 이미지를 선택합니다. 드롭다운에서 이미지는 `<canonicalName>@<architecture>` 형식으로 표시되므로 CPU 아키텍처별로 이미지를 구분할 수 있습니다. 이 형식은 폼의 검토 단계에도 동일하게 표시됩니다.
- **클러스터 및 리소스**: CPU, 메모리, 가속기 할당을 설정합니다.
- **고급 설정** *(접을 수 있음)*: 추가 스토리지 폴더를 마운트합니다.

:::note[필수 런타임 파라미터]
`vLLM` 또는 `SGLang` 런타임을 선택한 경우 관리자는 개별 런타임 파라미터를 필수로 지정할 수 있습니다(빨간 별표 ★로 표시). 필수 파라미터가 비어 있는 상태로 프리셋 폼을 제출하면 인라인 유효성 검사 오류와 함께 차단됩니다. 이 유효성 검사 규칙은 프리셋에서 리비전을 추가할 때도 동일하게 적용됩니다.
:::

구성된 런타임 파라미터 값은 프리셋과 함께 저장되며, 이 프리셋으로 배포를 생성할 때 적용됩니다. 변경하지 않은 파라미터는 런타임의 기본값이 유지됩니다. 구성된 값은 프리셋을 저장하기 전에 **검토** 단계의 **기본 정보** 항목에서도 확인할 수 있습니다.

### 배포 프리셋 편집

프리셋 행의 편집 아이콘을 클릭하면 현재 값이 미리 채워진 **배포 프리셋 편집** 폼이 열립니다. 필드와 유효성 검사 규칙은 프리셋 생성과 동일합니다.

### 프리셋 상세 모달

리비전 추가 모달의 **프리셋 모드**에서 프리셋 선택기 옆의 **ⓘ** 버튼을 클릭하면 **프리셋 상세** 모달이 열립니다. 이 모달에서는 다음 카드를 포함한 프리셋 구성 요약을 확인할 수 있습니다:

- **기본 정보**: 이름, 런타임 변형, 이미지, 복제본 수, 클러스터 모드, 시작 명령어, 환경 변수.
- **리소스**: CPU, 메모리, 가속기 할당.
- **헬스 체크**: 이 프리셋의 헬스 체크 활성화 여부입니다.
   * **활성화**된 경우: 구성된 경로, 간격, 최대 재시도 횟수, 최대 대기 시간, 예상 상태 코드, 초기 헬스 체크 유예 시간이 표시됩니다.
   * **비활성화**된 경우: `비활성화`로 표시됩니다.
...
슈퍼관리자는 모델 스토어 관리 패널에서 모델 카드를 생성, 편집, 삭제할 수 있습니다.

모델 카드를 생성하거나 편집할 때:

- **라벨(Label)** 및 **프레임워크(Framework)** 태그 필드는 **Enter** 또는 **쉼표(,)** 를 토큰 구분자로 사용할 수 있습니다. 예를 들어 `pytorch, transformers,`를 입력하면 각 항목 사이에 Enter를 누르지 않고도 여러 태그를 한 번에 추가할 수 있습니다.
- **도메인(Domain)** 필드는 새 카드를 생성할 때 관리자의 현재 활성 도메인으로 미리 채워져 도메인을 수동으로 선택하는 단계를 줄여줍니다.

:::tip
여러 라벨 또는 프레임워크 태그를 빠르게 추가하려면 쉼표로 구분하여 입력하세요 (예: `nlp, text-generation, llm`). 모두 한 번에 추가됩니다.
:::
