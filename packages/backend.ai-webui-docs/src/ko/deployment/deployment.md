...
:::warning[자원 그룹과 공개 여부는 생성 시점에 확정됩니다]
모달이 열리는 순간부터 **자원 그룹** 필드 아래에는 *"생성 시 선택한 자원 그룹은 변경할 수 없습니다."* 라는 경고 문구가, **외부에 공개** 필드 아래에는 *"생성 시 선택한 공개 여부는 변경할 수 없습니다."* 라는 경고 문구가 표시됩니다.

자원 그룹은 리비전이 아니라 배포에 속하는 값이므로, 배포를 수정하거나 새 리비전을 추가하는 방법으로도 다른 자원 그룹으로 옮길 수 없습니다. **외부에 공개** 역시 같은 방식으로 확정되며, **배포 수정** 모달에서 이 체크박스는 읽기 전용이고 그 아래에 동일한 문구가 표시됩니다. 배포를 생성하기 전에 두 값을 모두 올바르게 선택하고, 다른 값이 필요하다면 배포를 새로 생성하세요.
:::

`생성`을 클릭하면 배포가 생성되며, 배포 상세 페이지로 이동합니다. 첫 번째 리비전을 추가하기 전까지는 **배포된 리비전이 없습니다** 경고가 표시됩니다.

생성 후 배포 수준 설정(이름, 목표 복제본 수, 태그)을 변경하려면 **기본 정보** 카드의 **수정** 버튼을 클릭한 다음, **배포 수정** 모달에서 값을 변경하고 저장합니다. 자원 그룹과 **외부에 공개** 설정은 위에서 설명한 대로 생성 시점에 고정되므로 여기서는 변경할 수 없습니다.
...
배포에 현재 리비전이 있으면 두 모드 위에 **현재 리비전 불러오기** 안내 배너가 표시됩니다. **현재 리비전 불러오기**를 클릭하면 현재 적용 중인 리비전의 설정을 시작점으로 폼에 미리 채웁니다(필요한 경우 자동으로 고급 모드로 전환됩니다). 아직 현재 리비전이 없는 배포에서는 배너가 표시되지 않으며, 설정을 한 번 불러오면 배너는 사라집니다.

#### 프리셋 모드

미리 정의된 배포 프리셋으로 리비전을 빠르게 추가합니다.

- **모델 소스**: 이 리비전의 모델을 어디에서 가져올지 선택합니다. **모델 폴더** 또는 **모델 카드** 중 하나를 선택하며, 선택에 따라 아래 선택기가 바뀝니다. 소스를 변경하면 이미 선택한 모델과 프리셋 선택이 초기화됩니다.
- **모델 폴더** *(모델 폴더 소스)*: 각 복제본에 마운트될 스토리지 폴더입니다.
- **모델 카드** *(모델 카드 소스)*: 배포할 모델 카드입니다. 해당 카드의 모델 폴더가 각 복제본에 마운트되며, 그 카드와 호환되는 프리셋만 표시됩니다. 선택기 옆의 ⓘ 버튼을 클릭하면 선택한 카드의 **모델 카드 상세** 드로어가 열립니다.
- **프리셋**: 배포의 자원 그룹과 호환되는 배포 프리셋입니다. 옆의 ⓘ 버튼을 클릭하면 프리셋의 전체 구성을 확인할 수 있습니다. **모델 카드** 소스에서는 카드를 선택하기 전까지 *"모델 카드를 먼저 선택해 주세요."* 안내와 함께 선택기가 비활성화된 상태로 유지됩니다.

선택한 모델 카드에 마운트할 모델 폴더가 없으면 리비전이 생성되지 않고 *"선택한 모델 소스에 마운트할 모델 폴더가 없습니다. 다른 모델 카드나 모델 폴더를 선택하세요."* 오류 메시지가 표시됩니다. 다른 모델 카드를 선택하거나 소스를 다시 **모델 폴더**로 전환하세요.
...
- **모델 및 런타임**: 모델 폴더와 마운트 경로, 런타임 변형을 선택합니다. `vLLM` / `SGLang` 변형에서는 런타임 파라미터 패널이, `Custom` 변형에서는 서비스 구성 섹션이 나타납니다. 런타임별 상세 필드는 아래 섹션을 참고하세요.
- **헬스 체크** 및 **시작 전 작업**: 헬스 체크와 추론 서버 시작 전에 실행할 작업을 구성합니다. 두 항목 모두 모든 런타임 변형에서 사용할 수 있습니다.
- **실행 환경**: 컨테이너 이미지(실행 환경 / 버전)와 환경 변수를 설정합니다.
- **클러스터 및 리소스**: CPU, 메모리, 가속기, 클러스터 모드 및 크기 등 자원을 할당합니다.
- **고급 설정** *(접을 수 있음)*: 모델 정의 파일 경로를 지정하고, 모델 폴더 외에 추가로 마운트할 스토리지 폴더를 설정합니다.

모달 하단의 **추가 후 바로 적용** 체크박스를 선택하면 리비전이 생성된 직후 바로 활성화됩니다. 선택하지 않으면 비활성 상태로 저장되며, 이후 리비전 탭에서 직접 적용할 수 있습니다.

### 리비전 추가: 필드 참고

아래 하위 섹션에서는 리비전 단위 필드를 자세히 설명합니다. **고급 모드**에서 리비전을 수동으로 추가할 때와 각 필드가 무엇을 제어하는지 이해하고자 할 때 모두 적용됩니다.
...
- **서브경로**: 폴더 루트 대신 마운트할 모델 폴더 내부의 하위 폴더입니다. 필드를 클릭하면 디렉터리 선택 창이 열리며, 선택한 모델 폴더의 디렉터리 트리에서 하위 폴더를 고릅니다. 경로를 직접 입력할 수는 없습니다. **모델 폴더**를 선택하기 전까지는 이 필드를 사용할 수 없으며, 모델 폴더를 변경하면 선택했던 경로가 초기화됩니다. 선택한 경로는 앞에 `/`가 붙어 표시됩니다. 비워 두면 폴더 루트가 마운트됩니다.

<a id="service-configuration"></a>

#### 서비스 구성 (Custom 런타임만 해당)

모델 폴더에서 설정을 읽는 런타임 변형(예: **Custom**)을 선택하면 **런타임** 선택기 아래에 **서비스 구성** 섹션이 나타납니다. 이 섹션은 기본적으로 펼쳐져 있으며, 추론 서버 프로세스의 시작 방식을 정의합니다.

- **실행 방식**: 명령어를 실행하는 방법입니다.
   * **Shell**: 셸(`bash -c "..."`)을 통해 명령어를 실행하므로 셸 연산자를 사용할 수 있습니다. 대부분의 경우 권장됩니다.
   * **Exec**: 셸을 거치지 않고 명령어를 인수 형태로 직접 실행합니다. 셸 없이 명령어를 실행해야 하는 경우(예: 셸 바이너리가 없는 이미지)에만 사용하세요.
- **셸**: 명령어 실행에 사용할 셸 바이너리입니다(예: `/bin/bash`). **Shell** 방식에서만 표시되며 필수 입력 항목입니다. 해당 바이너리가 컨테이너 이미지에 없으면 서비스가 시작되지 않습니다.
- **명령어**: 추론 서버를 실행하는 명령어입니다. 예를 들어 `python -m http.server 8000`. **Shell** 방식에서는 여러 줄 입력 상자이며, **Exec** 방식에서는 **명령어 (argv)** 라는 이름의 한 줄 입력 필드로 바뀝니다.
- **포트**: 추론 서버가 수신하는 컨테이너 포트입니다(2~65535).

:::tip[셸 연산자는 Shell 방식에서만 동작합니다]
**Shell** 방식에서는 명령어가 셸로 전달되므로 `;`, `|`, `&&`, `$VAR`, 리다이렉션 같은 연산자가 터미널에서와 동일하게 해석됩니다.

```bash
chmod +x /setup.sh; vllm serve /models
```

**Exec** 방식에서는 같은 문자열이 인수 단위로 분리되어 직접 실행되므로, 이러한 연산자는 일반 텍스트로 전달됩니다. 인수는 공백으로 구분하고, 공백이 포함된 인수는 따옴표로 묶으세요(예: `--name "my model"`). 셸 문법이 필요하다면 명령어를 `/bin/bash -c "..."`로 감싸는 대신 **실행 방식**을 **Shell** 로 되돌리세요.
:::
...
**헬스 체크 활성화** 와 **시작 전 작업** 필드는 서비스 구성 섹션 아래에 이어지며, 모든 런타임 변형에서 표시됩니다.

- **헬스 체크 활성화**: 활성화하면 시스템이 추론 서버에 주기적으로 HTTP 요청을 보내 정상적으로 응답하는지 확인합니다. 비활성화된 경우(새 리비전의 기본값) 헬스 체크가 구성되지 않으며, 비정상 복제본이 자동으로 감지되지 않습니다. 운영 환경 배포에서는 활성화하는 것을 권장합니다. **헬스 체크 활성화** 를 선택하면 다음 필드가 추가로 표시됩니다.
   * **경로**: 서비스 헬스 체크 시 호출되는 HTTP 엔드포인트 경로입니다 (기본값: `/health`).
   * **간격**: 연속된 헬스 체크 사이의 시간(초)입니다.
   * **최대 재시도 횟수**: 복제본이 `UNHEALTHY`로 표시되기 전까지 허용되는 최대 연속 실패 횟수입니다.
   * **최대 대기 시간**: 개별 헬스 체크 요청의 타임아웃 시간(초)입니다.
   * **상태 코드**: 서비스가 정상임을 나타내는 HTTP 응답 상태 코드입니다.
   * **초기 헬스 체크 유예 시간**: 컨테이너 시작 후 헬스 체크 실패를 허용하는 유예 시간(초)입니다. 첫 성공 응답 시점에 복제본이 활성화됩니다. 로딩에 오래 걸리는 대형 모델에서는 값을 늘리세요.
- **시작 전 작업**: 모델 서비스가 시작되기 전에 실행할 작업입니다. **사전 실행 동작 추가** 를 클릭해 행을 추가한 뒤 **동작**(작업 이름, 예: `wait_for_file`)과 **인수 (JSON)**(JSON 객체 형식의 인수, 예: `{}`)를 입력합니다. 두 필드 모두 각 행에서 필수이며, 인수는 올바른 JSON이어야 합니다. 지원되는 작업 목록은 [Backend.AI 모델 서빙에서 지원하는 서비스 동작 설명](#prestart-actions) 을 참고하세요.

모델 정의 파일 경로 자체는 폼 하단의 **고급 설정** 패널에 있습니다. 모델 정의 파일 작성 방법은 [모델 정의 파일 생성](#model-definition-guide) 섹션을 참고하세요.

<a id="runtime-parameters"></a>

#### 런타임 파라미터 (vLLM / SGLang)

`vLLM` 또는 `SGLang` 런타임 변형을 선택하면, 서비스 구성 섹션 대신 **런타임 파라미터** 섹션이 나타납니다. 이 섹션에서는 구성 파일을 수동으로 편집하지 않고도 서빙 프레임워크를 구성할 수 있습니다.
...
**헬스 체크 활성화** 와 **시작 전 작업** 필드는 런타임 파라미터 섹션 아래에 위치하며, [헬스 체크 및 시작 전 작업](#health-check-and-pre-start-actions) 에서 설명한 것과 동일하게 동작합니다. `Custom` 변형과 마찬가지로 `vLLM` 및 `SGLang` 에서도 표시됩니다.

런타임 파라미터 외에도 `vLLM` 및 `SGLang` 런타임 변형은 **환경** 섹션에 다음 환경 변수를 미리 채웁니다.
...
- **환경 이름 (수동)**: 이미지 참조를 직접 입력하는 필드로, 서버에서 이미지 이름 직접 입력을 허용한 경우에만 표시됩니다. 이 필드에 값을 입력하면 **실행 환경 / 버전** 선택이 해제되고, 반대로 목록에서 이미지를 선택하면 이 필드가 비워집니다. 두 방식은 같은 이미지를 지정하는 두 가지 방법입니다.
- **환경 변수**: 추론 서버 컨테이너에 전달되는 키/값 쌍입니다. `vLLM` 및 `SGLang` 의 경우 위에 나열한 런타임 전용 변수가 미리 채워집니다. 항목을 자유롭게 추가, 편집, 삭제할 수 있습니다. 변수 이름을 입력하면 `HF_TOKEN`, `WANDB_API_KEY`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_DEFAULT_REGION`, `HTTP_PROXY`, `HTTPS_PROXY`, `NO_PROXY` 등 자주 사용하는 환경 변수 이름이 선택한 런타임 변형의 변수와 함께 추천 목록으로 표시됩니다. 다른 행에서 이미 사용 중인 이름은 목록에서 제외됩니다. 알려진 이름을 입력하면 값 입력란에 해당 변수에 맞는 안내 문구가 표시됩니다. 예를 들어 `HF_TOKEN` 에는 *"Hugging Face 액세스 토큰"* 이 표시됩니다.

##### 이미지 이름을 직접 입력하는 경우

목록에서 선택하는 대신 **환경 이름 (수동)** 필드에 이미지 이름을 입력하면, 폼을 제출하는 시점에 입력한 참조를 서버에 등록된 이미지와 대조하여 일치하는 이미지로 리비전을 생성합니다.

- 레지스트리에서 사용하는 것과 동일한 형식으로 입력합니다. 예: `cr.backend.ai/stable/python:3.10-ubuntu20.04`
- `@` 뒤에 아키텍처를 덧붙여 특정 아키텍처를 지정할 수 있습니다. 예: `cr.backend.ai/stable/python:3.10-ubuntu20.04@aarch64` 아키텍처를 생략하면 매니저의 기본 아키텍처가 사용됩니다.

입력한 값과 일치하는 이미지가 없으면 리비전은 생성되지 **않으며**, 해당 필드에 *"입력한 이미지를 찾을 수 없습니다. 등록된 이미지 참조를 입력하거나 목록에서 선택하세요."* 라는 오류가 표시되고 화면이 그 필드로 이동합니다. 참조를 수정하거나, 필드를 비우고 **실행 환경 / 버전** 목록에서 이미지를 선택하세요.

   이미지 조회는 제출 시점에 이루어지므로, 오류는 입력하는 도중이 아니라 저장 버튼을 클릭한 뒤에 표시됩니다.
...
**고급 설정** 펼침 패널을 열면 모델 정의 파일 경로를 지정하고, 모델 스토리지 폴더 외에 추가 스토리지 폴더를 마운트할 수 있습니다.

- **모델 정의 파일 경로**: 모델 스토리지 폴더 내 모델 정의 파일의 경로입니다 (기본값: `model-definition.yaml`). `Custom` 처럼 모델 폴더에서 설정을 읽는 런타임 변형에서만 표시됩니다.
- **추가 마운트**: 추론 서버 컨테이너에 마운트할 스토리지 폴더 목록입니다. 준비 상태(`ready`)의 비모델 폴더만 선택할 수 있으며, 모델 타입 폴더와 현재 선택한 모델 폴더는 목록에서 제외됩니다. 이름이 `.`으로 시작하는 자동 마운트 폴더는 선택할 수 없고, 테이블 하단에 자동으로 마운트될 폴더 목록으로 별도 표시됩니다.
  ...
  (명령 스크립트 및 코드 포함).
   ...
   - `pre_start_actions`: `start_command` 이전에 실행되는 작업입니다. 이러한 작업들은
     ...
     작업은 정의된 순서대로 순차적으로 실행됩니다.
      ...
      - `action`: 수행할 작업 유형입니다. 사용 가능한 작업 유형과 해당 파라미터는
        [사전 시작 작업](#prestart-actions)을 참조하세요.
      - `args`: 작업별 파라미터입니다. 각 작업 유형마다 다른 필수 인수가 있습니다.
...
모델 폴더에 `deployment-config.yaml` 파일을 배치하여 모델에서 배포를 생성할 때 사용하는 리소스, 환경 및 런타임 설정을 미리 구성할 수 있습니다. 파일이 존재하면 해당 값들이 **기본값**으로 사용되며, 배포 시점에 설정하는 모든 항목(리비전 추가 양식 또는 API 요청)이 이를 재정의합니다.

:::note
같은 폴더에 있는 `service-definition.toml`(TOML 형식) 파일도 폴백으로 읽히지만 **더 이상 권장되지 않습니다** — 모델 폴더에는 `deployment-config.yaml`을 사용하세요.
:::

`deployment-config.yaml`은 **선택 사항**이며 배포를 막지 않습니다. 파일의 존재 여부와 관계없이 모델을 배포할 수 있으며, 누락되거나 형식이 잘못된 파일은 단순히 건너뜁니다. 모델 및 추론 서버를 설명하는 선택 사항인 `model-definition.yaml`과 함께 사용하면 관리자는 모델과 함께 합리적인 기본값을 제공할 수 있습니다.

값은 우선순위가 가장 낮은 것부터 가장 높은 것 순으로 **3단계 재정의 계층**을 통해 결정됩니다.

1. **루트 수준 기본값** — 모든 런타임 변형에 적용됩니다.
2. **런타임 변형 섹션**(예: `vllm`, `sglang`, `tgi`) — 해당 변형에 한해 개별 필드만 재정의합니다.
3. **배포 시점 입력**(리비전 추가 양식 / API 요청) — 모든 항목을 재정의합니다.
...
- `environment`: 추론 서버의 컨테이너 이미지 및 아키텍처입니다.
   - `image`: 컨테이너 이미지의 전체 경로입니다(예: `example.com/model-server:latest`).
   - `architecture`: 이미지의 CPU 아키텍처입니다(예: `x86_64`, `aarch64`).
- `resource_slots`: 각 복제본에 할당되는 연산 리소스입니다.
   - `cpu`: CPU 코어 수입니다(예: `4`, `8`).
   - `mem`: 단위 접미사가 붙은 메모리 양입니다(예: `"16gb"`).
   - `"cuda.shares"` / `"cuda.device"`: 분할 또는 전체 GPU 할당입니다. 점을 포함하는 키는 따옴표로 감쌉니다.
- `environ`: 추론 서버 컨테이너로 전달되는 환경 변수입니다. 루트 값과 변형 값이 병합됩니다.
- `resource_opts`: `shmem`(공유 메모리 크기)과 같은 추가 리소스 옵션입니다.
...
- [모델 스토어](#model-store)에서 사전 구성된 모델 카드를 탐색하고 `배포` 버튼을 클릭하면 프리셋 기반으로 바로 배포할 수 있습니다.
- 데이터 페이지의 **모델** 탭에서 모델 폴더의 `서비스로 배포` 버튼을 클릭하면 **프리셋으로 새 배포 생성** 모달이 열립니다. 프리셋과 자원 그룹을 선택하면 별도의 설정 없이 바로 배포됩니다.

전체 모델 배포 흐름은 [모델 스토어](#model-store) 섹션을 참고하세요.
...
- **비공개 배포입니다. 엔드포인트에 접근하려면 액세스 토큰이 필요합니다.**: **외부에 공개** 옵션이 비활성화되어 있고, 엔드포인트 URL이 발급되었으며, 아직 액세스 토큰이 하나도 없을 때 표시됩니다. 배너에서 바로 액세스 토큰 생성 대화 상자를 여는 **액세스 토큰 추가** 버튼이 포함되며, 첫 토큰을 생성하면 배너는 사라집니다. 자세한 내용은 [토큰 생성](#generating-tokens)을 참고하세요.
...
상태 태그 옆의 **스케줄링 기록** 링크 버튼을 클릭하면 **배포 스케줄링 기록** 모달이 열립니다. 해당 배포에 기록된 스케줄링 이벤트가 최신순으로 표시되고 속성 필터 바(ID, 단계, 결과, 이전 상태, 이후 상태, 오류 코드, 메시지, 생성일, 업데이트일)와 새로 고침 버튼도 있습니다.

![](../images/deployment_scheduling_history.png)

<a id="scheduling-history-substeps"></a>

세부단계가 기록된 이벤트에는 행 왼쪽에 확장 화살표가 표시됩니다. 이를 클릭하면 해당 단계의 세부단계가 한 행에 하나씩 표시되는 간결한 인라인 표가 열리며, 다음 컬럼으로 구성됩니다.

- **세부단계**: 세부단계의 이름입니다.
- **결과**: 세부단계의 결과로, 색상으로 구분된 마커로 표시됩니다.
- **소요 시간**: 세부단계에 걸린 시간입니다(예: `120 ms`, `1.35 s`).
- **시간**: 세부단계가 시작된 시각으로, 밀리초까지 표시됩니다.
- **오류 코드**: 세부단계에 기록된 오류 코드이며, 없으면 `-` 로 표시됩니다.
- **메시지**: 세부단계의 상세 내용이나 오류 문구로, 한 줄로 축약되어 표시됩니다.

복제본 탭에서 여는 **복제본 스케줄링 기록** 모달도 동일한 표를 사용합니다. [복제본](#replicas-tab-history)을 참고하세요.

<a id="revisions-tab"></a>

### 리비전 탭

배포 상세 페이지의 **리비전** 카드에는 **현재 리비전**, **리비전 히스토리**, **감사 로그** 세 개의 탭이 있습니다. 카드 상단의 `리비전 추가` 버튼은 모든 탭에서 사용할 수 있으며, 리비전 추가 모달을 엽니다([리비전 추가](#add-revision) 참고). 이 버튼은 중지 중이거나 더 이상 활성 상태가 아닌 배포, 그리고 현재 선택된 프로젝트와 다른 프로젝트에 속한 배포에서는 비활성화되며, 비활성화된 동안에는 툴팁에 *"배포가 중지 중이거나 더 이상 활성 상태가 아닙니다."* 또는 *"리비전을 추가하려면 이 배포의 프로젝트로 전환하세요."* 라고 이유가 표시됩니다.
...
- **모델 폴더**: 각 복제본에 마운트된 모델 폴더로, 링크와 함께 **모델 폴더 마운트할 경로**가 표시됩니다. 하위 폴더를 마운트한 리비전에서는 `서브경로: <값>` 줄도 함께 표시됩니다.
- **모델 정의 파일 경로**: 모델 폴더 내 모델 정의 파일 경로입니다.
- **추가 마운트**: 각 복제본에 마운트된 추가 스토리지 폴더입니다.
- **런타임**: 서빙 런타임입니다(예: `vLLM`, `SGLang`, `Custom`).
- **런타임 파라미터**: 이 리비전에 구성된 런타임 변형 프리셋 파라미터 값입니다(예: DType: float32, Quantization: awq). 프리셋 파라미터 값이 구성된 `vLLM` 또는 `SGLang` 리비전에서만 표시됩니다. `Custom` 런타임이나 프리셋 값이 없는 리비전에서는 대시(-)로 표시됩니다.
- **런타임 구성**: `vLLM` 또는 `SGLang` 런타임에서 추가 설정이 존재하는 경우 JSON 형태로 표시됩니다.
...
- **생성일**
- **런타임**: 이 리비전에 사용된 서빙 런타임입니다(예: `vLLM`, `SGLang`, `Custom`).
...
- **적용** 버튼은 현재 활성 리비전과 적용 중인 리비전에 대해 비활성화됩니다. 배포가 중지된 상태이거나 적용 또는 롤백이 이미 진행 중인 경우에도 사용할 수 없습니다.

행의 리비전 번호를 클릭하면 해당 리비전의 전체 설정을 보여주는 리비전 상세 드로어가 열립니다. 드로어에도 **적용** 버튼과 **이 리비전 기반으로 리비전 생성** 버튼이 있으며, 표와 동일한 조건에서 **적용** 버튼이 비활성화됩니다.
...
복제본 탭은 배포를 구성하는 라우팅 노드를 보여줍니다. 탭 상단의 **실행 중 / 종료됨** 라디오 컨트롤로 항목을 필터링합니다.
...
각 복제본 행에는 서로 **독립적인** 세 가지 상태 필드가 표시됩니다. 세 필드는 서로 다른 축을 나타내므로 함께 읽어야 합니다. 예를 들어 복제본의 라이프사이클은 *실행 중*이면서 헬스 상태는 아직 *대기 중*일 수 있습니다.

- **라이프사이클 상태**: 복제본이 라이프사이클의 어느 단계에 있는지를 나타냅니다(예: *프로비저닝 중*, *준비 중*, *실행 중*, *종료 중*, *종료됨*). **준비 중** 단계에서 복제본은 초기 헬스 체크 유예 시간 안에 있습니다. 즉 시작되는 중이며 첫 번째 헬스 체크 성공을 기다립니다. 이 구간에서는 헬스 체크 실패가 허용되며, 첫 성공 시 *실행 중*으로 전환되고 구간이 끝날 때까지 한 번도 성공하지 못하면 종료됩니다. 컬럼 헤더에는 **라이프사이클**로 표시됩니다.
- **헬스 상태**: 복제본 프로세스의 현재 헬스 상태입니다(*정상*, *비정상*, *저하됨*, *대기 중*).
   * **대기 중**은 아직 첫 헬스 체크가 완료되지 않았음을 의미합니다. 복제본은 대개 **준비 중** 유예 시간 안에 있습니다. 그 자체로 오류는 아니며, 복제본의 현재 단계는 **라이프사이클 상태**를 보고 확인합니다.
- **트래픽 상태**: 복제본이 현재 요청을 처리 중인지 여부입니다. 복제본은 헬스 상태와 무관하게 트래픽에서 제외될 수 있으므로(예: 수동으로 비활성화한 경우) 트래픽을 헬스 상태에 합치지 않고 별도의 상태로 표시합니다.

:::note
세 가지 상태는 서로 독립적인 축입니다. **준비 중** 라이프사이클 단계에서 복제본은 초기 헬스 체크 유예 시간 안에 있으며, 첫 체크가 완료되기 전까지 **헬스 상태**는 대개 *대기 중*으로 표시됩니다. 이는 그 자체로 문제를 나타내지 않습니다.
:::
...
- 서버 측 페이지네이션.
- 메트릭 소스(Metric Source), 조건(Condition), 쿨다운 초(Cooldown Sec.), 단계 크기(Step Size), 최소 / 최대 복제본 수(Min / Max Replicas), 생성 시간(Created At), 최근 실행 시점(Last Triggered) 컬럼. **단계 크기** 컬럼은 설정한 조건에서 도출된 방향에 따라 `+`, `−`, `±` 부호가 자동으로 표시됩니다.
- 각 행의 조건 요약 옆에 표시되는 행별 편집 및 삭제 아이콘.

`규칙 추가` 버튼을 클릭하면 **오토스케일링 규칙 추가** 편집기가 열립니다. 이 버튼은 중지 중이거나 더 이상 활성 상태가 아닌 배포, 그리고 본인이 소유하지 않은 배포에서는 비활성화되며, 비활성화된 동안에는 툴팁에 *"배포가 중지 중이거나 더 이상 활성 상태가 아닙니다."* 또는 *"배포 소유자만 이 배포를 관리할 수 있습니다."* 라고 이유가 표시됩니다.

기존 규칙을 수정하려면 해당 행의 편집 아이콘을 클릭하세요. 규칙 값이 미리 채워진 상태로 **오토스케일링 규칙 수정** 편집기가 열립니다. 편집기에는 다음 필드가 순서대로 포함됩니다:

- **메트릭 소스(Metric Source)**: `Kernel` 또는 `Prometheus` 중 하나를 선택합니다.
- **메트릭 이름(Metric Name)**: `Kernel`의 경우 메트릭 이름을 입력합니다. `cpu_util`, `mem`, `net_rx`, `net_tx`와 같은 일반적인 메트릭이 자동 완성 제안으로 제공되며, 사용자 정의 이름을 자유롭게 입력할 수도 있습니다.
- **메트릭 이름 프리셋(Metric Name (Prometheus Preset))**: 메트릭 소스가 `Prometheus`일 때만 표시됩니다. 드롭다운에서 프리셋을 선택하면 프리셋의 메트릭 이름, 쿼리 템플릿, 그리고 (정의된 경우) 쿨다운 초(Cooldown Sec.)가 자동으로 채워집니다.
...
- **단계 크기(Step Size)**: 스케일링 이벤트마다 추가하거나 제거할 복제본 수를 지정하는 양의 정수입니다. 선택한 조건(스케일 인 / 스케일 아웃 / 스케일 인 & 아웃)에 따라 `-`, `+`, `±` 부호가 자동으로 표시됩니다.
   ...
   * 둘 다 설정: `[metric] < [minThreshold]`이면 스케일 **인**되고, `[metric] > [maxThreshold]`이면 스케일 **아웃**됩니다.
...
배포 상태가 `HEALTHY`가 되면 배포 목록에서 해당 배포 이름을 클릭하여 배포 상세 페이지를 엽니다. **기본 정보** 카드에서 **엔드포인트 URL**을 확인할 수 있습니다. **외부에 공개**가 활성화된 경우 최종 사용자는 토큰 없이 배포에 접근할 수 있습니다. 비활성화된 경우 아래 설명에 따라 토큰을 생성하세요.
...
1. 카드 헤더의 `액세스 토큰 생성` 버튼을 클릭합니다. 페이지 상단의 **비공개 배포** 배너에 있는 **액세스 토큰 추가** 버튼을 클릭해도 동일한 대화 상자가 열립니다.
2. 토큰의 **만료일**을 선택합니다.
   * `7일`, `30일`, `90일`: 지금부터 해당 일수 뒤에 만료됩니다. 기본값은 `7일`입니다.
   * **만료일 직접 설정**: 날짜·시간 선택기가 나타나 만료 시점을 직접 지정할 수 있습니다. 지정한 시점은 현재보다 미래여야 합니다.
...
토큰이 발급되면 **토큰** 대화 상자가 열려 토큰 값과 복사 버튼, 그리고 선택한 만료일을 한 번 보여줍니다. 이 대화 상자에서 토큰을 복사하거나, 이후 목록에서 복사할 수 있습니다.

#### 액세스 토큰 목록

카드에는 이 배포에 발급된 모든 액세스 토큰이 최신순으로 표시됩니다.

- **토큰**: 토큰 값입니다. 축약되어 표시되며 복사 버튼도 함께 있습니다. 같은 셀의 삭제 아이콘으로 토큰을 삭제할 수 있습니다.
- **생성일**: 토큰이 발급된 시점입니다.
- **만료일**: 토큰이 만료되는 시점입니다. **만료 없음**으로 생성한 토큰은 `2099년 12월 31일`로 표시됩니다.

![](../images/generated_token_copy.png)

토큰은 `Authorization` 요청 헤더로 전달합니다.
...
토큰 행의 삭제 아이콘을 클릭합니다. **액세스 토큰 삭제** 대화 상자가 열리면 확인을 위해 `삭제`를 입력해야 하며, 입력한 값이 일치할 때까지 확인 버튼은 비활성화됩니다.

:::danger
액세스 토큰 삭제는 **되돌릴 수 없으며** 즉시 적용됩니다. 해당 토큰을 계속 사용하는 클라이언트는 인증에 실패하게 됩니다.
:::
...
배포에 접근할 수 있도록 배포 URL을 최종 사용자와 공유합니다. **외부에 공개**가 활성화된 경우 배포 상세 페이지의 **서비스 엔드포인트** URL을 그대로 공유할 수 있습니다. 비공개 배포의 경우 URL과 액세스 토큰을 함께 공유합니다.
...
생성한 모델이 자동으로 선택된 채팅 페이지로 리디렉션되며, 여기서 바로 LLM 모델을 테스트할 수 있습니다.
채팅 기능에 대한 자세한 내용은 [채팅 페이지](#chat-page)를 참조하세요.

![](../images/LLM_chat.png)

#### 채팅 페이지에서 토큰 선택하기

채팅 페이지가 Backend.AI 배포에 연결되어 있으면, 토큰 입력란은 일반 텍스트 입력창이 아니라 해당 배포의 액세스 토큰 목록을 보여주는 선택기로 표시됩니다.

- 각 항목에는 토큰의 **마지막 6자리**가 코드 형태로 표시되고, 그 옆에 만료일(예: `~ 2026년 8월 26일`, **만료 없음**으로 생성한 토큰은 `~ 2099년 12월 31일`)이 함께 표시됩니다. 모든 토큰은 동일한 JWT 헤더로 시작하므로, 실제로 서로를 구분해 주는 부분은 끝부분입니다. 항목에 마우스를 올리면 생성 시각 → 만료 시각 전체를 확인할 수 있습니다.
- **만료된 토큰은 표시되지 않습니다.** 아직 유효한 토큰만 목록에 나타납니다.
- 입력란이 비어 있으면 **가장 최근에 생성된 유효한 토큰**이 자동으로 선택됩니다. 사용자가 직접 선택을 해제한 경우에는 다시 채워지지 않습니다.
- 입력란 옆의 톱니바퀴 아이콘(**액세스 토큰 설정**)을 클릭하면 해당 배포의 액세스 토큰 영역으로 이동하여 새 토큰을 발급할 수 있습니다. 돌아오면 목록이 다시 로드되므로 방금 생성한 토큰을 바로 선택할 수 있습니다.

배포에 유효한 토큰이 하나도 없으면 선택기는 비어 있습니다. [토큰 생성](#generating-tokens)을 참고하여 먼저 토큰을 발급하세요.

![](../images/chat_deployment_token_select.png)

API 연결에 문제가 발생하면, 채팅 페이지에 모델 설정을 수동으로 구성할 수 있는 옵션이 표시됩니다.
...
페이지 상단은 필터 및 정렬 레이아웃을 사용합니다:

- **속성 필터 바**: 조건을 하나 이상 추가하여 카드 목록을 좁힐 수 있습니다. 사용할 수 있는 속성은 두 가지입니다.
   * **모델 이름**: 이름으로 모델 카드를 검색합니다.
   * **스토리지 호스트**: 모델 폴더가 위치한 스토리지 호스트로 카드를 필터링합니다. 호스트 이름을 직접 입력하는 대신 서버에 등록된 스토리지 호스트 목록에서 선택하며, 선택하는 즉시 필터 조건으로 추가됩니다. 해당 호스트만 보려면 `equals`, 해당 호스트를 제외하려면 `not equals` 연산자를 사용합니다.
- **정렬(Sort)**: 결과 정렬 방식을 선택합니다. 사용 가능한 옵션은 `이름 (A→Z)`, `이름 (Z→A)`, `오래된 순`, `최신 순`입니다.
- **새로 고침(Refresh)**: 새로 고침 버튼을 클릭하여 카드 목록을 다시 로드합니다.

   호스트를 직접 입력하지 않고 목록에서 선택하므로, 호스트 이름을 잘못 입력해 결과가 비어 버리는 일이 없습니다. 조건은 `AND`로 결합되므로 이름 조건과 스토리지 호스트 조건을 함께 사용할 수 있습니다.
