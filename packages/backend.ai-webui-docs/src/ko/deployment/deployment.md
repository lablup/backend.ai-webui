...
목록 상단의 새로고침 버튼에서 자동 새로고침도 함께 설정할 수 있으며([자동 새로고침](#common-auto-refresh) 참고), 배포 목록은 `15초` 주기로 시작합니다.
...
하위 단계가 있는 이벤트는 행 앞의 `+` 아이콘을 클릭하여 펼칠 수 있으며, 해당 이벤트의 하위 단계 표가 표시됩니다. 펼침 열 헤더의 메뉴에서는 모든 행 열기, 모든 행 닫기, 오류가 있는 행만 펼치기의 세 가지 보기 방식을 선택할 수 있습니다. 기본값은 **오류만 펼치기**이므로, 모달을 열자마자 실패·재시도·만료 항목을 바로 확인할 수 있습니다.
...
복제본 카드의 새로고침 버튼에도 자동 새로고침 주기 선택 기능이 있습니다([자동 새로고침](#common-auto-refresh) 참고). 복제본 상태는 빠르게 변하므로 이 카드는 `10초`로 시작하며, 선택한 주기는 배포 목록의 설정과 별도로 저장됩니다.
