...
:::note[필수 파라미터]
관리자는 개별 파라미터를 필수로 지정할 수 있습니다. 필수 파라미터는 레이블 옆에 빨간색 별표(★)로 표시됩니다. 필수 파라미터가 비어 있으면 폼 제출이 차단되며, 아직 열어 보지 않은 탭에 있는 파라미터라도 해당 필드에 인라인 검증 오류가 표시됩니다.
:::
...
- **환경 변수**: 추론 서버 컨테이너에 전달되는 키/값 쌍입니다. `vLLM` 및 `SGLang` 의 경우 위에 나열한 런타임 전용 변수가 미리 채워집니다. 항목을 자유롭게 추가, 편집, 삭제할 수 있습니다. 변수 이름을 입력하면 `HF_TOKEN`, `AWS_ACCESS_KEY_ID`, `HTTP_PROXY` 등 자주 사용하는 환경 변수 이름이 추천 목록으로 표시됩니다. 추천은 이름의 어느 위치에나 일치하므로 `proxy` 를 입력하면 `HTTP_PROXY`, `HTTPS_PROXY`, `NO_PROXY` 가 함께 표시되며, 다른 행에서 이미 사용 중인 이름은 다시 추천되지 않습니다. 이름 필드에 추천 변수가 입력되면 옆의 값 필드 플레이스홀더에 형식 힌트가 표시됩니다(예: `HF_TOKEN` 의 경우 *"Hugging Face 액세스 토큰"*).
...
상태 태그 옆의 **스케줄링 기록** 링크 버튼을 클릭하면 **배포 스케줄링 기록** 모달이 열립니다. 이 모달에는 해당 배포에 기록된 스케줄링 이벤트가 최신순으로 표시되며, 속성 필터 바(ID, 단계, 결과, 이전 상태, 이후 상태, 오류 코드, 메시지, 생성일, 업데이트일)와 새로 고침 버튼도 있습니다.
