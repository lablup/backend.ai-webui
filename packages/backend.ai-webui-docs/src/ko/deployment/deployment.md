...
- **서브경로**: 폴더 루트 대신 마운트할 모델 폴더 내부의 하위 폴더입니다. 비워 두면 폴더 루트가 마운트됩니다.
...
상태 태그 옆의 **스케줄링 기록** 링크 버튼을 클릭하면 **배포 스케줄링 기록** 모달이 열립니다. 모달은 전체 화면으로 열리며, 해당 배포에 기록된 스케줄링 이벤트가 최신순으로 표시되고 속성 필터 바(ID, 단계, 결과, 이전 상태, 이후 상태, 오류 코드, 메시지, 생성일, 업데이트일)와 새로 고침 버튼도 있습니다.
...
배포 단계의 세부단계 표에서 마지막 행은 수행된 작업이 아니라 해당 단계 자체의 결과를 다시 표시하는 음영 처리된 결과 마커입니다. 이 행에는 소요 시간이 없으며 메시지는 *"이 시도의 결과 마커"* 로 표시됩니다. 세부단계가 이 마커뿐인 행은 위 행을 그대로 반복하게 되므로 확장할 수 없습니다.
...
**라이프사이클** 컬럼의 상태 태그 옆에는 기록 아이콘 버튼이 있습니다. 클릭하면 해당 복제본의 **복제본 스케줄링 기록** 모달이 열리며, 복제본의 스케줄링 이벤트 기록을 날짜 범위, 상태 등으로 필터링하여 확인할 수 있습니다. 이 모달도 전체 화면으로 열리며, 세부단계는 [스케줄링 기록](#scheduling-history-substeps)에서 설명한 것과 동일한 인라인 표로 확장됩니다.
