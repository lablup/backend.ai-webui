...
- **서브경로**: 폴더 루트 대신 마운트할 모델 폴더 내부의 하위 폴더입니다. 경로를 직접 입력할 수는 없습니다. 먼저 **모델 폴더**를 선택해야 하며(선택하기 전까지 이 필드는 비활성화됩니다), 그다음 필드를 클릭하여 폴더의 디렉터리를 탐색한 후 **이 위치 선택**을 클릭해 경로를 지정합니다. 선택한 경로는 앞에 `/`가 붙어 표시되므로 폴더 루트는 `/`로 나타납니다. **모델 폴더**를 변경하면 이미 선택한 서브경로는 초기화됩니다. 비워 두면 폴더 루트가 마운트됩니다.
