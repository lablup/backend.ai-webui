...
테이블 위에는 속성 필터 바와 새로 고침 버튼이 있습니다. **리비전 번호**, **생성일**, **클러스터 모드**, **이미지**, **모델 폴더** 로 목록을 좁힐 수 있습니다. **이미지** 와 **모델 폴더** 값은 식별자를 직접 입력하는 대신 드롭다운에서 선택하며, 적용된 필터 태그에는 선택한 이름이 표시됩니다. **모델 폴더** 드롭다운은 해당 배포가 속한 프로젝트로 범위가 한정되고 삭제된 폴더를 제외한 모델 용도 폴더만 표시하며, 이는 리비전 추가 모달의 폴더 선택기와 동일한 범위입니다.
