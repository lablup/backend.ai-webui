...
- **서브경로**: 폴더 루트 대신 마운트할 모델 폴더 내부의 하위 폴더입니다. 비워 두면 폴더 루트가 마운트됩니다. 서버가 마운트 서브경로를 지원하는 경우에만 표시됩니다.
...
:::
   ...
   실행 방식 설정을 지원하지 않는 서버에서는 **실행 방식**과 **셸** 필드가 표시되지 않고, **실행 명령어** 필드 하나와 **포트** 만 제공됩니다. 이 경우 명령어는 셸 없이 실행되므로, 셸 연산자를 사용하려면 여전히 `/bin/bash -c "..."`로 명시적으로 감싸야 합니다.
...
:::note
이 파일은 이전에 `service-definition.toml`(TOML 형식)로 명명되었습니다. 레거시 `service-definition.toml`은 여전히 폴백으로 읽히지만 **더 이상 권장되지 않습니다** — 새로운 모델 폴더에는 `deployment-config.yaml`을 사용하세요.
:::
