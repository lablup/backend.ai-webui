...
생성 후 배포 수준 설정(이름, 목표 복제본 수, 태그)을 변경하려면 **기본 정보** 카드의 **수정** 버튼을 클릭합니다. 현재 값이 채워진 상태로 **배포 수정** 모달이 열리며, 저장 버튼은 `저장`으로 표시됩니다. 이 모달의 **자원 그룹** 필드는 읽기 전용으로 동일한 경고 문구를 함께 보여주고, **외부에 공개** 체크박스는 비활성화됩니다.
...
- **모델 폴더 마운트할 경로**: 컨테이너 내부에 모델 폴더가 마운트되는 경로입니다 (기본값: `/models`). 필드의 툴팁에도 *"컨테이너 내부에 모델 폴더가 마운트되는 경로입니다."* 라는 동일한 설명이 표시됩니다.
...
**기본 정보** 카드 헤더에는 새로 고침 버튼, **수정** 버튼, **더보기** 메뉴가 제공됩니다. **수정** 버튼은 [배포 생성 모달](#create-deployment)에서 설명한 **배포 수정** 모달을 열며, 저장 버튼은 `저장`으로 표시됩니다. 더보기 메뉴에는 현재 **배포 삭제** 동작이 포함됩니다.
...
- **토큰**: 토큰 값입니다. 축약되어 표시되며 복사 버튼이 함께 제공됩니다. 같은 셀의 삭제 아이콘으로 토큰을 삭제할 수 있습니다.
- **생성일**: 토큰이 발급된 시점입니다.
...
Chat 페이지가 Backend.AI 배포에 연결되어 있으면, 토큰 입력란은 일반 텍스트 입력창이 아니라 해당 배포의 액세스 토큰 목록을 보여주는 선택기로 표시됩니다.
