...
선택한 배포가 현재 활성화된 프로젝트가 아닌 다른 프로젝트에 속해 있는 경우(예: 프로젝트를 전환한 뒤에도 카드가 기존 선택을 유지하고 있는 경우)에는 **다른 프로젝트로 전환할까요?**라는 확인 대화상자가 먼저 표시됩니다. 확인하면 해당 배포의 프로젝트로 전환된 다음 상세 페이지가 열리고, 취소하면 현재 프로젝트가 그대로 유지됩니다.
