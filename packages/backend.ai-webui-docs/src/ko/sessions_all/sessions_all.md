...
새로운 연산 세션을 시작하려면 `세션 시작` 버튼을 클릭하세요.
...
첫 번째 페이지에서는 세션 형태로 Interactive 세션 또는 Batch 세션을 선택합니다. 세션 이름은 선택적으로 지정할 수 있습니다.
...
`슈퍼 관리자` 계정으로 세션을 생성하는 경우, 세션 소유자를 추가로 지정할 수 있습니다. 토글을 활성화하면 사용자 이메일 필드가 나타납니다.
...
호스트 메모리 풀을 함께 사용합니다. 이러한 가속기를 선택하면 가속기 메모리를
별도로 설정할 수 없으며, 가속기가 호스트 메모리 풀에서 자동으로 메모리를 사용합니다.
:::
...
:::
  ...
  * 클러스터 모드: 하나의 연산 세션을 에이전트에 어떻게 배치할지 선택합니다.
    **단일 노드**를 선택하면 메인 노드와 모든 워커 노드가 하나의 에이전트(물리 노드 또는
    가상 머신)에 배치되고, **다중 노드**를 선택하면 하나의 메인 노드와 하나 이상의 워커
    노드가 여러 에이전트에 나누어 배치됩니다. 각 옵션 옆의 (`?`) 아이콘에 마우스를 올리면
    간단한 설명을 확인할 수 있습니다. 모드를 선택한 후에는 슬라이더 또는 숫자 입력으로
    **클러스터 크기**를 지정합니다. 단일 노드 모드에서 크기는 하나의 에이전트에 생성되는
    **컨테이너** 수를 의미하고, 다중 노드 모드에서는 여러 에이전트에 분산되는 **노드** 수를
    의미합니다. 슬라이더의 *남은 자원* 표시는 현재 할당된 자원으로 즉시 시작할 수 있는 최대
    크기를 나타냅니다. 더 큰 크기를 선택할 수도 있지만, 자원이 확보될 때까지 세션이 대기
    상태로 유지될 수 있습니다. 크기가 1인 다중 노드 세션은 단일 노드 세션으로 생성됩니다.
...
- 고성능 컴퓨팅(HPC) 최적화: HPC 최적화 관련 값을 설정할 수 있습니다.
...
`폴더를 선택해주세요` 드롭다운에서 연산 세션에 마운트할 스토리지 폴더를 지정합니다. 목록의 각 항목에는 폴더 이름과 함께 소유자(사용자 또는 프로젝트), 사용 모드, 호스트, 생성일, 해당 폴더에 대한 권한(`R`, `W`, `D`)이 표시됩니다. 드롭다운에 검색어를 입력하면 폴더 이름으로 목록을 필터링할 수 있습니다. 자동으로 마운트되는 폴더와 현재 프로젝트에서 마운트할 수 없는 폴더는 목록에 표시되지 않습니다.

드롭다운 옆에는 두 개의 버튼이 있습니다.

- `+`(`새 폴더 추가`): 폴더 생성 다이얼로그를 엽니다. 생성한 폴더는 목록에 추가되고 마운트할 폴더로 선택됩니다. 자세한 내용은 [스토리지 폴더 생성](#create-storage-folder) 섹션을 참고하세요.
- `새로고침`: 폴더 목록을 다시 불러옵니다. 다른 화면에서 폴더를 생성하거나 공유받은 경우에 사용합니다.
...
`세션 런처` 페이지는 세션을 생성하기 위한 다양한 옵션을 제공합니다. `최근 기록` 기능은 이전에 생성한 세션의 정보를 기억합니다.
...
:::note
슈퍼 관리자의 경우 현재 클러스터에서 실행 중인 (또는 종료된) 모든 세션 정보를 확인할 수 있고, 사용자의 경우에는 자신이 사용한 세션만 조회 가능합니다.
:::
...
세션 목록은 세션 타입에 따라 **전체**, **Interactive**, **Batch**, **Inference**,
**업로드 세션** 탭으로 구분되며 각 탭에는 **실행 중** / **종료됨** 전환 버튼과
**세션 이름**, **자원 그룹**, **실행 노드** 속성 필터가 함께 제공됩니다.

<a id="session-resource-grid"></a>

### 자원 그리드 보기

사용자 설정에서 실험적 기능인 **세션 리소스 그리드 뷰**를 활성화하면 목록 위 새로고침
버튼 옆에 **보기 모드** 전환 버튼이 나타나며, 목록을 **테이블** 또는 **그리드** 형태로
전환할 수 있습니다. 기능을 활성화하는 방법은 [실험적 기능](#experimental-features)
섹션을 참고하세요. 그리드 보기는 테이블과 동일한 타입 탭, **실행 중** / **종료됨**
전환 버튼, 속성 필터를 사용하므로 목록을 먼저 좁힌 다음 보기 모드를 전환할 수 있습니다.

![](../images/session_resource_grid.png)

그리드에서 각 세션은 작은 셀로 이루어진 판 형태로 표시되며, 셀의 색상은 해당 세션의
실시간 자원 사용률을 나타냅니다. 사용률이 50% 미만이면 기본 색상, 50% 이상 80% 미만
구간에서는 점점 진해지는 경고 색상, 80% 이상이면 빨간색으로 표시됩니다. 아직 실행되지
않은 세션처럼 실시간 데이터가 없는 세션은 **데이터 없음** 색상으로 표시되며, 자원이
아직 요청 상태인 세션은 점선 테두리로 표시됩니다. 각 색상의 의미는 그리드 위의 범례에서
확인할 수 있습니다.

세션 판 위에 마우스를 올리면 해당 세션의 상태, 타입, 클러스터 모드, 자원 그룹, 실행 환경,
시작 시간과 경과 시간, 할당된 각 자원의 사용률, 디스크 I/O 요약 정보를 확인할 수 있습니다.
판을 클릭하면 세션 상세 정보 패널이 열립니다.

그리드 위의 컨트롤은 셀 하나가 무엇을 의미하는지를 결정합니다.

- **그리드 모드**: **자원**을 선택하면 선택한 자원 한 단위마다 셀 하나를, **커널**을
  선택하면 세션의 커널 하나마다 셀 하나를 표시합니다.
- **자원** (자원 모드 전용): 시각화할 자원을 선택합니다. **CPU**, **메모리** 또는
  목록에 표시된 세션이 사용하는 가속기를 선택할 수 있습니다. 셀 하나는 CPU 코어 한 개,
  메모리 한 단위, 가속기 한 개를 의미하며 가속기를 분할해 사용하는 경우에는 일부만 채워진
  셀로 표시됩니다. 디바이스 메모리를 보고하는 가속기는 사용률(`util`) 항목과 디바이스
  메모리(`mem`) 항목이 각각 제공됩니다.
- **메모리 단위** (자원 모드에서 메모리 자원을 선택한 경우): 셀 하나가 나타내는 메모리
  크기를 1, 2, 4, 8 GiB 중에서 선택합니다.
- **메트릭** (커널 모드 전용): 커널 셀의 색상을 결정하는 실시간 통계 항목을 선택합니다.
  목록에 표시된 세션 중 실시간 메트릭을 보고하는 세션이 없으면 선택란이 비활성화되고
  **실시간 메트릭 없음**으로 표시됩니다.
- **배치**: **지그재그**는 한 줄씩 방향을 번갈아 배치하여 여러 줄에 걸친 세션의 셀이
  이어져 보이도록 하고, **줄바꿈**은 모든 줄을 왼쪽 끝에서 시작합니다.

그리드 위에는 표시되지 않은 세션에 대한 안내가 함께 표시됩니다.

- 현재 필터에 해당하는 세션이 그리드에 표시할 수 있는 수보다 많으면 처음 100개의 세션만
  표시되고 전체 개수를 알리는 안내가 표시됩니다.
- 자원 모드에서 선택한 자원을 할당받지 않은 세션은 표시되지 않으며, 표시하지 않은 세션
  개수를 알리는 안내가 표시됩니다.
...
세션 스케줄링 기록 모달은 연산 세션의 내부 스케줄링 과정을 투명하게 보여줍니다. 세션이 거쳐간 모든 스케줄링 단계의 상세 기록을 표시하며, 상태 전환, 결과, 재시도 횟수, 발생한 오류 등의 정보를 포함합니다. 이 기능은 세션이 지연되거나, `PENDING` 상태에 머물러 있거나, 시작에 실패한 원인을 파악하는 데 특히 유용합니다.

![](../images/session_scheduling_history_modal.png)

#### 필터 바

모달 상단의 필터 바를 사용하여 스케줄링 기록을 좁혀볼 수 있습니다. 다음 속성으로 필터링할 수 있습니다:

- **ID**: 기록 ID (UUID, 정확히 일치)로 필터링
- **단계**: 스케줄링 단계 이름으로 필터링
- **결과**: 결과 유형 (SUCCESS, FAILURE, STALE, NEED_RETRY, EXPIRED, GIVE_UP, SKIPPED)으로 필터링
- **이전 상태**: 스케줄링 단계 이전의 세션 상태로 필터링
- **이후 상태**: 스케줄링 단계 이후의 세션 상태로 필터링
- **오류 코드**: 오류 코드로 필터링
- **메시지**: 메시지 내용으로 필터링
- **생성 시간**: 기록 생성 시각으로 필터링
- **수정 시간**: 기록 수정 시각으로 필터링

필터 바 옆에 있는 새로고침 버튼을 사용하여 스케줄링 기록 데이터를 다시 불러올 수 있습니다.

#### 기록 테이블

스케줄링 기록 테이블에는 다음 컬럼이 표시됩니다:

- **수정일**: 이 기록이 마지막으로 업데이트된 시각
- **생성일**: 이 기록이 생성된 시각
- **단계**: 스케줄링 단계의 이름
- **결과**: 해당 스케줄링 단계의 결과로, 색상으로 구분된 배지로 표시됩니다
- **이전 상태**: 해당 스케줄링 단계 이전의 세션 상태
- **이후 상태**: 해당 스케줄링 단계 이후의 세션 상태
- **시도 횟수**: 해당 스케줄링 단계가 시도된 횟수
- **메시지**: 해당 스케줄링 단계에 대한 요약 메시지

**생성일** 또는 **수정일** 컬럼을 기준으로 테이블을 정렬하여 원하는 순서로 기록을 확인할 수 있습니다.

#### 기록 테이블 페이지네이션

스케줄링 기록 테이블은 오프셋 기반 페이지네이션을 사용합니다. 기본 페이지당 표시 건수는 10건입니다. 필터나 정렬 순서를 변경하면 자동으로 첫 번째 페이지로 이동합니다.

#### 세부 단계 확장

일부 기록에는 세부 단계가 포함되어 있습니다. 세부 단계가 있는 경우, 행 왼쪽에 확장 화살표가 표시됩니다. 이를 클릭하면 해당 스케줄링 단계의 개별 세부 단계를 보여주는 중첩 테이블이 나타납니다.

![](../images/session_scheduling_history_substeps.png)

세부 단계 테이블에는 다음 컬럼이 포함됩니다:

- **세부단계**: 세부 단계의 이름
- **결과**: 세부 단계의 결과로, 색상으로 구분된 배지로 표시됩니다
- **오류 코드**: 단계 실패 시의 특정 오류 코드
- **메시지**: 상세 정보 또는 오류 설명
- **시작일**: 세부 단계가 시작된 시각
- **종료일**: 세부 단계가 완료된 시각
...
각 스케줄링 단계와 세부 단계에는 결과를 나타내는 색상 구분 배지가 표시됩니다:

| 결과 | 색상 | 의미 |
|--------|-------|---------|
| SUCCESS | 초록색 | 단계가 성공적으로 완료됨 |
| FAILURE | 빨간색 | 단계가 실패함 |
| STALE | 회색 | 기록이 오래되었거나 최신이 아님 |
| NEED_RETRY | 노란색 | 단계를 재시도해야 함 |
| EXPIRED | 빨간색 | 스케줄링 시도가 만료됨 |
| GIVE_UP | 빨간색 | 시스템이 스케줄링을 포기함 |
| SKIPPED | 회색 | 단계가 건너뛰어짐 |
...
테이블 위의 필터 바를 사용하여 로그 항목을 좁혀볼 수 있습니다:

- **상태**: 작업 결과로 필터링 (`SUCCESS`, `ERROR`, `RUNNING`, `UNKNOWN`)
- **작업**: 작업 이름 또는 키워드로 검색
- **수행자**: 수행자 ID로 검색
- **시각**: 날짜 범위 선택기로 필터링

**새로고침** 버튼을 클릭하면 로그를 다시 불러옵니다. 감사 로그 쿼리는 지연 실행되며, **감사 로그** 탭을 처음 활성화할 때만 전송됩니다.
...
세션 상세 정보 패널에서 `컨테이너 로그 보기` 아이콘을 클릭하면 연산 세션의 로그를 조회할 수 있습니다.

![](../images/session_log.png)

로그 창의 로그 영역 위에는 다음 컨트롤이 제공됩니다.

- **커널 선택기**: 로그를 확인할 세션의 커널을 선택합니다. 단일 컨테이너 세션에는 항목이
  하나뿐이며 클러스터 세션에서는 각 컨테이너가 호스트명과 ID 앞부분과 함께 표시됩니다.
- **다운로드**: 현재 표시된 로그를 텍스트 파일로 저장합니다.
...
실행 중인 세션에 적용되는 기준은 세션 상세 정보 패널의 **자원 수거 상태** 항목에서 각 기준이
...
:::note
환경 설정 값에 따라 적용되는 유휴 상태 검사 기준과 사용량 기반 자원 수거에 표시되는 자원 항목이 다르게 보일 수 있습니다.
:::
...
세션 목록에는 각 세션이 사용량 기반 자원 수거 대상에 얼마나 가까운지를 한눈에 보여주는
**자원 수거 상태** 컬럼이 있습니다. 세션 상세 정보 패널의 **자원 수거 상태** 항목에서도
사용량 기반 자원 수거 항목의 이름 대신 동일한 배지가 표시됩니다.

배지는 다음 세 단계로 표시되며 팝오버 범례에도 같은 설명이 나옵니다.

- **안전**: 현재 평균 사용률이 기준값보다 충분히 높습니다.
- **경고**: 현재 평균 사용률이 기준값보다 높지만 기준값에 근접해 있습니다.
- **위험**: 현재 평균 사용률이 기준값보다 낮습니다.

배지는 평균 사용률이 실제로 기준값 아래로 떨어지기 전에 미리 단계를 올리므로 현재 평균
사용률이 아직 기준값보다 높은 상태에서도 **위험**으로 표시될 수 있습니다. 기준값을 넘었다는
사실을 알리는 표시가 아니라 대응할 시간을 두고 미리 알려 주는 경고로 보면 됩니다.

사용량 기반 자원 수거가 적용되지 않는 세션이나 아직 측정값이 없는 세션은 컬럼에 `-`가
표시됩니다. 세션 상세 정보 패널에서는 최초 회수 판단이 아직 이루어지지 않은 항목에 남은 시간
태그 대신 **회수 여부 확인 중**이 표시됩니다.

배지 옆의 정보(i) 아이콘에 마우스를 올리면 단계 판단 근거를 보여주는 팝오버가 열립니다.

- 나열된 조건 중 **하나라도** 위험 단계로 유지될 때 자원이 수거되는지, 아니면 **모두**
  위험 단계일 때만 수거되는지를 알려주는 조건 문장. 어떤 문장이 표시되는지는 관리자가
  검사기를 어떻게 설정했는지에 따라 달라집니다.
- 측정된 자원별 행. 각 행에는 해당 자원의 단계 배지와 함께 현재 평균 사용률 및 설정된
  기준값이 `(현 12% | 기준 5%)` 형태로 표시됩니다.
- 세 단계 각각의 판단 기준을 설명하는 범례.

측정값이 없는 자원은 대시로 표시되며 전체 단계 판단에 영향을 주지 않습니다.

사용량 기반 자원 수거 항목의 남은 시간이 얼마 남지 않으면 그 옆의 남은 시간 태그가 배지와
동일한 색상으로 바뀝니다. 남은 시간 태그가 빨간색으로 표시된 세션이 곧 자원 수거 대상입니다.
반면 시간이 넉넉히 남아 있는 동안에는 배지가 이미 위험 단계를 나타내고 있어도 태그에 색상이
나타나지 않습니다.
...
Backend.AI는 "세션을 이미지로 변환" 기능을 지원합니다. `RUNNING` 상태의 세션을 커밋하면 세션의 현재 상태가 새로운 이미지로 저장됩니다. 세션 상세 정보 패널에서 `커밋` 버튼(네 번째 아이콘)을 클릭하면 세션 정보가 표시된 대화 상자가 열립니다. 세션 이름을 입력한 후 세션을 새 이미지로 변환할 수 있습니다. 세션 이름은 4자에서 32자 사이여야 하며, 영숫자, 하이픈(`-`) 또는 밑줄(`_`)만 포함할 수 있습니다.
