...
탭으로 이동했다가 돌아오면 초기화되지 않고 이전에 보던 상태가 그대로 복원됩니다.

<a id="auto-refresh-interval"></a>

### 자동 새로고침 주기

목록 툴바 오른쪽의 새로고침 버튼을 클릭하면 세션 목록을 즉시 다시 불러오며, 공통 자동
새로고침 주기 선택기가 함께 제공됩니다([자동 새로고침](#common-auto-refresh) 참고).
연산 세션 화면에서는 자동 새로고침이 기본적으로 켜져 있습니다. **세션 목록**은 15초
주기로, **세션 상세 정보 패널**과 **컨테이너 로그** 창은 10초 주기로 시작하며, 선택한
주기는 각 화면별로 따로 저장됩니다.
...
확장 컬럼 헤더의 케밥 메뉴(⋮)를 클릭하면 **펼치기 옵션** 메뉴가 열리며, 모든 행 펼치기, 모든 행 접기, 결과가 SUCCESS가 아닌 행만 펼치기 중에서 선택할 수 있습니다. 기본값은 **오류만 펼치기**로, 이 모드에서는 펼쳐진 행에서 SUCCESS 세부 단계도 숨겨지므로 세션의 스케줄링 과정에서 문제가 발생한 원인이 바로 드러납니다.
...
시작합니다. 새로고침될 때마다 로그 화면은 가장 최근 줄로 스크롤되고 이전 새로고침 이후에
추가된 줄이 강조 표시됩니다.
