...
배지는 평균 사용률이 실제로 기준값 아래로 떨어지기 전에 미리 단계를 올립니다. 따라서 현재
평균 사용률이 아직 기준값보다 높은 상태에서도 **위험**으로 표시될 수 있습니다. 기준값을
넘었다는 사실을 알리는 표시가 아니라, 대응할 시간을 두고 미리 알려 주는 경고로 보면 됩니다.
...
동일한 색상으로 바뀝니다. 따라서 남은 시간 태그가 빨간색으로 표시된 세션이 곧 자원 수거
대상이 되는 세션입니다. 반면 시간이 넉넉히 남아 있는 동안에는 배지가 이미 위험 단계를
나타내고 있어도 태그에 색상이 나타나지 않습니다. 즉 태그에 색상이 나타나면 자원 수거가
임박했다는 뜻입니다.
