...
각 탭은 페이지에 머무르는 동안 자체적인 필터, 정렬 순서, 페이지 번호를 유지합니다. 다른
탭으로 이동했다가 돌아오면 초기화되지 않고 이전에 보던 상태가 그대로 복원됩니다. 현재
보고 있는 탭의 보기 상태는 페이지 URL에도 저장되므로 브라우저를 새로고침해도 해당 상태가
재현되며 URL을 북마크하거나 공유해 같은 상태의 목록을 열 수 있습니다. 단, URL에 담기는 것은
현재 보고 있는 탭뿐이므로 새로고침한 뒤 다른 탭은 기본값에서 시작합니다.
...
선택한 주기는 각 화면별로 따로 저장되며 새로고침 후에도 유지됩니다. 예를 들어 컨테이너 로그
