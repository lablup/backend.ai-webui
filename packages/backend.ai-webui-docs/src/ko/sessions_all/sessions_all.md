...
각 탭은 자체적인 필터, 정렬 순서, 페이지 번호를 유지하며, 이 보기 상태는 페이지 URL에
저장됩니다. 다른 탭으로 이동했다가 돌아오면 초기화되지 않고 이전에 보던 상태가 그대로
복원되며, 브라우저를 새로고침해도 동일한 상태가 재현됩니다. URL을 북마크하거나 공유하면
동일한 상태의 목록을 그대로 열 수 있습니다.
