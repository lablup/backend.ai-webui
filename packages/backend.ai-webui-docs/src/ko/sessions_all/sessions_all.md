...
세션 목록의 구성, 필터링, 새로고침 방법은
[세션 목록 보기 및 새로고침](#session-list-view-and-refresh) 섹션을 참고하세요.
...
각 탭은 페이지에 머무르는 동안 자체적인 필터, 정렬 순서, 페이지 번호를 유지합니다. 따라서
다른 탭으로 이동했다가 돌아오면 초기화되지 않고 이전에 보던 상태가 그대로 복원됩니다. 현재
보고 있는 탭의 보기 상태는 페이지 URL에도 저장되므로 브라우저를 새로고침해도 해당 상태가
재현되며 URL을 북마크하거나 공유해 같은 상태의 목록을 열 수 있습니다. 단, URL에 담기는 것은
현재 탭의 상태뿐이며 새로고침 후 다른 탭은 기본값에서 시작합니다.

<a id="auto-refresh-interval"></a>

### 자동 새로고침 주기

목록 툴바 오른쪽의 새로고침 버튼을 클릭하면 세션 목록을 즉시 다시 불러옵니다. 이 버튼에는
자동 새로고침 주기 선택기가 함께 제공됩니다. 버튼 옆의 화살표를 클릭하면 **끄기** 또는
5초에서 60초 사이의 사전 정의된 주기를 선택할 수 있습니다. 자동 새로고침이 켜져 있는 동안에는
다음 새로고침까지 남은 시간을 나타내는 카운트다운 테두리가 버튼에 표시되며 선택한 주기가
선택기에 레이블(예: `15초`)로 나타납니다.

연산 세션 화면에서는 자동 새로고침이 기본적으로 켜져 있습니다.

- **세션 목록**은 15초 주기로 시작합니다.
- **세션 상세 정보 패널**과 **컨테이너 로그** 창은 10초 주기로 시작합니다.
...
패널 헤더의 새로고침 버튼을 클릭하면 세션 정보를 다시 불러오며 목록 툴바와 마찬가지로 자동
새로고침 주기 선택기를 제공합니다. 주기 설정 방법은
[자동 새로고침 주기](#auto-refresh-interval) 섹션을 참고하세요.
...
:::note
아래에서 설명하는 **오류만 펼치기** 모드는 전체 기록이 아닌 현재 페이지에 표시된 기록에만 적용됩니다.
:::
...
확장 컬럼 헤더의 케밥 메뉴(⋮)를 클릭하면 **펼치기 옵션** 메뉴가 열리며, 테이블 전체의 행 펼치기 방식을 제어할 수 있습니다. 다음 세 가지 보기 모드를 사용할 수 있습니다:

- **모두 열기**: 모든 행을 펼쳐 세부 단계를 즉시 확인할 수 있습니다.
- **모두 닫기**: 모든 행을 접어 최상위 기록만 표시합니다.
- **오류만 펼치기** (기본값): 결과가 SUCCESS가 아닌 행을 자동으로 펼치고, 펼쳐진 행에서 SUCCESS 세부 단계를 숨깁니다. 세션의 스케줄링 과정에서 문제가 발생한 원인을 빠르게 파악하는 데 가장 유용한 모드입니다.

선택된 모드는 사용자별로 저장되며 모달을 닫았다가 다시 열어도 유지됩니다. 활성 모드와 관계없이 언제든지 개별 행을 수동으로 펼치거나 접을 수 있습니다.
...
- **새로고침**: 로그를 다시 불러옵니다. 이 버튼에는
  [자동 새로고침 주기](#auto-refresh-interval) 섹션에서 설명한 자동 새로고침 주기 선택기가
  함께 제공됩니다.

로그 창은 새로운 로그가 쌓이는 것을 지켜보기 위해 여는 화면이므로 자동 새로고침이 10초 주기로
시작합니다. 새로고침 버튼 옆의 선택기에서 다른 주기 또는 **끄기**를 선택할 수 있으며 선택한
값은 다음에 로그 창을 열 때도 그대로 유지됩니다. 새로고침될 때마다 로그 화면은 가장 최근
줄로 스크롤되고 이전 새로고침 이후에 추가된 줄이 강조 표시됩니다.
...
중 하나 이상이 남은 시간을 보고하면 나타나며, 남은 시간이 얼마나 긴지는 관계가 없습니다.
만료가 한참 남은 검사도 만료가 임박한 검사와 동일하게 표시됩니다.
...
단계는 기준값과의 단순 비교가 아니라 여유 폭을 두고 판정되므로, 현재 평균 사용률이 아직
기준값보다 높은 상태에서도 **위험**으로 표시될 수 있습니다. 배지는 평균 사용률이 기준값을
넘었는지를 그대로 알려주는 값이 아니라, 대응할 시간을 벌어 주는 상대적인 경고 단계로
이해하면 됩니다.
...
동일한 색상을 사용합니다. 따라서 남은 시간 태그가 빨간색으로 표시된 세션이 곧 자원 수거
대상이 되는 세션입니다. 반면 시간이 넉넉히 남아 있는 동안에는 배지가 이미 위험 단계를
나타내고 있어도 태그에는 색상이 적용되지 않습니다. 즉, 색상이 적용된 남은 시간 태그는 위험이
존재한다는 뜻을 넘어 임박했다는 신호입니다.
