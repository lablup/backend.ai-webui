...
사용자 Web-UI에서 간단한 방법으로 작업 스케줄러의 동작을 확인할 수 있습니다. GPU 호스트가 최대 2 fGPU를 할당할 수 있는 경우, 각각 1 fGPU 할당을 요청하는 3개의 연산 세션을 동시에 생성해 보겠습니다. 세션 시작 다이얼로그의 **사용자 설정 자원 할당** 섹션에서 GPU 값을 1로 설정하고, **세션 개수** 값을 3으로 설정합니다. **세션 개수**에 1보다 큰 값을 지정하고 **LAUNCH** 버튼을 클릭하면 해당 수만큼의 세션이 동시에 요청됩니다. (**세션 개수**는 단일 다중 노드 세션을 구성하는 노드 수를 제어하는 **클러스터 크기**와는 다른 개념입니다.) 이는 fGPU가 2 단위밖에 없는 상황에서 총 3 fGPU를 요청하는 3개의 세션을 생성하는 상황입니다.
...
세션 페이지로 이동하여 세션 시작 다이얼로그를 엽니다. 설치 환경에 따라 다양한 커널 이미지가 제공될 수 있습니다. 사용 가능한 프레임워크와 버전은 관리자가 등록한 항목에 따라 달라지므로, 사용자 환경의 목록은 아래 그림과 다를 수 있습니다.
...
여기서 특정 TensorFlow 버전을 선택하여 세션을 생성합니다.

![](../images/session_launch_dialog_tf23.png)

생성된 세션의 웹 터미널을 열고 다음 Python 명령을 실행합니다. 선택한 TensorFlow 버전이 설치되어 있는 것을 확인할 수 있습니다.

![](../images/tf23_version_print.png)

이번에는 다른 TensorFlow 버전을 선택하여 연산 세션을 생성합니다. 자원이 부족한 경우 이전 세션을 삭제합니다.

![](../images/session_launch_dialog_tf115.png)

생성된 세션의 웹 터미널을 열고 이전과 동일한 Python 명령을 실행합니다. 다른 TensorFlow 버전이 설치되어 있는 것을 확인할 수 있습니다.

![](../images/tf115_version_print.png)

마지막으로, PyTorch 이미지를 사용하여 연산 세션을 생성합니다.

![](../images/session_launch_dialog_pytorch17.png)

생성된 세션의 웹 터미널을 열고 다음 Python 명령을 실행합니다. 선택한 PyTorch 버전이 설치되어 있는 것을 확인할 수 있습니다.
...
실행 중인 연산 세션(컨테이너)을 새로운 Docker 이미지로 변환하여 추후 연산 세션 생성 시 재사용하려는 경우, Backend.AI는 셀프 서비스 방식의 "세션을 이미지로 변환" 기능을 제공합니다. 관리자에게 요청하지 않고도 WebUI에서 실행 중인 세션을 직접 커밋할 수 있으며, 생성된 이미지는 본인에게만 표시되는 비공개 사용자 정의 이미지가 됩니다.
...
Python 패키지를 설치할 때는 [자동 마운트 폴더](#using-automount-folder)를 사용하여 [pip를 통해 설치](#install_pip_pkg)하는 것을 권장합니다. 그러나 새로운 이미지에 Python 패키지를 포함시키려면, `sudo pip install <패키지명>` 명령을 사용하여 홈 디렉터리가 아닌 시스템 디렉터리에 설치해야 합니다. 홈 디렉터리(보통 `/home/work/`)의 내용은 연산 세션을 Docker 이미지로 변환할 때 저장되지 않습니다.
:::

- 연산 세션 준비가 완료되면, 세션 상세 패널을 열고 **세션을 사용자 정의된 이미지로 푸시**(**세션을 이미지로 커밋하기** 작업)를 선택합니다. 이미지 이름을 입력하고 확인합니다. 전체 단계별 절차는 [세션 커밋 저장](#save-session-commit) 섹션을 참고하세요.
- 커밋된 이미지는 비공개 사용자 정의 이미지로 생성됩니다. [내 환경](#my-environments) 페이지에서 해당 이미지를 확인하고 관리할 수 있으며, 다음에 세션을 시작할 때 환경 목록에서 선택할 수 있습니다.

:::note
"세션을 이미지로 변환" 기능은 버전 24.03부터 사용할 수 있으며, 세션이 `INTERACTIVE` 모드일 때만 사용할 수 있습니다. 이 기능은 배포 환경에서 활성화되어 있어야 합니다(`enableContainerCommit`). **세션을 사용자 정의된 이미지로 푸시** 작업이 보이지 않는 경우, 해당 설치 환경에서 컨테이너 커밋이 비활성화되어 있는 것입니다. 이 경우에는 관리자에게 세션을 새로운 이미지로 변환해 달라고 요청하세요. 세션 이름 또는 ID와 플랫폼에 등록된 이메일 주소를 알려주면, 관리자가 전체 이미지 이름과 태그를 안내해 줍니다. 그런 다음 세션 시작 다이얼로그에서 해당 이미지 이름을 직접 입력할 수 있습니다. 해당 이미지는 비공개이며 다른 사용자에게는 표시되지 않습니다.
