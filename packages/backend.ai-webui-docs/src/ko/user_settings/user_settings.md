...
열리는 대화 상자는 서버 버전에 따라 다릅니다. 이전 버전의 서버에서는 **내 키페어
정보** 대화 상자에 액세스 키와 비밀 키가 표시되는 읽기 전용 테이블과 **닫기**
버튼이 나타납니다. 기본 액세스 키에는 **기본 액세스 키** 태그가 표시됩니다.

![](../images/my_keypair_information.png)

키페어 관리를 지원하는 서버에서는 Config 버튼을 클릭하면 아래에서 설명하는 전체
**내 키페어 관리** 대화 상자가 대신 열립니다.

<a id="my-keypair-management"></a>

### 내 키페어 관리

키페어 관리를 지원하는 서버에서는 Config 버튼을 클릭하면 **내 키페어 관리**
대화 상자가 열립니다. 여기에서 여러 개의 키페어를 동시에 보유하고, 새 키페어를
발급하며, 어떤 키페어를 기본 액세스 키로 사용할지 선택하고, 키페어를 비활성화,
복구 또는 영구 삭제할 수 있습니다.
