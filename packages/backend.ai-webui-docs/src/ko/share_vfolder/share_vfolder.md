...
사용자 B의 계정으로 전환한 후 데이터 페이지로 이동합니다. 사이드바의 데이터 메뉴 항목 옆과 폴더 목록 상단의 폴더 상태 요약 옆에 초대 배지가 표시되며, 배지에는 아직 응답하지 않은 대기 중인 초대 개수가 표시됩니다.
...
:::note
여기에서 설명하는 권한은 개별 스토리지 폴더에 적용되며 폴더 소유자가 관리합니다.
스토리지 호스트 전체에 적용되는 폴더 권한은 관리자 전용 설정으로, 스토리지 호스트 상세
패널의 **프로젝트 폴더 권한** 및 **사용자 폴더 권한** 탭에서 구성합니다. 이때 대상
도메인이나 사용자는 이름을 입력하지 않고 선택기에서 고릅니다. 자세한 내용은
[사용자 폴더 권한](#user-folder-permission) 섹션을 참고하세요.
:::
...
초대자가 폴더 공유를 중단하려면, 폴더 목록의 제어 열에서 공유 버튼을 클릭하여 폴더 공유 모달을 엽니다. **공유된 사용자** 테이블에서 접근을 해제할 사용자 행의 권한 드롭다운 옆에 있는 공유 중지 아이콘(빨간색 닫기 원)을 클릭합니다. 확인 대화상자가 표시되면 `확인` 버튼을 클릭하여 해당 사용자의 접근을 해제합니다.
