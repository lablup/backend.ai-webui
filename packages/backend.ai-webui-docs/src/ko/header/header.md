...
메뉴 아이콘을 누르면 사이드바와 동일한 메뉴가 담긴 내비게이션 드로어가 화면 옆에서 슬라이드되어 나타나며, 드로어 상단에는 Backend.AI 로고가 있는 브랜드 색상 헤더가 표시됩니다. 드로어는 헤더의 닫기 버튼을 클릭하거나 드로어 바깥 영역을 클릭하여 닫을 수 있으며, 메뉴 항목을 선택해 페이지를 이동하면 자동으로 닫힙니다.
