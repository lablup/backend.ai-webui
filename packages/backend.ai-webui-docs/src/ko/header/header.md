...
프로젝트 선택기에는 사용자가 접근할 수 있는 프로젝트가 표시됩니다. 존재하지 않거나 접근 권한이 없는
...
돋보기 모양 버튼을 클릭하면 검색 팔레트가 열리며, 키보드만으로 원하는 페이지, 페이지 탭, 설정 항목으로 바로 이동할 수 있습니다.

:::note
검색 팔레트는 실험적 기능이며 기본적으로 비활성화되어 있습니다. 사용자 설정 페이지의 **실험적 기능**에서 **커맨드 팔레트** 항목을 활성화한 경우에만 상단 바에 **검색** 버튼이 표시되고 단축키가 동작합니다.
:::

버튼을 클릭하거나 `Ctrl-K`(macOS에서는 `Cmd-K`)를 누르면 팔레트가 열립니다. 입력 필드에 커서가 있는 상태에서도 단축키를 사용할 수 있습니다.

![](../images/global_search_palette.png)

`페이지, 탭, 설정 검색`이라는 안내 문구가 표시된 검색 입력창에 검색어를 입력하면 페이지, 페이지 탭, 설정 항목을 한 번에 찾을 수 있습니다. 항목의 이름이 아니라 설명이 검색어와 일치한 경우에는 검색어가 발견된 위치를 알려주는 `...에서 찾음` 문구가 함께 표시되며, 관리자 메뉴에 속한 결과에는 `관리자` 또는 `프로젝트 관리자` 태그가 표시됩니다. 입력한 검색어와 일치하는 항목이 없으면 `검색 결과 없음`이 표시됩니다.

팔레트에서는 다음 세 가지 그룹의 빠른 작업도 제공합니다.

- `만들기`: 새 세션 시작, 스토리지 폴더 생성, 배포 생성
- `화면`: 다크 모드 또는 라이트 모드로 전환, 시스템 설정 사용, UI 언어 변경
- `패널 및 도움말`: 사용자 설정 열기, 알림 열기, 사이드바 접기/펼치기, 현재 페이지 매뉴얼 열기

검색어를 입력하기 전에는 최근에 연 결과가 `최근 항목` 아래에 먼저 표시됩니다.
...
- `설정`: 현재 페이지 위에 사용자 설정 대화 상자를 엽니다. **로그** 범주에서 클라이언트 측에 기록된 로그 및 오류 내역을 확인할 수 있습니다.
