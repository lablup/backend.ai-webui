...
스토리지 폴더에는 `사용자`와 `프로젝트` 두 가지 유형이 있으며, '종류' 열에서 구분합니다.
...
- **종류**: 생성할 폴더 유형을 결정합니다. 사용자 또는 프로젝트로 설정합니다. 사용자 폴더는 사용자가 단독으로 생성해 사용하는 폴더이며, 프로젝트 폴더는 관리자가 생성하고 프로젝트의 사용자가 공유하는 폴더입니다.
...
왼쪽 패널에서 폴더 내의 모든 디렉토리와 파일을 확인합니다. 이름 열에 있는 디렉토리 이름을 클릭하면 해당 디렉토리로 이동합니다. 작업 열의 다운로드 버튼이나 삭제 버튼을 클릭하면 파일을 다운로드하거나 디렉토리에서 완전히 삭제합니다. 파일이나 디렉토리의 이름도 변경할 수 있습니다. 보다 세밀한 파일 작업이 필요하다면 이 폴더를 연산 세션 생성 시 마운트한 뒤 터미널이나 Jupyter Notebook 등과 같은 서비스로 수행하세요.
...
회색으로 변하며, 툴팁에 업로드가 허용되지 않는다는 안내가 표시됩니다.
...
폴더 탐색기에서 텍스트 파일을 직접 편집합니다. 폴더 이름을 클릭하여 파일 탐색기를 연 다음, 텍스트 파일의 제어 열에서 **파일 편집** 버튼을 클릭합니다.
...
편집기는 UI 환경 설정과 일치하는 라이트 및 다크 테마를 모두 지원합니다. 파일 내용을 편집한 후 **저장**을 클릭하면 수정된 파일이 업로드되고, **취소**를 클릭하면 변경 사항이 취소됩니다.

:::note
파일 편집 버튼은 해당 스토리지 폴더에 대한 사용자 접근 권한에 `write_content` 권한이 포함되어 있을 때에만 사용할 수 있습니다(폴더 공유 권한 또는 폴더에 부여된 역할로 부여됨). 제어판의 스토리지 호스트 수준 설정은 이 동작에 영향을 주지 않습니다. 파일 로드에 실패하면 오류 메시지가 표시됩니다.
:::
...
스토리지 폴더를 삭제할 권한이 있으면, 제어 열의 `휴지통` 아이콘을 클릭하여 폴더를 `휴지통` 탭으로 이동시킵니다. 휴지통 탭으로 이동된 폴더는 삭제 대기(delete-pending) 상태로 표시됩니다.
...
이 상태에서 제어 열의 복원 버튼을 클릭하면 폴더가 복원됩니다. 폴더를 영구적으로 삭제하려면 같은 열의 `휴지통` 버튼을 클릭합니다.
...
Backend.AI는 [FileBrowser](https://filebrowser.org)를 지원합니다. FileBrowser는 웹 브라우저에서 원격 서버의 파일을 관리하도록 도와주는 프로그램입니다. 특히 사용자의 로컬 머신에서 디렉토리를 업로드할 때 유용합니다.
