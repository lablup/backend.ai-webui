...
- **프로젝트**: 프로젝트 타입을 선택한 경우에만 표시됩니다. 새 프로젝트 폴더를 만들 때 폴더가 속한 프로젝트를 지정할 수 있습니다. 프로젝트 폴더는 반드시 프로젝트에 속해야 하며, 프로젝트 폴더의 경우 상단 바에서 현재 선택된 프로젝트가 대상 프로젝트로 자동 선택됩니다. 다만 사용자 폴더를 생성할 때에는 별도의 역할을 수행하지 않습니다.
...
다음 필터를 사용하여 감사 로그를 필터링할 수 있습니다.

- **상태**: 작업 결과로 필터링합니다(예: SUCCESS 또는 ERROR).
- **작업**: 작업 유형으로 필터링합니다(예: create 또는 delete).
- **실행자**: 작업을 수행한 사용자 ID로 검색합니다. 항목은 "이메일 (id)" 형식으로 표시됩니다.
- **시간**: 특정 날짜 및 시간 범위로 결과를 좁힙니다.

자동 새로 고침 버튼을 사용하면 전체 페이지를 새로 고치지 않고도 로그를 최신 상태로 유지할 수 있습니다. 로그는 지연 로딩 방식을 사용하므로, 탭을 처음 열 때만 서버에 쿼리를 전송합니다.
