...
Backend.AI는 사용자의 파일을 안전하게 보관할 수 있도록 전용 저장소를 제공합니다. 연산 세션이 종료되면 세션 내에서 생성된 모든 파일과 디렉터리가 삭제되기 때문에, 중요한 데이터는 반드시 스토리지 폴더에 저장하는 것이 좋습니다. 스토리지 폴더 목록은 사이드바의 데이터 페이지에서 확인할 수 있습니다. 이 목록에서는 폴더 이름과 ID, 폴더가 위치한 스토리지 호스트(위치), 폴더의 접근 권한(권한) 등 주요 정보를 확인할 수 있습니다.


![](../images/data_page.png)
<!-- TODO: Re-capture data_page.png — 폴더 목록 위의 새로고침 버튼이 간격 선택 셰브론이 붙은 분할 컨트롤로 변경됨 -->
...
폴더 목록 위의 새로고침 버튼을 클릭하면 페이지 전체를 다시 불러오지 않고 폴더 목록만
다시 불러옵니다. 그 옆의 셰브론을 클릭하면 **자동 새로고침** 메뉴가 열리며, 목록이
스스로 갱신되는 주기를 `끄기`, `5초`, `10초`, `15초`, `30초`, `1분` 중에서 선택할 수
있습니다. 현재 적용 중인 주기에는 체크 표시가 붙습니다.

![](../images/vfolder_auto_refresh_menu.png)
<!-- TODO: Capture screenshot of vfolder_auto_refresh_menu.png — 데이터 페이지의 새로고침 버튼에서 자동 새로고침 메뉴를 연 상태. 끄기 / 5초 / 10초 / 15초 / 30초 / 1분 항목과 현재 선택된 항목의 체크 표시가 보여야 함 -->

폴더 목록은 폴더 내용이 스스로 바뀌는 경우가 드물기 때문에 자동 새로고침이 `끄기`
상태로 시작합니다. 주기를 한 번 선택하면 그 선택은 해당 목록에 대해 기억되어 다음에
페이지를 열 때 다시 적용됩니다. 다시 `끄기`를 선택한 경우에도 마찬가지입니다.

자동 새로고침이 켜져 있는 동안에는 선택한 주기가 셰브론 옆에 표시되고, 다음 갱신
시점까지 버튼 주위에 카운트다운 테두리가 채워집니다. 수동으로 새로고침하면 카운트다운이
다시 시작됩니다. 페이지가 보이지 않는 동안(예: 다른 브라우저 탭으로 전환한 경우)에는
자동 새로고침이 일시 중지되었다가, 다시 돌아오면 재개됩니다.
...
- **종류**: 생성할 폴더 유형을 결정합니다. 사용자 또는 프로젝트로 설정할 수 있습니다. 사용자 폴더는 사용자가 단독으로 생성하여 사용할 수 있는 폴더이며, 프로젝트 폴더는 관리자가 생성하고 프로젝트의 사용자가 공유하는 폴더입니다.
- **프로젝트**: 종류를 프로젝트로 선택한 경우에만 표시됩니다. 폴더는 상단 바에서 현재 선택된 프로젝트에 속하게 되며, 별도의 프로젝트 선택기는 없습니다. 어떤 프로젝트가 폴더를 소유하게 되는지는 확인 메시지로 안내됩니다. 사용자 폴더를 생성할 때는 이 필드가 적용되지 않습니다.
- **권한**: 폴더가 연산 세션에 마운트될 때 적용되는 마운트 권한입니다. **읽기 및 쓰기**는 세션 내에서 폴더에 쓰기를 허용하고, **읽기 전용**은 쓰기를 막습니다. 이 설정은 사용자 폴더와 프로젝트 폴더 모두에 적용되며, 프로젝트 폴더의 경우 모든 프로젝트 멤버의 접근 권한을 결정합니다.
- **복사 가능 여부**: 사용 방식이 "모델"로 설정된 경우에만 표시됩니다. 생성하는 가상 폴더(vfolder)가 복사 가능한지 여부를 선택합니다.

여기서 생성한 폴더는 연산 세션 생성 시 [마운트](#session-mounts)할 수 있습니다. 폴더는 사용자의 기본 작업 디렉토리인 `/home/work/` 아래에 마운트되며, 마운트된 디렉토리에 저장된 파일은 연산 세션이 종료되어도 삭제되지 않습니다.
...
넓은 화면(xl 이상)에서는 드래그 가능한 구분선으로 두 패널을 분리하여 워크플로우에 맞게 크기를 조절할 수 있습니다. 좁은 화면에서는 패널이 세로로 쌓입니다.
...
왼쪽 패널에서 폴더 내의 모든 디렉토리와 파일을 확인할 수 있습니다. 이름 열에 있는 디렉토리 이름을 클릭하면 해당 디렉토리로 이동할 수 있습니다. 작업 열의 다운로드 버튼이나 삭제 버튼을 클릭하여 파일을 다운로드하거나 디렉토리에서 완전히 삭제할 수 있습니다. 파일이나 디렉토리의 이름을 변경하는 것도 가능합니다. 보다 세밀한 파일 작업이 필요하다면, 이 폴더를 연산 세션 생성 시 마운트한 뒤 터미널이나 Jupyter Notebook 등과 같은 서비스를 활용하여 수행할 수 있습니다.

**폴더 생성** 버튼으로 현재 경로에 새로운 폴더를 생성할 수 있으며, **업로드** 버튼으로 현재 경로에 로컬 파일 혹은 폴더를 업로드할 수도 있습니다. 이러한 파일 작업은 앞서 설명한 연산 세션 마운트 방식으로도 모두 수행 가능합니다.
...
`upload-file`은 호스트 수준의 권한으로, 도메인 권한, 프로젝트 권한, 또는
...
:::note
파일 편집 버튼은 해당 스토리지 폴더에 대한 사용자 접근 권한에 `write_content` 권한이 포함되어 있을 때에만 사용할 수 있습니다(폴더 공유 권한 또는 폴더에 부여된 역할을 통해 부여됨). 제어판의 스토리지 호스트 수준 설정은 이 동작에 영향을 주지 않습니다. 파일 로드에 실패하면 오류 메시지가 표시됩니다.
:::
...
이 탭에도 [폴더 목록 새로고침](#refresh-folder-list)에서 설명한 것과 동일한
**자동 새로고침** 주기 메뉴가 달린 새로고침 버튼이 있습니다. 이 버튼의 주기도 `끄기`
상태로 시작하며, 폴더 목록과는 별도로 기억됩니다.

## 폴더 이름 변경


스토리지 폴더의 이름을 변경할 수 있는 권한을 가지고 있는 경우, 폴더 상세 드로어를 열고 폴더 이름 옆의 수정 버튼을 클릭하여 이름을 변경할 수 있습니다. 이름 변경 작업은 상세 드로어 내부에서만 수행됩니다.
...
제외됩니다."*라는 제목의 알림이 나타나고 해당 폴더 이름이 함께 표시됩니다. 확인하기 전에
이 목록을 반드시 확인하세요. 나머지 폴더만 휴지통 탭으로 이동하며, 알림 아래의 확인
메시지도 실제로 이동하는 폴더만 세어서 표시합니다. 제외되는 폴더가 많으면 목록은 모달을
늘리는 대신 알림 안에서 스크롤됩니다.
...
폴더 이름을 입력하라는 확인 모달이 나타납니다. 폴더 이름을 정확하게 입력하면 **영구 삭제** 버튼이 활성화되며, 이를 클릭하여 폴더를 완전히 삭제할 수 있습니다.
...
Backend.AI는 [FileBrowser](https://filebrowser.org)를 지원합니다. FileBrowser는 웹 브라우저를 통해 원격 서버의 파일을 관리할 수 있도록 도와주는 프로그램입니다. 특히 사용자의 로컬 머신에서 디렉토리를 업로드할 때 유용합니다.
...
FileBrowser가 새 창에서 열린 것을 확인할 수 있습니다. 탐색기를 열었던 스토리지 폴더가 FileBrowser의 루트 디렉토리가 됩니다. FileBrowser 창에서 디렉토리와 파일을 자유롭게 업로드하고, 수정하고, 삭제할 수 있습니다.
...
Backend.AI는 데스크톱 앱과 웹 기반 WebUI 모두에서 SSH / SFTP 파일 업로드를 지원합니다. SFTP 서버를 사용하면 안정적인 데이터 스트림을 통해 파일을 빠르게 업로드할 수 있습니다.
...
연결을 위해 `SSH 키 다운로드` 버튼을 클릭하여 SSH 개인 키(`id_container`)를 다운로드합니다. 또한 호스트와 포트 번호를 기억해 두세요. 이후 대화 상자에 표시된 연결 예시 코드를 사용하여 세션에 파일을 복사하거나, 다음 가이드를 참고할 수 있습니다: [SFTP 연결 가이드](#ssh-sftp-container). 파일을 보존하려면 스토리지 폴더로 파일을 전송해야 합니다. 또한 일정 시간 동안 전송이 없으면 세션이 자동으로 종료됩니다.
...
모델 탭은 간편한 배포를 지원합니다. [배포](#model-serving)에 필요한 입력 데이터와 학습 데이터를 포함한 필요한 데이터를 모델 폴더에 저장할 수 있습니다.

![](../images/models.png)

모델 폴더 목록에서 폴더의 `서비스로 배포` 버튼을 클릭하면, 별도의 설정 파일 없이 배포 프리셋을 선택해 바로 배포할 수 있습니다. 자세한 내용은 [빠른 배포](#quick-deploy) 섹션을 참고하세요.
