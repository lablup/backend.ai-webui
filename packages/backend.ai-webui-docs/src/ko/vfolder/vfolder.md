...
- **마운트 권한**: 폴더가 연산 세션에 마운트될 때 적용되는 마운트 권한입니다. **읽기 및 쓰기**는 세션 내에서 폴더에 쓰기를 허용하고, **읽기 전용**은 쓰기를 막습니다.
- **복사 가능 여부**: 사용 방식이 "모델"로 설정된 경우에만 표시됩니다. 생성하는 가상 폴더(vfolder)가 복사 가능한지 여부를 선택합니다.

:::note
데이터 페이지에서 생성한 폴더는 항상 **사용자 폴더**입니다. **프로젝트 폴더**는 관리자가 관리자용 데이터 페이지에서 생성합니다.
:::
...
SFTP 세션을 시작할 수 없는 경우 SSH / SFTP 연결 대화 상자 대신 `SFTP 세션 생성에 실패했습니다.` 대화 상자가 나타납니다. 이 대화 상자에는 서버가 보고한 실패 원인이 표시되며, 그 뒤에 HTTP 상태 코드와 오류 코드가 괄호 안에 표시됩니다(예: `... (HTTP 500, BAI_E0001)`). 따라서 장애 내용을 그대로 관리자에게 전달할 수 있습니다. 원인 아래에는 사용하지 않는 업로드 세션을 정리하면 SFTP 호스트의 자원을 확보할 수 있다는 안내가 표시됩니다.

대화 상자에는 다음 두 개의 버튼이 있습니다.

- `업로드 세션으로 이동`: `업로드 세션` 유형으로 필터링된 세션 페이지를 엽니다. 여기서 더 이상 필요하지 않은 업로드 세션을 종료할 수 있습니다.
- `닫기`: 다른 동작 없이 대화 상자를 닫습니다.

이 오류는 대부분 키페어 자원 정책에서 허용하는 동시 업로드(SFTP) 세션 수의 상한에 도달했을 때 발생합니다.
