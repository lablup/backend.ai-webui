...
:::note
감사 로그 탭은 모든 사용자에게 표시되지만, 백엔드에서 접근 제한이 적용됩니다. 슈퍼관리자만 감사 로그 데이터를 받을 수 있습니다. 일반 사용자에게는 빈 목록이 표시됩니다.
:::
...
연결을 위해 'SSH 키 다운로드' 버튼을 클릭하여 SSH 개인 키(`id_container`)를 다운로드합니다. 또한 호스트와 포트 번호를 기억해 두세요. 이후 대화 상자에 표시된 연결 예시 코드를 사용하여 세션에 파일을 복사하거나, 다음 가이드를 참고할 수 있습니다: [SFTP 연결 가이드](../sftp_to_container/sftp_to_container.md#ssh-sftp-container). 파일을 보존하려면 스토리지 폴더로 파일을 전송해야 합니다. 또한 일정 시간 동안 전송이 없으면 세션이 자동으로 종료됩니다.
