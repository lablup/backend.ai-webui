...
해당 폴더에 대한 사용자 권한에 쓰기 또는 삭제 권한이 포함되어 있지 않으면, 예를 들어 **읽기 전용**으로 공유받은 폴더라면 **삭제**, **이름 변경**, **폴더 생성** 버튼이 비활성화됩니다. 이 경우에는 폴더를 탐색하고 내용을 다운로드하는 작업만 가능합니다.

:::warning
해당 폴더가 위치한 스토리지 호스트에 대한 `upload-file` 권한과 폴더 자체에 대한 쓰기 권한을
**모두** 보유해야 업로드할 수 있습니다. 둘 중 하나라도 없으면 **업로드** 버튼(및 드래그 앤
드롭 업로드)은 **비활성화**됩니다. 버튼 자체는 표시되지만 회색으로 변하며, 툴팁에 업로드가
허용되지 않는다는 안내가 표시됩니다.
...
**위치** 열이나 폴더 상세 드로어에서 확인합니다. 폴더 자체에 대한 쓰기 권한은 폴더를
소유하고 있거나 폴더를 공유받을 때 부여받은 권한으로 결정됩니다.
:::
