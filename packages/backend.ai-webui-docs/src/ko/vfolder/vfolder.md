...
왼쪽 패널에서 폴더 내의 모든 디렉토리와 파일을 확인합니다. **이름** 열에 있는 디렉토리 이름을 클릭하면 해당 디렉토리로 이동합니다. 각 행의 작업 버튼은 **이름** 열의 항목 이름 옆에 있습니다. **다운로드** 버튼을 클릭하면 파일이나 디렉토리를 다운로드하고, **삭제** 버튼을 클릭하면 삭제하며, 이름 옆의 연필(**Rename**) 버튼을 클릭하면 이름을 그 자리에서 변경합니다. 열 너비가 좁아 모든 버튼을 표시할 수 없는 경우, 나머지 작업은 같은 행의 **추가 작업**(⋮) 메뉴로 이동합니다. 보다 세밀한 파일 작업이 필요하다면 이 폴더를 연산 세션 생성 시 마운트한 뒤 터미널이나 Jupyter Notebook 등과 같은 서비스로 수행하세요.
...
:::note
**파일 편집** 작업은 해당 스토리지 폴더에 대한 사용자 접근 권한에 `write_content` 권한이 포함되어 있을 때에만 사용할 수 있습니다(폴더 공유 권한 또는 폴더에 부여된 역할로 부여됨). 제어판의 스토리지 호스트 수준 설정은 이 동작에 영향을 주지 않습니다. 파일 로드에 실패하면 오류 메시지가 표시됩니다.
:::
...
이 상태에서 **이름** 열에 있는 해당 폴더 행의 복원 버튼을 클릭하면 폴더가 복원됩니다. 폴더를 영구적으로 삭제하려면 같은 행의 `휴지통` 버튼을 클릭합니다.
