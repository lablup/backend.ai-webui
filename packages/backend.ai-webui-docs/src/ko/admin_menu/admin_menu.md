...
- **배포**: 모든 프로젝트의 배포 목록을 표시하며, 사용자용 배포 페이지와 동일한 라이프사이클 및 속성 필터를 제공합니다. 프로젝트를 넘나드는 이 화면에는 **프로젝트** 필터 속성이 추가로 제공되며, 프로젝트 ID를 직접 입력하는 대신 검색 가능한 프로젝트 드롭다운에서 선택합니다.
...
- **프로젝트(Project)**: 소유 프로젝트로 검색합니다. 프로젝트 식별자를 직접 입력하는 대신 모델 스토어(Model Store) 프로젝트가 나열된 검색 가능한 드롭다운에서 선택하며, 필터 태그에는 프로젝트 이름이 표시됩니다.
...
테이블 상단의 속성 필터로 목록을 검색하고 좁힐 수 있으며, 컬럼 헤더를 클릭하여 정렬 순서를 변경할 수 있습니다. **카테고리** 속성은 카테고리 ID를 직접 입력하는 대신 이 클러스터에 정의된 카테고리 목록에서 선택하며, 필터 태그에는 카테고리 이름이 표시됩니다.
