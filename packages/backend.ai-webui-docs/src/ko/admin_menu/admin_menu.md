...
2. *프리셋 생성* 대화 상자의 각 섹션을 입력합니다.
   ...
   - **기본 정보**:
      ...
      * **런타임 파라미터** (선택 사항): 선택한 런타임 변형의 파라미터 값(예: 양자화 방식, 데이터 타입)입니다. vLLM, SGLang 등 비-Custom 런타임을 선택한 경우에만 표시됩니다.
   ...
   - **모델 정의** (선택 사항): 토글을 활성화하면 구조화된 모델 정의를 설정할 수 있습니다. 활성화 시 다음 항목을 구성합니다.
      ...
      * **헬스 체크** (선택 사항): 경로, 간격, 최대 재시도 횟수, 최대 대기 시간, 예상 상태 코드, 초기 유예 시간.
