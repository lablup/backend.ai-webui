...
- **최대 요청수 (15분당)**: 15분 동안 Backend.AI 서버에 보낼 수 있는 최대 요청 수입니다. 예를 들어 1000으로
  ...
  않습니다. 기본값을 사용하고 사용자의 패턴에 따라 API 요청 빈도가 높아지면 값을 증가시키는 것이 좋습니다.
   ...
   `최대 요청수는 0보다 크고 50000보다 작아야 합니다.` 라는 검증 메시지가 표시되고 제출이 차단되므로, 입력한
...
프로젝트 페이지에는 클러스터의 모든 프로젝트가 나열되며, 관리자는 여기에서 프로젝트를 생성, 수정, 비활성화,
활성화, 영구 삭제할 수 있습니다. 각 행에는 프로젝트 관리자 권한을 부여할 수 있는 바로 가기도 제공됩니다.
