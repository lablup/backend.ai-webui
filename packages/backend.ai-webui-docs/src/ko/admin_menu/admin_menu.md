...
해당 사용자의 **이메일** 컬럼 행에 있는 `설정` 아이콘을 클릭하면 이미 존재하는 사용자의 정보를 수정할 수 있습니다. 사용자의 이름, 비밀번호, 활성화 상태 등을 변경할 수 있습니다. 사용자 ID(이메일)는 변경할 수 없습니다.
...
- **password**: 초기 비밀번호입니다. 단일 사용자 생성과 동일한 비밀번호 규칙이 적용됩니다 (8자 이상, 알파벳/특수문자/숫자 각 1개 이상 포함).
...
미리보기를 검토하고 모든 행이 유효한지 확인한 후 **생성**을 클릭하여 제출합니다. 일부 행이 서버 측에서 실패한 경우(예: 이메일이나 사용자 이름이 이미 존재하는 경우) 대화 상자가 열린 상태로 유지되며 행별 오류가 표시되어 충돌을 식별하고 해결할 수 있습니다.
...
사용자 탭과 마찬가지로 키페어 행에 있는 인라인 버튼을 사용하여 키페어 세부 정보를 보거나 업데이트할 수 있습니다. 정보 아이콘 버튼을 클릭하면 키페어의 구체적인 세부 정보를 확인할 수 있습니다. 필요한 경우 복사 버튼을 클릭하여 secret key를 복사할 수 있습니다.

![](../images/keypair_detail_dialog.png)

`설정 (톱니바퀴)` 버튼을 클릭하여 키페어의 자원 정책과 최대 요청수를 수정할 수 있습니다. '최대 요청수' 값이 작으면 로그인과 같은 API 작업이 차단될 수 있으므로 주의하시기 바랍니다.
...
실수로 키페어를 삭제한 경우, 오른쪽 상단의 `+ 자격증명 추가` 버튼을 클릭하여 사용자를 위한 키페어를 다시 생성할 수 있습니다.

최대 요청수 필드는 15분 동안 Backend.AI 서버에 보낼 수 있는 최대 요청 수를 지정하는 곳입니다. 예를 들어 1000으로 설정하면 키페어가 15분 동안 1000개가 넘는 API 요청을 보낼 경우 서버에서 오류를 발생시키고 요청을 수락하지 않습니다. 기본값을 사용하고 사용자의 패턴에 따라 API 요청 빈도가 높아지면 값을 증가시키는 것이 좋습니다.
...
:::note
각 관리자용 배포는 `/admin-deployments/:id` 전용 경로를 갖습니다. 관리자 배포 페이지에서 배포를 열면 URL이 이 경로로 변경되어 배포 상세 페이지를 직접 링크하거나 북마크할 수 있습니다.
:::
...
상단의 필터 바에서 모델 카드 이름으로 목록을 검색하여 좁힐 수 있습니다. 필터 바는 이름(텍스트) 검색만 제공하며, 도메인 선택기는 제공하지 않습니다.
...
- **카테고리**: 연관된 프리셋을 묶기 위한 선택적 카테고리입니다. 비워두면 **카테고리 없음**으로 표시됩니다.
...
자원 정책을 수정하려면 default 정책 그룹의 이름 열에 있는 `설정 (톱니바퀴)` 버튼을 클릭합니다. 자원 정책 수정 대화 상자에서 목록의 자원 정책을 구분하는 기본 키 역할을 하는 자원 정책 이름을 제외한 모든 옵션을 편집할 수 있습니다. CPU, RAM, fGPU 하단의 무제한 체크박스를 해제하고 자원 제한을 원하는 값으로 설정합니다. 할당된 자원이 전체 하드웨어 용량보다 적은지 확인하세요. 이 경우 CPU, RAM, fGPU를 각각 2, 4, 1로 설정합니다. 확인 버튼을 클릭하여 업데이트된 자원 정책을 적용합니다.
...
`+ 생성` 버튼을 클릭하여 새 자원 정책을 생성할 수 있습니다. 각 설정 값은 위에서 설명한 것과 동일합니다.

자원 정책을 생성하고 키페어와 연결하려면 사용자 페이지의 자격 증명 탭으로 이동하여 원하는 키페어의 이름 열에 있는 톱니바퀴 버튼을 클릭하고 자원 정책 선택 필드를 클릭하여 선택합니다.
...
- 최대 폴더 크기: 사용자의 스토리지 공간의 최대 크기입니다. 사용자의 스토리지 공간이 이 값을 초과하면 새 스토리지 폴더를 생성할 수 없습니다. 무제한으로 설정하면 "∞"로 표시됩니다.
...
업데이트하려면 이름 열의 `설정 (톱니바퀴)` 버튼을 클릭합니다. 삭제하려면 휴지통 버튼을 클릭합니다.
...
변경하려면 이름 열의 `설정` 버튼을 클릭합니다. 자원 정책 이름은 편집할 수 없습니다. 삭제는 휴지통 아이콘 버튼을 클릭하여 수행할 수 있습니다.
...
관리자용 세션 페이지에서는 선택한 자원 그룹 내의 모든 대기 중인 세션을 한눈에
...
`제어` 패널의 `설정 (톱니바퀴)`를 클릭하여 각 이미지의 최소 자원 요구 사항을 변경할 수 있습니다. 각 이미지에는 최소 작동을 위한 하드웨어 및 자원 요구 사항이 있습니다. (예를 들어 GPU 전용 이미지의 경우 최소 할당 GPU가 있어야 합니다.) 최소 자원 양의 기본값은 이미지의 메타데이터에 포함되어 제공됩니다. 각 이미지에 지정된 자원 양보다 적은 자원으로 연산 세션을 생성하려고 하면 요청이 취소되지 않고 이미지의 최소 자원 요구 사항으로 자동 조정된 후 생성됩니다.
...
이름 열에 있는 `설정 (톱니바퀴)` 버튼을 클릭하여 자원 프리셋에서 제공하는 CPU, RAM, fGPU 등의 자원을 설정할 수 있습니다. Create or Modify Resource Preset 모달에는 현재 사용 가능한 자원의 필드가 표시됩니다. 서버 설정에 따라 특정 자원이 표시되지 않을 수 있습니다. 원하는 값으로 자원을 설정한 후 저장하고, 연산 세션을 생성할 때 해당 프리셋이 표시되는지 확인합니다. 사용 가능한 자원이 프리셋에 정의된 자원 양보다 적으면 해당 프리셋이 표시되지 않습니다.
...
슈퍼 관리자는 `자원` 페이지를 방문하여 현재 Backend.AI에 연결된 에이전트 노드의 목록을 볼 수 있습니다. 에이전트 노드의 IP, 연결 시간, 현재 실제 사용 중인 자원 등을 확인할 수 있습니다. WebUI에서는 에이전트 노드를 직접 조작하는 기능을 제공하지 않습니다.
...
또한 Control 패널의 노트 아이콘을 클릭하여 에이전트 노드의 자원에 대한 정확한 사용량을 확인할 수 있습니다.

![](../images/detailed_agent_node_usage_information.png)

종료됨 탭에서는 한번 연결되었다가 종료되거나 연결이 끊긴 에이전트의 정보를 확인할 수 있습니다. 이는 노드 관리를 위한 참고 자료로 사용할 수 있습니다. 목록이 비어 있다면 연결 끊김이나 종료가 발생하지 않았음을 의미합니다.
...
'제어' 열의 `설정` 버튼을 클릭하여 자원 그룹을 편집할 수 있습니다. '스케줄러 선택' 필드에서 연산 세션을 생성하기 위한 스케줄링 방법을 선택할 수 있습니다. 현재 `FIFO`, `LIFO`, `DRF`, `FAIR_SHARE` 네 가지 유형이 있습니다. `FIFO`와 `LIFO`는 작업 대기열에서 먼저 또는 마지막으로 대기열에 등록된 연산 세션을 생성하는 스케줄링 방법입니다. `DRF`는 Dominant Resource Fairness의 약자로, 각 사용자에게 가능한 한 공정하게 자원을 제공하는 것을 목표로 합니다. `FAIR_SHARE`는 과거 사용 패턴을 기반으로 연산 자원을 할당합니다. 자세한 내용은 [Fair Share 스케줄러](#fair-share-scheduler) 섹션을 참고하세요. '활성' 상태를 끄면 자원 정책을 비활성화할 수 있습니다.
...
`+ 생성` 버튼을 클릭하여 새 자원 그룹을 생성할 수 있습니다. 다른 생성 옵션과 마찬가지로 이미 존재하는 이름으로는 자원 그룹을 생성할 수 없습니다. 이름은 키 값이기 때문입니다.
...
사용자 폴더 권한은 키페어 자원 정책에 설정되는 권한입니다. 우측의 사용자 선택기를 통해 사용자를 선택하면 해당 사용자의 키페어에 할당된 키페어 자원 정책만 필터링되어 제공됩니다. **할당 키페어** 열에서 해당 사용자의 기본 액세스 키를 확인할 수 있습니다.
...
관리 페이지로 이동하면 서버를 관리하는 몇 가지 버튼이 표시됩니다.

- 사용량 재계산: 때때로 불안정한 네트워크 연결이나 Docker daemon의 컨테이너 관리 문제로 인해 Backend.AI가 점유한 자원이 컨테이너가 실제로 사용하는 자원과 일치하지 않는 경우가 있을 수 있습니다. 이 경우 사용량 재계산 버튼을 클릭하여 자원 점유율을 수동으로 수정합니다.
- 이미지 리스캔: 등록된 모든 Docker registry에서 이미지 메타 정보를 업데이트합니다. Backend.AI에 연결된 docker registry에 새 이미지가 푸시될 때 사용할 수 있습니다.
