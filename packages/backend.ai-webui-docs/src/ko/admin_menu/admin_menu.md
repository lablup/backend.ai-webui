...
관리자 계정으로 로그인하면 사이드바 상단에 **관리자 설정** 메뉴가 추가됩니다. 이 메뉴를 선택하면 사이드바가 관리자 메뉴만 표시하도록 전환됩니다. Backend.AI에 등록된 사용자 정보는 **사용자** 메뉴에서 볼 수 있습니다. 슈퍼관리자는 모든 사용자의 정보를 확인하고, 사용자를 생성하거나 비활성화할 수 있습니다.
...
일상적인 계정 관리는 비활성화에 의존하지만, 슈퍼관리자는 아래에서 설명하는 영구 삭제(Purge) 기능을 사용하여 이미 비활성화된 계정을 영구적으로 제거할 **수 있습니다**.

<a id="purge-inactive-users"></a>

### 비활성 사용자 영구 삭제

슈퍼관리자는 이미 비활성화된 사용자 계정을 영구적으로 삭제(purge)할 수 있습니다. 영구 삭제는 **비활성** 탭에 있는 사용자에 대해서만 가능하며, 활성 사용자는 먼저 비활성화해야 합니다. 비활성화와 달리 영구 삭제는 되돌릴 수 없으며 사용자의 관련 데이터도 함께 제거합니다.
...
관리자 및 슈퍼관리자는 클러스터에 있는 모든 배포를 한눈에 볼 수 있는 관리자 배포 페이지(`/admin-deployments`)에 접근할 수 있습니다. **프로젝트** 열은 배포 목록에서 제공되지만 기본적으로 숨겨져 있으며, 컬럼 설정에서 표시하도록 활성화할 수 있습니다.
...
:::note[버전 요구 사항]
**런타임 파라미터** 행은 Backend.AI Manager 26.4.4 이상에서만 표시됩니다.
:::
...
슈퍼관리자는 관리자 배포 페이지의 모델 스토어 관리(Model Store Management) 탭을 통해 모델 카드를 관리할 수 있습니다.
...
:::note[슈퍼관리자 전용: 오토 스케일링 규칙 편집기 미리보기]
슈퍼관리자로 배포의 오토 스케일링 규칙 편집기를 열고 메트릭 소스를 `Prometheus`로 설정한 뒤 프리셋을 선택하면, 프리셋 선택기 아래에 해당 프리셋의 **현재 값(Current value)** 이 실시간으로 표시됩니다. 이 미리보기는 슈퍼관리자 계정에서만 제공되며, 일반 사용자 및 도메인 관리자에게는 표시되지 않습니다.
:::
...
:::danger
프로메테우스 쿼리 프리셋의 삭제는 **영구적이며 되돌릴 수 없습니다**. 삭제된 프리셋을 참조하는 오토 스케일링 규칙 등은 쿼리 템플릿을 잃게 되며, 다른 프리셋을 가리키도록 재구성하기 전까지 정상적으로 동작하지 않을 수 있습니다.
:::
...
:::note[버전 요구 사항]
필수 런타임 파라미터에 대한 검증과 **헬스 체크 활성화** 토글은 Backend.AI Manager 26.4.4 이상에서만 사용할 수 있습니다. vLLM 및 SGLang의 고급 모드(Advanced Mode)에도 동일하게 적용됩니다.
:::
...
- 최대 폴더 크기: 사용자의 스토리지 공간의 최대 크기입니다. 사용자의 스토리지 공간이 이 값을 초과하면 새 데이터 폴더를 생성할 수 없습니다. 무제한으로 설정하면 "∞"로 표시됩니다.
...
다음에 나열된 사전 정의된 자원 프리셋은 연산 세션을 생성할 때 자원 할당 패널에 표시됩니다. 슈퍼관리자는 이러한 자원 프리셋을 관리할 수 있습니다.
