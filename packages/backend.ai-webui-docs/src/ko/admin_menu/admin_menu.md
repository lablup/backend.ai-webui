...
테이블 상단의 속성 필터 바를 사용하여 목록을 좁힐 수 있습니다. 기본 속성은 사용자 ID(이메일), ID, 이름(사용자 이름), 프로젝트, 역할입니다. 컬럼 헤더를 클릭하여 목록을 정렬할 수 있으며, 필터와 정렬 상태는 현재 페이지뿐만 아니라 전체 데이터에 적용됩니다.

기본 컬럼 외에도 슈퍼관리자 사용자 목록에는 사용자 레코드에서 가져온 다음과 같은 추가 컬럼이 표시됩니다.

- **기본 액세스 키 (Main Access Key)**: API 인증에 사용되는 사용자의 기본 액세스 키입니다. 값은 복사할 수 있습니다.
- **2FA 활성화 시점**: 사용자에 대해 이중 인증이 활성화된 시점입니다.
- **허용된 클라이언트 IP**: 사용자가 시스템에 접근할 수 있는 IP 주소 또는 CIDR 범위입니다.
- **비밀번호 변경 필요 여부**: 다음 로그인 시 사용자가 비밀번호를 변경하도록 표시되었는지 여부입니다.
- **컨테이너 UID / 컨테이너 기본 GID / 컨테이너 GIDs**: 사용자의 컨테이너 내부 프로세스에 할당되는 숫자 사용자 ID, 기본 그룹 ID, 보조 그룹 ID입니다.

:::note
연결된 Backend.AI Manager가 26.4.4 버전 이상인 경우 속성 필터 바에서 추가 필터 속성을 사용할 수 있습니다. 전체 이름, 소개, 상태 정보, 자원 정책(문자열 일치), 비밀번호 변경 필요 여부, 2FA 활성화, sudo 세션 사용 여부(boolean), 컨테이너 UID 및 컨테이너 기본 GID(정수), 컨테이너 GIDs(단일 GID 포함 여부)가 이에 해당합니다. 이러한 필터는 Manager가 해당 기능을 지원하는 경우에만 표시됩니다.
:::
...
- **자원 정책**: Backend.AI 버전 24.09부터, 사용자가 속한 사용자 자원 정책을 선택할 수 있습니다. 사용자 자원 정책에 대한 자세한 내용은 [사용자 자원 정책](#user-resource-policy) 섹션을 참고하세요.
...
기존 명부나 내보낸 데이터에서 여러 사용자 계정을 생성해야 할 때는 순번 접두사를 지정하는 대신 CSV 파일을 직접 업로드할 수 있습니다. **사용자 추가** 버튼 옆의 말줄임표(`...`) 드롭다운을 클릭하고 **CSV 파일로 사용자 일괄 생성**을 선택하면 CSV 업로드 대화 상자가 열립니다.
...
사용자별 사용 통계 추적, 메트릭 보존, 실수로 인한 계정 손실 방지를 위해, 사용자가 로그인하지 못하도록 하는 권장 방법은 계정을 삭제하는 것이 아니라 **비활성화**하는 것입니다. 비활성화는 사용자의 레코드를 그대로 유지하면서 로그인만 차단합니다. Controls 패널의 삭제 아이콘을 클릭합니다. 확인을 요청하는 팝오버가 나타나며, Deactivate 버튼을 클릭하여 사용자를 비활성화할 수 있습니다.
...
슈퍼관리자는 이미 비활성화된 사용자 계정을 영구적으로 삭제(purge)할 수 있습니다. 영구 삭제는 **Inactive** 탭에 있는 사용자에 대해서만 가능하며, 활성 사용자는 먼저 비활성화해야 합니다. 비활성화와 달리 영구 삭제는 되돌릴 수 없으며 사용자의 관련 데이터도 함께 제거합니다.

Users 페이지에서 **Inactive** 탭으로 전환합니다(상태 선택기에는 영구 삭제가 사용자의 키페어에도 영향을 준다는 것을 나타내기 위해 **비활성 (키페어 포함)** 으로 표시됩니다). 두 가지 방법으로 사용자를 영구 삭제할 수 있습니다.

- **개별 영구 삭제**: 비활성 사용자 한 명의 행에서 Controls 열의 휴지통(영구 삭제) 아이콘을 클릭합니다.
- **일괄 영구 삭제**: 행 체크박스로 하나 이상의 비활성 사용자를 선택한 다음, 선택 개수 옆에 나타나는 **유저 영구 삭제** 버튼(빨간색 휴지통 버튼)을 클릭합니다.
...
:::note
옵션 체크박스를 포함한 일괄 영구 삭제는 Backend.AI Manager 26.3.0 버전 이상이 필요합니다. 이전 버전의 Manager에서는 선택한 사용자를 한 명씩 차례로 삭제하는 방식으로 동작합니다.
:::
...
모델 정의 파일 형식에 대한 자세한 설명은 배포 문서의 [모델 정의 가이드](../deployment/deployment.md#model-definition-guide) 를 참고하세요.

:::note
모델 스토어에서 모델 카드의 **Deploy** 버튼을 활성화하려면 연결된 폴더에 `service-definition.toml`이 포함되어 있어야 합니다. `model-definition.yaml`은 `Custom` 런타임 변형을 사용할 때만 추가로 필요하며, `vLLM`, `SGLang`, `NVIDIA NIM`, `Modular MAX` 같은 프리셋 런타임 변형에는 필요하지 않습니다. 서비스 정의 파일에 대한 자세한 내용은 배포 문서의 [서비스 정의 파일](../deployment/deployment.md#service-definition-file) 섹션을 참고하세요.
:::
...
관리자 및 슈퍼관리자는 클러스터에 있는 모든 배포를 한눈에 볼 수 있는 관리자 배포 페이지(`/admin-deployments`)에 접근할 수 있습니다. 이 페이지는 표준 배포 목록 열에 추가로 **프로젝트** 열을 표시하여, 관리자가 모든 프로젝트의 배포를 관리할 수 있게 합니다.

![](../images/admin_serving_page.png)

관리자 배포 페이지에는 최대 네 개의 탭이 있습니다:

- **배포**: 항상 표시됩니다. 모든 프로젝트의 배포 목록을 표시하며, 사용자용 배포 페이지와 동일한 라이프사이클 및 속성 필터를 제공합니다.
- **모델 스토어 관리**: 항상 표시됩니다. 슈퍼관리자에게만 제공됩니다. 아래 [관리자 모델 스토어 관리](#admin-model-store-management) 섹션을 참고하세요.
...
상단의 속성 필터 바를 사용하여 목록을 좁힐 수 있으며, 다음 속성으로 필터링할 수 있습니다:

- **이름(Name)**: 모델 카드 이름으로 필터링합니다 (문자열 일치).
- **도메인(Domain)**: 소유 도메인으로 필터링합니다 (문자열 일치).
- **프로젝트(Project)**: 소유 프로젝트의 UUID로 필터링합니다.
- **스토리지 호스트(Storage Host)**: 연결된 폴더의 스토리지 호스트로 필터링하며, 같음 또는 같지 않음 연산자와 직접 입력한 값을 사용합니다.
...
삭제는 되돌릴 수 없기 때문에, 다이얼로그는 **삭제** 버튼이 활성화되기 전에 확인 입력란에 **프리셋 이름을 직접 입력**하도록 요구합니다. 이 입력 확인 패턴(`BAIConfirmModalWithInput`)은 Backend.AI 전반에서 영구 삭제 작업에 일관되게 사용됩니다. 다이얼로그 제목에 표시된 프리셋 이름을 정확히 입력한 뒤 **삭제**를 클릭하여 확정합니다.
...
특정 사용자에 대한 keypair 자원 정책을 선택할 때, 선택 테이블에는 각 정책에 현재 어떤 사용자의 키페어가 연결되어 있는지 보여주는 **할당 키페어** 컬럼이 포함되어 있어, 정책을 선택하기 전에 사용자의 기존 할당 상태를 확인할 수 있습니다.
...
정렬 가능한 컬럼 헤더를 클릭하면 현재 보이는 페이지뿐만 아니라 전체 이미지 목록이 서버 측에서 정렬되며, 적용된 정렬 상태는 페이지 URL에 보존되어 새로고침하거나 링크를 공유할 때도 유지됩니다.
...
- **전역 레지스트리로 설정(Set as Global Registry)**: 활성화하면 모든 프로젝트에서 해당 레지스트리에 접근할 수 있도록 허용하는 토글입니다. 이 필드는 24.09.0 버전부터 사용할 수 있습니다.
- **허용된 프로젝트(Allowed Projects)**: **전역 레지스트리로 설정** 토글을 끈 경우, 이 필드에서 해당 레지스트리를 사용할 수 있는 특정 프로젝트를 선택합니다. 이 필드는 25.3.0 버전부터 사용할 수 있습니다.
...
스토리지 호스트를 관리하려면 Storages 목록에서 스토리지 호스트 이름(또는 control 열의 '설정 (톱니바퀴)')을 클릭합니다. 그러면 가용량(쿼터)과 폴더 권한을 구성하는 **스토리지 호스트 상세 Drawer**가 열립니다.

:::note
쿼터 설정은 쿼터 설정을 제공하는 스토리지(예: XFS, CephFS, NetApp, Purestorage 등)에서만 사용할 수 있습니다. Capacity 탭에서 스토리지와 관계없이 스토리지 사용량을 볼 수 있지만, 내부적으로 쿼터 구성을 지원하지 않는 쿼터는 구성할 수 없습니다.
...
스토리지 호스트 상세 Drawer는 스토리지 호스트를 확인하고 관리하는 단일 공간입니다. Storages 목록에서 열리며, 상단에 호스트 이름과 마운트 경로, 자원 요약, 그리고 여러 탭이 표시됩니다. 이전에 존재하던 별도의 스토리지 설정 페이지는 이 Drawer의 **Capacity** 탭으로 통합되었습니다.
...
Drawer에는 다음과 같은 탭이 있습니다.

- **프로젝트 폴더 권한 (Project Folder Permissions)**: 어떤 도메인과 프로젝트가 호스트의 프로젝트 폴더에 접근할 수 있는지 확인하고 관리합니다. 아래의 [프로젝트 폴더 권한](#project-folder-permission) 섹션을 참고하세요.
- **사용자 폴더 권한 (User Folder Permissions)**: 호스트의 사용자 폴더에 적용되는 권한을 확인하고 관리합니다. 사용자 폴더 권한은 사용자의 기본 액세스 키에 연결된 키페어 자원 정책에 의해 결정됩니다.
- **Capacity**: 사용자별/프로젝트별 스토리지 쿼터를 구성하고, 호스트의 사용량과 기능을 확인합니다. 아래의 [쿼터 설정 패널](#quota-setting-panel) 섹션을 참고하세요. 쿼터 구성을 지원하지 않는 스토리지 호스트에서는 이 탭을 사용할 수 없습니다.

<a id="quota-setting-panel"></a>

#### 쿼터 설정 패널

스토리지 호스트 상세 Drawer의 **Capacity** 탭에는 두 개의 패널이 있습니다.
...
Backend.AI에는 사용자와 관리자(프로젝트)가 생성한 두 가지 유형의 vfolder가 있습니다. 이 섹션에서는 사용자별 현재 쿼터 설정을 확인하고 구성하는 방법을 보여드리고자 합니다. 먼저 Capacity 탭에서 Quota Settings 패널의 활성 하위 탭이 `For User`인지 확인합니다. 그런 다음 쿼터를 확인하고 편집하려는 사용자를 선택합니다. 이미 쿼터를 설정한 경우 사용자 ID에 해당하는 쿼터 ID와 이미 설정된 구성을 테이블에서 볼 수 있습니다.
...
[스토리지 호스트 상세 Drawer](#storage-host-detail-drawer)의 **프로젝트 폴더 권한** 탭에서 관리자는 선택한 스토리지 호스트의 프로젝트 폴더에 어떤 도메인과 프로젝트가 액세스 권한을 가지는지 확인하고 관리할 수 있습니다.
...
이 탭은 선택한 도메인에 대한 **Domains** 테이블과 해당 도메인에 속한 프로젝트에 대한 **Projects** 테이블, 두 섹션으로 구성됩니다.

:::tip
**프로젝트 폴더 권한** 탭의 도메인 선택기는 drawer가 열릴 때 현재 활성화된 도메인으로 기본 설정되므로, 해당 도메인의 폴더 권한을 즉시 확인할 수 있습니다. 다른 도메인으로 전환하거나 선택을 해제하면 다른 도메인의 권한도 확인할 수 있습니다.
:::

프로젝트 폴더에 적용되는 유효 권한은 **선택한 도메인의 권한과 프로젝트 자체 권한의 합집합(union)** 입니다. 즉, 도메인 수준이나 프로젝트 수준 중 어느 한 곳에서 부여된 권한은 해당 프로젝트의 폴더에 적용됩니다. 이를 명확하게 표시하기 위해 Projects 테이블은 각 권한에 대해 세 가지 상태(tri-state) 표시를 사용합니다.

- **프로젝트에 설정됨**: 권한이 프로젝트에 직접 부여되어 있습니다.
- **도메인에서 상속됨**: 권한이 도메인 수준에서 부여되어 프로젝트가 이를 상속합니다.
- 둘 다 아님: 어느 수준에서도 권한이 부여되지 않았습니다.

Domains, Projects 또는 사용자 폴더 권한 테이블에서 행 체크박스로 여러 행을 선택하여 권한 집합을 나란히 비교할 수 있습니다. 행을 선택한 상태에서 **권한 수정** 을 클릭하면 일괄 편집 모달이 열리며, 선택한 권한 집합을 선택된 모든 대상에 한 번에 적용합니다. 이 모달은 기본적으로 모든 권한이 선택된 상태로 열리며, 저장하면 선택한 대상의 권한이 사용자가 선택한 집합으로 정확히 덮어쓰여집니다.
...
:::tip
Backend.AI에서 내보낸 모든 CSV 파일은 파일 맨 앞에 UTF-8 BOM(Byte Order Mark)이 포함됩니다. 이를 통해 비 UTF-8 시스템(예: CP949를 사용하는 한국어 Windows)에서 Microsoft Excel이 인코딩을 올바르게 인식하여 한국어, 일본어, 중국어 등 멀티바이트 문자를 깨짐 없이 표시할 수 있습니다.
:::
