/**
 @license
 Copyright (c) 2015-2026 Lablup Inc. All rights reserved.

 The antd-parity data the Astryx theme recipe (`react/src/astryx-theme`) is
 built from: the measured alignment tables and the BUI custom-token builder.
 What is left of the theme-shim after the `useToken()` consumers moved onto
 `useTheme()` (FR-3605).
 */
export { ANTD_ALIGN_TOKENS, ANTD_REVERSED_BAND_OVERLAYS } from './antdParity';
export {
  BAI_SELF_COLOR_TOKENS,
  buildBaiCustomTokens,
  toSeedTuple,
  type BaiCustomTokenSeeds,
  type BrandSeedPair,
} from './baiCustomTokens';
