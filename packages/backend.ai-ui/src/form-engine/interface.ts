/**
 ...
 Public type surface of the self-hosted form engine (to-astryx ticket 34).

 Shapes are chosen to be assignment-compatible with the antd / rc-field-form
 declarations the 100+ call sites were written against, so the migration is an
 import rewrite and nothing else. Where the upstream surface is measurably
 unused in this repository it is omitted rather than stubbed — see
 `MEASURED SURFACE` below.

 MEASURED SURFACE (source: answers/08 §1, a TS-AST scan of 977 files)
 --------------------------------------------------------------------
 FormInstance methods kept (13): getFieldValue, getFieldsValue, getFieldError,
   getFieldWarning, getFieldsError, setFieldValue, setFieldsValue, setFields,
   resetFields, validateFields, submit, isFieldsTouched, scrollToField.
   Also kept for internal completeness (free, used by the engine itself):
   isFieldTouched, isFieldValidating, isFieldsValidating, getFieldInstance,
   focusField.
 FormInstance methods dropped: none that the repo calls. `getInternalHooks`
   stays but is engine-private (0 external call sites).
 Rule keys kept (9): required, message, validator, type, max, min, pattern,
   warningOnly, whitespace.
 Rule keys dropped (0 call sites): enum, len, transform, defaultField, fields,
   and the date/regexp/hex/float/integer/method/array/boolean `type` values.
 Form.Item props dropped (0 call sites): normalize.
 Form.List operations dropped (0 call sites): move.
 */
import type { InternalNamePath, NamePath, Store } from './namePath';
...
export interface ValidateMessages {
  ...
  array?: RangeMessages;
  pattern?: { mismatch?: ValidateMessage };
  [key: string]: any;
}
...
export interface FormInstance<Values = any> {
  ...
  getFieldsValue: ((nameList?: NamePath[]) => Values) &
    ((nameList: true, filterFunc?: (meta: Meta) => boolean) => Values) &
    ((config: {
      strict?: boolean;
...
type ValidateFields<Values = any> = ((
  ...
  ((options?: ValidateOptions) => Promise<Values>);
...
export type NotifyInfo =
  | { type: 'valueUpdate'; source: 'internal' | 'external'; store: Store }
  | { type: 'validateFinish'; store: Store }
  | { type: 'reset'; store: Store }
  | { type: 'remove'; store: Store }
  | { type: 'setField'; data: FieldData; store: Store }
  | {
