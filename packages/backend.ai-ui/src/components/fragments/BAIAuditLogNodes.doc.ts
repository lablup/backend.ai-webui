...
export const docs = {
  ...
  usage: {
    description:
      'The shared audit-log table used by the session, vfolder and model-deployment detail surfaces. It reads the plural Relay fragment `BAIAuditLogNodesFragment` on `AuditLogV2`, so the caller spreads that fragment on each audit-log node in its own query and passes the array as `auditLogFrgmt`; null and undefined entries are dropped and rows are keyed by `id`. Up to eleven columns are built internally — time, operation, status (a BAIAuditLogStatusBadge), description, duration, triggered-by and client IP are visible by default, while entity type, entity id, request id and action id ship as `defaultHidden`. The client IP column is rendered only when the connected client reports the `client-ip-of-audit-log` capability (manager 26.9.0), so the component must sit under BAIClientProvider; the value is shown exactly as the manager returns it, since the server applies the client IP masking policy, so the cell may hold a masked address or `-` when the policy records none. Filtering, pagination and refetch stay in the consuming surface; the component renders BAITable with resizing on and size "small", and every BAITable prop except `dataSource`, `columns` and `onChangeOrder` passes through.',
    bestPractices: [
      {
        guidance: true,
        description:
