/**
 ...
 PILOT-DECISIONs:
  ...
  - The antd original's explicit `onChange?: (value, option: any) => void`
    ...
    used by every other Astryx sibling (`BAIUserSelectAstryx` included) — no
...
*/
import { BAIAdminSessionSelectAstryxPaginatedQuery } from '../../__generated__/BAIAdminSessionSelectAstryxPaginatedQuery.graphql';
import { BAIAdminSessionSelectAstryxValueQuery } from '../../__generated__/BAIAdminSessionSelectAstryxValueQuery.graphql';
...
export type AstryxAdminSessionNode = NonNullable<
  NonNullable<
    BAIAdminSessionSelectAstryxPaginatedQuery['response']['adminSessionsV2']
  >['edges'][number]
>['node'];

export interface BAIAdminSessionSelectAstryxRef {
  refetch: () => void;
}

export interface BAIAdminSessionSelectAstryxProps extends Omit<
...
> {
  ...
  ref?: React.Ref<BAIAdminSessionSelectAstryxRef>;
}

const BAIAdminSessionSelectAstryx: React.FC<
...
> = ({ multiple = false, isLoading, ref, ...selectProps }) => {
  ...
  const { adminSessionsV2: selectedSessionNodes } =
    useLazyLoadQuery<BAIAdminSessionSelectAstryxValueQuery>(
      graphql`
        query BAIAdminSessionSelectAstryxValueQuery(
  ...
  const { paginationData, result, loadNext, isLoadingNext } =
    useLazyPaginatedQuery<
      BAIAdminSessionSelectAstryxPaginatedQuery,
      AstryxAdminSessionNode
    >(
      graphql`
        query BAIAdminSessionSelectAstryxPaginatedQuery(
          $offset: Int!
          $limit: Int!
          $filter: SessionV2Filter
        ) {
...
export default BAIAdminSessionSelectAstryx;
