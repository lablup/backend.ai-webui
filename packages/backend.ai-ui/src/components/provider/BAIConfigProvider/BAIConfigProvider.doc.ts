...
export const docs = {
  ...
  usage: {
    description:
      "The root provider every `backend.ai-ui` consumer mounts once, at the top of the app. It wires four things: a shared TanStack `QueryClientProvider` (created at module scope, with `refetchOnWindowFocus` and `retry` disabled), Astryx's `InternationalizationProvider` (locale, `dir` from `getLocaleDirection`, and the `@astryx.*` and ui-common `uic.*` strings carried on `locale.astryxLocale`), BUI's own i18next instance plus the dayjs locale — both re-synced in an effect whenever `locale.lang` changes — and, when the client props are present, `BAIClientProvider`. It deliberately does not render an i18next React Context: BUI components bind to BUI's instance explicitly through `useBAIi18n()`, so the host's own i18n context is left untouched. Its props are standalone — there is no underlying component to pass extra props through to.",
    bestPractices: [
      {
        guidance: true,
        description:
