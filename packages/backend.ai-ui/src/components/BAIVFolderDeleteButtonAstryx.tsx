 ...
 **Native** rebuild: the only consumer in the pilot graph is the page's bulk
 ...
 `...BAIVFolderDeleteButtonAstryxFragment` instead of BUI's. The selection set
...
export interface BAIVFolderDeleteButtonAstryxProps {
  vfolderFrgmt: BAIVFolderDeleteButtonAstryxFragment$key;
...
const BAIVFolderDeleteButtonAstryx: React.FC<
  BAIVFolderDeleteButtonAstryxProps
> = ({ vfolderFrgmt, label, tooltip, isDisabled, onClick, size = 'md' }) => {
  'use memo';
  const vfolders = useFragment<BAIVFolderDeleteButtonAstryxFragment$key>(
    graphql`
...
export default BAIVFolderDeleteButtonAstryx;
