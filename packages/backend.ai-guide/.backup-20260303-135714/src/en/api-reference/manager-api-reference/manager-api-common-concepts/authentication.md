...
To make requests to the API server, an API access key and a secret key pair are required. These credentials can be obtained from the [Backend.AI cloud service](https://cloud.backend.ai/) or from the administrator of the Backend.AI cluster.

\
The server uses the API access key to identify each client and the secret key to verify the integrity of API requests and authenticate clients.
...
For local deployments, a master dummy key pair can be created in the configuration file.
...
| HTTP Headers | Values |
| --- | --- |
| Status code | API-specific HTTP-standard status codes. Responses commonly used throughout all APIs include 200, 201, 204, 400, 401, 403, 404, 429, and 500, but not limited to. |
...
The following is a Python code that derives the signing key from the secret key. The key is nestedly signed against the current date (without time) and the API endpoint address.
...
The success example in [Example Requests and Responses](authentication.md#example-requests-and-responses) makes a string to sign as follows (where the newlines are `"\n"`):
...
<pre data-overflow="wrap"><code><strong>import hashlib, hmac
</strong>
str_to_sign = 'GET\n/v2...'
...
### Example: Success checking the latest API version
...
### Example: Failure with a missing authorization header
...
<pre data-overflow="wrap"><code><strong>HTTP/1.1 401 Unauthorized
</strong>Content-Type: application/problem+json
Content-Language: en
Content-Length: 139
X-RateLimit-Limit: 2000
