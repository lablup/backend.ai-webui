...
/** Only `search` spans domains; `docs` and `schema` each fix their own. */
export const DOMAIN_FLAG: FlagSpec = {
  flag: '--domain <name>',
...
/** What a single-domain search surface takes: everything but `--domain`. */
export const SCOPED_SEARCH_FLAGS: FlagSpec[] = [
  {
    flag: '--limit <n>',
...
export const SEARCH_FLAGS: FlagSpec[] = [DOMAIN_FLAG, ...SCOPED_SEARCH_FLAGS];
