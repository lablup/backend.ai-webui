...
/** How many releases the automatic lookup ranks, newest-created first. */
...
export interface ResolvedRelease {
  tag: string;
  /** `flag` when `--tag` named it, `latest` when the CLI ranked for it. */
...
/**
 * The highest version among the published releases — deliberately NOT GitHub's
 * `releases/latest`, which is the `Latest` badge: the backend moves that with
 * whichever maintenance branch shipped last, so it answered 26.4.10 while
 * 26.8.1 was out, and syncing to it downgrades the SDL.
 */
export function highestRelease(
  releases: ReleasePayload[],
): ReleasePayload | undefined {
  let best: ReleasePayload | undefined;
  let bestTag = '';
  for (const release of releases) {
    // A pre-release is never picked automatically; `--tag` still reaches one.
    if (release.draft || release.prerelease) continue;
    const tag = release.tag_name?.trim();
    if (!tag) continue;
    if (!best || compareVersions(tag, bestTag) > 0) {
      best = release;
      bestTag = tag;
    }
  }
...
/**
 * Resolve `--tag` (or the highest published release) to the
 * `supergraph.graphql` asset. Two calls at most, so an unauthenticated run
 * stays well inside the anonymous rate limit.
 */
export async function resolveRelease(
  ...
  const url = tag
    ...
    : `${GITHUB_API}/repos/${BACKEND_REPO}/releases?per_page=${RELEASE_PAGE_SIZE}`;
  ...
  const body = (await response.json()) as ReleasePayload | ReleasePayload[];
  const payload = Array.isArray(body) ? highestRelease(body) : body;
  if (!payload) {
    throw new CliError(
      'not_found',
      `${BACKEND_REPO} has published no release the CLI can rank.`,
      { hint: `${CLI_NAME} schema sync --tag <tag> --dry-run` },
    );
  }
