...
// The `Client` constructor reads `localStorage` (session id, log stack), which
// is absent in vitest's default `node` environment. Provide a minimal
// in-memory stub so the client can be instantiated without jsdom.
beforeAll(() => {
