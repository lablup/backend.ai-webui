...
3. Apply context filters to cut false positives. Drive them off the entry's `context` field plus the `conceptId`, not off hardcoded literals:
   ...
   - `organization` (entry `context: "for the Backend.AI domain concept"`, `conceptId: domain`) — only flag when the word stands in for the Backend.AI **domain** hierarchy unit (e.g. "users in the same organization share resource policies", "organization admin"). Skip references to the customer's real-world company/institution: possessive forms (`your organization's`, `the organization's` hardware/policy/identity), SSO/IdP prose (SAML, OIDC login), corporate-branding prose, the model-card `Author` field ("creator or organization"), and everything in `license_agreement/` (legal text). These were the recurring 13 false positives in the 2026-07-06 run.
