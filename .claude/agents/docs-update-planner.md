...
**Rules:**
- Lowercase kebab-case: `file-editing`, `session-launcher`, `vfolder-sharing`
- 2-4 words maximum
- Derive from feature name, PR title, or user description
- Examples:
  - PR "feat(FR-2050): add bulk delete for vfolders" → `vfolder-bulk-delete`
  - "document the new file editing feature" → `file-editing`
  - PR stack about model serving → `model-serving`

### Step 6: Create the Documentation Update Plan

Produce a structured plan saved as a Markdown file at `packages/backend.ai-webui-docs/.agent-output/docs-update-plan-{topic}.md` (where `{topic}` is the slug from Step 5.5):
...
8. **Content Exclusions (FR-3480, PR #8619)**: Never plan documentation for topics deliberately removed from the manual: auto-refresh controls / default refresh intervals on list pages (only genuinely feature-specific polling like the deployment revision apply's 5s polling is allowed), view-state persistence boilerplate (filters/sort/pagination in the URL, surviving reload, or per-tab state being restored when switching tabs and back), and self-evident table expand-mode / table-state enumerations (including the Expand options kebab menu and its per-user mode memory). These are discoverable from the UI and were judged not worth manual space — planning them forces the same removal work again. The docs-update-writer carries the matching skip rule.
...
Ensure the output directory exists: `mkdir -p packages/backend.ai-webui-docs/.agent-output/`

Save the plan to `packages/backend.ai-webui-docs/.agent-output/docs-update-plan-{topic}.md` (using the topic slug from Step 5.5) and present a summary to the user, including the exact output filename so downstream agents can find it.
