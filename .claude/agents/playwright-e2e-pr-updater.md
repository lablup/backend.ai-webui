...
description: Record e2e test cases as GIFs and attach them to a GitHub PR description. Records one GIF per test via Playwright video, uploads them to GitHub CDN via Chrome DevTools, and updates the PR body with a GIF table. Invoke when a PR needs visual proof of its e2e tests.
