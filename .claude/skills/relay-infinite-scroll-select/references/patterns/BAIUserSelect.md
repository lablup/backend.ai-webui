...
This component demonstrates **Pattern B (Dynamic)**: Using `useLazyLoadQuery` + `useLazyPaginatedQuery` for infinite scroll with **full control over query parameters**. Uses email as value/label, but the pattern works with any value type (name, id, row_id, etc.) when dynamic control is needed.

## Key Characteristics

- **Value Type**: `email` (String) - but pattern works with any type
...
- **Search**: Optimistic updates with `useOptimistic`
- **Complexity**: 🟡 Moderate
- **Key Features**:
...
const BAIUserSelect: React.FC<BAIUserSelectProps> = ({
  ...
  const [optimisticSearchStr, setOptimisticSearchStr] = useOptimistic(
    searchStr,
    (_s, newS: string) => newS,
  ); // 🔑 Optimistic search for immediate UI feedback
  ...
  const { user_nodes: selectedUserNodes } =
    useLazyLoadQuery<BAIUserSelectValueQuery>(
      ...
      `,
      {
        ...
        first: _.castArray(deferredControllableValue).length, // 🔑 Dynamic first
        skipSelected: _.isEmpty(deferredControllableValue),
      },
      {
        fetchPolicy: !_.isEmpty(deferredControllableValue)
    ...
    useLazyPaginatedQuery<BAIUserSelectPaginatedQuery, UserNode>(
      ...
      { limit: 10 },
      {
        filter: mergeFilterValues([
          mergedFilter,
          searchStr ? `email ilike "%${searchStr}%"` : null, // 🔑 Use actual state in query
        ]),
        order: 'email',
      },
      {
  ...
  return (
    <BAISelect
      ...
      searchAction={async (value) => {
        setOptimisticSearchStr(value); // 🔑 Set optimistic value first
        await selectProps.searchAction?.(value);
        setSearchStr(value); // 🔑 Set actual value after
      }}
      showSearch={{
  ...
  [
    !_.isEmpty(deferredControllableValue)
      ? mergeFilterValues(
          ...
          '|', // OR operator for multiple emails
        )
      : null,
...
### 3. Value-to-Label Mapping (Simplified)
...
- **Simpler than Pattern B (ID-based)**: No Global ID conversion needed
- **Maintain selection order**: Map in the same order as `deferredControllableValue`
- **Fallback**: Use email as label if query not yet resolved
...
**Key Points:**
- Use `_.castArray()` for uniform handling
...
**Note:** No need for `searchStr !== optimisticSearchStr` comparison - `useOptimistic` handles search UI updates automatically through BAISelect's internal transition.
...
✅ **Simple value model**: Email is both value and label
✅ **Dynamic first parameter**: Always fetch complete data
✅ **No ID conversion**: No Global ID complexity
...
|---------|-------------------|--------------------------|----------------------------|
| Value Type | Name | Email | ID / row_id |
...
### 3. Optimistic Search Updates
```typescript
// ✅ Good: Optimistic search for immediate UI feedback
const [searchStr, setSearchStr] = useState<string>();
const [optimisticSearchStr, setOptimisticSearchStr] = useOptimistic(
  searchStr,
  (_s, newS: string) => newS,
...
// Use actual searchStr in query filter
filter: mergeFilterValues([
  mergedFilter,
  searchStr ? `email ilike "%${searchStr}%"` : null,
])

// Set optimistic value first, actual value after
searchAction={async (value) => {
  setOptimisticSearchStr(value);  // Optimistic - immediate UI update
  await selectProps.searchAction?.(value);
  setSearchStr(value);  // Actual state - triggers query
}}
...
useLazyLoadQuery(query, {
  filter: actualSearchStr,  // Not deferred for search
  fetchKey: deferredFetchKey,  // Deferred for refetch
})

// ❌ Bad: Not using useOptimistic for search
filter: mergeFilterValues([
  mergedFilter,
