...
| **`MediaTheme` leaking into a `<dialog>`** — **fixed for modals by FR-3578** | Astryx `Dialog` is a native, non-portalled `<dialog>`, so the top layer does not change DOM ancestry and a `MediaTheme` ancestor keeps forcing its surface tokens inside every modal below it. **Modals no longer hit this**: they render through `BAIDialogPortal` into `document.body` and escape the band. The lab `Drawer` is still native, so it does. | For a drawer: scope the `MediaTheme` to the content actually on the band, and do not wrap it in a counter-`MediaTheme` (that declares *surface luminance*, not app mode). Never re-introduce a native `<dialog>` modal — see the next row. |
| **Assuming a modal is a native `<dialog>`** | FR-3578 moved every modal to `BAIDialogPortal` (portal → `document.body`), because `showModal()` marks the whole document inert and made notifications unclickable. So `document.querySelector('dialog[open]')` no longer finds a modal, and nothing about a modal is in the CSS top layer. | Query open modals through `react/src/helper/openModalRoot.ts` (`[data-bai-modal-open]`, plus `dialog[open]` for drawers). Stacking is a **z-index** question now: take a named layer from `packages/backend.ai-ui/src/styles/zIndexLadder.ts`, never a literal. A portalled modal also loses any *nested* theme wrapper it used to inherit — `BAIDialogPortal` re-emits the nearest theme's name for that reason. |
