...
`,is:"paper-dialog-scrollable",properties:{dialogElement:{type:Object}},get scrollTarget(){return this.$.scrollable},ready:function(){this._ensureTarget();this.classList.add("no-padding")},attached:function(){this._ensureTarget();requestAnimationFrame(this.updateScrollState.bind(this))},updateScrollState:function(){this.toggleClass("is-scrolled",0<this.scrollTarget.scrollTop);this.toggleClass("can-scroll",this.scrollTarget.offsetHeight<this.scrollTarget.scrollHeight);this.toggleClass("scrolled-to-bottom",this.scrollTarget.scrollTop+this.scrollTarget.offsetHeight>=this.scrollTarget.scrollHeight)},_ensureTarget:function(){this.dialogElement=this.dialogElement||this.parentElement;if(this.dialogElement&&this.dialogElement.behaviors&&0<=this.dialogElement.behaviors.indexOf(_backendAiConsole.PaperDialogBehaviorImpl)){this.dialogElement.sizingTarget=this.scrollTarget;this.scrollTarget.classList.remove("fit")}else if(this.dialogElement){this.scrollTarget.classList.add("fit")}}});const template$1=_backendAiConsole.html$1`
  <style>
    :host {
      @apply --layout;
...
`}static get is(){return"vaadin-dialog"}static get version(){return"2.2.1"}static get properties(){return{opened:{type:Boolean,value:!1,notify:!0},noCloseOnOutsideClick:{type:Boolean,value:!1},noCloseOnEsc:{type:Boolean,value:!1},ariaLabel:{type:String},theme:String,_contentTemplate:Object,renderer:Function,_oldTemplate:Object,_oldRenderer:Object}}static get observers(){return["_openedChanged(opened)","_ariaLabelChanged(ariaLabel)","_templateOrRendererChanged(_contentTemplate, renderer)"]}ready(){super.ready();this.$.overlay.setAttribute("role","dialog");this.$.overlay.addEventListener("vaadin-overlay-outside-click",this._handleOutsideClick.bind(this));this.$.overlay.addEventListener("vaadin-overlay-escape-press",this._handleEscPress.bind(this));this._observer=new _backendAiConsole.FlattenedNodesObserver(this,info=>{this._setTemplateFromNodes(info.addedNodes)})}_setTemplateFromNodes(nodes){this._contentTemplate=nodes.filter(node=>node.localName&&"template"===node.localName)[0]||this._contentTemplate}_removeNewRendererOrTemplate(template,oldTemplate,renderer,oldRenderer){if(template!==oldTemplate){this._contentTemplate=void 0}else if(renderer!==oldRenderer){this.renderer=void 0}}render(){this.$.overlay.render()}_templateOrRendererChanged(template,renderer){if(template&&renderer){this._removeNewRendererOrTemplate(template,this._oldTemplate,renderer,this._oldRenderer);throw new Error("You should only use either a renderer or a template for dialog content")}this._oldTemplate=template;this._oldRenderer=renderer;if(renderer){this.$.overlay.setProperties({owner:this,renderer:renderer})}}disconnectedCallback(){super.disconnectedCallback();this.opened=!1}_openedChanged(opened){if(opened){this.$.overlay.template=this.querySelector("template")}this.$.overlay.opened=opened}_ariaLabelChanged(ariaLabel){if(ariaLabel!==void 0&&null!==ariaLabel){this.$.overlay.setAttribute("aria-label",ariaLabel)}else{this.$.overlay.removeAttribute("aria-label")}}_onOverlayOpened(e){if(!1===e.detail.value){this.opened=!1}}_handleOutsideClick(e){if(this.noCloseOnOutsideClick){e.preventDefault()}}_handleEscPress(e){if(this.noCloseOnEsc){e.preventDefault()}}}_exports.DialogElement=DialogElement;customElements.define(DialogElement.is,DialogElement);var vaadinDialog={DialogElement:DialogElement};_exports.$vaadinDialog=vaadinDialog;const $_documentContainer$1=document.createElement("template");$_documentContainer$1.innerHTML=`<dom-module id="lumo-overlay">
  ...
  <template>
    ...
    `}}customElements.define(BackendAIIndicator.is,BackendAIIndicator);class BackendAIJobList extends _backendAiConsole.PolymerElement{static get is(){return"backend-ai-job-list"}static get properties(){return{active:{type:Boolean,value:!1},condition:{type:String,default:"running"},jobs:{type:Object,value:{}},compute_sessions:{type:Object,value:{}},terminationQueue:{type:Array,value:[]},filterAccessKey:{type:String,value:""}}}ready(){super.ready();this.refreshTimer=null;if("running"!==this.condition||!window.backendaiclient||!window.backendaiclient.is_admin){this.$["access-key-filter"].parentNode.removeChild(this.$["access-key-filter"])}}connectedCallback(){super.connectedCallback();(0,_backendAiConsole.afterNextRender)(this,function(){})}shouldUpdate(){return this.active}static get observers(){return["_menuChanged(active)"]}is_admin(){return window.backendaiclient.is_admin}_menuChanged(active){if(!active){return}if(window.backendaiclient==void 0||null==window.backendaiclient){document.addEventListener("backend-ai-connected",()=>{this._refreshJobData()},!0)}else{this._refreshJobData()}}refreshList(){return this._refreshJobData()}_refreshJobData(){this.shadowRoot.querySelector("#loading-indicator").show();if(!0!==this.active){return}let status="RUNNING";switch(this.condition){case"running":status="RUNNING";break;case"finished":status="TERMINATED";break;case"archived":default:status="RUNNING";}let fields=["sess_id","lang","created_at","terminated_at","status","occupied_slots","cpu_used","io_read_bytes","io_write_bytes","access_key"];window.backendaiclient.computeSession.list(fields,status,this.filterAccessKey).then(response=>{this.shadowRoot.querySelector("#loading-indicator").hide();var sessions=response.compute_sessions;if(sessions!==void 0&&0!=sessions.length){Object.keys(sessions).map((objectKey,index)=>{var session=sessions[objectKey],occupied_slots=JSON.parse(session.occupied_slots);const kernelImage=sessions[objectKey].lang.split("/")[2];sessions[objectKey].cpu_slot=parseInt(occupied_slots.cpu);sessions[objectKey].mem_slot=parseFloat(window.backendaiclient.utils.changeBinaryUnit(occupied_slots.mem,"g"));if("cuda.device"in occupied_slots){sessions[objectKey].gpu_slot=parseInt(occupied_slots["cuda.device"])}if("cuda.shares"in occupied_slots){sessions[objectKey].vgpu_slot=parseFloat(occupied_slots["cuda.shares"])}sessions[objectKey].kernel_image=kernelImage})}this.compute_sessions=sessions;let refreshTime;if(!0===this.active){if("running"===this.condition){refreshTime=5e3;this.refreshTimer=setTimeout(()=>{this._refreshJobData(status)},refreshTime)}else{refreshTime=15e3}}}).catch(err=>{console.log(err);if(err&&err.message){this.$.notification.text=err.message;this.$.notification.show()}})}_startProgressDialog(){this.$["app-progress"].value=0;this.$["app-progress-text"].textContent="Initializing...";this.$["app-progress-dialog"].open()}_setProgressDialog(value,text=""){this.$["app-progress-text"].textContent=text;this.$["app-progress"].value=value}_endProgressDialog(){this.$["app-progress-dialog"].close()}_isRunning(){return"running"===this.condition}_humanReadableTime(d){var d=new Date(d);return d.toLocaleString()}_isAppRunning(lang){if("running"!=this.condition)return!1;let support_kernels=["python","python-tensorflow","python-pytorch","ngc-digits","ngc-tensorflow","ngc-pytorch","julia","r"];lang=lang.split("/")[2].split(":")[0];return"running"===this.condition&&support_kernels.includes(lang)}_byteToMB(value){return _Mathfloor(value/1e6)}_byteToGB(value){return _Mathfloor(value/1e9)}_MBToGB(value){return value/1024}_msecToSec(value){return(+(value/1e3)).toFixed(2)}_elapsed(start,end){return window.backendaiclient.utils.elapsedTime(start,end)}_indexFrom1(index){return index+1}_terminateKernel(e){const termButton=e.target,controls=e.target.closest("#controls"),kernelId=controls.kernelId;if(this.terminationQueue.includes(kernelId)){this.$.notification.text="Already terminating the session.";this.$.notification.show();return!1}this.$.notification.text="Terminating session...";this.$.notification.show();this.terminationQueue.push(kernelId);this._terminateApp(kernelId).then(()=>{window.backendaiclient.destroyKernel(kernelId).then(req=>{setTimeout(()=>{this.terminationQueue=[];this.refreshList()},1e3)}).catch(err=>{this.$.notification.text="Problem occurred during termination.";this.$.notification.show()})}).catch(err=>{console.log(err);if(err&&err.message){this.$.notification.text=err.message;this.$.notification.show()}})}sendRequest(rqst){return babelHelpers.asyncToGenerator(function*(){let resp,body;try{if("GET"==rqst.method){rqst.body=void 0}resp=yield fetch(rqst.uri,rqst);let contentType=resp.headers.get("Content-Type");if(contentType.startsWith("application/json")||contentType.startsWith("application/problem+json")){body=yield resp.json()}else if(contentType.startsWith("text/")){body=yield resp.text()}else{if(resp.blob===void 0)body=yield resp.buffer();else body=yield resp.blob()}if(!resp.ok){throw body}}catch(e){console.log(e)}return body})()}_terminateApp(kernelId){let accessKey=window.backendaiclient._config.accessKey,rqst={method:"GET",uri:window.backendaiclient._config.proxyURL+"proxy/"+accessKey+"/"+kernelId};return this.sendRequest(rqst).then(response=>{let accessKey=window.backendaiclient._config.accessKey;if(response!==void 0&&404!==response.code){let rqst={method:"GET",uri:window.backendaiclient._config.proxyURL+"proxy/"+accessKey+"/"+kernelId+"/delete"};return this.sendRequest(rqst)}}).catch(err=>{console.log(err);if(err&&err.message){this.$.notification.text=err.message;this.$.notification.show()}})}_showLogs(e){const controls=e.target.closest("#controls"),kernelId=controls.kernelId;window.backendaiclient.getLogs(kernelId).then(req=>{setTimeout(()=>{this.$["work-title"].innerHTML=`${kernelId}`;this.$["work-area"].innerHTML=`<pre>${req.result.logs}</pre>`||"No logs.";this.$["work-dialog"].open()},100)}).catch(err=>{if(err&&err.message){this.$.notification.text=err.message;this.$.notification.show()}else if(err&&err.title){this.$.notification.text=err.title;this.$.notification.show()}})}_open_wsproxy(kernelId,app="jupyter"){var _this=this;return babelHelpers.asyncToGenerator(function*(){if(window.backendaiclient==void 0||null==window.backendaiclient){return!1}if(window.backendaiclient._config.proxyURL===void 0){window.backendaiclient._config.proxyURL="http://127.0.0.1:5050/"}let param={access_key:window.backendaiclient._config.accessKey,secret_key:window.backendaiclient._config.secretKey,endpoint:window.backendaiclient._config.endpoint},rqst={method:"PUT",body:JSON.stringify(param),headers:{Accept:"application/json","Content-Type":"application/json"},uri:window.backendaiclient._config.proxyURL+"conf"};try{let response=yield _this.sendRequest(rqst),token=response.token;_this.$.indicator.set(50,"Adding kernel to socket queue...");rqst={method:"GET",app:app,uri:window.backendaiclient._config.proxyURL+"proxy/"+token+"/"+kernelId+"/add"};return yield _this.sendRequest(rqst)}catch(err){throw err}})()}_runJupyter(e){const termButton=e.target,controls=e.target.closest("#controls"),kernelId=controls.kernelId;if(window.backendaiwsproxy==void 0||null==window.backendaiwsproxy){this.$.indicator.start();this._open_wsproxy(kernelId,"jupyter").then(response=>{if(response.url){this.$.indicator.set(100,"Prepared.");setTimeout(()=>{window.open(response.url+"&redirect=/tree","_blank");this.$.indicator.end();console.log("Jupyter proxy loaded: ");console.log(kernelId)},1e3)}})}}_runJupyterLab(e){const termButton=e.target,controls=e.target.closest("#controls"),kernelId=controls.kernelId;if(window.backendaiwsproxy==void 0||null==window.backendaiwsproxy){this.$.indicator.start();this._open_wsproxy(kernelId,"jupyterlab").then(response=>{if(response.url){this.$.indicator.set(100,"Prepared.");setTimeout(()=>{window.open(response.url+"&redirect=/lab","_blank");this.$.indicator.end();console.log("Jupyter proxy loaded: ");console.log(kernelId)},1e3)}})}}_runJupyterTerminal(e){const termButton=e.target,controls=e.target.closest("#controls"),kernelId=controls.kernelId;let accessKey=window.backendaiclient._config.accessKey;if(window.backendaiwsproxy==void 0||null==window.backendaiwsproxy){this.$.indicator.start();this._open_wsproxy(kernelId,"jupyter").then(response=>{if(response.url){this.$.indicator.set(100,"Prepared.");setTimeout(()=>{window.open(response.url+"&redirect=/terminals/1","_blank");this.$.indicator.end();console.log("Jupyter proxy loaded: ");console.log(kernelId)},1e3)}})}}_updateFilterAccessKey(e){this.filterAccessKey=e.target.value;if(this.refreshTimer){clearTimeout(this.refreshTimer);this._refreshJobData()}}static get template(){return _backendAiConsole.html`
    ...
    `}}customElements.define(BackendAIDropdownMenu.is,BackendAIDropdownMenu);class BackendAIJobView extends(0,_backendAiConsole.OverlayPatchMixin)(_backendAiConsole.PolymerElement){static get is(){return"backend-ai-job-view"}constructor(){super();(0,_backendAiConsole.setPassiveTouchGestures)(!0)}static get properties(){return{active:{type:Boolean,value:!1},supports:{type:Object,value:{}},resourceLimits:{type:Object,value:{}},userResourceLimit:{type:Object,value:{}},aliases:{type:Object,value:{TensorFlow:"python-tensorflow",Python:"python",PyTorch:"python-pytorch",Chainer:"chainer",R:"r",Julia:"julia",Lua:"lua"}},versions:{type:Array,value:["3.6"]},languages:{type:Array,value:[]},gpu_mode:{type:String,value:"gpu"},gpu_step:{type:Number,value:.05},cpu_metric:{type:Object,value:{min:"1",max:"1"}},mem_metric:{type:Object,value:{min:"1",max:"1"}},gpu_metric:{type:Object,value:{min:"0",max:"0"}},tpu_metric:{type:Object,value:{min:"1",max:"1"}},images:{type:Object,value:{}}}}ready(){super.ready();this.$["launch-session"].addEventListener("tap",this._launchSessionDialog.bind(this));this.$["launch-button"].addEventListener("tap",this._newSession.bind(this));this.$.environment.addEventListener("selected-item-label-changed",this.updateLanguage.bind(this));this.$.version.addEventListener("selected-item-label-changed",this.updateMetric.bind(this));this._initAliases();var gpu_resource=this.$["gpu-resource"];gpu_resource.addEventListener("value-change",()=>{if(0<gpu_resource.value){this.$["use-gpu-checkbox"].checked=!0}else{this.$["use-gpu-checkbox"].checked=!1}});this.$["use-gpu-checkbox"].addEventListener("change",()=>{if(!0===this.$["use-gpu-checkbox"].checked){this.$["gpu-resource"].disabled=!1}else{this.$["gpu-resource"].disabled=!0}})}_initAliases(){for(let item in this.aliases){this.aliases[this.aliases[item]]=item}}connectedCallback(){super.connectedCallback();(0,_backendAiConsole.afterNextRender)(this,function(){})}shouldUpdate(){return this.active}static get observers(){return["_menuChanged(active)"]}_menuChanged(active){if(!active){this.$["running-jobs"].active=!1;this.$["finished-jobs"].active=!1;return}this.$["running-jobs"].active=!0;this.$["finished-jobs"].active=!0;if(window.backendaiclient==void 0||null==window.backendaiclient){document.addEventListener("backend-ai-connected",()=>{this._refreshResourcePolicy()},!0)}else{this._refreshResourcePolicy()}}_refreshResourcePolicy(){window.backendaiclient.keypair.info(window.backendaiclient._config.accessKey,["resource_policy"]).then(response=>{let policyName=response.keypair.resource_policy;if("student"===policyName){return new Promise(function(resolve,reject){var resource={cpu:4,mem:"8g","cuda.shares":.25},result={keypair_resource_policy:{default_for_unspecified:"LIMITED",total_resource_slots:JSON.stringify(resource),max_concurrent_sessions:3,max_containers_per_session:1}};resolve(result)})}else{return window.backendaiclient.resourcePolicy.get(policyName,["default_for_unspecified","total_resource_slots","max_concurrent_sessions","max_containers_per_session"])}}).then(response=>{let resource_policy=response.keypair_resource_policy;if("UNLIMITED"===resource_policy.default_for_unspecified){}this.userResourceLimit=JSON.parse(response.keypair_resource_policy.total_resource_slots);this._refreshResourceValues()}).catch(err=>{console.log(err);if(err&&err.message){this.$.notification.text=err.message;this.$.notification.show()}else if(err&&err.title){this.$.notification.text=err.title;this.$.notification.show()}})}_refreshResourceValues(){this._refreshImageList();this._updateGPUMode();this._updateVirtualFolderList();this.updateMetric();var cpu_resource=this.$["cpu-resource"],ram_resource=this.$["ram-resource"],gpu_resource=this.$["gpu-resource"]}_launchSessionDialog(){var gpu_resource=this.$["gpu-resource"];if(0<gpu_resource.value){this.$["use-gpu-checkbox"].checked=!0}else{this.$["use-gpu-checkbox"].checked=!1}this.$["new-session-dialog"].open()}_updateGPUMode(){window.backendaiclient.getResourceSlots().then(response=>{let results=response;if("cuda.device"in results){this.gpu_mode="gpu";this.gpu_step=1;console.log("gpu")}if("cuda.shares"in results){this.gpu_mode="vgpu";this.gpu_step=.05;console.log("vgpu")}})}_generateKernelIndex(kernel,version){if(kernel in this.aliases){return this.aliases[kernel]+":"+version}return kernel+":"+version}_newSession(){let kernel=this.$.environment.value,version=this.$.version.value,sessionName=this.$["session-name"].value,vfolder=this.$.vfolder.selectedValues,config={};config.cpu=this.$["cpu-resource"].value;if("vgpu"==this.gpu_mode){config.vgpu=this.$["gpu-resource"].value}else{config.gpu=this.$["gpu-resource"].value}config.mem=this.$["ram-resource"].value+""+"g";if(!0!==this.$["use-gpu-checkbox"].checked){if("vgpu"==this.gpu_mode){config.vgpu=0}else{config.gpu=0}}if(4>sessionName.length){sessionName=void 0}if(0!==vfolder.length){config.mounts=vfolder}const kernelName=this._generateKernelIndex(kernel,version);this.$["launch-button"].disabled=!0;this.$["launch-button-msg"].textContent="Preparing...";this.$.notification.text="Preparing session...";this.$.notification.show();window.backendaiclient.createKernel(kernelName,sessionName,config).then(req=>{this.$["running-jobs"].refreshList();this.$["new-session-dialog"].close();this.$["launch-button"].disabled=!1;this.$["launch-button-msg"].textContent="Launch"}).catch(err=>{console.log(err);if(err&&err.message){this.$.notification.text=err.message;this.$.notification.show()}else if(err&&err.title){this.$.notification.text=err.title;this.$.notification.show()}this.$["launch-button"].disabled=!1;this.$["launch-button-msg"].textContent="Launch"})}_guessHumanizedNames(kernelName){const candidate={cpp:"C++",gcc:"C",go:"Go",haskell:"Haskell",java:"Java",julia:"Julia",lua:"Lua","ngc-rapid":"RAPID (NGC)","ngc-digits":"DIGITS (NGC)","ngc-pytorch":"PyTorch (NGC)","ngc-tensorflow":"TensorFlow (NGC)",nodejs:"Node.js",octave:"Octave",php:"PHP",python:"Python","python-cntk":"CNTK","python-pytorch":"PyTorch","python-tensorflow":"TensorFlow","r-base":"R",rust:"Rust",scala:"Scala",scheme:"Scheme"};let humanizedName=null,matchedString="abcdefghijklmnopqrstuvwxyz1234567890!@#$%^&*()";Object.keys(candidate).forEach((item,index)=>{if(kernelName.endsWith(item)&&item.length<matchedString.length){humanizedName=candidate[item];matchedString=item}});return humanizedName}_updateEnvironment(){const langs=Object.keys(this.supports);if(langs===void 0)return;langs.sort();this.languages=[];langs.forEach((item,index)=>{if(!Object.keys(this.aliases).includes(item)){const humanizedName=this._guessHumanizedNames(item);if(null!==humanizedName){this.aliases[item]=humanizedName}}const alias=this.aliases[item];if(alias!==void 0){this.languages.push({name:item,alias:alias})}});this._initAliases()}_updateVersions(lang){if(this.aliases[lang]in this.supports){this.versions=this.supports[this.aliases[lang]];this.versions=this.versions.sort()}if(this.versions!==void 0){this.$.version.value=this.versions[0];this.updateMetric()}}_updateVirtualFolderList(){let l=window.backendaiclient.vfolder.list();l.then(value=>{this.vfolders=value})}_supportLanguages(){return Object.keys(this.supports)}_supportVersions(){let lang=this.$.environment.value;return this.supports[lang]}updateMetric(){if(this.$.environment.value in this.aliases){let currentLang=this.aliases[this.$.environment.value],currentVersion=this.$.version.value,kernelName=currentLang+":"+currentVersion,currentResource=this.resourceLimits[kernelName],userResource={};if(!currentResource)return;currentResource.forEach(item=>{if("cpu"===item.key){let cpu_metric=item;cpu_metric.min=parseInt(cpu_metric.min);if("cpu"in this.userResourceLimit){if(0!==parseInt(cpu_metric.max)){cpu_metric.max=_Mathmin(parseInt(cpu_metric.max),parseInt(this.userResourceLimit.cpu))}else{cpu_metric.max=parseInt(this.userResourceLimit.cpu)}}else{if(0!==parseInt(cpu_metric.max)){cpu_metric.max=parseInt(cpu_metric.max)}else{cpu_metric.max=4}}if(cpu_metric.min>cpu_metric.max){}this.cpu_metric=cpu_metric}if("cuda.device"===item.key){let gpu_metric=item;gpu_metric.min=parseInt(gpu_metric.min);if("cuda.device"in this.userResourceLimit){if(0!==parseInt(gpu_metric.max)){gpu_metric.max=_Mathmin(parseInt(gpu_metric.max),parseInt(this.userResourceLimit["cuda.device"]))}else{gpu_metric.max=parseInt(this.userResourceLimit["cuda.device"])}}else{if(0!==parseInt(gpu_metric.max)){gpu_metric.max=parseInt(gpu_metric.max)}else{gpu_metric.max=0}}if(gpu_metric.min>gpu_metric.max){}this.gpu_metric=gpu_metric}if("cuda.shares"===item.key){let vgpu_metric=item;console.log("LETS VGPU",item);console.log(this.userResourceLimit["cuda.shares"]);vgpu_metric.min=parseInt(vgpu_metric.min);if("cuda.shares"in this.userResourceLimit){if(0!==parseInt(vgpu_metric.max)){vgpu_metric.max=_Mathmin(parseFloat(vgpu_metric.max),parseFloat(this.userResourceLimit["cuda.shares"]))}else{vgpu_metric.max=parseInt(this.userResourceLimit["cuda.shares"])}}else{if(0!==parseInt(vgpu_metric.max)){vgpu_metric.max=parseInt(vgpu_metric.max)}else{vgpu_metric.max=0}}if(vgpu_metric.min>vgpu_metric.max){}this.vgpu_metric=vgpu_metric;if(0<vgpu_metric.max){this.gpu_metric=vgpu_metric}}if("tpu"===item.key){let tpu_metric=item;tpu_metric.min=parseInt(tpu_metric.min);tpu_metric.max=parseInt(tpu_metric.max);if(tpu_metric.min>tpu_metric.max){}this.tpu_metric=tpu_metric}if("mem"===item.key){let mem_metric=item;mem_metric.min=window.window.backendaiclient.utils.changeBinaryUnit(mem_metric.min,"g","g");let image_mem_max=window.window.backendaiclient.utils.changeBinaryUnit(mem_metric.max,"g","g");if("mem"in this.userResourceLimit){let user_mem_max=window.window.backendaiclient.utils.changeBinaryUnit(this.userResourceLimit.mem,"g","g");if(0!==parseInt(image_mem_max)){mem_metric.max=_Mathmin(parseFloat(image_mem_max),parseFloat(user_mem_max))}else{mem_metric.max=parseFloat(user_mem_max)}}else{if(0!==parseInt(mem_metric.max)){mem_metric.max=parseFloat(window.window.backendaiclient.utils.changeBinaryUnit(mem_metric.max,"g","g"))}else{mem_metric.max=8}}if(mem_metric.min>mem_metric.max){}this.mem_metric=mem_metric}});if({}===this.gpu_metric){this.gpu_metric={min:0,max:0};this.$["use-gpu-checkbox"].checked=!1;this.$["gpu-resource"].disabled=!0;this.$["gpu-resource"].value=0}else{this.$["use-gpu-checkbox"].checked=!0;this.$["gpu-resource"].disabled=!1;this.$["gpu-resource"].value=this.gpu_metric.max}}}updateLanguage(){this._updateVersions(this.$.environment.selectedItemLabel)}_refreshImageList(){const fields=["name","humanized_name","tag","registry","digest","installed","resource_limits { key min max }"];window.backendaiclient.image.list(fields).then(response=>{const images=[];Object.keys(response.images).map((objectKey,index)=>{const item=response.images[objectKey];if(!0===item.installed){images.push(item)}});if(0===images.length){return}this.images=images;this.supports={};Object.keys(this.images).map((objectKey,index)=>{const item=this.images[objectKey],supportsKey=`${item.registry}/${item.name}`;if(!(supportsKey in this.supports)){this.supports[supportsKey]=[]}this.supports[supportsKey].push(item.tag);this.resourceLimits[`${supportsKey}:${item.tag}`]=item.resource_limits});this._updateEnvironment()}).catch(err=>{if(err&&err.message){this.$.notification.text=err.message;this.$.notification.show()}})}changed(e){console.log(e)}static get template(){return _backendAiConsole.html`
