...
## Case 3: Pretendard (vendored by script)

Hangul is served by [Pretendard](https://github.com/orioncactus/pretendard), vendored as the
`unicode-range`-sliced **variable** woff2 files its npm package ships, so a page downloads only
the slices its text uses and a Latin-only page downloads none of them.

```bash
node scripts/vendor-pretendard.mjs                # pinned default version
node scripts/vendor-pretendard.mjs --version 1.3.9
```

The script rebuilds `resources/fonts/pretendard/` from scratch: the `.woff2` slices, `OFL.txt`,
and the entry `pretendard.css`. Two deliberate rewrites happen on the way:

- the `@font-face` family is renamed from upstream's `Pretendard Variable` to `Pretendard`, so
  `theme.json` lists `Pretendard` and the auto-loader resolves `resources/fonts/pretendard/pretendard.css`;
- no `local()` source is declared — a host-installed Pretendard may be a different version, and the
  point of vendoring is host-independent rendering.

Keep `'Ubuntu', Roboto` **before** `Pretendard` in `fontFamily`. Latin then stays in Ubuntu and only
the code points Ubuntu/Roboto lack (Hangul) fall through to Pretendard.
...
| `pretendard/`   | `pretendard.css`     | variable woff2, 92 `unicode-range` slices; generated by `scripts/vendor-pretendard.mjs` |
