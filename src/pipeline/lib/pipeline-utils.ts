...
export default class PipelineUtils extends LitElement {
  ...
  static _parseTaskListInfo(rawData, scalingGroup = '') {
    ...
    if (taskNodes.length > 0) {
      taskList = taskNodes.map((task: any) => {
        ...
        const taskData: PipelineTaskDetail = JSON.parse(
          task.data,
        ...
        return {
          ...
          dependencies: getTaskNameFromNodeId(
            task.inputs?.input_1?.connections,
          ),
        } as PipelineTask;
      });
    } else {
   ...
   */
  static _stringifyPipelineInfo(
    pipelineInfo: PipelineInfo | PipelineInfoExtended,
  ) {
    if (Object.keys(pipelineInfo).length !== 0) {
      return {
        ...pipelineInfo,
        yaml:
          typeof pipelineInfo.yaml !== 'string'
            ? YAML.dump(pipelineInfo.yaml ?? {})
            : pipelineInfo.yaml,
        dataflow:
          typeof pipelineInfo.dataflow !== 'string'
            ? JSON.stringify(pipelineInfo.dataflow ?? `{}`)
            : pipelineInfo.dataflow,
      };
    } else {
      return pipelineInfo;
    }
  ...
  static _parsePipelineInfo(pipelineInfo: PipelineInfo | PipelineInfoExtended) {
    if (Object.keys(pipelineInfo).length !== 0) {
      return {
        ...
        yaml:
          typeof pipelineInfo.yaml === 'string'
            ? YAML.load(pipelineInfo.yaml === '' ? `{}` : pipelineInfo.yaml)
            : pipelineInfo.yaml,
        dataflow:
          typeof pipelineInfo.dataflow === 'string'
            ? JSON.parse(pipelineInfo.dataflow ?? `{}`)
            : pipelineInfo.dataflow,
      };
    } else {
      return pipelineInfo;
