...
export default class PipelineJobView extends BackendAIPage {
  ...
  @queryAll('vaadin-grid#pipeline-task-instance-list vaadin-grid-column')
  taskInstanceGridColumnList;
  @queryAll('vaadin-grid#pipeline-task-instance-list vaadin-grid-filter-column')
  taskInstanceGridFilterColumnList;
  @queryAll('vaadin-grid#pipeline-task-instance-list vaadin-grid-sort-column')
  taskInstanceGridSortColumnList;
  ...
  firstUpdated() {
    ...
    document.addEventListener(
      'pipeline-job-view-active-tab-change',
      async (e: any) => {
        if (e.detail) {
          this._initPipelineTaskListItem();
          const tabGroup = [
            ...this.shadowRoot.querySelector('#pipeline-job-pane').children,
          ];
          this.shadowRoot.querySelector('#pipeline-job-pane').activeIndex =
            tabGroup.map((tab) => tab.title).indexOf(e.detail.activeTab.title);
          this._showTabContent(e.detail.activeTab);
          this.pipelineJobInfo = PipelineUtils._parsePipelineInfo(
            e.detail.pipelineJob,
          );
          this.pipelineJobs = e.detail.pipelineJobs;
          this._loadTaskNodesByCurrentPipelineJob();
          await this._loadCurrentFlowData();
          PipelineUtils._loadTaskInstances(this.pipelineJobInfo.id)
            .then((res) => {
  ...
  async _refreshPipelineJob(repeat = true) {
    ...
    globalThis.backendaiclient.pipelineJob
      .info(this.pipelineJobInfo.id)
      .then((res) => {
      ...
      .then((res) => {
        ...
        this.taskInstanceList.forEach((task: PipelineTaskInstance) => {
          const updateTaskStatusEvent = new CustomEvent('update-task-status', {
            detail: {
  ...
  _loadTaskNodesByCurrentPipelineJob() {
    const dataflowObj =
      typeof this.pipelineJobInfo.dataflow === 'string'
        ? JSON.parse(this.pipelineJobInfo.dataflow)
        : this.pipelineJobInfo.dataflow;
    ...
    try {
      ...
      if (
        ...
        document.addEventListener(
          ...
          async () => {
            this.pipelineJobs =
              await globalThis.backendaiclient.pipelineJob.list();
          },
          true,
        );
      } else {
  ...
  async _loadCurrentFlowData() {
    ...
    if (
      ...
      this.pipelineJobInfo.id !== undefined
    ) {
      await this._refreshPipelineJob();
      this.requestUpdate();
    }
  ...
  async _changePipelineJob(e) {
    ...
    this._initPipelineTaskListItem();
    this.pipelineJobInfo = this.pipelineJobs.filter(
      (job) => job.name === selectedPipelineJobName,
    )[0];
    await this._loadCurrentFlowData();
  }
  ...
  _launchWorkFlowDialog(pipelineYaml: PipelineYAML) {
    const codemirror = this.shadowRoot.querySelector(
      'lablup-codemirror#workflow-file',
    );
  ...
  _setVaadinGridRenderers() {
    ...
    this.taskInstanceGridColumnList[1].renderer = (root, column, rowData) => {
      // id
      render(
        html`
    ...
    this.taskInstanceGridColumnList[2].renderer = (root, column, rowData) => {
      // status
    ...
    this.taskInstanceGridColumnList[3].renderer = (root, column, rowData) => {
      // resources
      render(
        html`
    ...
    this.taskInstanceGridColumnList[4].renderer = (root, column, rowData) => {
      // command
      render(
        html`
    ...
    // filter columns
    this.taskInstanceGridFilterColumnList[0].renderer = (
      root,
      column,
      rowData,
    ) => {
    ...
    // sortable columns
    this.taskInstanceGridSortColumnList[0].renderer = (
      root,
      column,
      rowData,
    ) => {
      ...
      <lablup-activity-panel noheader narrow autowidth>
        <div slot="message">
          <h3 class="tab horizontal center layout">
            <mwc-tab-bar
              id="pipeline-job-pane"
              @MDCTabBar:activated="${() => this._refreshViewPane()}"
            >
              <mwc-tab
                title="pipeline-job-list"
                label="Job List"
          ...
          <div id="pipeline-job-list" class="tab-content">
            <pipeline-job-list
              ?active="${this._activeTab === 'pipeline-job-list'}"
            ></pipeline-job-list>
          </div>
          <div
            id="pipeline-job-view"
            class="tab-content item card"
            style="display:none;"
          >
            <h4 class="horizontal flex center center-justified layout">
              <mwc-select
                id="pipeline-list"
                label="Pipeline Job"
                @change=${this._loadTaskNodesByCurrentPipelineJob}
              ...
              <mwc-list-item twoline>
                ...
                <span class="monospace" slot="secondary">
                  ${this.pipelineJobInfo.created_at !== ''
                    ? PipelineUtils._humanReadableTimeDuration(
                        this.pipelineJobInfo.created_at,
                        this.pipelineJobInfo.terminated_at,
                      )
                    : `-`}
                </span>
              </mwc-list-item>
              <span class="flex"></span>
              ${['WAITING', 'RUNNING', 'STOPPED'].includes(
                this.pipelineJobInfo.status,
              )
                ? html`
                    <mwc-button
                      label="Start"
                      icon="play_arrow"
                      @click="${(e) => this._toggleRunning(e)}"
                    ></mwc-button>
                  `
                : html``}
              <div id="dropdown-menu-container">
                <mwc-icon-button
                  ...
                  ?disabled="${this.pipelineJobInfo?.name === ''}"
                ></mwc-icon-button>
                <mwc-menu id="dropdown-menu">
                  <mwc-list-item
                    class="horizontal layout center"
                    @click="${() =>
                      this._launchWorkFlowDialog(this.pipelineJobInfo.yaml)}"
                  >
            ...
            <vaadin-grid
              ...
              theme="row-stripes column-borders compact wrap-cell-content ${globalThis.isDarkMode
                ? 'dark'
                : ''}"
