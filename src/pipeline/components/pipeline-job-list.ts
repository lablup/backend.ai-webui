...
export default class PipelineJobList extends BackendAIPage {
  ...
  @queryAll('vaadin-grid#pipeline-job-list vaadin-grid-column')
  pipelineJobGridColumnList;
  @queryAll('vaadin-grid#pipeline-job-list vaadin-grid-filter-column')
  pipelineJobGridFilterColumnList;
  ...
  _createTaskProgressChart() {
    PipelineUtils._loadTaskInstances(this.pipelineJobInfo.id)
      .then((res) => {
        ...
        const numActiveTasks =
          tasksGraph?.filter((task) =>
            [
              'PENDING',
              'SCHEDULED',
              'PREPARING',
        ...
        const numFinishedTasks =
          tasksGraph?.filter((task) =>
            ['TERMINATED', 'ERROR', 'CANCELLED'].includes(task.status),
          ).length ?? 0;
        this.tasksGraph = {
          labels: [`${numActiveTasks} ACTIVE`, `${numFinishedTasks} FINISHED`],
          datasets: [
            {
              data: [numActiveTasks, numFinishedTasks],
              backgroundColor: [
                // '#007ec6',
                '#97ca00',
                '#9f9f9f',
  ...
  _stopPipelineJobExecution(pipelineJob: PipelineJob) {
     ...
     *
     */
    return globalThis.backendaiclient.pipelineJob
      .stop(this.pipelineJobInfo.id)
      .then((res) => {
        console.log(res);
      })
      .catch((err) => {
  ...
  _loadPipelineJobView(pipelineJob: PipelineJob) {
    this.pipelineJobInfo = pipelineJob;
    const activeTabChangeEvent = new CustomEvent(
  ...
  _setVaadinGridRenderers() {
    ...
    this.pipelineJobGridColumnList[1].renderer = (root, column, rowData) => {
      // status
    ...
    this.pipelineJobGridColumnList[2].renderer = (root, column, rowData) => {
      // result
    ...
    this.pipelineJobGridColumnList[3].renderer = (root, column, rowData) => {
      // duration
      const createdAt = PipelineUtils._humanReadablePassedTime(
        rowData.item.created_at,
      );
      const duration = PipelineUtils._humanReadableTimeDuration(
        rowData.item.created_at,
        rowData.item.terminated_at,
      );
      render(
        html`
          <mwc-list-item
    ...
    this.pipelineJobGridColumnList[4].renderer = (root, column, rowData) => {
      // control
      const isFinished = ['SUCCESS', 'FAILURE'].includes(rowData.item.result);
      const isActive = ['PENDING', 'RUNNING', 'WAITING'].includes(
        rowData.item.status,
      );
          ...
          ${isRunning
              ? html`
                  <mwc-icon-button
                    ...
                    icon="stop"
                    @click="${() => {
                      this._stopPipelineJobExecution(rowData.item);
                    }}"
                  ></mwc-icon-button>
                `
              : html``}-->
    ...
    // filter columns
    this.pipelineJobGridFilterColumnList[0].renderer = (
      root,
      column,
      rowData,
    ) => {
    ...
    this.pipelineJobGridFilterColumnList[1].renderer = (
      root,
      column,
      rowData,
    ) => {
      ...
      const mountedFolderList: Array<string> = rowData.item.yaml.mounts.map(
        (elem: string) => {
        ...
        html`
          <div class="layout horizontal center flex">
            <div class="layout horizontal center configuration">
              ${rowData.item.yaml.mounts.length > 0
                ? html`
        ...
        <div slot="content" role="listbox" class="horizontal center layout">
          <mwc-list class="vertical center layout">
            ...
            <mwc-list-item twoline>
              ...
              <span class="monospace" slot="secondary">
                ${PipelineUtils._humanReadableDate(
            ...
            <mwc-list-item twoline>
              ...
              <span class="monospace" slot="secondary">
                ${PipelineUtils._humanReadableDate(
                  this.pipelineJobInfo.last_modified,
                )}
              </span>
            </mwc-list-item>
            <mwc-list-item twoline>
          ...
          <mwc-list class="vertical center layout left-border">
            <mwc-list-item id="status" twoline>
              ...
              <div slot="secondary" class="horizontal center layout">
                <lablup-shields
                  id="pipeline-status"
                  description="${this.pipelineJobInfo.status}"
                  color="${PipelineUtils._getStatusColor(
