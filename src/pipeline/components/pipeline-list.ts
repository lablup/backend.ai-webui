...
export default class PipelineList extends BackendAIPage {
  ...
  @property({ type: Object }) _boundIndexRenderer =
    this.indexRenderer.bind(this);
  @property({ type: Object }) _boundControlRenderer =
    this.controlRenderer.bind(this);
  @property({ type: Object }) _boundCreateAtRenderer =
    this.createdAtRenderer.bind(this);
  @property({ type: Object }) _boundModifiedAtRenderer =
    this.modifiedAtRenderer.bind(this);

  @query('vaadin-grid#pipeline-list') pipelineGrid;
  @query('pipeline-configuration-form')
  ...
  _createPipeline() {
    ...
    this.pipelineInfo =
      this.pipelineConfigurationForm.inputFieldListAsInstance();
    globalThis.backendaiclient.pipeline
      .create(this.pipelineInfo)
      .then((res: PipelineInfoExtended) => {
  ...
  _deletePipeline() {
    globalThis.backendaiclient.pipeline
      .delete(this.pipelineInfoExtended.id)
      .then((res) => {
    ...
    globalThis.backendaiclient.pipeline
      ...
      .then((res) => {
        this.pipelines = res.map((pipeline: PipelineInfoExtended) => {
          ...
          pipeline.created_at = PipelineUtils._humanReadableDate(
            pipeline.created_at,
          );
          pipeline.last_modified = PipelineUtils._humanReadableDate(
  ...
  _launchPipelineDetailDialog(pipeline: PipelineInfoExtended) {
    ...
    parsedPipelineInfo.yaml.tasks = PipelineUtils._parseTaskListInfo(
      parsedPipelineInfo.dataflow,
   ...
   */
  moveToViewTab() {
    const moveToViewEvent = new CustomEvent('pipeline-view-active-tab-change', {
      detail: {
        activeTab: {
          title: 'pipeline-view',
        },
  ...
  controlRenderer(root, column, rowData) {
    render(
      html`
        <div
          id="controls"
          class="layout horizontal flex center wrap"
          .pipeline-id="${rowData.item.name}"
        >
          <mwc-icon-button
            icon="info"
            class="fg green"
    ...
    return html`
      <div class="horizontal layout flex center end-justified">
        <mwc-button
          ...
          icon="add"
          label="New Pipeline"
          @click="${() => this._launchPipelineDialog()}"
          ?disabled="${!this._isLaunchable}"
        ></mwc-button>
      </div>
      <vaadin-grid
        id="pipeline-list"
      ...
      >
        ...
        <vaadin-grid-column
          ...
          header="Created At"
          resizable
          .renderer="${this._boundCreateAtRenderer}"
        ></vaadin-grid-column>
        <vaadin-grid-column
          id="modified-at"
          header="Last Modified"
          resizable
          .renderer="${this._boundModifiedAtRenderer}"
        ></vaadin-grid-column>
      </vaadin-grid>
