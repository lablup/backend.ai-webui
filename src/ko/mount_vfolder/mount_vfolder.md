...
'데이터 & 폴더' 단계로 이동한 후 **폴더를 선택해주세요** 드롭다운을 열면 사용자가 마운트할 수 있는 폴더를 확인할 수 있습니다.
이 드롭다운에서 하나 이상의 폴더를 선택하면, 선택한 폴더가 마운트할 폴더 목록에 추가됩니다.
...
마운트 경로를 사용자 지정하려면 해당 폴더 행의 **컨테이너 경로 (destination)** 필드에 절대 경로를 입력합니다.
`test` 폴더의 필드에 `/workspace`를 입력하면 세션 내부의 `/workspace`에 마운트됩니다.
상대 경로를 입력하면 `/home/work/` 아래에 해당 경로로 폴더가 마운트됩니다.
:::

:::note
폴더 전체가 아닌 하위 폴더를 마운트하려면 해당 폴더 행의 **폴더 안의 경로 (source)** 필드를 입력합니다.
필드를 클릭하면 폴더를 탐색하며 경로를 선택할 수 있고, 비워 두면 폴더 루트가 마운트됩니다.
:::
