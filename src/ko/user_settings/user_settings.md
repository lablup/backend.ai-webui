...
사용자 설정 대화 상자에서는 Backend.AI WebUI 환경을 사용자 정의할 수 있습니다.
우측 상단의 사람 아이콘을 클릭한 후 설정 메뉴를 선택하면 보고 있던 페이지 위에
열리므로, 닫으면 하던 작업으로 그대로 돌아갑니다.
...
선택하면 해당 화면이 열리며 뒤로 가기 버튼으로 목록에 돌아올 수 있습니다.
...
일반 범주에는 **설정**, **쉘 스크립트 환경**, **실험적 기능** 그룹으로 구성된
모든 환경 설정 항목이 포함되어 있습니다. 모든 그룹이 차례로 표시되며, 찾는
항목은 검색 바로 좁혀 나갈 수 있습니다.
...
나타납니다. 현재 등록된 SSH 공개 키는 **현재 공개 키** 항목에 표시되며, 항목
옆의 복사 버튼을 클릭하면 복사할 수 있습니다. 다이얼로그 하단의 `생성하기`
버튼을 클릭하면 SSH 키페어를 갱신할 수 있습니다. SSH 공개/비밀 키는 랜덤으로
...
자신의 키를 사용하여 Backend.AI 세션에 접속할 수 있습니다. 서버가 키를 거부한
경우(예: 키 형식이 올바르지 않거나 공개 키와 비밀 키가 서로 맞지 않는 경우)에는
...
- **AI 에이전트**: AI 에이전트 기능을 활성화합니다. 이 기능은 WebUI 내에서
  ...
  사용할 수 있습니다.
- **사용자 정의 대시보드 패널**: 대시보드 페이지에 나만의 사용자 정의 패널을 추가할 수
  있게 해줍니다. 각 패널은 선택한 데이터 소스를 지정한 조건으로 좁혀 보여
  줍니다. 이 기능이 활성화된 동안에만 패널이 표시되며, 나중에
  비활성화하면 패널이 삭제되지 않고 숨겨지고, 다시 활성화하면 원래대로
  복원됩니다. 추가 및 관리 방법은 [사용자 정의 패널](#custom-panels) 섹션을
  참고하세요.
- **커맨드 팔레트**: 헤더에 **검색** 버튼을 추가하고 `Ctrl+K`
  (macOS에서는 `⌘K`) 단축키를 활성화합니다. 두 가지 모두 페이지, 페이지 탭,
  설정 항목을 검색할 수 있는 팔레트를 엽니다. 팔레트에서는 다크 모드와 라이트
  모드 전환, UI 언어 변경, 알림 열기, 사이드바 접기/펼치기, 현재 페이지의 매뉴얼
  열기와 같은 빠른 작업도 사용할 수 있습니다. 이 버튼과 단축키는 해당 기능을
  활성화한 경우에만 사용할 수 있습니다. 자세한 내용은 [검색](#search) 섹션을
  참고하세요.
- **Hugging Face에서 가져오기**: 시작 페이지의 **URL 로 시작** 대화 상자에
  **Hugging Face 모델 가져오기** 탭을 추가하여, Hugging Face의 모델을 모델
  ...
  사용할 수 있는 경우에만 표시됩니다.
- **세션 리소스 그리드 뷰**: 세션 페이지, 관리자용 세션 페이지, 그리고
  `자원` 페이지의 **에이전트 정보** 패널 **세션** 탭에 있는 세션 목록
  위에 **보기 모드** 전환(**테이블** 또는 **그리드**)을 추가합니다.
  **그리드**를 선택하면 테이블 대신 세션마다 셀이 하나씩 표시되며, 셀의 색은
  해당 세션의 실시간 자원 사용률을 나타냅니다. 이 전환 버튼은 해당 기능을
  활성화한 경우에만 표시됩니다. 그리드 자체의 조작 방법은
...
발생했을 때 자세한 내용을 확인하고 싶다면 이 범주를 살펴볼 수 있습니다.
로그를 검색하거나 에러 메시지를 필터링하고, 로그를 새로고침하거나 우측 상단의
...
작동하지 않는 것처럼 보일 수 있습니다. 로그는 서버에 대한 요청과 서버의 응답을
모아둔 것인데, 이 대화 상자를 보고 있는 동안에는 뒤쪽 페이지가 서버에 요청을
보내지 않기 때문입니다. 로그가 제대로 쌓이는지 확인하려면 대화 상자를 닫고 다른
페이지에서 작업한 뒤 **로그**를 다시 여십시오.
:::
...
로그인 세션 범주에는 현재 내 계정에 연결된 로그인 세션이 표시됩니다. Backend.AI에
...
로그인 기록 범주에는 내 계정에 대해 기록된 로그인 시도가 표시됩니다. 로그인 성공,
로그인 실패는 물론 로그아웃이나 만료와 같은 로그인 세션 관련 이벤트도 함께
표시됩니다. 이 범주는 조회 전용이며 행에 대한 별도의 작업은 없습니다.
...
- **클라이언트 IP**: 로그인을 시도한 IP 주소로, 서버가 보고한 그대로 표시됩니다.
  관리자의 클라이언트 IP 마스킹 정책에 따라 주소 일부가 가려질 수 있으며, 기록되지
  않은 경우 `-` 로 표시됩니다.
