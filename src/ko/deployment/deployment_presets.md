...
관리자 배포 페이지에는 **배포**, **모델 스토어 관리**, **프로메테우스 프리셋**, **배포 프리셋** 등 여러 탭이 있습니다. 각 탭의 필터, 정렬, 페이지 상태는 URL에 저장되므로 다음 두 가지 이점이 있습니다.

- 다른 탭으로 이동했다가 돌아와도 배포 프리셋 탭이 떠날 때의 상태 그대로 복원됩니다.
- 필터를 적용한 프리셋 목록의 URL을 북마크하거나 공유하면, 받는 사람도 동일한 필터·정렬·페이지 상태로 화면을 열게 됩니다.
