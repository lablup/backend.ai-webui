...
class Client {
    ...
    async createIfNotExists(kernelType, sessionId, resources = {}, timeout = 0, architecture = '') {
        ...
        // @ts-ignore: Legacy code - preserve original behavior to ensure same functionality
        if (resources != {}) {
            let config = {};
            if (resources['cpu']) {
    ...
    newPublicRequest(method, queryString, body, urlPrefix) {
        ...
        else if (this._config.connectionMode === 'SESSION' &&
...
class VFolder {
    ...
    async delete_files(files, recursive = false, name = null) {
        ...
        let rqst = this.client.newSignedRequest('POST', `${this.urlPrefix}/${name}/delete-files`, body);
        return this.client._wrapWithPromise(rqst);
    }
    /**
...
class Agent {
     ...
     */
    async list(status = 'ALIVE', fields = [
        'id',
        'status',
        'region',
        'first_contact',
...
class ContainerImage {
    ...
    ], installed_only = false, system_images = false) {
        let q, v;
    ...
    async install(name, architecture, resource = {}, registry = 'index.docker.io') {
        ...
        return this.client
            .createIfNotExists(registry + name, sessionId, resource, 600000, architecture)
            .then((response) => {
...
class ComputeSession {
    ...
    async list(fields = [
        ...
        'starts_at',
    ], status = 'RUNNING', accessKey = '', limit = 30, offset = 0, group = '', timeout = 0) {
    ...
    async listAll(fields = [
        ...
        'occupied_slots',
        'containers {live_stat last_stat}',
    ], status = 'RUNNING,RESTARTING,TERMINATING,PENDING,SCHEDULED,CREATING,PULLING,TERMINATED,CANCELLED,ERROR', accessKey = '', limit = 100, offset = 0, group = '', timeout = 0) {
                ...
                Object.keys(this.agents).map((objectKey, index) => {
                    ...
                    this.resources.mem.total =
                        parseFloat(this.resources.mem.total) +
                            parseInt(this.client.utils.changeBinaryUnit(available_slots.mem, 'b'));
                    this.resources.mem.allocated =
                        parseInt(this.resources.mem.allocated) +
                            parseInt(this.client.utils.changeBinaryUnit(occupied_slots.mem, 'b'));
                    this.resources.mem.used =
                        parseInt(this.resources.mem.used) +
                            parseInt(this.client.utils.changeBinaryUnit(value.mem_cur_bytes, 'b'));
...
class Group {
     ...
     * };
     */
    async list(is_active = true, domain_name = false, fields = [
        'id',
        'name',
        'description',
        'is_active',
...
class Domain {
     ...
     * };
     */
    async get(domain_name = false, fields = [
        'name',
        'description',
        'is_active',
        'created_at',
...
class User {
     ...
     * };
     */
    async list(is_active = true, fields = [
        'username',
        'password',
...
class EduApp {
     ...
     */
    async get_user_credential(stoken) {
        const rqst = this.client.newSignedRequest('GET', `/eduapp/credential?${new URLSearchParams({ sToken: stoken }).toString()}`);
        return this.client._wrapWithPromise(rqst);
    }
}
class utils {
    ...
    changeBinaryUnit(value, targetUnit = 'g', defaultUnit = 'b') {
        ...
        return (value *
            Math.pow(1024, Math.floor(binaryUnits.indexOf(sourceUnit) - binaryUnits.indexOf(targetUnit))));
    }
