...
2. กรอกฟิลด์ต่าง ๆ ฟอร์มนี้เป็นตัวช่วยสามขั้นตอน ได้แก่ **ข้อมูลพื้นฐาน**, **โมเดล & การประมวลผล** และ **การทบทวน** โดยมีรายการขั้นตอนอยู่ทางขวาและปุ่ม `ก่อนหน้า` / `ถัดไป` อยู่ด้านล่าง ใช้ `ข้ามไปยังการทบทวน` เพื่อไปยังขั้นตอนสุดท้ายได้ทันที ฟิลด์จัดเป็นส่วนต่าง ๆ ดังนี้:
   ...
   - **ข้อมูลพื้นฐาน**:
      * **ชื่อ**: ชื่อพรีเซ็ตที่ไม่ซ้ำกัน (เช่น `vLLM-GPU-Large`) ชื่อต้องไม่มีช่องว่าง
...
เหนือตารางมีตัวกรองคุณสมบัติ (**ชื่อ**, **รันไทม์**) ปุ่มรีเฟรช และปุ่ม **สร้างพารามิเตอร์** โดยค่าของ **รันไทม์** จะเลือกจากรายการแบบเลื่อนลงของรันไทม์ที่กำหนดไว้บนเซิร์ฟเวอร์นี้แทนการพิมพ์ ID และแท็กตัวกรองที่ได้จะแสดงชื่อของรันไทม์ คอลัมน์ที่แสดงตามค่าเริ่มต้นมีดังนี้

- **ชื่อ**: ชื่อของพารามิเตอร์ คอลัมน์นี้ยังมีปุ่มแก้ไขและลบของแต่ละแถวด้วย
...
- **เป้าหมายพารามิเตอร์**: วิธีที่ค่าถูกส่งไปยังคอนเทนเนอร์ ระหว่าง **ตัวแปรสภาพแวดล้อม** หรือ **อาร์กิวเมนต์บรรทัดคำสั่ง**
...
- **อันดับ**: ลำดับการแสดงผลระหว่างพารามิเตอร์ของ runtime เดียวกัน ค่าที่น้อยกว่าจะแสดงก่อน
- **วันที่สร้าง**: เวลาที่สร้างพารามิเตอร์
...
### การสร้างและแก้ไขพารามิเตอร์ Runtime

คลิกปุ่ม **สร้างพารามิเตอร์** เหนือตารางเพื่อเปิดหน้าต่าง **สร้างพารามิเตอร์** หรือคลิกปุ่มแก้ไขบนแถวเพื่อเปิดหน้าต่าง **แก้ไขพารามิเตอร์** ที่กรอกค่าปัจจุบันไว้แล้ว ทั้งนี้ไม่สามารถเปลี่ยนตัวแปรรันไทม์ของพารามิเตอร์ที่มีอยู่แล้วได้
...
- **เป้าหมายพารามิเตอร์**: **ตัวแปรสภาพแวดล้อม** หรือ **อาร์กิวเมนต์บรรทัดคำสั่ง** จำเป็นต้องระบุ
...
- **อันดับ** *(แสดงเฉพาะตอนแก้ไข)*: ลำดับการแสดงผลระหว่างพารามิเตอร์ของ runtime เดียวกัน ค่าที่น้อยกว่าจะแสดงก่อน

:::note
หากเปิดพารามิเตอร์ที่บันทึกตัวควบคุมและประเภทค่าที่เข้ากันไม่ได้ไว้ เช่น พารามิเตอร์ที่สร้างผ่าน API ระบบจะแสดงคำเตือนว่าประเภทค่าที่บันทึกไว้ไม่สามารถแสดงด้วยประเภท UI ที่บันทึกไว้ได้ และฟิลด์ที่เกี่ยวข้องจะมีข้อความ **ประเภท UI ที่เลือกไม่สามารถแสดงประเภทค่านี้ได้** กำกับไว้ ให้เปลี่ยนอย่างใดอย่างหนึ่งก่อนบันทึก

บนเซิร์ฟเวอร์ที่ไม่มีฟิลด์ **ประเภท UI** ระบบจะบังคับความเข้ากันได้จากอีกด้านหนึ่งแทน กล่าวคือ ตัวควบคุมที่บันทึกไว้กับพารามิเตอร์จะเป็นตัวจำกัดว่าเลือก **ประเภทค่า** ใดได้บ้าง
:::

คลิก `สร้าง` (หรือ `บันทึก` เมื่อแก้ไข) เพื่อบันทึกพารามิเตอร์ ระบบจะแจ้งเตือนว่าพารามิเตอร์ Runtime ถูกสร้างหรืออัปเดตแล้ว และรายการจะรีเฟรช

### การลบพารามิเตอร์ Runtime

คลิกปุ่มลบบนแถวของพารามิเตอร์ จะมีกล่องยืนยันที่ให้พิมพ์ชื่อพารามิเตอร์ปรากฏขึ้น ปุ่ม **ลบ** จะยังคงถูกปิดใช้งานจนกว่าข้อความที่พิมพ์จะตรงกันทุกประการ

:::danger
การลบพารามิเตอร์ Runtime **ไม่สามารถย้อนกลับได้** พารามิเตอร์นี้จะหายไปจากส่วน **พารามิเตอร์ Runtime** ของทุกแบบฟอร์มการปรับใช้ที่ใช้ตัวแปรรันไทม์นี้
:::
