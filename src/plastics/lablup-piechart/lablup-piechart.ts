...
export default class LablupPiechart extends LitElement {
  ...
  firstUpdated() {
    ...
    let chart: HTMLElement = this.shadowRoot?.querySelector(
      '#chart',
    ) as HTMLElement;
    let chartText: HTMLElement = this.shadowRoot?.querySelector(
      '#chart-text',
    ) as HTMLElement;
    let unitText: HTMLElement = this.shadowRoot?.querySelector(
      '#unit-text',
    ) as HTMLElement;
    ...
    this.shadowRoot
      ?.querySelector('#pievalue')
      ?.setAttribute('d', this.indicatorPath);
    if (this.url !== undefined && this.url !== '') {
      this.shadowRoot
        ?.querySelector('#chart')
        ?.addEventListener('tap', this._moveTo.bind(this));
    }
    this.requestUpdate();
  }
