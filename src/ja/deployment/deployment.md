...
- **サブパス**: フォルダーのルートの代わりにマウントする、モデルフォルダー内のサブフォルダーです。フィールドをクリックするとディレクトリピッカーが開き、選択したモデルフォルダーのディレクトリツリーからサブフォルダーを選びます。未設定のままにするとフォルダーのルートがマウントされます。
...
- **コマンド**: 推論サーバを起動するコマンドです（例: `python -m http.server 8000`）。**Shell** 方式では複数行の入力ボックスになり、**Exec** 方式ではラベルが **コマンド (argv)** に変わり 1 行の入力フィールドになります。JSON 文字列を 1 つの引数として渡すには、JSON 全体をシングルクォートで囲み、内部のダブルクォートはそのままにします（例: `--override-generation-config '{"temperature": 0.7}'`）。
...
テーブルの上部には、プロパティフィルターバーと更新ボタンがあります。**Revision 番号**、**作成日**、**クラスターモード**、**イメージ**、**モデルフォルダー** で一覧を絞り込めます。**イメージ** と **モデルフォルダー** の値は識別子を入力するのではなくドロップダウンから選択し、適用されたフィルタータグには選択した名前が表示されます。**モデルフォルダー** のドロップダウンはこのデプロイメント自身のプロジェクトに限定され、削除済みを除くモデル用途のフォルダーのみを表示します。これは Revision 追加モーダルのフォルダー選択と同じ範囲です。
