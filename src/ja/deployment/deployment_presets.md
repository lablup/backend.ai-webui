...
2. 各フィールドに入力します。この画面は **基本情報**、**モデル & 実行**、**レビュー** の 3 ステップからなるウィザードで、右側にステップ一覧、下部に `前へ` / `次のページ` のナビゲーションが表示されます。`レビューへスキップ` をクリックすると最後のステップへ直接移動できます。フィールドは次のセクションに分かれています。
   ...
   - **基本情報**：
      * **名前**：一意のプリセット名（例：`vLLM-GPU-Large`）。名前に空白を含めることはできません。
...
**管理者デプロイ** ページ（`/admin/deployments`）の **ランタイムパラメータ** タブでは、ランタイムが提供する個々のパラメータを定義します。各エントリは 1 つのパラメータを表し、コンテナに渡されるキー、値の型、デフォルト値、そして画面上での表示方法を持ちます。これらのエントリが集まって、ユーザーがデプロイメントの Revision を追加する際に入力する **ランタイムパラメータ** のタブを構成します（デプロイメントページの [ランタイムパラメータ](#runtime-parameters) を参照）。
...
- **パラメータ対象**: 値がコンテナに渡される方法で、**環境変数** または **コマンドライン引数** です。
- **値の型**: **文字列**、**整数**、**浮動小数点数**、**ブール値**、**フラグ** のいずれかです。
- **UIタイプ**: デプロイメントフォームでこのパラメータを表示する際に使用するコントロールで、**テキスト入力**、**数値入力**、**チェックボックス**、**セレクト**、**スライダー** のいずれかです。このバージョンの WebUI が認識できないコントロールは保存された値がそのまま表示され、`-` はコントロールが設定されていないことを示します。デプロイメントフォームでは、設定されたコントロールが値の型を表示できない場合、通常のテキスト入力に切り替わります。
- **キー**: 値が渡される環境変数名またはコマンドライン引数です。
- **デフォルト値**: ユーザーがパラメータを変更しなかった場合にランタイムが使用する値です。
- **必須**: このランタイムで Revision を作成する際に、このパラメータの指定が必須かどうかです。
- **ランク**: 同じランタイムのパラメータ間の表示順序です。値が小さいほど先に表示されます。
- **作成日**: パラメータが作成された日時です。
...
表の上部にある **パラメータを作成** ボタンをクリックすると **パラメータを作成** モーダルが開き、行の編集ボタンをクリックすると現在の値が入力された **パラメータを編集** モーダルが開きます。既存パラメータのランタイムバリアントは変更できません。
...
- **パラメータ対象**: **環境変数** または **コマンドライン引数** です。必須項目です。
- **値の型**: **文字列**、**整数**、**浮動小数点数**、**ブール値**、**フラグ** のいずれかです。必須項目です。以下の **UIタイプ** より先に選択してください。値の型を変更すると、選択済みのコントロールは解除されます。以前の型に合わせて選んだコントロールが、新しい型を表示できるとは限らないためです。
- **UIタイプ**: デプロイメントフォームでこのパラメータを表示する際に使用するコントロールです。空のままにすると、通常のテキスト入力として表示されます。選択した **値の型** を表示できるコントロールのみ選択でき、それ以外は無効になります。タイプを選択すると、そのタイプ固有の設定が表示されます。
   * **テキスト入力**（文字列、整数、浮動小数点数）: **入力プレースホルダー** — フィールドが空のときに表示されるヒント文です。
   * **数値入力**（整数、浮動小数点数）: **最小値** と **最大値**。範囲外の値はデプロイメントフォームで検証エラーとして表示されます。
   * **チェックボックス**（ブール値、フラグ）: 追加設定はありません。
   * **セレクト**（文字列）: **選択肢** — 選択肢ごとに **値** / **ラベル** を 1 行ずつ入力します。**選択肢を追加** ボタンで行を追加し、ゴミ箱ボタンで削除します。少なくとも 1 つの選択肢が必要です。
   * **スライダー**（整数、浮動小数点数）: **最小値** と **最大値**（いずれも必須で、最大値は最小値より大きくする必要があります）、およびスライダーの増分である **ステップ** です（既定値は `1`）。
...
- **必須の有無**: **必須** チェックボックスをオンにすると、ユーザーが Revision を作成する際にこのパラメータの指定が必須になります。必須パラメータはデプロイメントフォームで赤いアスタリスク（★）が付きます。
- **ランク** *(編集時のみ表示)*: 同じランタイムのパラメータ間の表示順序です。値が小さいほど先に表示されます。

:::note
API などで作成され、互いに適合しないコントロールと値の型が保存されているパラメータを開くと、保存された値の型を保存された UI タイプでは表示できないという警告が表示され、該当のフィールドに **選択した UI タイプではこの値の型を表示できません。** というメッセージが表示されます。保存する前にどちらか一方を変更してください。

**UIタイプ** のフィールドが表示されないサーバーでは、逆方向に制約がかかります。つまり、パラメータにすでに保存されているコントロールによって、選択できる **値の型** が制限されます。
:::

`作成`（編集時は `保存`）をクリックすると保存されます。ランタイムパラメータが作成または更新されたことを知らせる通知が表示され、一覧が更新されます。

### ランタイムパラメータの削除

パラメータ行の削除ボタンをクリックします。パラメータ名を入力して確認するダイアログが開き、入力した値が完全に一致するまで **削除** ボタンは無効のままです。

:::danger
ランタイムパラメータの削除は **元に戻せません**。このランタイムを使用するすべてのデプロイメントフォームの **ランタイムパラメータ** セクションから、このパラメータが消えます。
:::
