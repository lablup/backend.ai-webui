...
「データとストレージ」ステップに進み、**フォルダを選択** ドロップダウンを開くと、ユーザーがマウントできるフォルダが表示されます。
このドロップダウンから1つ以上のフォルダを選択すると、選択したフォルダがマウントするフォルダの一覧に追加されます。
...
「データとストレージ」ステップでは、現在のユーザーがマウント可能なストレージフォルダのみが表示される点に注意してください。例えば、他のプロジェクトに属するフォルダは表示されません。
:::
...
マウントパスをカスタマイズするには、該当フォルダの行の **コンテナのパス (destination)** フィールドに絶対パスを記入します。
`test` フォルダのフィールドに `/workspace` と記入すると、セッション内の `/workspace` にマウントされます。
相対パスを記入した場合は、そのパスを付けて `/home/work/` 配下にフォルダがマウントされます。
:::

:::note
フォルダ全体ではなくサブフォルダをマウントするには、該当フォルダの行の **フォルダ内のパス (source)** フィールドを入力します。
フィールドをクリックするとフォルダを参照してパスを選択でき、空のままにするとフォルダのルートがマウントされます。
:::
