// PILOT 10 — GAP COMPONENTS: screenshot each component's key states in
...
const BASE = process.env.GAP_URL ?? 'http://127.0.0.1:5312/gap.html';
const browser = await chromium.launch();
const errors = [];

/** Full-page frames of the four in-flow components. */
const pages = [
...
for (const mode of ['light', 'dark']) {
  for (const state of pages) {
    ...
    await page.screenshot({ path: `${OUT}${state.name}-${mode}.png`, fullPage: true });
...
for (const mode of ['light', 'dark']) {
  ...
  page.on('pageerror', (e) => errors.push(`[${mode}/columns] pageerror: ${e.message}`));
  page.on('console', (m) => {
    if (m.type() === 'error') errors.push(`[${mode}/columns] console: ${m.text()}`);
  });
...
// Breakpoint proof: the SAME hook at three viewport widths. One frame per
// width, clipped to the breakpoint column, so the booleans are legible.
...
// SAFE action (Cancel), Escape closes it, and focus returns to the trigger.
{
  ...
  page.on('pageerror', (e) => errors.push(`[popconfirm-a11y] pageerror: ${e.message}`));
  page.on('console', (m) => {
    if (m.type() === 'error') errors.push(`[popconfirm-a11y] console: ${m.text()}`);
  });
  await page.goto(BASE, { waitUntil: 'networkidle' });
  ...
  await page.waitForTimeout(300);
  const stillOpen = await page.getByText('This keypair becomes the main').isVisible();
...
{
  const page = await browser.newPage({ viewport: { width: 1500, height: 1100 } });
  page.on('pageerror', (e) => errors.push(`[notif-close] pageerror: ${e.message}`));
  await page.goto(`${BASE}?state=notifications`, { waitUntil: 'networkidle' });
  const before = await page.locator('[data-notification-key]').count();
  await page
    .locator('[data-notification-key="import"] button[aria-label], [data-notification-key="import"] button')
    .last()
    .click();
