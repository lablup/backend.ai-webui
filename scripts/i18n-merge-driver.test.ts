...
describe("i18n-merge-driver utility functions", () => {
  ...
  describe("sortKeyRank", () => {
    it("should give $schema the highest priority", () => {
      expect(sortKeyRank("$schema")).toBe(-1);
    });

    it("should rank lowercase letters as 0", () => {
      expect(sortKeyRank("apple")).toBe(0);
      expect(sortKeyRank("zebra")).toBe(0);
    });

    it("should rank uppercase letters as 1", () => {
      expect(sortKeyRank("Apple")).toBe(1);
      expect(sortKeyRank("ZEBRA")).toBe(1);
    });

    it("should rank other characters as 2", () => {
      expect(sortKeyRank("123")).toBe(2);
      expect(sortKeyRank("_test")).toBe(2);
      expect(sortKeyRank("@symbol")).toBe(2);
    });
  });

  describe("compareKeyRank", () => {
    it("should prioritize $schema first", () => {
      expect(compareKeyRank("$schema", "apple")).toBeLessThan(0);
      expect(compareKeyRank("$schema", "Apple")).toBeLessThan(0);
      expect(compareKeyRank("$schema", "123")).toBeLessThan(0);
    });

    it("should sort lowercase before uppercase", () => {
      expect(compareKeyRank("apple", "Apple")).toBeLessThan(0);
      expect(compareKeyRank("zebra", "Apple")).toBeLessThan(0);
    });

    it("should sort alphabetically within same rank", () => {
      expect(compareKeyRank("apple", "banana")).toBeLessThan(0);
      expect(compareKeyRank("banana", "apple")).toBeGreaterThan(0);
      expect(compareKeyRank("Apple", "Banana")).toBeLessThan(0);
    });

    it("should handle case-insensitive comparison", () => {
      expect(compareKeyRank("apple", "Apple")).toBeLessThan(0);
      expect(compareKeyRank("APPLE", "apple")).toBeGreaterThan(0);
    });
  });

  describe("sortObject", () => {
    it("should sort object keys according to ranking rules", () => {
      const obj = {
        Zebra: "uppercase",
        apple: "lowercase",
        $schema: "special",
      ...
      // The actual sorting puts $schema first, then lowercase, then uppercase, then others
      expect(keys).toEqual(["$schema", "apple", "banana", "Zebra"]);
    });
    ...
    it("should recursively sort nested objects", () => {
      ...
      expect(Object.keys(sorted.z)).toEqual(["apple", "Zebra"]);
    });
  });
...
describe("i18n-merge-driver integration scenarios", () => {
  ...
  it("should handle typical i18n merge scenario without conflicts", () => {
    ...
    const sortedBase = sortObject(base);
    const sortedOurs = sortObject(ours);
    const sortedTheirs = sortObject(theirs);

    const dOurs = diffLeaves(sortedBase, sortedOurs);
    const dTheirs = diffLeaves(sortedBase, sortedTheirs);
    ...
    // Apply changes
    let result = JSON.parse(JSON.stringify(sortedBase));
