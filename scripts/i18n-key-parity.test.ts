/**
 * Every key of `resources/i18n/en.json` must exist in every other catalog with a
 * non-empty value, and no catalog may carry the extractor's `__NOT_TRANSLATED__`
 * placeholder (FR-3729, FR-3839). A key missing from a catalog silently falls
 * back to English at runtime, which is how the FR-3839 backlog accumulated.
 */
...
function readCatalog(file: string) {
  return flatten(JSON.parse(readFileSync(join(I18N_DIR, file), "utf8")));
}
...
describe("resources/i18n key parity with en.json", () => {
  ...
  it.each(languages)("%s carries every en key with a real value", (file) => {
    ...
    // A blank is legitimate where the language drops the token (th has no
    // sentence-final period, ko moves a verb fragment into the next key), so
    // only an empty string standing in for wordful English counts.
