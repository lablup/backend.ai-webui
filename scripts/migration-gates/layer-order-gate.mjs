...
export function runLayerOrderGate({ repoRoot = REPO_ROOT } = {}) {
  ...
  for (const rel of MIRRORS) {
    const parsed = parseLayerOrder(
      rel.endsWith(".css") ? stripCssComments(texts.get(rel)) : texts.get(rel),
    );
    if (parsed === null) {
      failures.push(
  ...
  // 2. In index.html the statement must precede every stylesheet, including
  ...
  const statementAt = parseLayerOrder(html).index;
  const firstSheetAt = html.search(/<link\b[^>]*rel=["']stylesheet["']/i);
